...
K777 已顯示模型在 native target 可能有「主場優勢」；K1072 在短 SPY 樣本發現 5 分鐘 RV 與 noise-robust proxy 接近。K1704 的增量是使用長期台指期逐筆資料，在每個 forecast origin 僅以過去 500 日估計各 proxy 的 log-bias 與相對 consensus residual，再建立 consensus-weighted heuristic target。它不能識別共同偏誤或宣稱是最適 measurement-error 權重，但能直接檢定 HAR/GJR 排名是否依賴單一不可靠目標。
...
3. 每個 origin 的 proxy bias/weight 只使用 `[t-500,t)`；當日 proxy 只能作評分 target，不能進權重估計。
4. 對五個 raw proxies 與 consensus-weighted target 同時報 actual/predicted QLIKE、Spearman、repo canonical HAC DM-HLN（h=1）及 stationary-bootstrap MCS（seed=42、1,000 reps）；實際值非正的日期在模型 feature、consensus calibration 與共同評估 ledger 都視為缺值，不得 clip 成極小正數。
...
- DM：只用 repo canonical automatic-bandwidth HAC；不可用 h=1→lag 0 的 local helper。
- 隨機程序：MCS seed 固定為 42。
- 輸出：結果 JSON 先寫 temp、解析驗證後 `os.replace`；每個 raw tick 檔、raw byte-inventory、canonical CSV、proxy cache、collector／實驗／MCS／evaluation code 皆記 SHA-256。快取內另保存 extraction-function builder hash；重跑若 extraction code 不符即 fail closed，純 forecast code 修正不會冒充 raw cache 改變。
...
5. Liu, Patton and Sheppard (2015), *Journal of Econometrics*, 187, 293–311. doi:10.1016/j.jeconom.2015.02.014.
...
在 2018-02-01 至 2026-07-14 的一步樣本外區間，HAR-RV5 在五個 raw proxy 與 past-only consensus target 上都是最低 QLIKE 模型；六個 target 的 stationary-bootstrap MCS 也都只保留 HAR-RV5。故依預註冊規則，結論為 `PROXY_ROBUST_STATISTICAL_RANKING`。

- consensus target：HAR-RV5 QLIKE 0.1782、EWMA-R2 0.2347、GJR-GARCH 0.2556；HAR 相對 EWMA 的 DM-HLN t = -4.49，相對 GJR 的 t = -4.25。
- 五個 raw proxy 的 HAR QLIKE 依序為 0.1311、0.1793、0.2328、0.4259、2.8554；每個 target 的模型次序皆為 HAR-RV5、EWMA-R2、GJR-GARCH。
- early/late OOS consensus split 仍維持同一次序，且兩段 MCS 都只保留 HAR-RV5。
- canonical compatibility、3,548 個 raw 檔與程式／快取皆有 SHA-256 audit；正式 parity 數字以最終 `K1704_results.json` 為準。

這是「統計排名對已觀察代理選擇穩健」的證據，不代表 consensus 是 latent integrated variance、也不代表 HAR 具有因果或交易優勢。完整數字以 byte-pinned `K1704_results.json` 與 `review_verdict.json` 為準。
