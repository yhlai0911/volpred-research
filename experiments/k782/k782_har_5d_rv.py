#!/usr/bin/env python3
"""
K782: HAR-based Multi-Horizon Volatility Forecasting (Daily Squared Returns)
============================================================================
[提出: 用戶, 執行: Claude]

Use daily-frequency squared returns to construct multi-day realized variance,
then forecast at 5-day (weekly), 22-day (monthly), and 66-day (quarterly)
horizons using HAR and competing models.
...
Models:
  ...
  2. HAR-RV (log):       log(RV_h(t+1:t+h)) = c + beta_d log(RV_d(t)) + beta_w log(RV_5d(t)) + beta_m log(RV_22d(t)) + eps
...
References:
  ...
  - K457: Weekly frequency volatility prediction
  - K489: VIX term structure for multi-horizon forecasting
  - K778: GJR best on r-squared target (DM=3.78 vs MEM)
"""
...
RESULTS_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'k782_har_5d_rv_results.json')
...
def gjr_garch_multi_step(df, oos_start, horizon):
    ...
    for i, dt in enumerate(oos_dates):
        ...
        # 1-step forecast
        # Use most recent return and conditional variance
        r_last = returns_full.iloc[loc]
...
def main():
    print("=" * 70)
    print("K782: HAR Multi-Horizon Volatility Forecasting (Daily Squared Returns)")
    print("=" * 70)
    ...
    final_results = {
        'experiment_id': 'K782',
        'title': 'HAR Multi-Horizon Volatility Forecasting (Daily Squared Returns)',
        'description': (
            'Multi-horizon (5d/22d/66d) volatility forecasting using daily squared returns '
            'as realized variance proxy. Compares HAR-RV, HAR-log, GJR-GARCH multi-step, '
            'EWMA, and rolling window benchmarks.'
        ),
