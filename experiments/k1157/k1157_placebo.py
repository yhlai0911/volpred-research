#!/usr/bin/env python3
"""
K1157 placebo — within-stock permutation of continuous surprise signal (JP).

Mirrors K1151_placebo.py exactly but on JP TOPIX panel:
  - For each stock, shuffle surp_z vector in place (preserve marginal
    distribution within stock, break time alignment).
  - Refit pooled panel.
  - Under H0 (no real time-aligned surprise → vol signal), refit
    θ_SURP should center at ~0.
...
def main():
    ...
    print('=== K1157 placebo: within-stock permutation of continuous surp_z (JP) ===')
    ...
    out = {
        'experiment_id': 'K1157_placebo',
        'description': 'Within-stock permutation of continuous surp_z; refit pooled MLE (JP TOPIX)',
    ...
    out_path = SCRIPT_DIR / 'k1157_placebo_results.json'
