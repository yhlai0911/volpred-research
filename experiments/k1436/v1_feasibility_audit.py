...
def search_text_mentions(term_list: tuple[str, ...], limit: int = 20) -> list[str]:
    ...
    for path in ROOT.rglob("*"):
        ...
        except Exception:  # silent-ok: unreadable/binary file during a best-effort repo scan; skipping it cannot change the audit's "no funding data found" conclusion, which is established by the files it CAN read
            continue
        lower = text.lower()
