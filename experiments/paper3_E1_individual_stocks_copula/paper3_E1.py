#!/usr/bin/env python3
"""
Paper3_E1: Copula-GARCH on Individual Stock Pairs (lambda_L threshold validation)
==============================================================================
[Boss directive 2026-05-29 / Claude design / Claude execute]

Parent: K1100b (ETF-level asset-class copula advantage).
...
(E1), other equity markets (E2), commodities (E3) before body rewrite.

Motivation:
  K1100b at ETF level:
    - Same-market equity pairs (SPY-QQQ/IWM/XLF, lambda_L > 0.4) NULL —
      mixing/averaging dominant; DCC already captures co-crash.
    - Asymmetric pair (SPY-GLD, lambda_L < 0.05) NULL — copula mismatch.
    - Cross-asset-class pairs (eq-bond, eq-gold in K1142/K1172) PASS.

  Paper 3 reframe: copula advantage scales with lambda_L threshold —
  same-sector (lambda_L > 0.4) NULL, cross-sector (lambda_L < 0.2) PASS.
  Boss directive: validate on individual stocks before body rewrite.

  H1: Same-sector individual stocks (AAPL-MSFT, JPM-BAC, XOM-CVX, JNJ-PFE,
      NVDA-AMD) — lambda_L > 0.4 + Copula-GARCH NULL vs DCC-A4f-ASYM.
  H2: Cross-sector pairs (AAPL-XOM, MSFT-JPM, GOOGL-CVX, NVDA-BAC, META-JNJ,
      AMD-XOM) — lambda_L < 0.2 + Copula-GARCH PASS at Harvey |t| > 3.0.
  H3: Cross-pair DM t-stat negatively correlated with lambda_L (N=12).
  H4: Same-sector individual stock null replicates K1100b SPY-QQQ-style.

Design: 12 pairs x 3 models (DCC-A4f-ASYM, Copula-t, Clayton) = 36 cells.

Marginal: all stocks A4f with VIX^2 regressor (no stock-specific IV index;
VIX is canonical systemic equity factor). A4f-ASYM per K1092 / K1100b.

Pairs (sector classification):
  Same-sector (expected lambda_L > 0.4, NULL):
    AAPL-MSFT, GOOGL-META, NVDA-AMD (tech),
    JPM-BAC, GS-MS (fin),
    XOM-CVX (energy)
  Cross-sector (expected lambda_L < 0.2, copula advantage):
    AAPL-XOM, MSFT-JPM, NVDA-BAC,
    GOOGL-CVX, META-JNJ, AMD-XOM

Evaluation:
  - Portfolio VaR/ES via CF-Rolling (DCC) or MC (copula), alpha=1%, 2.5%
  - Trinity (Kupiec + CC + Basel) + FZ + DM QLIKE (Harvey |t|>3)
  - Cross-pair scatter: DM t vs mean lambda_L (H3)

Data: yfinance 13 individual stocks + ^VIX (2010-01-01 to 2026-05-28).
  NVDA/META shorter listing not an issue (both start before 2010).
  AMD/AAPL/MSFT/GOOGL/JPM/BAC/GS/MS/XOM/CVX/JNJ/PFE/NVDA/META all 2010+ OK.
OOS: 2015-06-01 onwards (~2700 days per pair), window=1250, refit=63, seed=42.
MC paths: 5000/day.

Runtime estimate: K1100b 5 pairs ~28 min. 12 pairs ~67 min (no marginal
sharing across pairs — each pair refits both marginals). Boss directive
4-6 hr estimate covers pooled-MLE + bootstrap inference allowance.

References (additive to K1100b):
  - Christoffersen et al. (2012). RFS — international tail-dependence.
  - Ang & Chen (2002). JFE — asymmetric equity-pair correlation.
  - K1216c (2026-05) — pooled-MLE multistart methodology lesson.

Author: VolPred Research System
Date: 2026-05-29 (Paper 3 reframe E1 expansion per boss directive)
"""
...
EXPERIMENT_ID = "Paper3_E1"

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
RESULTS_PATH = os.path.join(SCRIPT_DIR, 'paper3_E1_results.json')
...
# All individual stocks use VIX^2 regressor (no stock-specific IV index).
# Same-sector pairs (H1, H4): expected lambda_L > 0.4 + Copula-GARCH NULL.
# Cross-sector pairs (H2): expected lambda_L < 0.2 + Copula-GARCH PASS.
PAIRS = [
    # Same-sector tech (expected NULL)
    ('AAPL-MSFT', 'AAPL', 'MSFT', 'vix2', 'vix2'),
    ('GOOGL-META', 'GOOGL', 'META', 'vix2', 'vix2'),
    ('NVDA-AMD', 'NVDA', 'AMD', 'vix2', 'vix2'),
    # Same-sector fin (expected NULL)
    ('JPM-BAC', 'JPM', 'BAC', 'vix2', 'vix2'),
    ('GS-MS', 'GS', 'MS', 'vix2', 'vix2'),
    # Same-sector energy (expected NULL)
    ('XOM-CVX', 'XOM', 'CVX', 'vix2', 'vix2'),
    # Cross-sector tech-energy (expected PASS)
    ('AAPL-XOM', 'AAPL', 'XOM', 'vix2', 'vix2'),
    ('GOOGL-CVX', 'GOOGL', 'CVX', 'vix2', 'vix2'),
    ('AMD-XOM', 'AMD', 'XOM', 'vix2', 'vix2'),
    # Cross-sector tech-fin (expected PASS)
    ('MSFT-JPM', 'MSFT', 'JPM', 'vix2', 'vix2'),
    ('NVDA-BAC', 'NVDA', 'BAC', 'vix2', 'vix2'),
    # Cross-sector tech-healthcare (expected PASS)
    ('META-JNJ', 'META', 'JNJ', 'vix2', 'vix2'),
]
...
print("=" * 72)
print(f"{EXPERIMENT_ID}: Copula-GARCH on Individual Stock Pairs (lambda_L threshold)")
print(f"  {len(PAIRS)} pairs x {len(MODELS)} models = {len(PAIRS)*len(MODELS)} cells")
print(f"  Same-sector: AAPL-MSFT/GOOGL-META/NVDA-AMD/JPM-BAC/GS-MS/XOM-CVX")
print(f"  Cross-sector: AAPL-XOM/GOOGL-CVX/AMD-XOM/MSFT-JPM/NVDA-BAC/META-JNJ")
...
def load_data():
    ...
    tickers = ['AAPL', 'MSFT', 'GOOGL', 'META', 'NVDA', 'AMD',
               ...
               'XOM', 'CVX',
               'JNJ']
    print(f"Downloading prices: {len(tickers)} individual stocks + ^VIX ...")
    ...
    # VIX must exist (all marginals use vix2 regressor)
    ...
    # VIX^2 (annualized, standard form). gvz2 alias = vix2 for downstream
    # PAIRS that may reference 'gvz2' (none in E1, but keep alias for safety).
    ...
    # Anchor on AAPL+MSFT (longest history, both core to PAIRS list)
    df = df.dropna(subset=['ret_aapl', 'ret_msft', 'vix2'])
    ...
    for t in tickers:
        ...
        print(f"  {t}: {valid} valid days, first valid {first_str}")
    return df
...
def main():
    ...
    results_final = {
        ...
        'metadata': {
            'experiment_id': EXPERIMENT_ID,
            'parent_experiments': ['K1100b', 'K1100', 'K1092', 'K1142', 'K1172'],
            'data_source': 'yfinance (13 individual stocks + ^VIX)',
            'data_period': f"{DATA_START} to {DATA_END}",
            'oos_start': OOS_START,
            'pairs': [p[0] for p in PAIRS],
            'proposer': 'Boss directive 2026-05-29 (Paper 3 reframe E1 expansion)',
...
def make_plots(pair_results, cross_table):
    ...
    plt.suptitle(f'{EXPERIMENT_ID}: Tail Dependence λ_L by Asset Pair',
                 fontsize=12)
    plt.tight_layout()
    p1 = os.path.join(SCRIPT_DIR, 'paper3_E1_tail_dependence_by_pair.png')
    ...
    plt.suptitle(
        ...
        fontsize=11)
    plt.tight_layout()
    p2 = os.path.join(SCRIPT_DIR, 'paper3_E1_dm_vs_lambdaL.png')
    ...
    plt.suptitle(f'{EXPERIMENT_ID}: Fissler-Ziegel Score Heatmap',
                 fontsize=12)
    plt.tight_layout()
    p3 = os.path.join(SCRIPT_DIR, 'paper3_E1_fz_heatmap.png')
