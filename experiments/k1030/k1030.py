...
Motivation:
  ...
  This experiment tests A4f on EURO STOXX 50 (^STOXX50E), a major non-US market.

  VSTOXX is not available via yfinance, so we test:
  M1: GJR-t(df=8)               -- baseline
  M2: A4f-VIX-t(df=8)           -- τ = θ₀ + θ₁×VIX² (US fear index)
  M3: A4f-RV20-t(df=8)          -- τ = θ₀ + θ₁×RV20² (own 20d realized vol)

  K997 showed that own 20d RV can serve as a local fear proxy when an implied
  vol index is unavailable (tested for EEM and 0050.TW). For ^STOXX50E, since
  European equities track closely with US markets, VIX may already be a good
  proxy (VIX-r² correlation ~ 0.44 from initial test).

  We also test FEZ (US-traded Euro STOXX 50 ETF) as a robustness check,
  since it trades during US hours and may have better VIX alignment.

Data: yfinance 2009-2026.
...
print("=" * 70)
print(f"{EXPERIMENT_ID}: A4f Cross-Market — European Equity (EURO STOXX 50 + FEZ)")
...
ASSETS = {
    '^STOXX50E': {
        ...
        'vix_lag': 0,  # European market trades same calendar day as US
    },
    'FEZ': {
        'label': 'SPDR EURO STOXX 50 ETF (US-traded)',
        'vix_lag': 0,  # Trades in US hours
    },
}
...
for asset_key, asset_info in ASSETS.items():
    ...
    ret_v = oos_ret[valid] if len(oos_ret) == len(fc_gjr) else ret[oos_indices][valid]
    n_valid = len(r2_v)
...
if n_completed > 0:
    # Check VIX results
    stoxx_r = all_results.get('^STOXX50E', {})
    fez_r = all_results.get('FEZ', {})

    if stoxx_r.get('status') == 'completed':
        dm_vix = stoxx_r['dm_tests']['A4f_VIX_vs_GJR']['dm_t']
        dm_rv20 = stoxx_r['dm_tests']['A4f_RV20_vs_GJR']['dm_t']
        imp_vix = stoxx_r['qlike_improvement_pct']['A4f_VIX_vs_GJR']
        imp_rv20 = stoxx_r['qlike_improvement_pct']['A4f_RV20_vs_GJR']

        if abs(dm_vix) > 3.0:
            conclusion_parts.append(f"A4f-VIX significantly improves vol forecasting for EURO STOXX 50 (DM t={dm_vix:.3f}, QLIKE improvement {imp_vix:+.1f}%)")
        elif abs(dm_vix) > 2.0:
            conclusion_parts.append(f"A4f-VIX marginally improves for EURO STOXX 50 (DM t={dm_vix:.3f}, below Harvey threshold)")
        else:
            conclusion_parts.append(f"A4f-VIX does NOT significantly improve for EURO STOXX 50 (DM t={dm_vix:.3f})")

        if abs(dm_rv20) > 3.0:
            conclusion_parts.append(f"A4f-RV20 also significant (DM t={dm_rv20:.3f})")
        else:
            conclusion_parts.append(f"A4f-RV20 not significant (DM t={dm_rv20:.3f})")
...
final_results = {
    ...
    'title': 'A4f Cross-Market Validation: European Equity (EURO STOXX 50 + FEZ)',
    'status': 'completed',
    ...
    'config': {
        ...
        'df_fixed': DF_FIXED,
        'rv_window': RV_WINDOW,
        'note': 'VSTOXX unavailable from yfinance; used own 20d RV as local fear proxy',
    },
    'assets': all_results,
    'summary': {
    ...
    'data_source': 'yfinance: ^STOXX50E, FEZ, ^VIX',
    'references': [
...
if completed_assets:
    ...
    ax.set_title(f'K1030: A4f European Equity — QLIKE Comparison\n'
    ...
    plt.tight_layout()
    chart1 = os.path.join(SCRIPT_DIR, 'k1030_qlike_comparison.png')
    ...
    ax.set_title('K1030: DM Test Summary — European Equity', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    chart2 = os.path.join(SCRIPT_DIR, 'k1030_dm_summary.png')
    plt.savefig(chart2, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  Saved: {chart2}")

    # Chart 3: VaR/ES scorecard heatmap
    if len(completed_assets) >= 1:
        fig, ax = plt.subplots(figsize=(10, 4))
        models = ['GJR', 'A4f_VIX', 'A4f_RV20']
        tests = ['VaR_2.5%', 'VaR_1%', 'ES_2.5%', 'ES_1%']
        a = completed_assets[0]  # Use first asset for scorecard
        r = all_results[a]
        ...
        ax.set_title(f'K1030: VaR/ES Scorecard — {a}', fontsize=12, fontweight='bold')
        ...
        plt.tight_layout()
        chart3 = os.path.join(SCRIPT_DIR, 'k1030_var_es_scorecard.png')
