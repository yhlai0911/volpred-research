#!/usr/bin/env python3
"""
K1030: A4f Sub-Period Robustness Analysis (Paper 9 必備 robustness)
===================================================================
[提出: 賴奕豪, 執行: Claude]
...
SUB_PERIODS = {
    ...
    'P3': ('2017-01-01', '2018-12-31', 'Low vol then vol spike'),
...
# Student-t log-likelihood constant
T_CONST = float(gammaln((STUDENT_T_DF + 1) / 2) - gammaln(STUDENT_T_DF / 2) - 0.5 * np.log(np.pi * (STUDENT_T_DF - 2)))


def gjr_t_loglik_scipy(params, returns, df_val=STUDENT_T_DF):
    ...
    for t in range(n):
        if h[t] > 0:
            ll += T_CONST - 0.5 * np.log(h[t]) - (df_val + 1) / 2 * np.log(1 + returns[t]**2 / (h[t] * (df_val - 2)))
    return -ll
...
def fit_gjr_t(returns):
    ...
    for s in starts:
        try:
            res = optimize.minimize(gjr_t_loglik_scipy, s, args=(returns,),
...
def fit_a4f_t(returns, vix_vals):
    ...
    tau_t = theta0 + theta1 * VIX^2_{t-1}
    g_t follows GJR on de-trended returns, with free omega
    Student-t innovations with df=STUDENT_T_DF
    """
    n = len(returns)
    ...
    def neg_loglik(params):
        ...
        for t in range(1, n):
            # denom = tau_t (current, predetermined from VIX_{t-1})
        ...
        for t in range(n):
            sigma2 = tau[t] * g[t]
            if sigma2 > 0:
                ll += T_CONST - 0.5 * np.log(sigma2) - (STUDENT_T_DF + 1) / 2 * np.log(1 + returns[t]**2 / (sigma2 * (STUDENT_T_DF - 2)))
...
for t_idx, abs_idx in enumerate(oos_indices):
    ...
    if need_refit:
        ...
        # GJR-t
        gjr_params = fit_gjr_t(train_ret)
        ...
        # A4f-t
        a4f_params = fit_a4f_t(train_ret, train_vix)
    ...
    if p is not None:
        ...
        # g recursion: use tau_t (current, predetermined) for denom
        g_prev = a4f_state['g']
        r_prev = ret[abs_idx - 1]
...
results = {
    ...
    'models': ['A4f-VIX2-t(df=8)', 'GJR-t(df=8)'],
    'evaluation': 'QLIKE on r2 (Patton 2011)',
    'sub_periods': {},
    'summary': {},
    'references': [
...
plt.tight_layout()
chart1_path = os.path.join(SCRIPT_DIR, 'k1030_dm_by_period.png')
...
vix_arr_plot = [p['avg_vix'] for p in period_data]
imp_arr_plot = [p['improvement_pct'] for p in period_data]

ax.scatter(vix_arr_plot, imp_arr_plot, s=120, c='steelblue', edgecolors='black', zorder=5)
...
if n_periods >= 3:
    z = np.polyfit(vix_arr_plot, imp_arr_plot, 1)
    x_line = np.linspace(min(vix_arr_plot) - 1, max(vix_arr_plot) + 1, 100)
...
plt.tight_layout()
chart2_path = os.path.join(SCRIPT_DIR, 'k1030_vix_vs_improvement.png')
...
bars2 = ax.bar(x + width/2, a4f_qlikes, width, label='A4f-VIX2-t(8)', color='#2ca02c', alpha=0.8, edgecolor='black', linewidth=0.5)
...
plt.tight_layout()
chart3_path = os.path.join(SCRIPT_DIR, 'k1030_qlike_comparison.png')
...
print("=" * 70)
print(f"  Models: A4f-VIX2-t(df={STUDENT_T_DF}) vs GJR-t(df={STUDENT_T_DF})")
