#!/usr/bin/env python3
"""
K1324: 0050.TW 5-min HAR-RV continuation checkpoint.

Continuation of K1322 with four more 0050.TW intraday files
...
def plot_rv_and_forecasts(daily: pd.DataFrame, feat: pd.DataFrame, actual: np.ndarray, har: np.ndarray, rw: np.ndarray) -> str:
    ...
    out = ROOT / "k1324_rv_forecasts.png"
    fig.savefig(out)
    plt.close(fig)
    return out.name


def load_k1322_baseline() -> dict:
    return json.loads(K1322_RESULTS_PATH.read_text())
...
def main() -> None:
    ...
    results = {
        ...
        "seed": SEED,
        "related_experiments": ["K1307", "K1318", "K1322"],
        "data_source": {
            "intraday_glob": str(SOURCE_DIR.relative_to(PROJECT_ROOT)),
            "symbol_prefix": SYMBOL,
            "daily_rv_snapshot": str((DATA_DIR / "0050_tw_daily_rv_rebuilt.csv").relative_to(PROJECT_ROOT)),
        },
        ...
        "comparison_to_k1322": {
            "prior_n_total_days": int(k1322["n_total_days"]),
            "prior_n_test": int(k1322["n_test"]),
            "delta_total_days": int(len(daily) - k1322["n_total_days"]),
            "delta_test_days": int(n_test - k1322["n_test"]),
            "prior_dm_hln_t": float(k1322["DM_HLN_t"]),
            "current_dm_hln_t": float(dm_t),
            "delta_dm_hln_t": float(dm_t - k1322["DM_HLN_t"]),
            "prior_har_qlike": float(k1322["HAR_QLIKE_test"]),
            "current_har_qlike": har_qlike,
            "delta_har_qlike": float(har_qlike - k1322["HAR_QLIKE_test"]),
            "note": (
                "Only four additional daily files moved DM-HLN t from about 1.82 to 0.99. "
                "This instability itself is evidence that the current sample remains too short "
        ...
        "key_findings": [
            ...
            (
                f"DM-HLN t shifted from {k1322['DM_HLN_t']:.2f} to {dm_t:.2f} after just four new days, "
                "showing strong small-sample fragility."
            ),
        ],
        "artifacts": {
            "figure": fig_name,
