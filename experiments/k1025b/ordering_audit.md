...
### 2.1 先位元級重現原缺陷（before/after 要用「量」的，不是用「說」的）

| 指標 | k1025b 發表值 | 本次重現 | 判定 |
|---|---|---|---|
| rolling `mean_net_btc` | −76.64pp | **−76.62pp** | ✅ 重現 |
| rolling `mean_total` | 90.09% | **90.09%** | ✅ 重現 |
| rolling 視窗數 | 512 | **512** | ✅ |
| VAR 樣本數 | 2,812 | **2,812** | ✅ |

→ 原缺陷在同一份資料上完整重現，因此以下 before/after 是**實測**，不是宣稱。

### 2.2 三組 NET 對照（全樣本）

| 估計量 | TCI | NET_BTC |
|---|---|---|
| k1025b 發表版（誤切 + Cholesky） | 90.09% | **−76.64pp**（「淨接收者」） |
| Cholesky（正確切片）6 種排序全枚舉 | 8.2–12.5% | **−2.60pp ~ +8.96pp**（跨度 11.55pp，**變號**） |
| **KPPS generalized（order-invariant）** | **13.72%** | **+2.70pp** |
