...
!! are WITHDRAWN. `compute_spillover_index` (inherited verbatim from k1025.py) sliced
!! statsmodels' FEVD as `decomp[-1]`, annotating it `(horizon, n_vars, n_vars)`. The real
!! array is `(n_vars, horizon, n_vars)`, so that slice returned the last VARIABLE's table
!! and the forecast horizon was read as assets. The published
!!     spillover_index.mean_total    = 90.09%
!!     spillover_index.mean_net_btc  = -76.64pp   ("BTC is a NET RECEIVER")
!! are artifacts of that mis-slice, reproducible on pure noise.
!!
!! Superseded by `k1025b_v2.py` -> `k1025b_v2_results.json`. Re-estimated on the SAME
!! data with the order-invariant KPPS generalized FEVD:
!!     full-sample BTC net = +2.70pp   (SIGN FLIP; ~28x smaller than -76.64)
!!     rolling 252d mean   = -0.11pp, 65% of windows negative -> NO stable sign
!!     total connectedness = 20.3%     (was 90.09%)
!! Cholesky NET spans 11.6pp across the 6 variable orderings and changes sign, so even a
!! correctly-sliced Cholesky cannot carry the directional claim.
!!
!! NOTE FOR ANYONE CITING THE "5/5 REPLICATION": K1025b was reported as replicating
!! K1025's DY net (-76.64pp vs -76.89pp, "near identical"). Both scripts contain the
!! IDENTICAL mis-slice, so that agreement corroborates a shared BUG, not an effect.
!! Order-invariant, the two panels do not even agree in sign (K1025 v3: -0.95pp;
!! K1025b v2: +2.70pp). Fact (4) of that replication does not survive.
!!
!! The decomposition below is now a real KPPS GFEVD, so RE-RUNNING THIS SCRIPT WILL NOT
!! REPRODUCE the withdrawn spillover fields still stored in `k1025b_results.json` --
!! that file is kept unedited as the historical record of what was published.
!! Granger / asymmetry / quantile / DCC / forecasting never touched the FEVD.
K1025b: Crypto Fear Channel — BTC Vol Spillover to NASDAQ-100 Equity (VXN target)
=================================================================================

Fork of K1025 with VIX → VXN target swap. Replicates same 6-method analysis
on the NASDAQ-100 fear gauge (^VXN) to address the cross-paper meta-evaluation
multi-asset OOS robustness blocker for P10 (paper/crypto-fear-channel).

Variable name `vix` is preserved throughout the script for code-reuse simplicity;
the underlying ticker is ^VXN. All output JSON keys rename `vix` → `vxn`,
experiment_id is set to "1025b", and the README documents the swap explicitly.

Purpose: Multi-asset OOS robustness check for P10 paper claims:
  - If asymmetric Granger / QR sign-reversal / regime-watershed / DY-net-receiver
    findings replicate on BTC→VXN, the family-level spillover claim is supported
    across two equity-fear gauges rather than relying on a single asset.

Methods (mirror K1025):
1. Granger causality: BTC realized vol (20d) → VXN, bidirectional
...
5. DCC correlation: BTC-QQQ dynamic conditional correlation by VXN regime
6. Forecasting test: BTC vol added to AR(VXN) model, DM test

Data: QQQ (NASDAQ-100 ETF), BTC-USD, ^VXN from yfinance, 2015-02-01 to 2026-04-09
References: Diebold & Yilmaz (2012), Engle (2002), Koenker & Bassett (1978)

Author: VolPred Research System (main thread, autonomous platform 2026-04-28)
Seed: 42
"""
...
spy = yf.download('QQQ', start=start_date, end=end_date, progress=False)  # K1025b: QQQ replaces SPY (NASDAQ-100 ETF, paired with VXN)
btc = yf.download('BTC-USD', start=start_date, end=end_date, progress=False)
vix = yf.download('^VXN', start=start_date, end=end_date, progress=False)  # K1025b: ^VXN (NASDAQ-100 fear gauge) replaces ^VIX
...
# VIX ~ alpha + beta * BTC_RV at different quantiles
X_qr = sm.add_constant(btc_rv20.loc[common_idx2])
y_qr = vix_level_aligned.loc[common_idx2]

# Align
common_qr = X_qr.index.intersection(y_qr.index)
X_qr = X_qr.loc[common_qr]
y_qr = y_qr.loc[common_qr]
...
for tau in quantiles:
    ...
    sig = "***" if p_val < 0.01 else "**" if p_val < 0.05 else "*" if p_val < 0.10 else ""
    print(f"  τ={tau:.2f}: β={beta:.4f}, SE={se:.4f}, t={t_stat:.2f}, p={p_val:.4f} {sig}")
...
forecast_data = pd.DataFrame({
    ...
    'BTC_RV': btc_rv20,
    'VIX_lag1': vix_level_aligned.shift(1),
    'VIX_lag2': vix_level_aligned.shift(2),
    'VIX_lag3': vix_level_aligned.shift(3),
    'VIX_lag5': vix_level_aligned.shift(5),
    'BTC_RV_lag1': btc_rv20.shift(1),
    'BTC_RV_lag2': btc_rv20.shift(2),
}).dropna()

# OOS split: 2019-01-01
oos_start = '2019-01-01'
is_data = forecast_data.loc[:oos_start]
oos_data = forecast_data.loc[oos_start:]
...
# Model 1: AR(5) for VIX (baseline)
# Model 2: AR(5) + BTC_RV lags (extended)
from sklearn.linear_model import LinearRegression

# Features
ar_features = ['VIX_lag1', 'VIX_lag2', 'VIX_lag3', 'VIX_lag5']
ext_features = ar_features + ['BTC_RV_lag1', 'BTC_RV_lag2']
...
# Use expanding window starting from IS
min_train = len(is_data)

for i in range(len(oos_data)):
    train_end = min_train + i
    train = forecast_data.iloc[:train_end]
    test_row = forecast_data.iloc[train_end:train_end+1]
...
for period_name, (start, end) in periods.items():
    ...
    if len(sub_data) > 50:
        try:
            gc = grangercausalitytests(sub_data[['VIX', 'BTC_RV']], maxlag=3, verbose=False)
            ...
            sig = "***" if p_val < 0.01 else "**" if p_val < 0.05 else "*" if p_val < 0.10 else ""
            print(f"  {period_name} (n={len(sub_data)}): lag={best_lag}, F={f_stat:.2f}, p={p_val:.4f} {sig}")
        except Exception as e:
...
ses = [qr_results[t]['se'] for t in qr_results.keys()]
...
plt.tight_layout()
chart_path = os.path.join(SCRIPT_DIR, 'k1025b_results.png')
