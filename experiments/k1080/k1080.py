#!/usr/bin/env python3
"""
K1080: A4f Extended History on IWM — Completing US Equity Size Axis
====================================================================
[提出: 用戶 (Claude 執行), 執行: Claude]

Motivation:
  Paper 9 US-equity cross-asset size axis:
    - K1075 SPY (large-cap S&P 500):   DM t=+7.92, Harvey-PASS, theta1 <1 order
    - K1078 QQQ (tech/Nasdaq-100):     DM t=+5.99, Harvey-PASS, theta1 1.91 orders
    - K1077 0050.TW (Taiwan ETF):      DM t=-0.49, NS, theta1 ~4 orders (unstable)

  IWM (Russell 2000 small-cap) is the missing rung. This experiment completes
  the "size axis" (large -> tech -> small-cap) within US equity ETFs. Small-cap
  has:
    - Higher idiosyncratic/noise component than SPX
    - Lower liquidity at tails
    - VIX (SPX IV) may be a less-matched driver than RVX (Russell 2000 IV)
    - Higher unconditional volatility (~22% vs SPY ~16%)

  If A4f wins for IWM, Paper 9 claim strengthens to "A4f works across US equity
  size spectrum"; if fails, we bound the claim.

Design (strict parity with K1075 / K1078):
  - Three non-overlapping OOS windows (2007-2012, 2013-2018, 2019-2026)
  - Rolling-window GARCH with 2000-day training, 63-day refit
  - Two models: GJR baseline vs A4f (tau = theta0 + theta1 * VIX^2_{t-1},
    g = GJR, free omega_g)
  - Crisis sub-period analysis: GFC, Euro, COVID, 2022 Bear
  - VIX bucket analysis: Low/Normal/High/Extreme/Crisis

  IWM IPO 2000-05-26. At 2007-01-01 there are ~1680 observations (vs
  WINDOW=2000 requested). The rolling-window code uses
  `max(0, abs_idx - WINDOW)` so first refit uses all available history -- same
  behavior as K1075 SPY (1758 obs at 2007 start) and K1078 QQQ (1965 obs).

  RVX (Russell 2000 VIX): yfinance provides 'RVX' ticker covering 2004-01 to
  2021-10 (4448 days). This ends in 2021 so cannot be used across the full
  2007-2026 OOS. We report the primary A4f-VIX analysis as the main result.
  A supplementary A4f-RVX vs A4f-VIX comparison is run over 2007-2021 where
  both are available, reusing the IV-family substitutability framework of
  K1079 (VIX vs VXN). If RVX comes back missing we skip the supplement.

Hypotheses:
  H1: IWM full OOS 2007-2026 A4f vs GJR DM Harvey-PASS (|t|>3)
  H2: IWM GFC sub-period A4f improves over GJR
  H3: All 3 OOS windows A4f wins directionally
  H4: A4f does NOT break down at extreme VIX (>40)
  H5: IWM theta1 stability intermediate between SPY and 0050.TW
      (prediction: 2.0-3.0 orders span, between QQQ 1.91 and 0050.TW ~4)
  H6 (supplementary, if RVX available): A4f-RVX marginally better than
      A4f-VIX for IWM (RVX is the native IV for Russell 2000)

Data: yfinance IWM + ^VIX, 2000-05-01 ~ 2026-04-12 (primary)
      yfinance RVX, 2004-01 ~ 2021-10 (supplementary)
Evaluation: QLIKE on r^2 (Patton 2011), DM test (Harvey 2016)
...
RESULTS_PATH = os.path.join(SCRIPT_DIR, 'k1080_results.json')

# Configuration
DATA_START = '2000-05-01'  # IWM IPO 2000-05-26
DATA_END = '2026-04-12'
WINDOW = 2000
...
# Reference result paths for four-asset comparison
...
K1078_RESULTS_PATH = os.path.abspath(os.path.join(SCRIPT_DIR, '..', 'k1078', 'k1078_results.json'))

print("=" * 70)
print(f"{EXPERIMENT_ID}: A4f Extended History on IWM (2007-2026)")
print(f"  3 OOS windows, 4 crisis sub-periods, 5 VIX buckets")
print(f"  Only 2 models: GJR vs A4f (VIX^2 free omega)")
print(f"  Strict parity with K1075 (SPY) / K1078 (QQQ)")
print("=" * 70)

# ============================================================
# SECTION 1: DATA LOADING
# ============================================================
print("\n[1] Loading IWM + ^VIX from yfinance...")
import yfinance as yf

raw = yf.download('IWM', start=DATA_START, end=DATA_END, progress=False, auto_adjust=False)
...
n_total = len(df)
print(f"  IWM joined data: {df.index[0].strftime('%Y-%m-%d')} to "
...
# Optional: load RVX for supplementary
print("\n[1b] Loading RVX (Russell 2000 VIX) for supplementary analysis...")
rvx_available = False
rvx_series = None
try:
    rvx_raw = yf.download('RVX', start='2004-01-01', end=DATA_END, progress=False, auto_adjust=False)
    if isinstance(rvx_raw.columns, pd.MultiIndex):
        rvx_raw.columns = rvx_raw.columns.get_level_values(0)
    rvx_close = rvx_raw['Close'].copy() if len(rvx_raw) > 0 else None
    if rvx_close is not None and len(rvx_close) > 500:
        rvx_series = rvx_close
        rvx_available = True
        print(f"  RVX loaded: {rvx_close.index[0].strftime('%Y-%m-%d')} to "
              f"{rvx_close.index[-1].strftime('%Y-%m-%d')}, n={len(rvx_close)}")
    else:
        print(f"  RVX insufficient, skipping supplementary")
except Exception as e:
    print(f"  RVX load failed: {e}, skipping supplementary")
...
def fit_a4f(returns, vol_vals):
    ...
    A4f specification (K988 winner):
      tau_t = max(theta0 + theta1 * VOL_{t-1}^2, eps)
    ...
    `vol_vals` is the IV series (VIX or RVX).
    """
    n = len(returns)
    vol_lag = np.empty(n)
...
# ============================================================
# SECTION 5b: SUPPLEMENTARY — A4f-RVX vs A4f-VIX on IWM (2007-2021)
# ============================================================
rvx_supplement = None
if rvx_available and rvx_series is not None:
    print("\n[5b] Supplementary: A4f-RVX vs A4f-VIX on IWM (2007-2021)...")
    try:
        # Join IWM + VIX + RVX
        df_rvx = pd.DataFrame({
            'price': prices, 'log_ret': log_ret,
            'VIX': vix_close, 'RVX': rvx_series,
        }).dropna()
        sup_start = '2007-01-01'
        sup_end = '2021-09-30'  # RVX ends 2021-10-01 approx
        df_sup = df_rvx[(df_rvx.index >= sup_start) & (df_rvx.index <= sup_end)].copy()
        n_sup = len(df_sup)
        print(f"  Joint IWM+VIX+RVX period: {df_sup.index[0].strftime('%Y-%m-%d')} to "
              f"{df_sup.index[-1].strftime('%Y-%m-%d')}, n={n_sup}")

        if n_sup < 500:
            print("  Insufficient joint data for supplement, skipping.")
        else:
            # Simpler single-window OOS: 50/50 split
            split_idx = n_sup // 2
            train_r = df_sup['log_ret'].values[:split_idx]
            train_v = df_sup['VIX'].values[:split_idx]
            train_x = df_sup['RVX'].values[:split_idx]

            # In-sample fits + static forecasts (simple static comparison of fit quality)
            vix_p, vix_c = fit_a4f(train_r, train_v)
            rvx_p, rvx_c = fit_a4f(train_r, train_x)

            print(f"  A4f-VIX fit: theta1={vix_p[1]:.3e}, conv={vix_c}")
            print(f"  A4f-RVX fit: theta1={rvx_p[1]:.3e}, conv={rvx_c}")

            # Out-of-sample rolling forecast (simple: reuse train params, forecast OOS)
            oos_r = df_sup['log_ret'].values[split_idx:]
            oos_v = df_sup['VIX'].values[split_idx:]
            oos_x = df_sup['RVX'].values[split_idx:]
            n_oos_sup = len(oos_r)

            def static_oos_a4f(params, ret_arr, vol_arr_lag):
                """Walk forward using fixed params; evaluate 1-step forecasts."""
                theta0, theta1, omega_g, alpha, gamma_p, beta_p = params
                persist = alpha + gamma_p/2.0 + beta_p
                g = omega_g / (1.0 - persist)
                fc = np.empty(len(ret_arr))
                for t in range(len(ret_arr)):
                    v_prev = vol_arr_lag[t]
                    tau_t = max(theta0 + theta1 * v_prev**2, 1e-16)
                    if t == 0:
                        r_prev_ = ret_arr[0]  # approximation
                    else:
                        r_prev_ = ret_arr[t-1]
                    u_prev = r_prev_ / np.sqrt(tau_t)
                    asym = gamma_p * u_prev**2 if u_prev < 0 else 0.0
                    g_new = omega_g + alpha * u_prev**2 + asym + beta_p * g
                    g_new = max(g_new, 1e-10)
                    fc[t] = tau_t * g_new
                    g = g_new
                return fc

            # lagged vol at time t is vol at t-1
            vix_lag_oos = np.concatenate(([train_v[-1]], oos_v[:-1]))
            rvx_lag_oos = np.concatenate(([train_x[-1]], oos_x[:-1]))

            fc_vix = static_oos_a4f(vix_p, oos_r, vix_lag_oos)
            fc_rvx = static_oos_a4f(rvx_p, oos_r, rvx_lag_oos)

            r2_sup = oos_r ** 2
            valid_sup = (fc_vix > 0) & (fc_rvx > 0)

            ql_vix = float(np.mean(qlike_loss(fc_vix[valid_sup], r2_sup[valid_sup])))
            ql_rvx = float(np.mean(qlike_loss(fc_rvx[valid_sup], r2_sup[valid_sup])))

            d_sup = qlike_loss(fc_vix[valid_sup], r2_sup[valid_sup]) - \
                    qlike_loss(fc_rvx[valid_sup], r2_sup[valid_sup])
            dm_t_sup, dm_p_sup, _ = hac_dm_test(d_sup)

            print(f"  OOS n={valid_sup.sum()}, QLIKE VIX={ql_vix:.6f}, RVX={ql_rvx:.6f}")
            print(f"  DM(VIX vs RVX) t={dm_t_sup:+.3f}, p={dm_p_sup:.4f}")

            rvx_supplement = {
                'period': f'{sup_start} to {sup_end}',
                'n_joint': int(n_sup),
                'n_oos': int(valid_sup.sum()),
                'theta1_vix': float(vix_p[1]),
                'theta1_rvx': float(rvx_p[1]),
                'qlike_a4f_vix': ql_vix,
                'qlike_a4f_rvx': ql_rvx,
                'rvx_minus_vix_pct': (ql_rvx - ql_vix) / abs(ql_vix) * 100,
                'dm_t_vix_vs_rvx': float(dm_t_sup) if np.isfinite(dm_t_sup) else None,
                'dm_p': float(dm_p_sup) if np.isfinite(dm_p_sup) else None,
                'rvx_better': bool(ql_rvx < ql_vix),
                'harvey_different': bool(abs(dm_t_sup) > 3.0) if np.isfinite(dm_t_sup) else False,
            }
            results['rvx_supplement'] = rvx_supplement
    except Exception as e:
        print(f"  RVX supplement error: {e}")
        import traceback
        traceback.print_exc()
...
print(f"  H1 (IWM Full OOS A4f > GJR, DM t>3): {h1_verdict} "
...
print(f"  H2 (IWM GFC A4f improves): {h2_verdict} "
...
iwm_orders = results.get('theta1_stability', {}).get('orders_of_magnitude_span')
h5_verdict = 'N/A'
if iwm_orders is not None:
    if iwm_orders <= 2.0:
...
print(f"  H5 (theta1 stability): {h5_verdict} "
      f"(orders span = {iwm_orders:.2f})" if iwm_orders is not None else "  H5: N/A")

# H6 — supplementary RVX vs VIX
if rvx_supplement is not None:
    rvx_diff = rvx_supplement.get('rvx_minus_vix_pct')
    if rvx_diff is not None and rvx_diff < 0:
        h6_verdict = 'PASS' if rvx_supplement.get('harvey_different') else 'DIRECTIONAL'
    else:
        h6_verdict = 'FAIL'
    print(f"  H6 (RVX marginally better than VIX for IWM): {h6_verdict} "
          f"(RVX-VIX = {rvx_diff:+.2f}%, DM t={rvx_supplement.get('dm_t_vix_vs_rvx'):+.3f})")
else:
    h6_verdict = 'N/A'
    print(f"  H6: N/A (RVX data unavailable)")
...
results['hypothesis_verdicts'] = {
    ...
    'H6_rvx_vs_vix': h6_verdict,
}

# ============================================================
# SECTION 7: FOUR-ASSET COMPARISON
# ============================================================
print("\n[7] Four-asset comparison (SPY K1075 / QQQ K1078 / IWM K1080 / 0050.TW K1077)...")

four_asset = {}
try:
    with open(K1075_RESULTS_PATH) as f:
        spy_res = json.load(f)
    four_asset['SPY_K1075'] = {
        ...
        'theta1_orders': spy_res.get('theta1_stability', {}).get('orders_of_magnitude_span'),
    }
except Exception as e:
    print(f"  Warning: cannot load K1075 ({e})")
    four_asset['SPY_K1075'] = None

try:
    with open(K1078_RESULTS_PATH) as f:
        qqq_res = json.load(f)
    four_asset['QQQ_K1078'] = {
        'full_dm': qqq_res['full_oos']['dm_t'],
        'full_diff_pct': qqq_res['full_oos']['qlike_diff_pct'],
        'harvey_pass': qqq_res['full_oos']['harvey_pass'],
        'per_window': {k: {'dm': v['dm_t'], 'diff_pct': v['qlike_diff_pct']}
                       for k, v in qqq_res['per_window'].items()},
        'gfc': {'dm': qqq_res['crisis_subperiods'].get('GFC', {}).get('dm_t'),
                'diff_pct': qqq_res['crisis_subperiods'].get('GFC', {}).get('qlike_diff_pct')},
        'theta1_orders': qqq_res.get('theta1_stability', {}).get('orders_of_magnitude_span'),
    }
except Exception as e:
    print(f"  Warning: cannot load K1078 ({e})")
    four_asset['QQQ_K1078'] = None
...
try:
    ...
    four_asset['TW_K1077'] = {
        ...
        'theta1_orders': tw_res.get('theta1_stability', {}).get('orders_of_magnitude_span'),
    }
except Exception as e:
    print(f"  Warning: cannot load K1077 ({e})")
    four_asset['TW_K1077'] = None

four_asset['IWM_K1080'] = {
    ...
    'theta1_orders': results.get('theta1_stability', {}).get('orders_of_magnitude_span'),
}
results['four_asset_comparison'] = four_asset

print(f"  {'Asset':<14} {'Full DM':>10} {'Full Diff%':>12} {'Harvey':>8} {'θ1 span':>10}")
for label, r in four_asset.items():
    if r is None:
        continue
    orders_str = f"{r.get('theta1_orders'):.2f}" if r.get('theta1_orders') is not None else 'N/A'
    print(f"  {label:<14} {r['full_dm']:>+10.3f} {r['full_diff_pct']:>+11.2f}% "
          f"{'PASS' if r['harvey_pass'] else 'FAIL':>8} {orders_str:>10}")
...
results['metadata'] = {
    ...
    'asset': 'IWM',
    'asset_description': 'iShares Russell 2000 ETF (small-cap)',
    ...
    'notes': (
        'IWM IPO 2000-05-26, so at 2007-01-01 start there are ~1680 training obs '
        ...
        'at 2007 start) and K1078 QQQ (1965 obs). RVX (Russell 2000 VIX) available '
        'from yfinance only 2004-01 to 2021-10, so primary analysis uses VIX '
        'throughout for consistency with K1075/K1078; a 2007-2021 supplementary '
        'compares A4f-VIX vs A4f-RVX on the joint sample.'
    ),
    'rvx_supplement_available': rvx_available,
}
...
try:
    ...
    # --- Plot 1: Extended DM — 3 OOS windows ---
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    win_names = [n for n, s, e in OOS_WINDOWS]
    ql_gjrs = [results['per_window'][n]['qlike_gjr'] for n in win_names if n in results['per_window']]
    ql_a4fs = [results['per_window'][n]['qlike_a4f'] for n in win_names if n in results['per_window']]
    dm_ts = [results['per_window'][n]['dm_t'] for n in win_names if n in results['per_window']]
    ...
    ax1.set_title(f'{EXPERIMENT_ID} IWM: QLIKE by OOS Window')
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    colors = ['green' if abs(t) > 3.0 else ('orange' if abs(t) > 1.96 else 'gray') for t in dm_ts]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1080_extended_dm.png'), dpi=120)
    plt.close()
    print("    k1080_extended_dm.png")
    ...
    ax.set_title(f'{EXPERIMENT_ID} IWM: A4f vs GJR across Crisis Sub-periods\n'
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1080_crisis_periods.png'), dpi=120)
    plt.close()
    print("    k1080_crisis_periods.png")
    ...
    ax.set_title(f'{EXPERIMENT_ID} IWM: A4f Performance Across VIX Regimes (OOS 2007-2026)')
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1080_vix_bucket.png'), dpi=120)
    plt.close()
    print("    k1080_vix_bucket.png")
    ...
    ax.set_yscale('log')
    ax.set_title(f'{EXPERIMENT_ID} IWM: A4f θ₁ Evolution across Rolling Refits (2007-2026)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1080_theta1_evolution.png'), dpi=120)
    plt.close()
    print("    k1080_theta1_evolution.png")

    # --- Plot 5: Four-asset comparison ---
    ...
    dms = []
    bar_colors = []
    for label, r in [('SPY (K1075)', four_asset.get('SPY_K1075')),
                     ('QQQ (K1078)', four_asset.get('QQQ_K1078')),
                     ('IWM (K1080)', four_asset.get('IWM_K1080')),
                     ('0050.TW (K1077)', four_asset.get('TW_K1077'))]:
        if r is None:
            continue
        assets.append(label)
    ...
    ax.set_title('Cross-Asset Size Axis: Full OOS DM')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    order_labels = []
    order_vals = []
    for label, r in [('SPY', four_asset.get('SPY_K1075')),
                     ('QQQ', four_asset.get('QQQ_K1078')),
                     ('IWM', four_asset.get('IWM_K1080')),
                     ('0050.TW', four_asset.get('TW_K1077'))]:
        if r is None or r.get('theta1_orders') is None:
            continue
        order_labels.append(label)
        order_vals.append(r['theta1_orders'])
    if order_vals:
        color_o = ['green' if v <= 2.0 else ('orange' if v <= 3.5 else 'red') for v in order_vals]
        ax.bar(order_labels, order_vals, color=color_o, alpha=0.75)
        ax.axhline(2.0, color='green', linestyle='--', alpha=0.4, label='Stable (≤2)')
        ax.axhline(3.5, color='red', linestyle='--', alpha=0.4, label='Unstable (>3.5)')
        ax.set_ylabel('θ₁ orders-of-magnitude span')
        ax.set_title('θ₁ Stability: Lower = more stable')
        ax.legend()
        ax.grid(True, alpha=0.3)
        for i, v in enumerate(order_vals):
            ax.text(i, v + 0.08, f'{v:.2f}', ha='center', fontsize=10, fontweight='bold')
    else:
        ax.text(0.5, 0.5, 'theta1 orders data unavailable', ha='center', va='center', transform=ax.transAxes)
        ax.axis('off')

    # 5c summary text panel
    ax = axes[2]
    ax.axis('off')
    lines = ['Paper 9 US-Equity Size Axis Summary\n']
    lines.append(f"{'Asset':<14}{'DM':>8}{'Diff%':>9}{'Harvey':>8}{'θ1span':>9}\n")
    lines.append('-' * 50 + '\n')
    for label, r in [('SPY K1075', four_asset.get('SPY_K1075')),
                     ('QQQ K1078', four_asset.get('QQQ_K1078')),
                     ('IWM K1080', four_asset.get('IWM_K1080')),
                     ('TW K1077', four_asset.get('TW_K1077'))]:
        if r is None:
            lines.append(f"{label:<14}{'N/A':>8}\n")
            continue
        orders_s = f"{r['theta1_orders']:.2f}" if r.get('theta1_orders') is not None else 'N/A'
        lines.append(f"{label:<14}{r['full_dm']:>+8.2f}"
                     f"{r['full_diff_pct']:>+8.2f}%"
                     f"{'P' if r['harvey_pass'] else 'F':>8}"
                     f"{orders_s:>9}\n")
    lines.append('\nGFC Sub-period:\n')
    for label, r in [('SPY', four_asset.get('SPY_K1075')),
                     ('QQQ', four_asset.get('QQQ_K1078')),
        ...
        if r is None:
            continue
        gfc = r.get('gfc', {})
        if gfc.get('dm') is not None:
            lines.append(f"  {label:<4} DM = {gfc['dm']:+.3f}, "
                         f"diff {gfc['diff_pct']:+.2f}%\n")
    if rvx_supplement:
        lines.append('\nRVX vs VIX (2007-2021):\n')
        lines.append(f"  diff = {rvx_supplement['rvx_minus_vix_pct']:+.2f}% "
                     f"DM {rvx_supplement['dm_t_vix_vs_rvx']:+.3f}\n")
    ax.text(0, 1, ''.join(lines), family='monospace', fontsize=10,
            verticalalignment='top')
    ax.set_title('Summary')

    plt.suptitle(f'{EXPERIMENT_ID}: US-Equity Size Axis + 0050.TW A4f Comparison',
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1080_four_asset_comparison.png'),
                dpi=120, bbox_inches='tight')
    plt.close()
    print("    k1080_four_asset_comparison.png")
