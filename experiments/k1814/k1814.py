...
def main() -> None:
    ...
    if not args.skip_ablations:
        ...
        # Name the ablations still outstanding, mirroring the robustness loop above: a
        # partial artifact cut mid-loop must say what is missing, not report `[]` and read
        # as complete.
        pending_ablations = list(specs)
        for name, (over, mdls) in specs.items():
            ...
            pending_ablations.remove(name)
            checkpoint(f"ablation:{name}",
                       [f"ablation not yet run: {n}" for n in pending_ablations])
