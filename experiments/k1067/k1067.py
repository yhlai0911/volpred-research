...
Motivation / Big question:
  K1064 tested A4f+EAV vs A4f on the 0050.TW ETF with three weighting schemes
  (equal, sector, top50). Every variant returned NULL (DM t=+1.08/+0.96/+2.36
  with A4f baseline *winning* every time, θ₂<0 across refits with p>0.15).

  Two hypotheses explain the null:
    (H_div)  Diversification dilution — 50 constituents' idiosyncratic earnings
             news cancel out when value-weighted into the ETF.
    (H_vix)  VIX absorption — VIX already captures all systematic uncertainty
             and EAV carries zero incremental information once VIX² is in τ.

  A single-stock test cleanly separates these. If we restrict to one company
  (TSMC, 2330.TW) and *still* see NULL, H_vix dominates (EAV provides no
  predictive increment even without diversification). If A4f+EAV is DM-significant
  on TSMC, H_div dominates (the signal exists, ETF aggregation just washed it out).

Hypotheses:
  H1 (main): A4f+EAV delivers DM-significant improvement over A4f baseline
             on TSMC (Harvey |t|>3.0) in OOS r² QLIKE.
  ...
  H4 (key):  TSMC A4f+EAV improvement percentage strictly exceeds K1064 sector-
             weighted variant improvement on 0050.TW ETF, evidencing
             diversification wash-out.

Model specification (no-lookahead, matches K1064/K1058):
  τ_{t+1} = max(θ₀ + θ₁·VIX²_t + θ₂·EAV_signal_t, ε)  (all info ≤ t close)
  u_t    = r_t / √τ_{t+1}
  g_t    = ω_g + α·u_{t-1}² + γ·u_{t-1}²·I(u<0) + β·g_{t-1}
  σ²_{t+1} = τ_{t+1} · g_{t+1}

  EAV_signal_t = 1 if day t is a TSMC earnings announcement (post-close), else 0.
...
Data:
  - 2330.TW daily OHLC (yfinance, auto_adjust=True; TSMC is an individual stock
    → no 0050.TW ETF split artifact; auto_adjust handles splits/dividends).
  - ^VIX daily (yfinance, Close).
  - 財報公告日.txt (Big5, filter code == '2330').
...
Evaluation:
  ...
  - θ₂ one-sample t-test across refits + 2000-rep bootstrap 95% CI
  - Event (T+1) vs non-event conditional DM
  - Robustness: EAV_t-1 (default) vs EAV_{t-2} vs rolling-3d
  - Sub-period stability (5 equal sub-periods)
...
References:
  ...
  - K1064: ETF A4f+EAV — all three variants NULL (DM t=+1.08/+0.96/+2.36).
...
RESULTS_PATH = SCRIPT_DIR / 'k1067_results.json'

# Configuration
TICKER = '2330.TW'
TICKER_CODE = '2330'   # for matching 財報公告日.txt
...
# K1064 reference (0050.TW ETF) for H4 comparison
K1064_SECTOR_IMPROVEMENT_PCT = -0.207   # sector-weighted variant vs A4f
K1064_TOP50_IMPROVEMENT_PCT = -0.419
K1064_EQUAL_IMPROVEMENT_PCT = -0.205

print("=" * 72)
print(f"{EXPERIMENT_ID}: TSMC single-stock A4f-EAV — diversification vs VIX absorption")
print("=" * 72)

# ==========================================================================
# SECTION 1: LOAD TSMC EARNINGS ANNOUNCEMENTS
# ==========================================================================
print("\n[1] Loading TSMC (2330) earnings announcements...")
...
# ==========================================================================
# SECTION 3: BUILD TSMC-SPECIFIC BINARY EAV SIGNAL
# ==========================================================================
print("\n[3] Building TSMC-specific EAV signal...")

trading_days = df.index
eav_binary = np.zeros(len(trading_days), dtype=float)

# Map each TSMC announcement to the *first trading day >= announcement date*
...
if event_mask_full.sum() > 5:
    ...
    print(f"  Full-sample r²_event/r²_nonevent (T+0): {r2_ratio_t0:.3f} "
          f"(K1060 TSMC T+0=0.75, T+1=0.98)")
...
results = {
    ...
    'title': 'TSMC (2330.TW) Single-Stock A4f-EAV — Diversification vs VIX absorption',
    'proposer': '賴奕豪',
    'executor': 'Claude',
    'asset': TICKER,
    'external_regressors': ['US_VIX', 'TSMC_EAV_binary'],
...
# ==========================================================================
# SECTION 12: H4 — DIVERSIFICATION COMPARISON vs K1064 ETF
# ==========================================================================
print("\n[12] H4 — TSMC improvement vs K1064 ETF improvement...")

tsmc_improvement = results['dm_tests'].get('A4f_EAV_vs_A4f', {}).get('qlike_improvement_pct', np.nan)
etf_improvement_best = max(K1064_SECTOR_IMPROVEMENT_PCT, K1064_EQUAL_IMPROVEMENT_PCT, K1064_TOP50_IMPROVEMENT_PCT)
# (best = least negative; the ETF results are all ≤ 0, so "best" is the closest to 0)

h4_verdict = 'PASS' if tsmc_improvement > etf_improvement_best else 'FAIL'
results['h4_diversification_comparison'] = {
    'tsmc_improvement_pct': tsmc_improvement,
    ...
    'etf_best_improvement_pct_k1064': etf_improvement_best,
    'tsmc_strictly_better_than_etf_best': bool(tsmc_improvement > etf_improvement_best),
    'h4_verdict': h4_verdict,
}
print(f"  TSMC improvement: {tsmc_improvement:+.3f}%")
print(f"  K1064 ETF best (closest to 0): {etf_improvement_best:+.3f}%")
...
results['metadata'] = {
    ...
    'references': [
        ...
        'K1060: Per-stock EAV (TSMC T+1 ratio=0.983, overall T+1=1.466).',
        'K1064: ETF A4f+EAV — all NULL (DM t=+1.08/+0.96/+2.36).',
    ],
    'notes': [
        'TSMC 2330.TW uses yfinance auto_adjust=True (individual stock, not ETF).',
        ...
        'K1060 showed TSMC T+1 ratio=0.983 (weakest among 10 stocks tested); '
        'this experiment tests whether even that weak signal can be extracted '
        'via a parametric A4f-EAV model (vs. ETF-level aggregation which was NULL in K1064).',
    ],
}
...
# --- Chart 1: DM comparison (TSMC A4f+EAV vs A4f, with K1064 ETF reference) ---
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# Left: QLIKE bar (TSMC vs ETF ref)
ax = axes[0]
bar_labels = ['A4f (TSMC)', 'A4f+EAV (TSMC)']
...
ax.set_title(f'QLIKE on r² — TSMC OOS ({OOS_START}+)')
for bar, val in zip(bars, bar_qlikes):
    if np.isfinite(val):
        ax.text(bar.get_width() * 1.001, bar.get_y() + bar.get_height() / 2,
                f'{val:.5f}', va='center', fontsize=9)

# Right: DM t-stat comparison TSMC vs ETF variants (H4)
ax = axes[1]
comp_labels = ['TSMC\nbinary', 'ETF\nequal (K1064)', 'ETF\nsector (K1064)', 'ETF\ntop50 (K1064)']
tsmc_t = results['dm_tests'].get('A4f_EAV_vs_A4f', {}).get('dm_t', np.nan)
# K1064 DM t-stats (from prompt / results)
comp_ts = [tsmc_t, 1.082, 0.959, 2.360]
...
ax.set_title('DM Tests: TSMC vs K1064 ETF variants\n(all A4f+EAV vs A4f baseline)')
...
plt.tight_layout()
chart1_path = SCRIPT_DIR / 'k1067_dm_comparison.png'
...
if labels_ev:
    ...
    ax.set_title('TSMC A4f+EAV: Event (T+1) vs Non-event days')
...
plt.tight_layout()
chart2_path = SCRIPT_DIR / 'k1067_event_window_analysis.png'
...
if a4f_eav_param_history:
    ...
    ax.set_title(r'TSMC A4f+EAV: $\theta_2$ Evolution')
...
if n_theta2 >= 2:
    ...
    ax.set_xticks([0])
    ax.set_xticklabels(['TSMC A4f+EAV'])
    ...
    ax.legend()
plt.tight_layout()
chart3_path = SCRIPT_DIR / 'k1067_theta2_evolution.png'
...
print(f"  H4 (TSMC > ETF best): {h4_verdict_s}")

# Research interpretation
print("\n  ===== Interpretation =====")
if h1_verdict == 'PASS':
    print("  → H_div DOMINATES: diversification dilution was the cause of K1064 NULL.")
    print("    Sector-weighted EAV index may work for Paper 2.")
elif h1_verdict == 'FAIL' and h4_verdict_s == 'PASS':
    print("  → MIXED: EAV carries some signal (TSMC > ETF) but insufficient for DM significance.")
    print("    Paper 2 should report descriptive association only.")
else:
    print("  → H_vix DOMINATES: VIX absorbs all earnings-related uncertainty.")
    print("    EAV provides no incremental prediction even at single-stock level.")
    print("    Paper 2 should NOT include EAV as A4f regressor.")
