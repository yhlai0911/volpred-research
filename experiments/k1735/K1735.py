...
# 20 reps pins a rejection rate only to about +/-0.08, which is too loose a guarantee for
# two of the three tests the verdict rests on. 100 reps halves that.
CALIB_REPS = 100
CALIB_DAYS = 800
CALIB_PERM = 400
...
def cross_grid_consistency(cells: dict) -> dict[str, Any]:
    ...
    for cell, grids in cells.items():
        by_bins: dict[int, list[tuple[str, dict]]] = {}
        for name, res in grids.items():
            by_bins.setdefault(res["n_bins"], []).append((name, res))
        for n_bins, members in by_bins.items():
            if len(members) < 2:
                continue
            members.sort(key=lambda kv: -kv[1]["m_sub_returns"])
            (na, ra), (nb, rb) = members[0], members[1]
            va = ra["components"]["v_stoch_raw"]
            vb = rb["components"]["v_stoch_raw"]
            ratio = vb / va if va else None
            out[f"{cell}/{n_bins}bins"] = {
                "grid_a": na,
                "grid_b": nb,
                "m_a": ra["m_sub_returns"],
                "m_b": rb["m_sub_returns"],
                "v_stoch_a": va,
                "v_stoch_b": vb,
                "ratio_b_over_a": ratio,
                "consistent_within_2x": bool(
                    va > 0 and vb > 0 and ratio is not None and 0.5 <= ratio <= 2.0
                ),
                "sigma2e_over_floor_a": ra["components"]["sigma2_e_over_floor"],
                "sigma2e_over_floor_b": rb["components"]["sigma2_e_over_floor"],
                "interpretation": (
                    "grid_b uses coarser sub-returns and so carries less microstructure "
                    "noise; agreement means the residual variance is not a noise artefact"
                    if rb["m_sub_returns"] > 1
                    else "grid_b is the m=1 grid, where price discreteness invalidates the "
                    "Gaussian floor -- disagreement here is expected and is exactly why "
                    "m=1 was pre-declared diagnostic-only"
                ),
            }
    return out
...
def _wilson(p_hat: float, n: int, z: float = 1.96) -> list[float]:
    """Wilson score interval -- a rejection rate of 0/n is not evidence of size 0."""
    denom = 1 + z**2 / n
...
def calibration_check(
    ...
    for tag, df in (("gaussian", None), ("student_t_df5", 5.0)):
        ...
        r1 = float(np.mean(np.array(p1) < 0.05))
        r2 = float(np.mean(np.array(p2) < 0.05))
        out[tag] = {
            "reps": CALIB_REPS,
            "v_stoch_mean": float(np.mean(v_stoch)),
            "v_stoch_max": float(np.max(v_stoch)),
            "v_stoch_relative_to_floor": float(np.mean(v_stoch) / floor),
            "T1_reject_rate_at_05": r1,
            "T1_reject_rate_wilson95": _wilson(r1, CALIB_REPS),
            "T2_reject_rate_at_05": r2,
            "T2_reject_rate_wilson95": _wilson(r2, CALIB_REPS),
        }
    g = out["gaussian"]
    ...
    out["gate_rule"] = (
        f"Gaussian H0: |mean V_stoch| < 5% of the noise floor AND T1/T2 rejection at "
        f"alpha=0.05 no worse than 0.20 across {CALIB_REPS} reps. The Student-t row is "
        "not gated; it quantifies how much fat tails alone inflate V_stoch, which is "
        "exactly why the conservative floor exists."
    )
    return out
