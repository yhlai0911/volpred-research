...
!! forecast horizon was read as assets. v2 inherited the defect from v1 unchanged: the
!! published total spillover 90.1% and BTC net -74.4pp are artifacts, reproducible on
!! pure noise. `results.fevd()` is additionally the CHOLESKY decomposition, whose NET
!! depends on variable ordering (K865b class defect).
!!
!! Superseded by `k1025_v3.py` -> `k1025_v3_results.json` (pinned snapshot, KPPS
!! generalized FEVD): TCI 19.52%, BTC net -0.95pp.
!! The decomposition below is now a real KPPS GFEVD, so RE-RUNNING THIS SCRIPT WILL NOT
!! REPRODUCE the withdrawn spillover fields still stored in `k1025_v2_results.json` --
!! that file is kept unedited as the historical record of what was published, and is
!! the source cited by paper/crypto-fear-channel main.tex (which needs the same
!! correction; see experiments/k1025b/ordering_audit.md).
