# K1696 — Term-spread realized volatility as leading indicator for SPY/HYG/IWM RV
...
**ID migration (2026-07-12)**: originally committed as `K1544`; renamed to
`K1696` after a governance audit found that the unrelated PRG fair-information
experiment already occupied the K1544 research identity. No code, sample,
estimate, figure data, or verdict changed in this migration. The figures were
label-only re-rendered from the original result commit `4ba6d9503`, rather than
from later-extended local CSVs.
...
| K | Topic | Difference from K1696 |
|---|-------|----------------------|
| K1054 | MOVE-vs-VIX rate-cut spillover | K1696 asks: does *spread vol* add anything ON TOP of MOVE? Orthogonal predictor axis. |
| K1488 | MOVE leadingness for SPY vol | K1488 establishes MOVE → SPY-vol; K1696 nests TSV inside MOVE-based model to test marginal contribution. |
...
experiments/K1696/
├── README.md                  # this file
├── K1696.py                   # full reproducible script
├── K1696_results.json         # all estimates, t-stats, DM stats, sample sizes, verdict
├── fig_tsv_timeseries.png     # term-spread vol time series + MOVE
└── fig_dm_heatmap.png         # DM-HLN heatmap (5 comparisons × 9 cells)
```

Reproduce: `uv run python experiments/K1696/K1696.py` (deterministic, seed=42).
