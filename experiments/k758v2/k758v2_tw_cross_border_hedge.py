"""
K758v2: Taiwan Investor Cross-Border Hedging — RERUN with clean_tw50_data
==========================================================================

This is a rerun of K758 using the clean_tw50_data utility to fix the
0050.TW stock split artifact (pre-2014 prices ~4x too high in yfinance).

Original K758 conclusion: "Don't hedge FX" — USD provides natural crisis
cushioning, and hedging costs ~3-4%/yr for retail investors.

Key question: Does "don't hedge FX" conclusion survive with clean 0050.TW data?
...
if n_changed > 0:
    # Show the breakpoint area
    bp = pd.Timestamp("2014-01-02")
    if bp in raw_0050_before.index:
        pre_date = raw_0050_before.index[raw_0050_before.index < bp][-1]
        print(f"  Before cleaning: {pre_date.date()}={raw_0050_before.loc[pre_date]:.2f}, "
              f"2014-01-02={raw_0050_before.loc[bp]:.2f}")
        print(f"  After cleaning:  {pre_date.date()}={clean_prices_0050.loc[pre_date]:.2f}, "
...
ret["FX_change"] = prices["USDTWD"].pct_change()
...
crash_days = ret["SPY_USD"] < ret["SPY_USD"].quantile(0.05)
...
print(f"\n  FX Behavior During Equity Crashes (worst 5% SPY days):")
print(f"    Crash: {fx_in_crash:.2f}%, Normal: {fx_in_normal:.2f}%")
print(f"    USD {'STRENGTHENS' if fx_in_crash > 0 else 'WEAKENS'} in crashes")
...
tnx_daily = prices["TNX"].reindex(ret.index) / 100
tw_rate = 0.015
...
print(f"  TW deposit rate: {tw_rate*100:.1f}%")
print(f"  Avg annual hedging cost: {avg_hedge_cost*100:.2f}%/yr")
...
def compute_portfolio_metrics(port_ret, name, ann=252):
    ...
    return {
        "name": name, "n_days": n, "years": round(years, 1),
        "cagr_pct": round(cagr * 100, 2), "vol_pct": round(vol * 100, 2),
        "sharpe": round(sharpe, 4), "mdd_pct": round(mdd * 100, 2),
...
print(f"\nPortfolio Comparison ({ret.index[0].date()} to {ret.index[-1].date()}):")
cols = ["name", "cagr_pct", "vol_pct", "sharpe", "mdd_pct", "sortino", "calmar"]
print(results_b_df[cols].to_string(index=False))

# ──────────────────────────────────────────────────────────────────
# PART C: OPTIMAL PORTFOLIO GRID SEARCH
# ──────────────────────────────────────────────────────────────────
print("\n" + "="*70)
print("PART C: OPTIMAL PORTFOLIO GRID SEARCH")
print("="*70)
...
for w0050 in weights:
    for w_spy in weights:
        ...
        for h_ratio in [0.0, 0.25, 0.5, 0.75, 1.0]:
            ...
            grid_results.append({
                "w_0050": round(w0050, 2), "w_spy": round(w_spy, 2),
                "w_gld": round(w_gld, 2), "hedge_ratio": h_ratio,
                "cagr_pct": round(cagr, 2), "vol_pct": round(ann_vol * 100, 2),
                "sharpe": round(sharpe, 4), "mdd_pct": round(mdd * 100, 2),
            })
...
print(f"\nBest Conservative (MDD>-25%): {best_conservative['w_0050']:.0%} 0050 + "
      f"{best_conservative['w_spy']:.0%} SPY + {best_conservative['w_gld']:.0%} GLD, "
      f"hedge={best_conservative['hedge_ratio']:.0%} → Sharpe={best_conservative['sharpe']:.3f}")
print(f"Best Moderate (MDD>-35%): {best_moderate['w_0050']:.0%} 0050 + "
      f"{best_moderate['w_spy']:.0%} SPY + {best_moderate['w_gld']:.0%} GLD, "
      f"hedge={best_moderate['hedge_ratio']:.0%} → Sharpe={best_moderate['sharpe']:.3f}")
print(f"Best Aggressive: {best_aggressive['w_0050']:.0%} 0050 + "
...
# ──────────────────────────────────────────────────────────────────
# PART D: PRACTICAL COSTS
# ──────────────────────────────────────────────────────────────────
ndf_spread_retail = 30
ndf_spread_inst = 5
margin_pct = 0.04
opp_cost_rate = 0.03
...
print(f"\nTotal Annual Hedging Cost:")
print(f"  Retail:        {total_retail*100:.2f}%/yr")
print(f"  Institutional: {total_inst*100:.2f}%/yr")

# ──────────────────────────────────────────────────────────────────
# COMPARISON WITH ORIGINAL K758
# ──────────────────────────────────────────────────────────────────
print("\n" + "="*70)
print("COMPARISON WITH ORIGINAL K758 (before clean_tw50_data)")
print("="*70)

# Load original results
orig_path = project_root / "experiments" / "k758_tw_cross_border_hedge_results.json"
try:
    with open(orig_path) as f:
        orig = json.load(f)

    orig_tw_vol = orig["part_a_fx_risk"]["tw0050_vol_pct"]
    new_tw_vol = round(tw0050_vol * 100, 2)
    print(f"\n  0050.TW vol: original={orig_tw_vol}%, new={new_tw_vol}%")
    print(f"  Change: {new_tw_vol - orig_tw_vol:+.2f}pp")

    # Compare portfolio Sharpes
    print(f"\n  Portfolio Sharpe comparison (original vs v2):")
    orig_strats = {s["name"]: s for s in orig["part_b_portfolio_comparison"]}
    new_strats = {s["name"]: s for s in results_b}
    for name in sorted(set(orig_strats.keys()) & set(new_strats.keys())):
        o_s = orig_strats[name]["sharpe"]
        n_s = new_strats[name]["sharpe"]
        delta = n_s - o_s
        print(f"    {name:<35} orig={o_s:.4f}  new={n_s:.4f}  Δ={delta:+.4f}")

    # Key question: does "don't hedge" conclusion survive?
    # Check: unhedged vs hedged Sharpes for key portfolios
    pairs_to_check = [
        ("100% SPY unhedged (TWD)", "100% SPY fully hedged"),
        ("20/80 TW+SPY unhedged", "20/80 TW+SPY hedged"),
        ("50/50 TW+SPY unhedged", "50/50 TW+SPY hedged"),
    ]
    print(f"\n  Hedge benefit analysis (new data):")
    hedge_hurts = 0
    for unh_name, hdg_name in pairs_to_check:
        if unh_name in new_strats and hdg_name in new_strats:
            unh_s = new_strats[unh_name]["sharpe"]
            hdg_s = new_strats[hdg_name]["sharpe"]
            better = "unhedged" if unh_s >= hdg_s else "hedged"
            print(f"    {unh_name.split('(')[0].strip()}: unhedged={unh_s:.4f} vs hedged={hdg_s:.4f} → {better} wins")
            if unh_s >= hdg_s:
                hedge_hurts += 1

    conclusion_unchanged = hedge_hurts >= 2  # majority of pairs still favor unhedged
except Exception as e:
    print(f"  Could not load original results: {e}")
    conclusion_unchanged = None

# ──────────────────────────────────────────────────────────────────
# SAVE RESULTS
# ──────────────────────────────────────────────────────────────────
results = {
    "experiment_id": "K758v2",
    "title": "Taiwan Cross-Border Hedging — Rerun with clean_tw50_data",
    "author": "[提出: User (rerun request), 執行: Claude]",
    "data_source": "yfinance (SPY, 0050.TW, GLD, USDTWD=X, ^TNX, ^VIX)",
    "data_cleaning": "0050.TW: clean_tw50_data (split fix). USDTWD=X: remove <25 and >5% daily FX moves.",
    "period": f"{ret.index[0].date()} to {ret.index[-1].date()}",
    "n_obs": len(ret),
    "fix_applied": "clean_tw50_data — fixes 2014-01-02 split breakpoint (pre-2014 prices /4)",
    "part_a_fx_risk": {
...
if conclusion_unchanged is True:
    results["conclusion"] = (
        "CONCLUSION UNCHANGED. With clean 0050.TW data, the 'don't hedge FX for retail' "
        "recommendation holds. Unhedged portfolios still dominate hedged portfolios on Sharpe, "
        "and hedging costs remain prohibitive for retail investors."
    )
elif conclusion_unchanged is False:
    results["conclusion"] = (
        "CONCLUSION CHANGED. With clean 0050.TW data, hedged portfolios now dominate. "
        "The original 'don't hedge' recommendation may need revision."
    )
else:
    results["conclusion"] = "Unable to compare — original results not available."

print(f"\n\n{'='*70}")
print("K758v2 CONCLUSION")
print("="*70)
print(f"\n{results['conclusion']}")

out_path = project_root / "experiments" / "k758v2_tw_cross_border_hedge_results.json"
with open(out_path, "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2, ensure_ascii=False, default=str)

print(f"\nResults saved to: {out_path}")
print("K758v2 complete.")
