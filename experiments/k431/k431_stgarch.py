"""
K431: Smooth Transition GARCH (STGARCH) Volatility Forecasting
==============================================================
[提出: 用戶, 執行: Claude]
...
def rolling_stgarch_oos(returns, tv, dates, oos_start, oos_end, window=2000, refit_every=63, tv_name='VIX'):
    ...
    for count, idx in enumerate(oos_idx):
        ...
        # Update h_t using the actual transition function
        h_t = stgarch_1step_forecast(params, h_t, eps_t, tv[idx-1])
...
def rolling_baseline_oos(returns_s, dates, model_type, oos_start, oos_end, window=2000, refit_every=63):
    ...
    for count, idx in enumerate(oos_idx):
        ...
        # Filter on recent data to get h_t
...
lagvol_arr = gjr_res.conditional_volatility.values
...
print("-" * 62)
models_is = [('GARCH(1,1)', 3, garch_full.loglikelihood, garch_full.aic, garch_full.bic),
             ('GJR-GARCH(1,1)', 4, gjr_res.loglikelihood, gjr_res.aic, gjr_res.bic)]
...
print("\n[5/5] STGARCH-lagvol...")
t0 = time.time()
f_sl, r_sl, d_sl = rolling_stgarch_oos(ret_arr, lagvol_arr, dates_arr, oos_start, oos_end, tv_name='lag_vol')
...
results = {
    'experiment_id': 'K431',
    'title': 'Smooth Transition GARCH (STGARCH) Volatility Forecasting',
...
out_path = 'experiments/k431_stgarch_results.json'
