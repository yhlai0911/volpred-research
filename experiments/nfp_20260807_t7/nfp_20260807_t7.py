...
VIX_SNAPSHOT_THROUGH = "2026-07-30"
TARGET_RELEASE = "2026-08-07"
# ``event_series_slot=T-7`` is a calendar-stage publishing label.  This
# experiment's conditioning variable is explicitly the seventh *trading-day*
# close before the release: 2026-07-29.  The snapshot also contains the next
# close (2026-07-30, trading-day T-6), which must never replace that information
# set merely because it is the last row on disk.
TARGET_T7_AS_OF = "2026-07-29"
PRE_LAG = 7
HORIZON = PRE_LAG - 1
PRIMARY_HAC_LAG = 22
...
def load_snapshots() -> tuple[pd.Series, pd.DatetimeIndex, dict[str, Any]]:
    ...
    if close.index[-1] != pd.Timestamp(VIX_SNAPSHOT_THROUGH):
        raise RuntimeError(
            "VIX snapshot as-of drift: "
            f"{close.index[-1]} != {VIX_SNAPSHOT_THROUGH}"
        )
    if not np.isfinite(close).all() or not (close > 0).all():
        raise RuntimeError("VIX snapshot contains non-finite/non-positive closes")
    if pd.Timestamp(TARGET_T7_AS_OF) not in close.index:
        raise RuntimeError(
            f"target trading-day T-7 close missing from snapshot: {TARGET_T7_AS_OF}"
        )
    ...
    if pd.Timestamp(TARGET_RELEASE) <= pd.Timestamp(TARGET_T7_AS_OF):
...
def analyze(
    ...
    target_t7_vix = float(close.loc[pd.Timestamp(TARGET_T7_AS_OF)])
    target_t7_regime = str(
        pd.cut([target_t7_vix], REGIME_BINS, labels=REGIME_LABELS, right=False)[0]
    )
    ...
    results: dict[str, Any] = {
        ...
        "data": {
            ...
            "vix_snapshot_through": VIX_SNAPSHOT_THROUGH,
        ...
        "current_state": {
            "as_of": TARGET_T7_AS_OF,
            "information_set_role": "trading_day_T-7",
            "vix_close": target_t7_vix,
            "regime": target_t7_regime,
            "latest_snapshot_date": VIX_SNAPSHOT_THROUGH,
            "latest_snapshot_role": "trading_day_T-6",
        ...
        "published_article_correction": {
            ...
            "reason": (
                ...
                "It also conflated the calendar-stage T-7 publication slot with the "
                "trading-day T-7 conditioning close."
            ),
            "archived_claims": {
