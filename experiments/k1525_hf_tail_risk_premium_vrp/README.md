# K1525: High-Frequency Tail Risk Premium as a Time-Varying Predictor of SPY Excess Return and VRP
...
K1525 deliberately **avoids** SKEW and search-volume proxies. Instead it uses the realized 5%-tail mean from intraday and daily returns — the model-free truncated moment that Kelly-Jiang argue captures cross-sectional tail-risk pricing.
...
- Frozen pointwise forecast/loss ledger: `k1525_oos_loss_ledger.csv` (SHA-256 `f3c205edc7973a0f060141e4b3ef5d3ce5caeec737b38aa9ef300db8323f98a3`).
...
```bash
cd <repo_root>
uv run --active python experiments/k1525_hf_tail_risk_premium_vrp/k1525_hf_tail_risk_premium_vrp.py
```

- Seed: 42 (bootstrap and all stochastic blocks).
- Output: `k1525_hf_tail_risk_premium_vrp_results.json`, frozen OOS ledger, and 3 figures.

## 8. Files

| File | Purpose |
|---|---|
| `k1525_hf_tail_risk_premium_vrp.py` | Main script |
| `k1525_hf_tail_risk_premium_vrp_results.json` | All numerical results |
| `k1525_oos_loss_ledger.csv` | Frozen monthly forecasts and pointwise squared losses for offline Clark-West verification |
...
- K447 (SKEW): K1525 confirms that *some* tail proxy correlates with excess return in-sample, but loses OOS. K447's SKEW NULL is consistent — observed-IV-based tail proxies and realized-downside proxies behave similarly OOS.
- K768 (Conformal VaR): different question (coverage calibration), not directly comparable.
- K789 (Google Trends): K1525 finds the realized-tail signal also weak OOS, consistent with K789's behavioral-proxy NULL. The tail-premium literature is robust in cross-section, brittle in time-series.
