"""
K1525: High-frequency tail risk premium (TRP) as a time-varying predictor of
...
def fig_trp_overlay(monthly: pd.DataFrame, out: Path):
    ...
    ax1.set_title("K1525 Fig 1: Daily-channel TRP (ES proxy) vs SPY excess return")
...
def fig_rolling_oos(e_model: pd.Series, e_base: pd.Series, out: Path):
    ...
    ax.set_title("K1525 Fig 2: Cumulative SSE difference (baseline - model). Up = model improving.")
...
def fig_trp_5m(trp5: pd.DataFrame, out: Path):
    ...
    ax.set_title(f"K1525 Fig 3: 5-min realized truncated downside TRP (n_months={len(trp5)}, concept only)")
...
def run() -> dict:
    ...
    ledger_path = EXP_DIR / "k1525_oos_loss_ledger.csv"
    _atomic_write_text(ledger_path, oos_ledger.to_csv(float_format="%.17g"))
    ledger_sha256 = hashlib.sha256(ledger_path.read_bytes()).hexdigest()

    out = {
        "experiment_id": "k1525_hf_tail_risk_premium_vrp",
        "seed": SEED,
        "data": {
    ...
    # Write results JSON
    res_path = EXP_DIR / "k1525_hf_tail_risk_premium_vrp_results.json"
