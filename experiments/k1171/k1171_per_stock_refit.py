#!/usr/bin/env python3
"""K1171 per-stock theta_EAV refit for AU (ASX Top 10) + pooled theta_EAV.

Exact same specification as K1165/K1166/K1168/K1172 (fair comparison):
...
Lookahead discipline:
 ...
 - earnings filtered past only (HAND_CODED dates < today at fetch time)
...
def main():
    t0 = time.time()
    print(f"\n{'='*72}\nK1171: per-stock refit + pooled MLE (AU ASX Top 10)\n{'='*72}\n")
    earnings_cache = json.load(open(DATA / "earnings_dates_k1171.json"))
    info_cache = json.load(open(DATA / "ticker_info_k1171.json"))
    ...
    ih = json.load(open(DATA / "institutional_ownership_k1171.json"))
    ih_map = {}
    ...
    csv_path = ROOT / "k1171_per_stock_table_newmkts.csv"
    ...
    with open(ROOT / "k1171_pooled_by_market.json", "w") as f:
        json.dump(pooled_out, f, indent=2, default=str)
    print(f"  wrote k1171_pooled_by_market.json")
