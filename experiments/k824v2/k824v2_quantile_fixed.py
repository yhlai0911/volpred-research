#!/usr/bin/env python3
"""
K824v2: Probabilistic RV Quantile Forecasting — Bug-fixed
==========================================================
[提出: 用戶, 執行: Claude]

Fixes from K824 (Codex HIGH findings):

  FIX 1: Student-t df estimation lacked scale term.
    K824 bug: z_t = r_t/σ_t has unit variance by construction, but
    scipy t_dist.logpdf(z, df=df) assumes standard Student-t whose
    variance = df/(df-2) ≠ 1. This biased df estimation upward.
    Fix: Use scaled log-likelihood: z_scaled = z * sqrt(df/(df-2)),
    i.e. fit the scaled Student-t distribution with explicit Jacobian.
    Equivalently, maximize Σ log[f_t(z_t; df, scale=sqrt((df-2)/df))]
    where scale converts unit-variance z to standard-t domain.

  FIX 2: Basel traffic light was custom ratio-based.
    K824 bug: Used pi_hat <= alpha * 1.5 (proportion-based, not standard).
    Fix: Standard Basel II/III 250-day lookback:
      Green:  0-4  violations in last 250 days
      Yellow: 5-9  violations in last 250 days
      Red:    ≥10 violations in last 250 days
    For OOS < 250 days, scale proportionally: threshold = floor(N/250 * cutoff).

Otherwise identical to K824: same GJR-GARCH, same 4 models, same evaluation.
...
References:
  ...
  - Basel Committee (1996, 2019) — Traffic light backtesting framework
  ...
  - K824: Original version with 2 HIGH bugs (df scale, Basel custom)
"""
...
RESULTS_PATH = os.path.join(os.path.dirname(__file__), 'k824v2_quantile_fixed_results.json')
...
# ==============================================================
# D. Student-t df estimation from standardized residuals (FIXED)
# ==============================================================
#
# FIX 1: K824 bug — z_t has unit variance (by GJR construction), but
# scipy's t_dist is a *standard* Student-t with variance = df/(df-2).
# Fitting t_dist.logpdf(z, df) without scale yields biased df.
#
# Correct approach: fit a scaled Student-t where we model
#   z ~ (1/s) * t(df)   with s = sqrt(df/(df-2))
# so that Var(z) = (1/s²) * df/(df-2) = 1.
#
# The log-likelihood for z_i given (df, s) is:
#   log f(z_i; df, s) = log_t_pdf(s * z_i; df) + log(s)
# where s = sqrt(df/(df-2)).
#
# Equivalently: use scipy's t_dist with loc=0, scale=sqrt((df-2)/df),
# which automatically handles the Jacobian.
...
def estimate_t_df(std_residuals, df_min=2.1, df_max=30.0):
    ...
    FIXED: Uses scale = sqrt((df-2)/df) so that the fitted distribution
    has unit variance, matching the standardized residuals.
    """
    ...
    def neg_loglik(log_df):
        ...
        if df < df_min or df > df_max:
            return 1e10
        # Scale parameter: sqrt((df-2)/df) gives unit-variance Student-t
        scale = np.sqrt((df - 2.0) / df)
        # scipy t_dist.logpdf with scale automatically includes Jacobian
...
def fit_quantile_regression(z_residuals, tau, n_lags=5):
    ...
    X = np.column_stack([
        abs_z[start - 1:-1],
        z_sq[start - 1:-1],
        (z[start - 1:-1] < 0).astype(float) * abs_z[start - 1:-1],
        ma_abs[start - 1:-1],
        ma_sq[start - 1:-1],
    ])
    X = sm.add_constant(X)
...
# ==============================================================
# F. VaR backtest (Kupiec + Christoffersen + Basel) (FIXED)
# ==============================================================
#
# FIX 2: Basel traffic light now uses standard 250-day lookback window
# with count-based thresholds per Basel Committee (1996, 2019):
#   Green:  0-4  violations in 250 days
#   Yellow: 5-9  violations in 250 days
#   Red:    ≥10 violations in 250 days
#
# For OOS windows shorter than 250 days, we scale thresholds
# proportionally: cutoff_scaled = floor(N/250 * cutoff_250).
...
def basel_traffic_light_250(violations_array, n_lookback=250):
    ...
    Basel Committee on Banking Supervision (1996, rev. 2019):
      Green zone:  0-4  exceptions in 250 trading days
      Yellow zone: 5-9  exceptions in 250 trading days
      Red zone:    ≥10 exceptions in 250 trading days

    For windows < 250 days, scale thresholds proportionally.
    Returns: ('green'|'yellow'|'red', n_violations_in_window, window_size)
    """
    v = np.asarray(violations_array, dtype=int)
    n = len(v)

    # Use the last n_lookback days (or all if shorter)
    window = min(n, n_lookback)
    v_window = v[-window:]
    n_viol = int(v_window.sum())

    # Scale thresholds for shorter windows
    # Basel standard: 4/250, 9/250 as cutoffs
    # For N < 250: green_max = floor(N * 4/250), yellow_max = floor(N * 9/250)
    if window >= 250:
        green_max = 4
        yellow_max = 9
    else:
...
def var_backtest(returns, var_series, alpha_var=0.01):
    ...
    # FIX 2: Standard Basel 250-day traffic light
...
def winkler_score(y, lower, upper, alpha):
    """Winkler (1972) score for (1-α) prediction intervals. Lower is better."""
    y = np.asarray(y, dtype=np.float64)
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("K824v2: Probabilistic RV Quantile Forecasting (Bug-Fixed)")
    print("  FIX 1: Student-t df estimation with proper scale term")
    print("  FIX 2: Standard Basel 250-day traffic light")
    print("=" * 70)
    ...
    # Track df estimates for comparison reporting
    df_history = []
    ...
    for i, oos_pos in enumerate(oos_idx):
        ...
        if oos_pos - last_fit_idx >= REFIT_EVERY:
            ...
            # FIX 1: Estimate Student-t df with proper scale term
            t_df = estimate_t_df(z_train)
            df_history.append({'oos_day': i, 'df': round(t_df, 2),
                               'z_var': round(float(np.var(z_train)), 4)})
            ...
            if i % 100 == 0 or i == 0:
                print(f"  Refit at OOS day {i}/{n_oos}: "
                      ...
                      f"t_df={t_df:.2f} (z_var={np.var(z_train):.4f}), "
        ...
        for tau in TAUS:
            ...
            # M2: Student-t quantiles (FIXED: proper scaling for unit variance)
            # The quantile of a unit-variance Student-t is:
            #   q_τ = t^{-1}(τ; df) * sqrt((df-2)/df)
            # where t^{-1} is the standard Student-t quantile function.
            if t_df > 2.0:
    ...
    results = {
        'experiment_id': 'K824v2',
        'title': 'K824v2: Probabilistic RV Quantile Forecasting (Bug-Fixed)',
        'asset': 'SPY',
        ...
        'fixes_from_k824': [
            'FIX 1: Student-t df estimation with scale=sqrt((df-2)/df) for unit-variance residuals',
            'FIX 2: Standard Basel 250-day traffic light (Green:0-4, Yellow:5-9, Red:>=10)',
        ],
    ...
    # 3d. VaR 1% backtest (with FIXED Basel traffic light)
    print("\n  --- VaR 1% Backtest (Kupiec + Christoffersen + Basel 250-day) ---")
    var_results = {}
    for model in model_names:
        ...
        print(f"  {model:<15} violations={vbt['n_violations']}/{vbt['n_total']} "
              ...
              f"(window={vbt['basel_window_size']}, viol_in_window={vbt['basel_violations_in_window']}) "
              f"→ {status}")
    ...
    for model in model_names:
        ...
        print(f"  {model:<15} violations={vbt5['n_violations']}/{vbt5['n_total']} "
              ...
              f"Basel={vbt5['basel_traffic_light']} "
              f"(window={vbt5['basel_window_size']}, viol_in_window={vbt5['basel_violations_in_window']}) "
              f"→ {status}")
    ...
    # 4. DM tests
    # ----------------------------------------------------------
    print("\n[4/5] DM tests (pinball loss)...")

    # M4_HistSim vs each (since HistSim was best in K824, check if still)
    dm_results = {}

    # Test all pairs of interest
    test_pairs = [
        ('M4_HistSim', 'M1_Normal'),
        ('M4_HistSim', 'M2_StudentT'),
        ('M4_HistSim', 'M3_QuantReg'),
        ('M2_StudentT', 'M1_Normal'),
        ('M3_QuantReg', 'M1_Normal'),
        ('M3_QuantReg', 'M2_StudentT'),
    ]
    ...
    for model_a, model_b in test_pairs:
        ...
        dm_results[key] = {}
        for tau in TAUS:
            pwa = pinball_pw.get(model_a, {}).get(str(tau))
            pwb = pinball_pw.get(model_b, {}).get(str(tau))
            if pwa is None or pwb is None:
                dm_results[key][str(tau)] = None
                continue
            min_n = min(len(pwa), len(pwb))
            t_stat, p_val = dm_test(pwa[:min_n], pwb[:min_n])
            dm_results[key][str(tau)] = {
            ...
            print(f"  {model_a} vs {model_b:<15} τ={tau}: "
                  f"DM t={t_stat:+.3f} p={p_val:.4f} "
                  f"{'→ ' + model_a + ' wins' if t_stat < 0 else '→ ' + model_b + ' wins'} "
                  f"{sig_mark}")
    ...
    print("\n  === FINAL RANKING (K824v2 — both fixes applied) ===")
    print(f"  {'Model':<15} {'Avg Pinball':<14} {'Cov. Dev.':<12} {'VaR 1%':<10} {'VaR 5%':<10} {'Basel 1%':<10}")
    for model in model_names:
        ...
        b1 = var_results.get(model, {}).get('basel_traffic_light', 'N/A') if var_results.get(model) else 'N/A'
        ap_str = f"{ap:.6f}" if ap is not None else "N/A"
        cd_str = f"{cd:.6f}" if cd is not None else "N/A"
        print(f"  {model:<15} {ap_str:<14} {cd_str:<12} {v1:<10} {v5:<10} {b1:<10}")

    # Best model per criterion
    valid_pinball = [(m, avg_pinball[m]) for m in model_names if avg_pinball.get(m) is not None]
    valid_coverage = [(m, coverage_deviation[m]) for m in model_names if coverage_deviation.get(m) is not None]

    best_pinball = min(valid_pinball, key=lambda x: x[1]) if valid_pinball else (None, None)
    best_coverage = min(valid_coverage, key=lambda x: x[1]) if valid_coverage else (None, None)
    var_pass_models = [m for m in model_names
                       if var_results.get(m) and var_results[m].get('trinity_pass')]
    var5_pass_models = [m for m in model_names
    ...
    results['summary'] = {
        ...
        'var_5pct_pass_models': var5_pass_models,
    }

    print(f"\n  Best pinball loss: {best_pinball[0]} ({best_pinball[1]:.6f})" if best_pinball[0] else "")
    print(f"  Best coverage calibration: {best_coverage[0]} ({best_coverage[1]:.6f})" if best_coverage[0] else "")
    print(f"  VaR 1% PASS: {var_pass_models if var_pass_models else 'NONE'}")
    print(f"  VaR 5% PASS: {var5_pass_models if var5_pass_models else 'NONE'}")

    # GJR params + df history
    if gjr_params:
        results['final_gjr_params'] = {k: round(v, 6) for k, v in gjr_params.items()}
        results['student_t_df'] = round(t_df, 2)
        results['df_estimation_history'] = df_history

    # Bug impact analysis
    print("\n  === BUG IMPACT ANALYSIS ===")
    if df_history:
        df_vals = [d['df'] for d in df_history]
        print(f"  Student-t df estimates (with scale fix): "
              f"mean={np.mean(df_vals):.2f}, range=[{np.min(df_vals):.2f}, {np.max(df_vals):.2f}]")
        print(f"  (K824 without scale: df was biased upward → thinner tails → underestimates tail risk)")

    results['bug_impact'] = {
        'fix1_description': 'Student-t df estimation with scale=sqrt((df-2)/df). '
                            'Without scale, df biased upward (thinner tails, underestimate tail risk).',
        'fix2_description': 'Basel 250-day lookback (Green:0-4, Yellow:5-9, Red:>=10). '
                            'K824 used custom ratio thresholds.',
        'df_estimates_fixed': [d['df'] for d in df_history] if df_history else [],
    }
