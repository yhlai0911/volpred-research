...
Motivation:
  Paper 9 alignment audit found that K1024 standardizes u_{t-1} with sqrt(tau_t)
  instead of sqrt(tau_{t-1}). This follow-up re-runs the full refit-frequency
  grid with the corrected two-step recursion and compares magnitudes to K1024.
...
EXPERIMENT_ID = "K1024b"

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, '..', '..'))
sys.path.insert(0, os.path.join(PROJECT_ROOT, 'src'))

RESULTS_PATH = os.path.join(SCRIPT_DIR, 'k1024b_results.json')
ORIGINAL_RESULTS_PATH = os.path.join(PROJECT_ROOT, 'experiments', 'k1024', 'k1024_results.json')
...
print("  Paper 9 tau-alignment follow-up")
print("=" * 70, flush=True)

# ============================================================
# SECTION 1: DATA LOADING
# ============================================================
print("\n[1] Loading data from local snapshot...", flush=True)
...
def a4f_negloglik_t(theta0, theta1, omega_g, alpha, gamma_p, beta,
    ...
    for t in range(n):
        ...
        if t > 0:
            tau_prev = theta0 + theta1 * vix_lag_sq[t-1]
            if tau_prev < 1e-16:
                tau_prev = 1e-16
    ...
    for t_idx, abs_idx in enumerate(oos_indices):
        ...
        if need_refit:
            ...
            if a4f_params_new is not None:
                ...
                a4f_tau_prev = max(a4f_params[0] + a4f_params[1] * train_vix[-1]**2, 1e-16)
...
plt.tight_layout()
chart1_path = os.path.join(SCRIPT_DIR, 'k1024b_qlike_vs_frequency.png')
...
plt.tight_layout()
chart2_path = os.path.join(SCRIPT_DIR, 'k1024b_dm_vs_frequency.png')
...
plt.tight_layout()
chart3_path = os.path.join(SCRIPT_DIR, 'k1024b_runtime_vs_frequency.png')
...
if os.path.exists(ORIGINAL_RESULTS_PATH):
    ...
    for refit_freq in REFIT_FREQS:
        freq_key = f'freq_{refit_freq}'
        old_model = original['models'][freq_key]
        new_model = results['models'][freq_key]
        old_dm = original['dm_tests_a4f_vs_gjr'][freq_key]
        new_dm = results['dm_tests_a4f_vs_gjr'][freq_key]
        comparison[freq_key] = {
            'label': new_model['label'],
            'a4f_qlike_old': old_model['a4f']['qlike'],
            'a4f_qlike_new': new_model['a4f']['qlike'],
            'a4f_qlike_delta': new_model['a4f']['qlike'] - old_model['a4f']['qlike'],
            'gjr_qlike_old': old_model['gjr']['qlike'],
            'gjr_qlike_new': new_model['gjr']['qlike'],
            'gjr_qlike_delta': new_model['gjr']['qlike'] - old_model['gjr']['qlike'],
            'dm_t_old': old_dm['t_stat'],
            'dm_t_new': new_dm['t_stat'],
            'dm_t_delta': new_dm['t_stat'] - old_dm['t_stat'],
            'direction_unchanged': new_dm['direction'] == old_dm['direction'],
        }
    results['comparison_to_k1024'] = comparison
