...
REPLACEMENTS: list[tuple[str, str]] = [
    ...
    (
        ...
        "（顯著性 0.11）—— 要注意同一組對照下，只看排序的無母數檢定反而是顯著的（顯著性 0.002），"
    ...
    (
        ...
        "**方向性結論全部維持不變**：對全體交易日基準在 Welch 平均差檢定下未達顯著"
        "（同組對照的無母數檢定則顯著，兩者不一致）、對週五基準達到顯著、"
        "進場 VIX 高低兩組之間的差距是本研究中最大的數字——這三點在官方日期下都成立，"
        "只有數值小幅調整（1.10→1.11 倍、1.17→1.19 倍、2.17→2.03 倍、相關係數 0.45→0.44）。"
