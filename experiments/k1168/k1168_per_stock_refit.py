#!/usr/bin/env python3
"""K1168 per-stock theta_EAV refit for 3 NEW markets (BR, CH, IN) + pooled theta_EAV.

Exact same specification as K1165/K1166:
...
Output:
 ...
 - k1168_pooled_by_market.json (pooled theta_EAV for BR, CH, IN)
"""
...
# =========================================================================
# Per-stock numba likelihood (identical to K1165/K1166)
# =========================================================================
...
# =========================================================================
# Pooled-per-market MLE (shared theta0, theta_VIX, theta_EAV across stocks;
# stock-specific GJR(alpha, gamma, beta))
# =========================================================================
@njit(cache=True, fastmath=True)
...
def main():
    t0 = time.time()
    print(f"\n{'='*72}\nK1168: per-stock refit + pooled MLE for new markets (BR, CH, IN)\n{'='*72}\n")
    earnings_cache = json.load(open(DATA / "earnings_dates_k1168.json"))
    info_cache = json.load(open(DATA / "ticker_info_k1168.json"))
    ...
    # Merge metadata
    ih = json.load(open(DATA / "institutional_ownership_k1168.json"))
    ih_map = {}
    ...
    csv_path = ROOT / "k1168_per_stock_table_newmkts.csv"
    ...
    with open(ROOT / "k1168_pooled_by_market.json", "w") as f:
        json.dump(pooled_out, f, indent=2, default=str)
    print(f"  wrote k1168_pooled_by_market.json")
