...
Motivation:
  K1034 showed CF-Rolling is the best VaR method (6/6 Trinity PASS) using GJR.
  K1035 showed A4f is the best volatility model (already 4/4 with Student-t).
  This experiment combines the best model (A4f) with the best VaR method
  (CF-Rolling) to test whether improvements are additive.

  We compare 6 combinations in a 2x3 factorial design:
    Model: GJR, A4f
    VaR method: Normal, Student-t(df=8), CF-Rolling(252d)

  Core questions:
    1. Does A4f + CF-Rolling achieve double improvement?
    2. Is the improvement mainly from A4f or CF-Rolling?
...
Evaluation:
  ...
  - 2x3 interaction analysis (model x method)
...
References:
  ...
  - K1034: CF-Rolling 6/6 Trinity PASS (best VaR method)
  - K1035: A4f already 4/4 with Student-t (best model)
  - K988: A4f champion for SPY (DM t=+4.48 vs GJR)
  - K995: A4f-t 12/12 VaR PASS
...
RESULTS_PATH = os.path.join(SCRIPT_DIR, 'k1036_results.json')
...
print("=" * 70)
print(f"{EXPERIMENT_ID}: A4f + CF-Rolling VaR -- Best Model x Best VaR Method")
print(f"  Models: GJR, A4f | Methods: Normal, Student-t, CF-Rolling")
...
def run_backtest_for_asset(ticker):
    """Run full 2x3 VaR/ES comparison for one asset."""
    print(f"\n{'='*60}")
    ...
    # ---- Build combined residual series for CF-Rolling ----
    models = {
        'GJR': {
    ...
    for model_name, model_info in models.items():
        ...
        # Build combined residual series (in-sample + OOS) for CF
        ...
        for alpha in ALPHA_LEVELS:
            for method_name in ['Normal', 'Student-t', 'CF-Rolling']:
...
print("\n" + "=" * 70)
print("RUNNING FULL 2x3 BACKTEST")
print("=" * 70)
...
# ============================================================
# SUMMARY ANALYSIS: 2x3 Interaction Table
# ============================================================

print("\n" + "=" * 70)
print("2x3 INTERACTION ANALYSIS (Model x VaR Method)")
print("=" * 70)

# Count Trinity PASS for each combination
combinations = {}
for model in ['GJR', 'A4f']:
    for method in ['Normal', 'Student-t', 'CF-Rolling']:
...
for model in ['GJR', 'A4f']:
    total = sum(combinations[f"{model}_{m}"]['total_tests'] for m in ['Normal', 'Student-t', 'CF-Rolling'])
    passed = sum(combinations[f"{model}_{m}"]['trinity_pass'] for m in ['Normal', 'Student-t', 'CF-Rolling'])
    rate = passed / total if total > 0 else 0
    print(f"  {model}: {passed}/{total} = {rate:.1%}")

print("\n--- Method Effect (averaging over models) ---")
for method in ['Normal', 'Student-t', 'CF-Rolling']:
    total = sum(combinations[f"{m}_{method}"]['total_tests'] for m in ['GJR', 'A4f'])
    passed = sum(combinations[f"{m}_{method}"]['trinity_pass'] for m in ['GJR', 'A4f'])
    rate = passed / total if total > 0 else 0
...
# Figure 1: Violation rates bar chart (grouped by model x method)
...
bar_width = 0.12
models_list = ['GJR', 'A4f']
methods_list = ['Normal', 'Student-t', 'CF-Rolling']
colors = {
    'GJR_Normal': '#d62728',
...
for ax_idx, ticker in enumerate(ASSETS):
    ...
    ax.set_xticks(x_positions + bar_width * 2.5)
...
if len(ASSETS) > 0:
    axes[0].legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3,
                   fontsize=8, frameon=True)

plt.suptitle('K1036: Violation Rates by Model x VaR Method', fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
fig1_path = os.path.join(SCRIPT_DIR, 'k1036_violation_rates.png')
plt.savefig(fig1_path, dpi=150, bbox_inches='tight')
plt.close()
print(f"  Saved: {fig1_path}")


# Figure 2: Trinity PASS heatmap (Model x Method)
...
ax.set_title('K1036: Trinity PASS Rate (Model x VaR Method)', fontsize=14, fontweight='bold')
ax.set_xlabel('VaR Method', fontsize=12)
ax.set_ylabel('Volatility Model', fontsize=12)
plt.tight_layout()
fig2_path = os.path.join(SCRIPT_DIR, 'k1036_trinity_heatmap.png')
plt.savefig(fig2_path, dpi=150, bbox_inches='tight')
plt.close()
print(f"  Saved: {fig2_path}")


# Figure 3: Detailed comparison per asset
    ...
    for model in models_list:
        for method in methods_list:
            ...
            for alpha in ALPHA_LEVELS:
                key = f"{model}_{method}_{alpha}"
                if key in all_asset_results[ticker]['results']:
                    r = all_asset_results[ticker]['results'][key]
                    if r['trinity'] == 'PASS':
                        n_trinity += 1
                    if r['kupiec'] == 'PASS':
                        n_kupiec += 1
                    if r['es'] == 'PASS':
                        n_es += 1
            trinity_pass_vals.append(n_trinity)
            kupiec_pass_vals.append(n_kupiec)
            es_pass_vals.append(n_es)

    x = np.arange(len(combo_labels))
    w = 0.25

    ax.bar(x - w, kupiec_pass_vals, w, label='Kupiec PASS', color='#ff7f0e', alpha=0.85)
    ax.bar(x, trinity_pass_vals, w, label='Trinity PASS', color='#2ca02c', alpha=0.85)
    ax.bar(x + w, es_pass_vals, w, label='ES PASS', color='#1f77b4', alpha=0.85)

    ax.axhline(y=len(ALPHA_LEVELS), color='gray', linestyle='--', alpha=0.5)
    ax.set_xticks(x)
    ax.set_xticklabels(combo_labels, fontsize=9)
    ax.set_ylabel('PASS Count (out of 2)')
    ax.set_title(f'{ticker}: Test Results by Combination', fontsize=12, fontweight='bold')
    ax.set_ylim(0, len(ALPHA_LEVELS) + 0.5)
    ax.legend(fontsize=9)

plt.suptitle('K1036: Detailed Test Results per Asset', fontsize=14, fontweight='bold')
plt.tight_layout()
fig3_path = os.path.join(SCRIPT_DIR, 'k1036_detail_comparison.png')
...
results_json = {
    ...
    'title': 'A4f + CF-Rolling VaR: Best Model x Best VaR Method',
    ...
    'var_methods': {
        ...
        'CF-Rolling': 'VaR = sigma * z_cf (CF with 252d rolling moments)',
    },
    ...
    'references': [
        ...
        'K1034: CF-Rolling 6/6 Trinity PASS (best VaR method with GJR)',
        'K1035: A4f already 4/4 with Student-t (best model)',
        'K988: A4f champion for SPY (DM t=+4.48 vs GJR)',
    ],
    'figures': [
        'k1036_violation_rates.png',
        'k1036_trinity_heatmap.png',
        'k1036_detail_comparison.png',
    ],
...
# Check if A4f+CF-Rolling achieves 100%
