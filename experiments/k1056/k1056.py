...
Motivation:
  Paper 9's core finding is A4f (multiplicative GARCH-X with VIX²) beats GJR
  with DM t=4.48 over full OOS (2019-2026). A reviewer may ask: "Is this
  advantage stable across time, or driven by a single special period (e.g. COVID)?"

  This experiment answers by splitting OOS into 5 non-overlapping sub-periods
  and testing A4f vs GJR in each.

Prior results:
  - K988: A4f vs GJR, full OOS DM t=4.48, QLIKE: -8.361 vs -8.277
  - K988b: 17 specs, A4f best
  - K1024/K1033: Refit frequency CV=0.1% (insensitive)
  - K994: Cross-asset (QQQ/GLD PASS)

Research questions:
  1. Does A4f's QLIKE advantage hold in each sub-period (Harvey |t|>3.0)?
  2. Does the improvement magnitude vary by regime?
  3. Is there any sub-period where A4f is worse than GJR?
  4. Is θ₁ (VIX² coefficient) stable across sub-periods?

Method:
  - SPY daily returns + VIX, 2005-2026, yfinance
...
Data: SPY 2005-2026, VIX from yfinance. OOS: 2015-01-01 to latest.
...
RESULTS_PATH = os.path.join(SCRIPT_DIR, 'k1056_results.json')
...
SUB_PERIODS = {
    ...
    'P5_Recent': ('2024-07-01', '2026-04-12'),
}
...
print("=" * 70)
print(f"{EXPERIMENT_ID}: A4f Sub-Period Stability Analysis")
print("  Paper 9 Robustness — 5 non-overlapping sub-periods")
print("=" * 70)
...
for t_idx, abs_idx in enumerate(oos_indices):
    ...
    if need_refit:
        ...
        if a4f_params is not None:
            ...
            for i in range(1, n_train):
                ...
                g = max(g, 1e-10)
            a4f_state['g'] = g
            # τ for the last training observation's VIX (to use for first OOS step)
    ...
    if a4f_state['params'] is not None and a4f_state['g'] is not None:
        ...
        # τ_t uses VIX_{t-1} (no lookahead)
        vix_prev = vix[abs_idx - 1]
        tau_t = max(theta0 + theta1 * vix_prev**2, 1e-16)

        # g_t update: u_{t-1} = r_{t-1} / sqrt(τ_t) [denominator = tau_t, per Engle 2013]
        r_prev = ret[abs_idx - 1]
...
plt.tight_layout()
plot1_path = os.path.join(SCRIPT_DIR, 'k1056_subperiod_dm.png')
...
if len(theta1_history) > 1:
    ...
    plt.tight_layout()
    plot2_path = os.path.join(SCRIPT_DIR, 'k1056_theta1_evolution.png')
...
plt.tight_layout()
plot3_path = os.path.join(SCRIPT_DIR, 'k1056_qlike_improvement.png')
...
if rolling_dm:
    ...
    ax.xaxis.set_major_locator(mdates.YearLocator())
    plt.tight_layout()
    plot4_path = os.path.join(SCRIPT_DIR, 'k1056_rolling_dm.png')
...
results = {
    ...
    'description': 'A4f sub-period stability analysis for Paper 9 robustness',
    ...
    'plots': [
        'k1056_subperiod_dm.png',
        'k1056_theta1_evolution.png',
        'k1056_qlike_improvement.png',
        'k1056_rolling_dm.png',
    ],
