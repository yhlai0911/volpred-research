#!/usr/bin/env python3
"""
K1081: A4f on EEM Emerging Markets
==================================
Distinguishing "Non-US" vs "Taiwan-Specific" A4f Failure
--------------------------------------------------------
[提出: 用戶 (Claude 執行), 執行: Claude]

Motivation
----------
K1075-K1080 established the Paper 9 US Equity cross-asset axis:
    - K1075 SPY     : DM t = +7.92, Harvey-PASS, theta1 span < 1 order
    - K1078 QQQ     : DM t = +5.99, Harvey-PASS, theta1 span = 1.91 orders
    - K1080 IWM     : DM t = +4.80, Harvey-PASS, theta1 span = 3.54 orders
    - K1077 0050.TW : DM t = -0.49, Harvey-FAIL, theta1 span ~ 4 orders

The 0050.TW null raises a critical question: is the failure because
    (H_non_us)    the instrument is non-US, or
    (H_tw_uniq)   Taiwan has unique factors (local retail flow, political
                  risk, concentrated TSMC exposure, USD/TWD currency shifts)?
...
EEM (iShares MSCI Emerging Markets ETF) is the natural discriminator:
    ...
    - VIX is a global risk-on/off signal, so A4f should still be relevant,
      but with beta < 1 relative to SPX (EEM is non-US exposure)
    - IPO 2003-04-15 -> > 22 years history, sufficient for 3 OOS windows

If EEM A4f PASSES Harvey : 0050.TW null is Taiwan-specific
If EEM A4f FAILS Harvey  : A4f requires US market structure (both fail)
If EEM marginal (1<t<3)  : A4f decays with US-ness, Taiwan at the tail

Design (strict parity with K1075/K1078/K1080)
---------------------------------------------
    - Three non-overlapping OOS windows:
        Early    2007-2012 (GFC + Euro crisis; EM collapsed harder than SPX)
        Middle   2013-2018 (China 2015 slowdown, Trump tariffs)
        Late     2019-2026 (COVID + rate hike + China tension)
    - Rolling-window GARCH with 2000-day training, 63-day refit
    - Two models only: GJR baseline vs A4f (tau = theta0 + theta1 * VIX^2_{t-1},
      g = GJR on r/sqrt(tau), free omega_g) — same as K1075/K1078/K1080
    - Crisis sub-period analysis: GFC, China 2015, COVID, Bear 2022
    - VIX bucket analysis: Low/Normal/High/Extreme/Crisis

    EEM IPO 2003-04-15. At 2007-01-01 there are ~930 training observations
    (vs WINDOW=2000). The rolling code uses max(0, abs_idx - WINDOW) so the
    first refit uses all available history — same behaviour as K1075 (1758),
    K1078 (1965), K1080 (~1680).

Hypotheses
----------
    H1 : EEM Full OOS 2007-2026 A4f > GJR with |DM t| > 3 (Harvey-PASS)
    H2 : EEM GFC sub-period A4f improves over GJR (EM collapsed harder)
    H3 : All 3 OOS windows show A4f directional win (diff% < 0)
    H4 : A4f does not break down at extreme VIX (>40)
    H5 : EEM theta1 stability intermediate between QQQ (1.91) and 0050.TW (4+).
         Prediction: 2.5-3.5 orders span.
    H6 : EEM Harvey verdict discriminates H_non_us vs H_tw_uniq:
             PASS  -> 0050.TW null is Taiwan-specific
             FAIL  -> A4f requires US market structure
             Marginal -> A4f decays with distance-to-US

Data
----
    EEM : yfinance  2003-04-15 -> 2026-04-12 (primary asset)
    ^VIX: yfinance  2003-04-15 -> 2026-04-12 (exogenous IV regressor)

Evaluation
----------
    QLIKE loss on r^2 (Patton 2011, proxy-robust)
    DM test HAC-Newey-West (Harvey 2016 |t| > 3.0 threshold)
    Spearman rank correlation
    Moving-block bootstrap 95% CI (n_boot=1000, block ~ n^{1/3}, seed 42)
...
--------------------
    K988  SPY A4f 2019-2026 DM t = 4.48 (first proof of concept)
    K994  Cross-asset MF-GJR-X
    K1075 SPY 2007-2026 DM t = +7.92, Harvey-PASS, theta1 span < 1
    K1077 0050.TW 2010-2025 DM t = -0.49, Harvey-FAIL, theta1 span ~ 4
    K1078 QQQ 2007-2026 DM t = +5.99, Harvey-PASS, theta1 span = 1.91
    K1079 VXN vs VIX IV-family substitutability
    K1080 IWM 2007-2026 DM t = +4.80, Harvey-PASS, theta1 span = 3.54
...
RESULTS_PATH = os.path.join(SCRIPT_DIR, 'k1081_results.json')

# ------------------------------------------------------------------
# Configuration
# ------------------------------------------------------------------
DATA_START = '2003-04-15'   # EEM IPO
DATA_END   = '2026-04-12'
...
CRISIS_PERIODS = [
    ...
    ('China_2015',   '2015-06-01', '2016-02-29'),     # China devaluation + crash
...
# Reference result paths for 5-asset Paper 9 table
...
K1080_RESULTS_PATH = os.path.abspath(os.path.join(SCRIPT_DIR, '..', 'k1080', 'k1080_results.json'))

print("=" * 70)
print(f"{EXPERIMENT_ID}: A4f Extended History on EEM (Emerging Markets 2007-2026)")
print(f"  3 OOS windows, 4 crisis sub-periods, 5 VIX buckets")
print(f"  Only 2 models: GJR vs A4f (VIX^2 free omega)")
print(f"  Strict parity with K1075 / K1078 / K1080")
print("=" * 70)

# ==================================================================
# SECTION 1: DATA LOADING
# ==================================================================
print("\n[1] Loading EEM + ^VIX from yfinance...")
import yfinance as yf

raw = yf.download('EEM', start=DATA_START, end=DATA_END, progress=False, auto_adjust=False)
...
n_total = len(df)
print(f"  EEM joined data: {df.index[0].strftime('%Y-%m-%d')} to "
...
for name, start, end in OOS_WINDOWS:
    ...
    if n_w > 0:
        print(f"  {name} ({start} to {end}): n={n_w}, VIX max={np.max(vix_w):.1f}, "
              f"ret std(ann)={np.std(ret_w)*np.sqrt(252):.3f}, "
              f"max drawdown return={np.min(ret_w)*100:.1f}%")

# ==================================================================
# SECTION 3: MODEL IMPLEMENTATIONS (identical to K1075/K1078/K1080)
# ==================================================================
...
def gjr_loglik(params, returns):
    """Standard GJR-GARCH(1,1) log-likelihood (negative)."""
...
def fit_gjr(returns):
    """Fit GJR-GARCH(1,1) via multi-start L-BFGS-B."""
    var0 = np.var(returns)
    best_ll = np.inf
    best_params = None
    converged = False
...
def fit_a4f(returns, vol_vals):
    ...
    A4f specification (K988 winner):
      ...
      g_t = omega_g + alpha * u_{t-1}^2
            + gamma * u_{t-1}^2 * I(u_{t-1} < 0) + beta * g_{t-1}
      sigma^2_t = tau_t * g_t
...
for t_idx, abs_idx in enumerate(oos_indices):
    ...
    # Trigger refit at window boundary, beginning, and every REFIT_EVERY days
    ...
    # --- Forecast for t_idx using most recent parameters ---
...
def hac_dm_test(d_array):
    """Newey-West HAC Diebold-Mariano test."""
...
# --- VIX buckets ---
print("\n  VIX bucket analysis (using lagged VIX to match A4f's tau_t = f(VIX_{t-1})):")
...
if valid_theta1:
    ...
    # P10-P90 core span (robust to outliers, matches K1080 reporting)
    ...
    print(f"    CV: {theta1_stats['cv']:.3f}, full span: "
          f"{theta1_stats['orders_of_magnitude_span']:.2f}, "
          f"P10-P90 span: {theta1_stats['orders_of_magnitude_span_p10p90']:.2f}")
...
if full_dm is not None and np.isfinite(full_dm):
    ...
    print(f"  H1 (EEM Full OOS A4f > GJR, DM t>3): {h1_verdict} (t={full_dm:+.3f})")
else:
    h1_verdict = 'N/A'
    print("  H1: N/A")
...
gfc_dm = gfc.get('dm_t')
if gfc_diff is not None:
    h2_verdict = 'PASS' if gfc_diff < 0 else 'FAIL'
    print(f"  H2 (EEM GFC A4f improves): {h2_verdict} "
...
eem_orders_full = results.get('theta1_stability', {}).get('orders_of_magnitude_span')
eem_orders_p90  = results.get('theta1_stability', {}).get('orders_of_magnitude_span_p10p90')
h5_verdict = 'N/A'
if eem_orders_full is not None:
    # Use P10-P90 core span (robust) for stability classification
    core = eem_orders_p90 if eem_orders_p90 is not None else eem_orders_full
    if core <= 2.0:
        h5_verdict = 'STABLE (SPY-like)'
    elif core <= 3.5:
        h5_verdict = 'INTERMEDIATE (QQQ/IWM-like)'
    else:
        h5_verdict = 'UNSTABLE (TW-like)'
    print(f"  H5 (theta1 stability): {h5_verdict} "
          f"(full span={eem_orders_full:.2f}, P10-P90={eem_orders_p90:.2f})")

# H6: discrimination verdict for H_non_us vs H_tw_uniq
if full_dm is not None and np.isfinite(full_dm):
    if abs(full_dm) > 3.0 and full_dm > 0:
        h6_verdict = 'Taiwan-specific (A4f works on EM diversified ETF)'
        paper9_claim = ('A4f works on USD-denominated liquid ETFs including EM. '
                        '0050.TW null is specific to Taiwan (local currency + '
                        'retail-dominated flow + concentrated TSMC exposure).')
    elif full_dm > 1.96:
        h6_verdict = 'Marginal: A4f directional but weakens outside US'
        paper9_claim = ('A4f effect weakens outside US but remains directional. '
                        'Cross-market effect attenuates with market isolation.')
    elif full_dm > -1.96:
        h6_verdict = 'Non-US failure: A4f requires US market structure'
        paper9_claim = ('A4f requires US market structure. Both EEM and 0050.TW '
                        'show null/marginal results, bounding the claim to US equity.')
    else:
        h6_verdict = 'Non-US failure (strong): A4f fails on EM too'
        paper9_claim = ('A4f fails outside US. Non-US assets (EEM, 0050.TW) both '
                        'reject. VIX-MIDAS captures US-specific reflexivity.')
    print(f"  H6 (Discrimination H_non_us vs H_tw_uniq): {h6_verdict}")
    print(f"     Paper 9 claim: {paper9_claim}")
else:
    h6_verdict = 'N/A'
    paper9_claim = 'N/A'
...
results['hypothesis_verdicts'] = {
    ...
    'H6_discrimination': h6_verdict,
    'paper9_claim': paper9_claim,
}

# ==================================================================
# SECTION 7: FIVE-ASSET COMPARISON (Paper 9 final table)
# ==================================================================
print("\n[7] Five-asset comparison "
      "(SPY K1075 / QQQ K1078 / IWM K1080 / EEM K1081 / 0050.TW K1077)...")
    ...
    try:
        ...
        out = {
            ...
            'per_window': {k: {'dm': v['dm_t'], 'diff_pct': v['qlike_diff_pct']}
                           for k, v in r['per_window'].items()},
            'gfc': {'dm': r.get('crisis_subperiods', {}).get('GFC', {}).get('dm_t'),
                    'diff_pct': r.get('crisis_subperiods', {}).get('GFC', {}).get('qlike_diff_pct')},
            'theta1_orders': r.get('theta1_stability', {}).get('orders_of_magnitude_span')
                             if r.get('theta1_stability') else None,
        }
        five_asset[key] = out
...
load_ref(K1075_RESULTS_PATH, 'SPY_K1075')
load_ref(K1078_RESULTS_PATH, 'QQQ_K1078')
load_ref(K1080_RESULTS_PATH, 'IWM_K1080')
load_ref(K1077_RESULTS_PATH, 'TW_K1077')

# EEM current experiment
eem_gfc = results['crisis_subperiods'].get('GFC', {})
five_asset['EEM_K1081'] = {
    'full_dm': results['full_oos']['dm_t'],
    'full_diff_pct': results['full_oos']['qlike_diff_pct'],
    'full_n': results['full_oos']['n'],
    'harvey_pass': results['full_oos']['harvey_pass'],
    'per_window': {k: {'dm': v['dm_t'], 'diff_pct': v['qlike_diff_pct']}
                   for k, v in results['per_window'].items()},
    'gfc': {'dm': eem_gfc.get('dm_t'), 'diff_pct': eem_gfc.get('qlike_diff_pct')},
    'theta1_orders': results.get('theta1_stability', {}).get('orders_of_magnitude_span'),
    'theta1_orders_p10p90': results.get('theta1_stability', {}).get('orders_of_magnitude_span_p10p90'),
}
results['five_asset_comparison'] = five_asset

print(f"  {'Asset':<14} {'Full DM':>10} {'Full Diff%':>12} {'Harvey':>8} {'theta1 span':>12}")
for label in ['SPY_K1075', 'QQQ_K1078', 'IWM_K1080', 'EEM_K1081', 'TW_K1077']:
    r = five_asset.get(label)
    if r is None:
        continue
    ...
    print(f"  {label:<14} {r['full_dm']:>+10.3f} {r['full_diff_pct']:>+11.2f}% "
          f"{'PASS' if r['harvey_pass'] else 'FAIL':>8} {orders_str:>12}")
...
results['metadata'] = {
    ...
    'asset': 'EEM',
    'asset_description': 'iShares MSCI Emerging Markets ETF (diversified EM basket, USD)',
    ...
    'upstream_experiments': [
        ...
        'K1075 SPY extended 2007-2026 DM t=+7.92 Harvey-PASS theta1 <1',
        'K1077 0050.TW extended 2010-2025 DM t=-0.49 NS theta1 ~4',
        'K1078 QQQ extended 2007-2026 DM t=+5.99 Harvey-PASS theta1 1.91',
        'K1080 IWM extended 2007-2026 DM t=+4.80 Harvey-PASS theta1 3.54',
    ],
    'notes': (
        'EEM IPO 2003-04-15. At 2007-01-01 there are ~930 training observations '
        '(vs WINDOW=2000 requested). Code uses max(0, abs_idx - WINDOW) so the '
        'first refit uses all available history — same behavior as K1075 SPY '
        '(1758), K1078 QQQ (1965), K1080 IWM (~1680). VIX is the exogenous IV '
        'regressor (not VXEEM) because (a) VXEEM has limited history from '
        'yfinance and (b) VIX as a global risk-off signal is the relevant '
        'test for the non-US vs Taiwan-specific discrimination.'
    ),
}
...
try:
    ...
    ax1.set_title(f'{EXPERIMENT_ID} EEM: QLIKE by OOS Window')
    ax1.legend()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1081_extended_dm.png'), dpi=120)
    plt.close()
    print("    k1081_extended_dm.png")
    ...
    ax.set_title(f'{EXPERIMENT_ID} EEM: A4f vs GJR across Crisis Sub-periods\n'
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1081_crisis_periods.png'), dpi=120)
    plt.close()
    print("    k1081_crisis_periods.png")
    ...
    ax.set_title(f'{EXPERIMENT_ID} EEM: A4f Performance Across VIX Regimes (OOS 2007-2026)')
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1081_vix_bucket.png'), dpi=120)
    plt.close()
    print("    k1081_vix_bucket.png")
    ...
    ax.set_yscale('log')
    ax.set_title(f'{EXPERIMENT_ID} EEM: A4f θ₁ Evolution across Rolling Refits (2007-2026)')
    ...
    for cname, cstart, cend in CRISIS_PERIODS:
        ax.axvspan(pd.to_datetime(cstart), pd.to_datetime(cend),
                   ...
                   label=('Crisis periods' if not gfc_labelled else None))
        gfc_labelled = True
    ax.legend()

    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1081_theta1_evolution.png'), dpi=120)
    plt.close()
    print("    k1081_theta1_evolution.png")

    # ------- Plot 5: Five-asset comparison -------
    fig, axes = plt.subplots(1, 3, figsize=(22, 6))

    # (a) Full-OOS DM for all 5
    ax = axes[0]
    assets_order = [
        ('SPY (K1075)',     five_asset.get('SPY_K1075')),
        ('QQQ (K1078)',     five_asset.get('QQQ_K1078')),
        ('IWM (K1080)',     five_asset.get('IWM_K1080')),
        ('EEM (K1081)',     five_asset.get('EEM_K1081')),
        ('0050.TW (K1077)', five_asset.get('TW_K1077')),
    ]
    labels_a = []
    dms = []
    bar_colors = []
    for label, r in assets_order:
        if r is None:
            continue
        labels_a.append(label)
        dms.append(r['full_dm'])
        if abs(r['full_dm']) > 3.0 and r['full_dm'] > 0:
            bar_colors.append('green')
        elif abs(r['full_dm']) > 1.96 and r['full_dm'] > 0:
            bar_colors.append('orange')
        elif abs(r['full_dm']) > 1.96:
            bar_colors.append('firebrick')
        else:
            bar_colors.append('gray')
    ax.bar(labels_a, dms, color=bar_colors, alpha=0.78)
    ax.axhline(3.0, color='red', linestyle='--', alpha=0.5, label='Harvey |t|=3')
    ax.axhline(-3.0, color='red', linestyle='--', alpha=0.5)
    ax.axhline(1.96, color='gray', linestyle=':', alpha=0.4, label='|t|=1.96')
    ax.axhline(-1.96, color='gray', linestyle=':', alpha=0.4)
    ax.axhline(0, color='black', lw=0.5)
    ax.set_ylabel('Full-OOS DM t-stat (A4f vs GJR)')
    ax.set_title('Paper 9 Cross-Asset Axis: Full OOS DM')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    # (b) theta1 span across 5 assets
    ax = axes[1]
    order_labels = []
    order_vals = []
    for label, r in [('SPY',     five_asset.get('SPY_K1075')),
                     ('QQQ',     five_asset.get('QQQ_K1078')),
                     ('IWM',     five_asset.get('IWM_K1080')),
                     ('EEM',     five_asset.get('EEM_K1081')),
                     ('0050.TW', five_asset.get('TW_K1077'))]:
        if r is None or r.get('theta1_orders') is None:
            continue
        order_labels.append(label)
        order_vals.append(r['theta1_orders'])
    if order_vals:
        color_o = ['green' if v <= 2.0 else ('orange' if v <= 3.5 else 'red')
                   for v in order_vals]
        ax.bar(order_labels, order_vals, color=color_o, alpha=0.78)
        ...
        ax.set_title('θ₁ Stability across 5 assets (lower = more stable)')
        ax.legend()
        ax.grid(True, alpha=0.3)
        for i, v in enumerate(order_vals):
            ax.text(i, v + 0.08, f'{v:.2f}', ha='center', fontsize=10, fontweight='bold')
    else:
        ax.text(0.5, 0.5, 'theta1 orders unavailable', ha='center', va='center',
                transform=ax.transAxes)
        ax.axis('off')

    # (c) summary text panel
    ax = axes[2]
    ax.axis('off')
    lines = ['Paper 9 Five-Asset A4f Table\n']
    lines.append(f"{'Asset':<14}{'DM':>8}{'Diff%':>9}{'Harvey':>8}{'θ1span':>9}\n")
    lines.append('-' * 50 + '\n')
    for label, r in [('SPY K1075',     five_asset.get('SPY_K1075')),
                     ('QQQ K1078',     five_asset.get('QQQ_K1078')),
                     ('IWM K1080',     five_asset.get('IWM_K1080')),
                     ('EEM K1081',     five_asset.get('EEM_K1081')),
                     ('TW K1077',      five_asset.get('TW_K1077'))]:
        if r is None:
    ...
    for label, r in [('SPY', five_asset.get('SPY_K1075')),
        ...
        if r is None:
            continue
        gfc_r = r.get('gfc', {})
        if gfc_r.get('dm') is not None:
            lines.append(f"  {label:<4} DM={gfc_r['dm']:+.3f}, "
                         f"diff {gfc_r['diff_pct']:+.2f}%\n")
    lines.append(f"\nDiscrimination verdict:\n  {h6_verdict}\n")
    ax.text(0, 1, ''.join(lines), family='monospace', fontsize=10,
            verticalalignment='top')
    ax.set_title('Summary')

    plt.suptitle(f'{EXPERIMENT_ID}: Five-Asset A4f — Distinguishing Non-US vs Taiwan-Specific',
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'k1081_five_asset_comparison.png'),
                dpi=120, bbox_inches='tight')
    plt.close()
    print("    k1081_five_asset_comparison.png")
