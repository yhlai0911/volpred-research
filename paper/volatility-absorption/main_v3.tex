...
\label{eq:nsi_rv}
\end{equation}
where $RV_t^{(20)} = \sum_{i=1}^{20} r_{t-i}^2$. We re-estimate the absorption regression using $\text{NSI}^{\text{RV}}$ as the dependent variable and the 20-day realized volatility as the independent variable. The slope remains strongly negative ($\hat{\beta}^{\text{RV}} = -0.01249$, $t = -8.2$, $N = 767$) under the 2026-04-19 pinned snapshot,\footnote{Under the paper's original data snapshot, the RV-normalized estimate was $-0.0031$ ($t = -2.76$). The pinned-snapshot estimate is substantially larger in magnitude and more significant, reflecting that the RV normalization is robust to---and in fact strengthened by---the yfinance data revision.} confirming that the absorption effect is not an artifact of VIX-specific measurement. The RV-normalized regression provides our strongest and most robust quantitative evidence for the absorption hypothesis.
