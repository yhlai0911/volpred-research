...
Elevated ($20 \leq V < 25$) & 27  & 1.046 & 1.19 & 1.14 & 0.254 \\
...
This pattern is directionally consistent with the absorption hypothesis. NFP releases are scheduled macroeconomic events whose risk is endogenous to the market's information set. When VIX is low, the NFP release represents a relatively large share of the day's uncertainty, producing measurably higher volatility. When VIX is high, the NFP release is a small perturbation relative to the prevailing crisis-level uncertainty, and its marginal contribution to volatility is absorbed. The small sample sizes in the elevated ($n = 27$) and high ($n = 28$) regimes limit the statistical power of regime-specific tests; the \textit{overall} NFP volatility effect (1.16$\times$ versus all non-NFP days, $p = 0.051$) and the monotone decline in the ratio from calm to crisis regimes (1.31$\times$, 1.23$\times$, 1.19$\times$, 0.94$\times$) provide the strongest directional evidence for the absorption interpretation. We stress that the overall effect sits at the 10\% margin rather than clearing conventional 5\% significance, and that the regime-level evidence---significant at the 1\% level in calm markets and absent in crisis markets---is what carries the absorption reading.
