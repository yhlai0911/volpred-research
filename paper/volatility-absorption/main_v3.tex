...
Event-level evidence provides directionally supportive evidence. We examine nonfarm payroll (NFP) release days---a scheduled macroeconomic event whose impact should, in theory, depend on the surprise component rather than the ambient fear level. Overall, NFP days produce absolute returns 1.16 times the all-non-NFP average ($p = 0.039$, borderline---the equal-variance variant of the same test gives $p = 0.051$; 1.19$\times$ versus Fridays only, $p = 0.036$; Welch's two-sample $t$-tests, $N_{\text{NFP}} = 194$, sample 2010--2026).\footnote{The overall NFP volatility ratios and all regime-specific ratios reported in this section are estimated from 194 NFP release dates over 4{,}084 SPY trading days (194 NFP plus 3{,}890 non-NFP days, January 2010 to March 2026). Release dates are taken from the official Bureau of Labor Statistics ``Employment Situation'' release calendar, retrieved from the Federal Reserve Bank of St.\ Louis ALFRED real-time archive (release id 50); we do not use a first-Friday-of-month approximation, which misdates 33 of the 194 releases in this window and additionally assigns an event to October 2025, a month in which no Employment Situation report was published. Where a release falls on a market holiday---five Good Fridays in this sample, on which BLS published but US equity markets were closed---the event is assigned forward to the next trading day, since no reaction is observable on the release date itself. Data source: yfinance (SPY, \^{}VIX); reproducible values are bound in \texttt{paper/volatility-absorption/reproduce.py}.} In calm markets (VIX $< 15$) the ratio is 1.31$\times$ ($p = 0.026$, $n = 63$); in the medium-VIX regime (15--20) it is 1.23$\times$ ($p = 0.029$). When VIX exceeds 25, the NFP effect vanishes: the ratio drops to 0.94$\times$ ($p = 0.707$). The ordered decline from low to high VIX is consistent with the absorption hypothesis, but we do not claim it as established, for two reasons. The four regime $p$-values are unadjusted and none survives Holm correction across that family (smallest adjusted $p = 0.104$); and tested directly, the calm-minus-crisis ratio difference is $+0.37$ with a 95\% bootstrap interval of $[-0.10,\ 0.79]$ that includes zero, with the crisis cell holding only $n = 28$ NFP days. The NFP evidence is descriptive support, not the basis of the paper's inference.
...
\item \textit{Test variant.} All tests are Welch's unequal-variance $t$-tests. We fix this a priori rather than selecting it from a variance pre-test, since the two-stage procedure inflates size \citep{zimmerman2004, ruxton2006, delacre2017}; a Brown-Forsythe test in fact finds no evidence of unequal variance here ($p = 0.48$). The choice is consequential and we report the alternative: under Student's equal-variance $t$-test the overall NFP effect gives $p = 0.051$ rather than $p = 0.039$, i.e.\ the two variants fall on opposite sides of the 5\% line. It is not a choice that flatters the results---see the multiplicity note. % source: k741c.test_variant_disclosure (levene_bf_p_vs_all, overall_p_student, overall_p_welch)
\item \textit{Multiple comparisons.} The four regime tests form a family and the $p_{\text{Holm}}$ column reports Holm-Bonferroni step-down adjusted values across it. \textbf{No regime effect survives correction at the 5\% level}: the smallest adjusted $p$ is 0.104. Readers should treat the unadjusted column as descriptive. The correction interacts with the variant choice in the direction that works against us---under Student's the calm regime would survive ($p_{\text{Holm}} = 0.036$), under the Welch specification we report it does not. The overall vs.-all and vs.-Friday tests are not folded into this family: they are two framings of a single hypothesis on one sample, reported jointly rather than screened for the smaller $p$. % source: k741c.test_variant_disclosure.multiplicity (adjusted_welch / adjusted_student)
\end{tablenotes}
\end{threeparttable}
\end{table}

This pattern is directionally consistent with the absorption hypothesis. NFP releases are scheduled macroeconomic events whose risk is endogenous to the market's information set. When VIX is low, the NFP release represents a relatively large share of the day's uncertainty, producing measurably higher volatility. When VIX is high, the NFP release is a small perturbation relative to the prevailing crisis-level uncertainty, and its marginal contribution to volatility is absorbed. The small sample sizes in the elevated ($n = 27$) and high ($n = 28$) regimes limit the statistical power of regime-specific tests; the \textit{overall} NFP volatility effect (1.16$\times$ versus all non-NFP days, $p = 0.039$) and the ordered decline in the ratio from calm to crisis regimes (1.31$\times$, 1.23$\times$, 1.19$\times$, 0.94$\times$) are directionally consistent with the absorption interpretation. We deliberately state this weakly, for three reasons. First, the overall effect is borderline rather than robust: it clears 5\% under the Welch specification we report ($p = 0.039$) but not under the equal-variance variant ($p = 0.051$), and a result that flips on that choice should not be described as significant without qualification. Second, once the four regime tests are treated as the family they are, \textit{no individual regime effect survives Holm correction}---the calm regime, which carries most of the section's apparent evidence, moves from $p = 0.026$ unadjusted to $p_{\text{Holm}} = 0.104$. The regime-level significance reported in earlier drafts of this section does not hold up. Third, and most importantly, the regime contrast is \textit{not} established by the fact that the calm-regime ratio is significant while the crisis-regime ratio is not: that is a difference in significance, not a significance of difference. Testing the contrast directly---the calm-minus-crisis ratio difference, under a 20-day circular moving-block bootstrap that preserves volatility clustering and re-derives every regime ratio per replicate ($B = 10{,}000$)---gives a point estimate of $+0.37$ with a 95\% interval of $[-0.10,\ 0.79]$ ($p = 0.115$), which does not exclude zero. The ordered-trend statistic behaves the same way: the four regime ratios are perfectly ordered in sample (Spearman $\rho = -1.00$), but resampling shows that ordering is not stable (bootstrap mean $\rho = -0.63$, 95\% interval $[-1.00,\ 0.40]$, which includes zero). With four regime cells this statistic is coarse and we do not lean on it. With only 28 NFP days in the crisis regime this test has little power, so the non-rejection is emphatically \textit{not} evidence that the regimes are equal---but it does mean the NFP evidence cannot carry the absorption claim on its own. We therefore treat the NFP results as a descriptive pattern consistent with absorption, and rest the paper's inference on the SAR evidence of Section~\ref{sec:results}.
