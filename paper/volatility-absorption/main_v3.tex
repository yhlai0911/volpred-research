...
Event-level evidence provides directionally supportive evidence. We examine nonfarm payroll (NFP) release days---a scheduled macroeconomic event whose impact should, in theory, depend on the surprise component rather than the ambient fear level. Overall, NFP days produce absolute returns 1.16 times the all-non-NFP average (unadjusted $p = 0.039$; 1.19$\times$ versus Fridays only, unadjusted $p = 0.036$; Welch's two-sample $t$-tests, $N_{\text{NFP}} = 194$, sample 2010--2026), but we do not claim significance for this: Holm correction across the two comparisons adjusts both to $p = 0.072$, and the equal-variance variant of the vs.-all test gives $p = 0.051$.\footnote{The overall NFP volatility ratios and all regime-specific ratios reported in this section are estimated from 194 NFP release dates over 4{,}084 SPY trading days (194 NFP plus 3{,}890 non-NFP days, January 2010 to March 2026). Release dates are taken from the official Bureau of Labor Statistics ``Employment Situation'' release calendar, retrieved from the Federal Reserve Bank of St.\ Louis ALFRED real-time archive (release id 50); we do not use a first-Friday-of-month approximation, which misdates 33 of the 194 releases in this window and additionally assigns an event to October 2025, a month in which no Employment Situation report was published. Where a release falls on a market holiday---five Good Fridays in this sample, on which BLS published but US equity markets were closed---the event is assigned forward to the next trading day, since no reaction is observable on the release date itself. Data source: yfinance (SPY, \^{}VIX); reproducible values are bound in \texttt{paper/volatility-absorption/reproduce.py}.} In calm markets (VIX $< 15$) the ratio is 1.31$\times$ (unadjusted $p = 0.026$, $n = 63$); in the medium-VIX regime (15--20) it is 1.23$\times$ ($p = 0.029$). When VIX exceeds 25 the measured effect disappears, the ratio falling to 0.94$\times$ ($p = 0.707$), though with $n = 28$ that cell is too imprecise to establish a reversal. The ordered decline from low to high VIX is consistent with the absorption hypothesis, but we do not claim it as established, for two reasons. The regime $p$-values are unadjusted and none survives Holm correction (smallest adjusted $p = 0.104$); and tested directly, the calm-minus-crisis ratio difference is $+0.37$ with a 95\% bootstrap interval of $[-0.10,\ 0.79]$ that includes zero. The NFP evidence is descriptive support, not the basis of the paper's inference.
...
Table~\ref{tab:nfp} reports the NFP-day volatility ratio across VIX regimes. Event dates are the official BLS ``Employment Situation'' release dates rather than a first-Friday-of-month approximation, and releases falling on market holidays are assigned forward to the next trading day.\footnote{The first-Friday proxy is wrong for 33 of the 194 releases in this window---most often because when the first of the month falls on a Friday the reference-month data are not yet ready and BLS releases on the second Friday---and it additionally assigns an event day to October 2025, when the report was cancelled during the federal government shutdown. We note for transparency that these two corrections do not act in the same direction: holding the mapping rule fixed, moving from the proxy calendar to the official calendar changes the overall ratio only marginally (1.149$\times$ to 1.151$\times$), whereas correcting the holiday mapping accounts for most of the movement. The corrected specification is reported throughout; the full $2\times2$ decomposition is in the replication package.} Overall, NFP days produce absolute SPY returns 1.16 times the all-non-NFP average (unadjusted $p = 0.039$; 1.19$\times$ versus Fridays only, unadjusted $p = 0.036$; Welch's two-sample $t$-tests). Neither figure should be read as significant. They are fragile in two separate ways. First, under the equal-variance variant of the same test the vs.-all comparison gives $p = 0.051$, so the result changes sides of the 5\% line on an auxiliary specification choice. Second, and more decisively, these two comparisons are themselves a family of two, and Holm correction across them adjusts \textit{both} to $p = 0.072$. In the low-VIX regime (VIX $< 15$), the NFP ratio is 1.31$\times$ ($t = 2.28$, unadjusted $p = 0.026$; $n = 63$ NFP days). In the medium-VIX regime (15--20), the ratio is 1.23$\times$ ($p = 0.029$). In the elevated regime (20--25), it weakens to 1.19$\times$ ($p = 0.266$). In the high-fear regime (VIX $\geq 25$), the ratio falls to 0.94$\times$ ($p = 0.707$)---the point estimate is below one, but it is imprecise and compatible with effects in either direction, so we do not read it as the NFP effect reversing. Every regime $p$-value quoted here is unadjusted; the table reports Holm values, and \textit{no} regime effect survives correction (smallest adjusted $p = 0.104$). As the table note sets out, we tried every reasonable definition of the testing family, and under all of them---including the two overall comparisons on their own---nothing in this section clears 5\%.
...
\toprule
VIX Regime & NFP Days & $\overline{|r|}_{\text{NFP}}$ (\%) & $\overline{|r|}_{\text{NFP}} / \overline{|r|}_{\text{non-NFP}}$ & $t$-stat & $p$-value & $p_{\text{Holm}}$ \\
\midrule
Low ($V < 15$)              & 63  & 0.527 & 1.31 & 2.28 & 0.026 & 0.104 \\ % source: experiments/k741/k741_nfp_event_study_canonical_results.json .part_b_vix_regimes (official BLS calendar re-run, 2026-07-19; Welch headline + Holm over the four-regime family); column sum 63+76+27+28=194 matching the stated total
% sources: K741-canonical (experiments/k741/k741_nfp_event_study_canonical_results.json .part_b_vix_regimes .p_value/.p_value_holm); corroborated by experiments/k904/k904_task_s4_nfp_canonical_results.json .by_vix_regime
Medium ($15 \leq V < 20$)   & 76  & 0.788 & 1.23 & 2.23 & 0.029 & 0.104 \\
Elevated ($20 \leq V < 25$) & 27  & 1.046 & 1.19 & 1.13 & 0.266 & 0.533 \\
High ($V \geq 25$)          & 28  & 1.417 & 0.94 & $-0.38$ & 0.707 & 0.707 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} NFP days are the official Bureau of Labor Statistics ``Employment Situation'' release dates, retrieved from the St.\ Louis Fed ALFRED real-time archive (release id 50); where a release falls on a market holiday the event is assigned to the next trading day. $|r|$ is the absolute daily return of SPY (\%). $p$-values are from a Welch two-sample $t$-test comparing NFP-day and non-NFP-day absolute returns within each VIX regime. Total NFP days: 194, against 3{,}890 non-NFP days over the sample period 2010--2026 ($N = 4{,}084$ trading days). Data source: yfinance (SPY, \^{}VIX), 2026-04-19 pinned snapshot. % source: experiments/k741/k741_nfp_event_study_canonical_results.json .part_a_historical (n_nfp=194, n_non_nfp=3890; headline cell official__forward_mapper)
\item \textit{Test variant.} All tests are Welch's unequal-variance $t$-tests, chosen because the unconditional use of Welch avoids the size inflation of selecting a variant from a variance pre-test \citep{zimmerman2004, ruxton2006, delacre2017}; a Brown-Forsythe test finds no evidence of unequal variance here ($p = 0.48$), so this is a default rather than a response to the data. We state plainly that this was settled during revision with both results known, not pre-specified, and we therefore report the alternative rather than resting on the choice: under Student's equal-variance $t$-test the overall NFP effect gives $p = 0.051$ rather than $p = 0.039$, so the two variants fall on opposite sides of the 5\% line. The variant we report is the less favourable one where this section's argument lives---see the multiplicity note. % source: k741c.test_variant_disclosure (chosen_a_priori=false, levene_bf_p_vs_all, overall_p_student, overall_p_welch)
\item \textit{Multiple comparisons.} The $p_{\text{Holm}}$ column reports Holm-Bonferroni step-down values across the four regime tests. Because the choice of family is itself a researcher degree of freedom, we report every grouping a reader might reasonably ask for rather than selecting one: the two overall comparisons alone (both adjust to 0.072), the four regime tests alone (smallest 0.104), the primary overall test plus the four regimes (smallest 0.130), and all six together (smallest 0.155). \textbf{Under every one of these families, nothing clears 5\%}---including the overall effect, whose unadjusted $p = 0.039$ does not survive correction against even its own companion test. The unadjusted columns should be read as descriptive only. The correction also interacts with the variant choice against us: under Student's the calm regime would survive the regime family ($p_{\text{Holm}} = 0.036$), under the Welch specification we report it does not. % source: k741c.test_variant_disclosure.multiplicity.min_adjusted_p_by_family (overall_pair / regimes_only / primary_overall_plus_regimes / all_six)
\end{tablenotes}
\end{threeparttable}
\end{table}

This pattern is directionally consistent with the absorption hypothesis. One interpretation runs as follows: NFP releases are scheduled macroeconomic events whose risk is endogenous to the market's information set, so when VIX is low the release would represent a relatively large share of the day's uncertainty, while when VIX is high it would be a small perturbation relative to prevailing crisis-level uncertainty and its marginal contribution would be absorbed. We stress that this is an interpretation offered to motivate the pattern, not a mechanism these data establish: the regime contrast is not statistically distinguishable from zero (see below), and the binary-event design carries no surprise controls, so we cannot separate absorption of a same-sized surprise from variation in surprise magnitude across regimes. The small sample sizes in the elevated ($n = 27$) and high ($n = 28$) regimes limit the statistical power of regime-specific tests; the \textit{overall} NFP volatility effect (1.16$\times$ versus all non-NFP days) and the ordered decline in the ratio from calm to crisis regimes (1.31$\times$, 1.23$\times$, 1.19$\times$, 0.94$\times$) are directionally consistent with the absorption interpretation. We state this weakly, and the reasons are worth being explicit about, because taken together they amount to a real retreat from how earlier drafts of this section read. First, no result in this section survives multiple-comparison correction under any reasonable definition of the family---not the regime tests (smallest adjusted $p = 0.104$), and not the overall effect, which adjusts to $p = 0.072$ against its own companion comparison alone. The regime-level and overall significance reported in earlier drafts does not hold up. Second, the overall effect is additionally fragile to the equal-variance assumption, giving $p = 0.039$ under the Welch specification we report and $p = 0.051$ under Student's; a result that flips on that choice would not warrant a significance claim even without the multiplicity problem. Third, and most importantly, the regime contrast is \textit{not} established by the fact that the calm-regime ratio has a smaller $p$-value than the crisis-regime ratio: that is a difference in significance, not a significance of difference. Testing the contrast directly---the calm-minus-crisis ratio difference, under a 20-day circular moving-block bootstrap that preserves volatility clustering and re-derives every regime ratio per replicate ($B = 10{,}000$)---gives a point estimate of $+0.37$ with a 95\% interval of $[-0.10,\ 0.79]$ ($p = 0.115$), which does not exclude zero. The ordered-trend statistic behaves the same way: the four regime ratios are perfectly ordered in sample (Spearman $\rho = -1.00$), but resampling shows that ordering is not stable (bootstrap mean $\rho = -0.63$, 95\% interval $[-1.00,\ 0.40]$, which includes zero). With four regime cells this statistic is coarse and we do not lean on it. With only 28 NFP days in the crisis regime this test has little power, so the non-rejection is emphatically \textit{not} evidence that the regimes are equal---but it does mean the NFP evidence cannot carry the absorption claim on its own. We therefore treat the NFP results as a descriptive pattern consistent with absorption, and rest the paper's inference on the SAR evidence of Section~\ref{sec:results}.
...
\bibitem[Delacre et al.(2017)]{delacre2017}
Delacre, M., Lakens, D., \& Leys, C. (2017). Why psychologists should by default use Welch's $t$-test instead of Student's $t$-test. \textit{International Review of Social Psychology}, 30(1), 92--101.
...
\bibitem[Ruxton(2006)]{ruxton2006}
Ruxton, G.~D. (2006). The unequal variance $t$-test is an underused alternative to Student's $t$-test and the Mann--Whitney $U$ test. \textit{Behavioral Ecology}, 17(4), 688--690.
...
\bibitem[Zimmerman(2004)]{zimmerman2004}
Zimmerman, D.~W. (2004). A note on preliminary tests of equality of variances. \textit{British Journal of Mathematical and Statistical Psychology}, 57(1), 173--181.
