...
Event-level evidence provides directionally supportive evidence. We examine nonfarm payroll (NFP) release days---a scheduled macroeconomic event whose impact should, in theory, depend on the surprise component rather than the ambient fear level. Overall, NFP days produce absolute returns 1.17 times normal ($p = 0.074$; Welch's $t$-test, $N_{\text{NFP}} = 195$, sample 2010--2026).\footnote{The 1.17$\times$ overall NFP volatility ratio and all regime-specific ratios reported in this section are estimated from 195 NFP release dates over 4,081 SPY trading days (January 2010 to March 2026), using Bureau of Labor Statistics release calendar dates and daily SPY absolute returns. Data source: yfinance (SPY, \^{}VIX). Replication code available upon request.} In the medium-VIX regime (15--20), the ratio is 1.30$\times$ ($p = 0.092$), while in calm markets (VIX $< 15$), the ratio is 1.24$\times$ though only marginally significant ($p = 0.069$, $n = 63$). When VIX exceeds 25, the NFP effect vanishes: the ratio drops to 0.95$\times$ ($p = 0.777$). The general decline from low/medium VIX to high VIX is consistent with the absorption hypothesis, though small sample sizes in high-VIX regimes ($n = 28$) limit the statistical power of regime-specific tests.
...
Table~\ref{tab:nfp} reports the NFP-day volatility ratio across VIX regimes. Overall, NFP days produce absolute SPY returns 1.17 times the non-NFP average ($p = 0.074$, Welch's $t$-test). In the low-VIX regime (VIX $< 15$), the NFP ratio is 1.24$\times$, though with limited statistical power ($t = 1.85$, $p = 0.069$; $n = 63$ NFP days). In the medium-VIX regime (15--20), the ratio strengthens to 1.30$\times$ ($p = 0.009$). In the elevated regime (20--25), the ratio weakens to 1.18$\times$ ($p = 0.279$). In the high-fear regime (VIX $\geq 25$), the ratio drops to 0.95$\times$ ($p = 0.777$)---NFP days are actually \textit{less} volatile than average, though the difference is not statistically significant.
...
\midrule
Rate shocks       & Endogenous & 87 & $+0.0004$ & 4.22 & \textbf{Most absorbed} \\
(TLT $|\Delta| > 2\sigma$) & & & & & \\
\addlinespace
Risk-off flights  & Endogenous & 186 & $+0.0001$ & 1.68 & Marginally absorbed \\
(GLD $> 0.5\%$, residual)  & & & & & \\
\addlinespace
Geopolitical shocks & Exogenous & 150 & $+0.0002$ & 2.36 & Absorbed \\
(GLD $> 0.5\%$, priority)  & & & & & \\
\bottomrule
\end{tabular}
\begin{tablenotes}
...
\subsection{Null Simulation: Can GARCH Produce Absorption?}

A natural concern is whether the declining SAR pattern can arise mechanically from a standard GARCH process with no absorption mechanism. We address this by simulating 10,000 return paths from a GJR-GARCH(1,1) with parameters fitted to SPY (persistence = 0.982, leverage $\gamma = 0.244$, Student-$t$ innovations with $\nu = 5.65$), computing the SAR across VIX-equivalent regime bins for each path, and comparing the distribution to the empirical SAR.

The results decisively reject the null. The empirical SAR ranges from 2.33$\times$ (high-VIX regime) to 3.15$\times$ (calm regime), whereas the simulated GARCH SAR is only 1.01--1.23$\times$ across all regimes---a difference of Cohen's $d = 3.9$ to $19.7$. More importantly, the empirical SAR \emph{decline} from calm to high-VIX regimes is 0.816, compared to 0.173 $\pm$ 0.211 under the null ($p = 0.002$, two-sided). Even with persistence raised to 0.99, the simulated decline reaches only 0.15. These results confirm that volatility absorption is a genuine market phenomenon that cannot be explained by GARCH mean reversion alone.
