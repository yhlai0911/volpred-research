...
We test whether the marginal value of daily portfolio rebalancing varies across VIX regimes. Using a simple volatility-targeting framework, unreported analysis (available from the author upon request) finds that the improvement from daily versus monthly rebalancing is primarily driven by the baseline VT mechanism: daily rebalancing achieves a net Sharpe of 1.42, compared to 0.82 for monthly rebalancing, but the tracking error between daily and monthly is only 1.68\% annualized with a return correlation of 0.976. The difference narrows substantially when transaction costs are accounted for, and high-VIX regimes do not disproportionately reward more frequent rebalancing.
...
\begin{enumerate}
    ...
    \item \textbf{VT strategies naturally handle absorption.} The standard $c/\text{VIX}$ weight rule is approximately optimal under the absorption hypothesis. Unreported analysis (available from the author upon request) suggests that adding overlays at high VIX tends to degrade performance, consistent with this framework.
\end{enumerate}
