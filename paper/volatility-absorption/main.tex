...
Table~\ref{tab:regime_dist} reports the distribution of trading days across the five VIX regimes. Calm markets (VIX $< 15$) account for approximately 38\% of trading days, normal markets (15--20) for 27\%, elevated markets (20--25) for 16\%, high-fear markets (25--30) for 8\%, and crisis markets (VIX $\geq 30$) for 11\%. The shock rate ($|\Delta\text{VIX}| > 2$) increases sharply with VIX level, from 7\% in calm markets to over 35\% in crisis markets, confirming that fear shocks are far more frequent when fear is already elevated.
...
\begin{tablenotes}
\small
\item \textit{Notes:} A shock day is defined as $|\Delta\text{VIX}| > 2$, where $\Delta\text{VIX} = V_t - V_{t-1}$. Sample period: January 2006 to March 2026. Total shock days reflect the five-bin classification ($N = 767$ for SPY).
\end{tablenotes}
\end{threeparttable}
\end{table}
