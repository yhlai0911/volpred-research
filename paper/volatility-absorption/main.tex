...
\begin{tablenotes}
\small
\item \textit{Notes:} A shock day is defined as $|\Delta\text{VIX}| > 2$, where $\Delta\text{VIX} = V_t - V_{t-1}$. Sample period: January 2006 to March 2026. The five-bin classification yields $N = 767$ shock days for SPY. The absorption regression (Tables~\ref{tab:cross_asset_detail} and~\ref{tab:robust_threshold}) uses $N = 893$ because it includes all days with $|\Delta\text{VIX}| > 2$ regardless of the direction of the VIX change, whereas the five-bin SAR analysis restricts to days where both SPY returns and VIX data are jointly available within the same regime partition.
\end{tablenotes}
\end{threeparttable}
\end{table}
