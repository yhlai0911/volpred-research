...
\begin{abstract}
\noindent
We document a novel empirical regularity: the marginal impact of fear shocks on realized asset returns diminishes as the ambient fear level rises---a phenomenon we term \textit{volatility absorption}. Using daily data on four assets (SPY, GLD, TLT, 0050.TW) and VIX from 2006 to 2026, we show that the ratio of shocked-day to normal-day absolute returns declines from 3.16$\times$ in calm markets (VIX $< 15$) to 2.32$\times$ in high-fear regimes (VIX 25--30), with a slight reversal to 2.43$\times$ in crisis regimes (VIX $\geq 30$) attributable to extreme outlier events and small samples. This absorption effect is confirmed across three of four assets, with the exception being the Taiwanese equity ETF, consistent with VIX being a US-specific fear gauge. We further decompose shocks by type and find that \textit{endogenous} risks---rate shocks and risk-off flights already reflected in VIX---are heavily absorbed (coefficient $+0.019$), whereas \textit{exogenous} risks such as geopolitical shocks are not ($-0.003$). Event-level evidence from nonfarm payroll releases is directionally consistent: NFP-day volatility is 1.24$\times$ normal in calm markets ($p = 0.069$) and 1.30$\times$ in the medium-VIX regime ($p = 0.009$), but indistinguishable from normal days when VIX exceeds 25 (ratio $= 0.95$, $p = 0.777$). These findings have direct implications for hedging, portfolio rebalancing, and volatility-targeting strategy design.
...
We formalize and quantify this observation under the label \textit{volatility absorption}: the diminishing marginal impact of fear shocks on realized asset price movements as the ambient level of market fear rises. Our central measure is the \textit{normalized shock impact}---the absolute return on a shock day divided by the prevailing VIX level---and we show that this quantity generally declines across VIX regimes. When VIX is below 15 (``calm''), a large VIX shock ($|\Delta\text{VIX}| > 2$) produces an absolute SPY return that is 3.16 times the normal-day average. This ratio declines to 2.77$\times$ (VIX 15--20), 2.37$\times$ (VIX 20--25), and 2.32$\times$ (VIX 25--30), with a slight uptick to 2.43$\times$ in crisis markets (VIX $\geq 30$). The overall decline from calm to stressed markets is economically large and statistically significant.

While we cannot fully rule out mechanical explanations from the contemporaneous VIX-return relationship, the differential absorption across shock types (which share the same VIX denominator) suggests a component beyond pure mechanical correlation. After normalizing returns by VIX level, we estimate a cross-sectional regression of normalized shock impact on VIX and find a slope of $-0.00028$, confirming that each additional VIX point reduces the marginal impact of the next shock. This ``paralysis'' coefficient is robust to alternative shock thresholds and sub-period splits.
...
For each regime $j$, we compute the \textit{shock amplification ratio}:
\begin{equation}
\text{SAR}_j = \frac{\overline{|r_t|}_{t \in S_j}}{\overline{|r_t|}_{t \in N_j}}
\label{eq:sar}
\end{equation}
where $S_j$ is the set of shock days in regime $j$ and $N_j$ is the set of non-shock days in regime $j$. Volatility absorption predicts a general decline in SAR across regimes, though we note that perfect monotonicity may not hold at extremes due to small samples and outlier events in crisis regimes.
...
Table~\ref{tab:absorption_core} presents the central result using a five-bin VIX regime classification. The shock amplification ratio (SAR) for SPY declines from 3.16$\times$ in calm markets (VIX $< 15$) through 2.77$\times$, 2.37$\times$, and 2.32$\times$ as VIX increases. The overall decline from calm to high is 0.84 (26.6\%), statistically significant at the 1\% level.
...
Geopolitical shocks, in contrast, show no statistically significant absorption ($-0.003$, $t = -0.68$, $p > 0.10$), suggesting that these shocks may retain their impact regardless of the ambient fear level. This is economically intuitive: VIX prices in financial and macroeconomic risks but does not price in exogenous geopolitical events. A geopolitical shock represents genuinely new information that is orthogonal to the risks already reflected in VIX, and therefore its impact appears not to diminish with the ambient fear level. However, the small sample size ($N = 89$) limits the statistical power of this test, and we cannot definitively distinguish between ``no absorption'' and ``absorption too small to detect.''

This endogenous/exogenous distinction has suggestive implications for risk management. Hedging against rate shocks appears to become progressively less valuable as VIX rises (because the market has already ``absorbed'' much of the rate risk), while our evidence suggests that hedging against geopolitical risks may retain its value across VIX regimes, though the small sample ($N = 89$) warrants caution in drawing definitive conclusions.
...
Several limitations should be noted. First, our central measure---the normalized shock impact $\text{NSI}_t = |r_t| / V_t$---uses VIX in the denominator while also conditioning on VIX level to define regimes. This creates a potential mechanical correlation: as VIX rises, NSI is deflated by a larger denominator, which could produce a declining NSI-VIX relationship even absent a genuine economic absorption effect. We address this concern in three ways. (i) The shock amplification ratio (SAR), which does \textit{not} divide by VIX, independently confirms the declining pattern (Table~\ref{tab:absorption_core}). (ii) The alternative normalization using realized volatility (Section~\ref{sec:robustness}) produces qualitatively identical results ($\hat{\beta}^{\text{RV}} = -0.0031$, $t = -2.76$). (iii) The endogenous/exogenous decomposition (Table~\ref{tab:shock_types}) shows that the absorption effect varies by shock type---rate shocks show significant absorption while geopolitical shocks do not---which cannot be explained by a purely mechanical VIX-denominator effect. Nevertheless, we acknowledge that the NSI construction introduces a degree of mechanical correlation, and future work using alternative conditioning variables (e.g., realized volatility as both normalizer and regime indicator) would provide a cleaner identification.
