...
| K994 | Cross-Asset Validation of A4f (QQQ / EEM / GLD / 0050.TW) | **Table 6 source**: cross-asset A4f vs GJR validation under the paper-period setup. For 0050.TW, this is the canonical source of the Table 6 row `DM t = 1.44` (stored as `dm_t = -1.4388` in `A4f_vs_GJR`, reported in absolute value because the table note defines positive DM as ``A4f better''). Distinct from K997/K1098, which are later local-fear robustness runs. | `experiments/k994/` |
