...
\begin{tablenotes}
\footnotesize
\item DM $t$: negative values indicate A4f has lower (better) QLIKE than GJR. Harvey significance: $|t| > 3.0$. OOS: 2019--2026. For GLD, A4f uses a single-factor model with the CBOE Gold VIX (GVZ): $\tau_t = \theta_0 + \theta_1 \mathrm{GVZ}_{t-1}^2$.
...
A natural concern is whether the ``source decomposition'' label is merely a relabeling of the standard long-run/short-run narrative in GARCH-MIDAS models. We address this by deriving three propositions that establish the economic content of the decomposition.
...
When $\tau_t = \theta_0 + \theta_1 \mathrm{VIX}_{t-1}^2$, the MLE of $\theta_1$ endogenously corrects for the variance risk premium. Specifically, the ratio $\theta_1 / [1/(252 \times 10{,}000)]$ measures the discount on implied variance relative to the ``no-VRP'' benchmark.
\end{proposition}
...
\begin{proposition}[g Tracks VRP Dynamics]
The dynamic factor $g_t$ reflects time-varying departures of realized variance from the VRP-corrected implied level. The GARCH autoregressive structure smooths the noisy daily ratio $r_t^2/\tau_t$, extracting its persistent VRP component.
\end{proposition}
...
These propositions distinguish source decomposition from mere relabeling: (i)~$\theta_1$ provides parametric identification of VRP magnitude, (ii)~the $\E[g_t]=1$ constraint pins the scale, making $\tau_t$ the sole determinant of unconditional variance, and (iii)~the forecasting gain (DM $t = 4.03$ vs.\ GJR) demonstrates that the decomposition captures information not available to standard models.
