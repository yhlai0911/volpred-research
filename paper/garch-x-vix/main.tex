...
\end{itemize}
These results indicate that A4f is statistically \emph{non-inferior} to HAR-type benchmarks under the $r_t^2$ proxy. Importantly, A4f achieves this parity without requiring high-frequency or realized-variance data as inputs---a practical advantage for daily risk management where intraday data may be unavailable or costly. We caution that, under the standard Patton QLIKE (which penalizes underestimation and is asymmetric in the opposite direction), the relative rankings between GARCH-X and HAR-type models may shift; a formal comparison on realized variance proxies is left for future work.
...
\newblock \url{https://www.risk.net/risk-management/2381658/back-testing-expected-shortfall}
