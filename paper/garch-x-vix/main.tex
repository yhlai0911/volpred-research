...
\end{align}
Contemporaneous normalization follows the recursive structure of \citet[Eq.~4]{engle2013}, where the current volatility environment determines how past shocks are interpreted. Despite the shared time subscript, there is no simultaneity: $\tau_t$ is a deterministic function of $\mathrm{VIX}_{t-1}$, which is observed before $r_t$. Lagged normalization reflects a conservative timing choice in which the normalization denominator $\tau_{t-1}$ is fully predetermined before the shock $r_{t-1}$ is observed, since both $\tau_t$ and $u_{t-1}$ use $\mathrm{VIX}_{t-1}$ information. Our baseline specifications use contemporaneous normalization; lagged normalization is included for comparison (see Section~\ref{sec:taxonomy}).
