...
% ─── Title ───────────────────────────────────────────────────
\title{Multiplicative GARCH-X with VIX: Daily Source Decomposition Outperforms GARCH-MIDAS for Volatility Forecasting and Risk Management\thanks{We thank the VolPred Research System for computational support. Data and replication code are available upon request.}}
...
\begin{abstract}
\noindent
We propose a daily multiplicative GARCH-X model, $\sigma_t^2 = \tau_t \times g_t$, where $\tau_t$ is a simple function of lagged VIX and $g_t$ follows a GJR-GARCH process on standardized returns. In a systematic horse race among 17 specifications---including ten daily GARCH-X variants, six GARCH-MIDAS alternatives with Beta-polynomial weighting, and a GJR-GARCH benchmark---the simplest daily model with $\tau_t = \theta_0 + \theta_1 \mathrm{VIX}_{t-1}^2$ and freely estimated intercept (A4f) achieves the lowest QLIKE loss function, significantly outperforming GJR-GARCH (Diebold-Mariano $t = 4.48$, surpassing the \citet{harvey2016} threshold of $|t| > 3.0$) and uniformly dominating all GARCH-MIDAS specifications. The result extends to QQQ ($t = 3.71$) and, when asset-specific fear indices are used, to GLD with GVZ ($t = 3.17$). We show that the $g_t$ component captures the dynamics of the variance risk premium (Spearman $\rho \approx 0.80$), providing a structural interpretation: $\tau_t$ reflects the option-market fear level while $g_t$ tracks deviations of realized variance from implied variance. However, $g_t$ has no out-of-sample predictive power for future VRP---it is a contemporaneous decomposition, not a forecasting tool. For risk management, A4f with Student-$t$ innovations passes VaR backtesting at two of three confidence levels (scorecard 3/4), whereas GJR-GARCH passes none (scorecard 1/4). These findings demonstrate that GARCH-MIDAS complexity is unnecessary when the external variable updates daily, and that a parsimonious multiplicative structure with VIX delivers both superior point forecasts and improved tail risk measurement.
...
\textbf{Third}, we validate the model's practical value through VaR and Expected Shortfall (ES) backtesting. A4f with Student-$t$ innovations achieves a scorecard of 3/4 (passing VaR at 1\% and 5\% levels plus ES at 2.5\%), while GJR-GARCH achieves only 1/4 regardless of distributional assumption. Cross-asset validation shows that the model generalizes to assets with high VIX correlation (QQQ: DM $t = 3.71$) and, when asset-specific implied volatility indices are employed, to gold (GLD with GVZ: DM $t = 3.39$). For assets with weak VIX linkage---emerging markets (EEM) and Taiwan (0050.TW)---the model improves directionally but does not reach statistical significance, consistent with the interpretation that the multiplicative structure requires a relevant fear index as input.

Our findings contribute to the literature on component volatility models \citep{engle2008, engle2013, conrad2015, conrad2020}, the role of VIX in volatility forecasting \citep{bekaert2014, bayer2020}, and the practical evaluation of volatility models for risk management \citep{patton2011, hansen2011}. The key message is one of parsimony: when the external variable is already daily, GARCH-MIDAS complexity is not only unnecessary but actively detrimental, and a simple multiplicative structure delivers both forecasting and risk management gains.
...
Subsequent work has expanded the framework in several directions. \citet{conrad2015} incorporated forward-looking variables, showing that consumer confidence and financial conditions indices improve long-horizon volatility forecasts. \citet{conrad2020} demonstrated that combining realized volatility with macroeconomic variables in a two-component MIDAS filter outperforms single-variable specifications. \citet{wang2017} compared GARCH-MIDAS with alternative component models and found that the choice of explanatory variable matters more than the component structure itself.

However, a limitation of existing GARCH-MIDAS studies is that they focus on mixed-frequency settings where the MIDAS aggregation is economically motivated (e.g., monthly macro data driving daily volatility). When the external variable is itself daily---as is the case with VIX---the MIDAS filter imposes unnecessary complexity. Our paper fills this gap by showing that a simple daily multiplicative model dominates the MIDAS alternatives in this setting.
...
Several papers have incorporated VIX directly into GARCH models. \citet{han2014} developed asymptotic theory for GARCH-X models, establishing consistency and asymptotic normality of quasi-maximum likelihood estimators when exogenous regressors are added to the variance equation. Their theoretical results justify the inclusion of VIX in GARCH specifications, provided the regressor is stationary and the model is correctly specified. \citet{bayer2020} compared various approaches to incorporating VIX information and concluded that simple approaches often perform as well as complex ones---a finding consistent with our results.
...
\end{align}
Contemporaneous normalization follows the recursive structure of \citet[Eq.~4]{engle2013}, where the current volatility environment determines how past shocks are interpreted. Lagged normalization avoids a simultaneity issue in which $\tau_t$ and $u_{t-1}$ both depend on $\mathrm{VIX}_{t-1}$. Our baseline specifications use contemporaneous normalization; lagged normalization is included for comparison (see Section~\ref{sec:taxonomy}).
...
\item \textbf{Normalization denominator}: Using $\tau_t$ (contemporaneous) follows the recursive structure of \citet[Eq.~4]{engle2013}, where the current environment determines the normalization. Using $\tau_{t-1}$ (lagged) avoids a simultaneity issue in which $\tau_t$ depends on $\mathrm{VIX}_{t-1}$ and $g_t$ depends on $r_{t-1}/\sqrt{\tau_t}$.
...
Optimization uses L-BFGS-B with three sets of starting values to mitigate local optima. Parameter bounds ensure positivity ($\alpha, \gamma, \beta > 0$) and stationarity ($\alpha + \gamma/2 + \beta < 1$).
...
\label{eq:qlike}
\end{equation}
evaluated on the squared return $r_t^2$ as the volatility proxy. \citet{patton2011} shows that QLIKE is robust to noise in the volatility proxy in the sense that using $r_t^2$ instead of realized variance preserves the ranking of correctly specified models.

Pairwise model comparisons use the \citet{diebold2002} test (hereafter DM test). For the full 16-comparison horse race, we adopt the conservative threshold $|t| > 3.0$, which is close to the Bonferroni-adjusted critical value of $z_{0.05/(2 \times 16)} \approx 2.95$ and consistent with the multiple-testing concerns raised by \citet{harvey2016}. For the primary A4f vs.\ GJR comparison, the standard $|t| > 1.96$ threshold applies and is comfortably exceeded.
...
\item \textbf{Acerbi \& Szekely (2014)} Z1/Z2 tests for ES adequacy.
\end{enumerate}
...
\midrule
1  & \textbf{A4f} (VIX$^2$, free $\omega$) & $\mathbf{-8.361}$ & $\mathbf{4.48}$ & \textbf{Yes} & GARCH-X \\
2  & A4 (VIX$^2$, constr.) & $-8.358$ & 4.17 & Yes & GARCH-X \\
3  & A2 (log-exp, $\tau_t$) & $-8.356$ & 3.46 & Yes & GARCH-X \\
4  & B1 (MIDAS-RW, $K\!=\!22$) & $-8.354$ & 3.37 & Yes & MIDAS-RW \\
5  & A2f (log-exp, free $\omega$) & $-8.353$ & 3.30 & Yes & GARCH-X \\
6  & A3f ($\tau_{t-1}$, free $\omega$) & $-8.350$ & 3.38 & Yes & GARCH-X \\
7  & A3 (log-exp, $\tau_{t-1}$) & $-8.350$ & 3.25 & Yes & GARCH-X \\
8  & A4n (VIX$^2$, norm) & $-8.348$ & 3.71 & Yes & GARCH-X \\
9  & A2n (log-exp, norm) & $-8.350$ & 3.41 & Yes & GARCH-X \\
10 & A1 (K889 original) & $-8.348$ & 3.06 & Yes & GARCH-X \\
11 & B2 (MIDAS-RW, $K\!=\!65$) & $-8.341$ & 3.15 & Yes & MIDAS-RW \\
12 & B3 (MIDAS-RW, $K\!=\!125$) & $-8.337$ & 3.26 & Yes & MIDAS-RW \\
13 & A5 (VIX level) & $-8.332$ & 1.84 & No & GARCH-X \\
14 & C1 (MIDAS-FS, $K_m\!=\!6$) & $-8.309$ & 3.55 & Yes & MIDAS-FS \\
15 & C3 (MIDAS-FS, $K_m\!=\!24$) & $-8.307$ & 2.85 & No & MIDAS-FS \\
16 & C2 (MIDAS-FS, $K_m\!=\!12$) & $-8.302$ & 2.33 & No & MIDAS-FS \\
17 & B0 (GJR-GARCH) & $-8.277$ & ref & --- & Benchmark \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
...
\textbf{Finding 3: Free omega improves the VIX-squared model.} Comparing A4f vs.\ A4, freeing $\omega_g$ reduces QLIKE from $-8.358$ to $-8.361$ and increases the DM $t$ from 4.17 to 4.48. This improvement is consistent with the presence of a positive variance risk premium: VIX$^2$ systematically exceeds realized variance, so constraining $\E[g_t] = 1$ forces an inappropriate normalization. However, free omega does \emph{not} improve the log-exp model (A2f vs.\ A2: $-8.353$ vs.\ $-8.356$), suggesting that the log-exp parameterization already absorbs some of this flexibility through its intercept $\theta_0$.

\textbf{Finding 4: Daily GARCH-X uniformly dominates GARCH-MIDAS.} All GARCH-X specifications in Table~\ref{tab:main_results} outperform all GARCH-MIDAS variants. The best MIDAS model (B1, rolling-window $K=22$) ranks 4th, and the fixed-span MIDAS models (C1--C3) cluster at the bottom. This result is intuitive: VIX information is concentrated in the most recent observation. The Beta-polynomial weighting scheme in MIDAS dilutes this signal by spreading it across 22--125 lags, and the monthly aggregation in fixed-span MIDAS further destroys daily information.
...
\item \textbf{Predictive regressions}: In specifications controlling for lagged VRP, the Newey-West $t$-statistic on $g_t$ ranges from $-1.28$ to $-1.95$. Augmenting the regression with additional controls (log-VIX, lagged returns) increases the $t$-statistic to $-2.34$ at the $h=5$ horizon, but no specification surpasses the Harvey $|t| > 3.0$ threshold.
...
\midrule
SPY (primary) & 0.52 & $-8.277$ & $-8.361$ & 4.48 & \textbf{Yes} \\
...
\begin{tablenotes}
\footnotesize
\item DM $t$ uses the \citet{harvey2016} threshold $|t| > 3.0$. OOS: 2019--2026. For GLD, A4f uses a single-factor model with the CBOE Gold VIX (GVZ) replacing equity VIX: $\tau_t = \theta_0 + \theta_1 \mathrm{GVZ}_{t-1}^2$.
\item $^a$ GVZ-$r^2$ correlation for GLD.
\end{tablenotes}
\end{threeparttable}
\end{table}

The A4f specification achieves Harvey-significant improvements for three of five assets: SPY ($t = 4.48$), QQQ ($t = 3.71$), and GLD with GVZ ($t = 3.17$). The model fails to reach significance for EEM ($t = -2.47$) and 0050.TW ($t = -1.44$), though the direction of improvement is correct in both cases.
...
\begin{tablenotes}
\footnotesize
\item ES tests following \citet{acerbi2014}. Z1 tests ES level adequacy; Z2 tests ES conditional calibration. All models pass both tests.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
With 17 model comparisons against the GJR benchmark, we acknowledge the multiple testing concern. However, our main result---A4f vs.\ GJR with $t = 4.48$---comfortably exceeds even conservative Bonferroni-adjusted thresholds. With 16 pairwise tests and a family-wise error rate of 5\%, the Bonferroni-adjusted critical value is $z_{0.05/(2 \times 16)} \approx 2.95$, well below our observed $t = 4.48$.
...
First, a simple daily multiplicative GARCH-X model with $\tau_t = \theta_0 + \theta_1 \mathrm{VIX}_{t-1}^2$ and a freely estimated $g_t$ intercept (A4f) significantly outperforms GJR-GARCH (DM $t = 4.48$) and uniformly dominates six GARCH-MIDAS alternatives in out-of-sample QLIKE loss. The key insight is that GARCH-MIDAS complexity is unnecessary when the external variable already updates at daily frequency---the Beta-polynomial MIDAS filter dilutes the concentrated information in recent VIX observations.
...
Fourth, the model generalizes to assets with relevant fear indices: QQQ (DM $t = 3.71$) and GLD with GVZ (DM $t = 3.17$). For assets without appropriate implied volatility indices (EEM, 0050.TW), the model improves directionally but not significantly, underscoring the requirement for a \emph{relevant} fear gauge.

Fifth, A4f with Student-$t$ innovations achieves a VaR/ES scorecard of 3/4, substantially outperforming GJR-GARCH (1/4), demonstrating practical value for risk management.
...
\newblock {\em Risk}, 27(11):76--81.

\bibitem[Bayer and Hackethal, 2020]{bayer2020}
Bayer, S. and Hackethal, A. (2020).
\newblock Combining VIX and GARCH: Does VIX help predict equity volatility?
\newblock {\em International Journal of Forecasting}, 36(4):1444--1458.
...
\newblock {\em Journal of Business and Economic Statistics}, 20(1):134--144.
...
\newblock {\em Journal of Business and Economic Statistics}, 33(3):338--358.
...
\newblock {\em Journal of Business and Economic Statistics}, 22(4):367--381.
...
\newblock {\em Journal of Business and Economic Statistics}, 32(3):416--429.
...
\bibitem[Wang and Ghysels, 2015]{wang2017}
