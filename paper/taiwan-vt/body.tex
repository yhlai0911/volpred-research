...
\end{tabular}
\begin{flushleft}
\small \textit{Notes:} GJR-GARCH(1,1) with rolling window $w = 2000$. $t$-statistics use Newey-West HAC standard errors. Persistence $= \alpha + \beta + \gamma/2$. Individual stocks use the full available sample (2008--2026). 0056.TW is itself a diversified ETF (30+ constituents); it is reported separately and excluded from the primary 9-stock individual-stock average used for the amplification ratio. \textit{Methodological note}: Canonical full-sample estimates (GJR-GARCH(1,1), Bollerslev-Wooldridge robust SE, 2008--2024) from experiment K1302 differ from rolling-window values for Hon Hai ($\gamma = 0.032$, $t = 1.74$), MediaTek ($\gamma = 0.041$, $t = 3.10$), Mega Financial ($\gamma = 0.038$, $t = 1.55$), and 0056.TW ($\gamma = 0.067$, $t = 1.91$). The differences reflect the rolling-window versus full-sample specification and Newey-West versus Bollerslev-Wooldridge standard errors; the 9-stock average decreases from 0.054 to approximately 0.038 under the canonical full-sample specification. % source: K1302 (CONFLICT_RESOLVED: rolling_w2000_NW-HAC vs canonical_full_BW-robust; 2026-05-15)
\end{flushleft}
\end{table}
