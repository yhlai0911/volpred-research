...
\end{tabular}
\begin{flushleft}
% DRAFT NOTE (2026-05-16): Table rows for Hon Hai/MediaTek/Mega/0056 currently show legacy rolling-w2000
% NW-HAC estimates (inconsistent with TAIEX/0050/TSMC/SPY which use canonical full-sample BW-robust spec).
% Decision: Option A adopted — update ALL individual stock rows to canonical full-sample GJR-GARCH(1,1)
% BW-robust SE (experiment K1302). K1302 canonical values: Hon Hai γ=0.032 t=1.74; MediaTek γ=0.041
% t=3.10; Mega Financial γ=0.038 t=1.55; 0056.TW γ=0.067 t=1.91.
% Pending K1302b (5 unlisted stocks: 2882/2891/2412/2885/2881) to recompute 9-stock average.
% Full Table 2 update + §5 amplification ratio revision scheduled after K1302b PASS + Codex review.
\small \textit{Notes:} TAIEX, 0050.TW, SPY, and TSMC use full-sample GJR-GARCH(1,1) with Bollerslev-Wooldridge robust standard errors (2008--2024). The four individual stocks and 0056.TW shown above currently use rolling-window estimates ($w = 2000$, Newey-West HAC) from the original draft and will be updated to the canonical specification upon completion of K1302b. Persistence $= \alpha + \beta + \gamma/2$. 0056.TW is itself a diversified ETF (30+ constituents); it is reported separately and excluded from the primary 9-stock individual-stock average used for the amplification ratio. \textit{Provisional canonical estimates from K1302} (full-sample BW-robust, 2008--2024): Hon Hai ($\gamma = 0.032$, $t = 1.74$), MediaTek ($\gamma = 0.041$, $t = 3.10$), Mega Financial ($\gamma = 0.038$, $t = 1.55$), 0056.TW ($\gamma = 0.067$, $t = 1.91$); the 9-stock average is expected to decrease from 0.054 to approximately 0.038 under the canonical specification. % source: K1302 (OPTION_A_DECIDED 2026-05-16; pending K1302b for 5 remaining stocks)
\end{flushleft}
\end{table}
