...
% DISPUTED PROVENANCE (2026-07-09, telegram-312 / provenance-sweep): gamma=0.272,t=3.18 UNTRACEABLE.
% Full-sample 1997-2026 re-estimate -> gamma=0.105,t=5.31 (experiments/paper2_twii_fullsample_gamma_provenance/results.json),
% corroborated by K892 full_sample gamma=0.109,t=5.62 (experiments/k892/k892_verify_tw_gamma_results.json).
% reproduce.py's "0.272 = 1997-2026 long-sample" NOTE is factually false (long-sample = 0.109). AWAITING OWNER SIGN-OFF before headline change; do NOT silently rewrite.
...
9-stock average (excl.\ 2330 \& 0056) & 0.026 & 1.74 & $-0.21$ & 3.62 & 0.054 & --- \\
10-security average (incl.\ 0056; excl.\ 2330) & 0.028 & 1.71 & $-0.22$ & 3.68 & 0.060 & --- \\
\bottomrule
\end{tabular}
\begin{flushleft}
\small \textit{Notes:} $\gamma_{\text{GJR}}$ is the asymmetric leverage parameter from GJR-GARCH(1,1). 0050.TW and TSMC values use full-sample MLE; other rows use rolling window $w = 2000$. The $t$-statistics for the full-sample 0050.TW and TSMC rows are based on Bollerslev--Wooldridge robust (quasi-)maximum-likelihood standard errors from the GJR-GARCH estimation \citep{bollerslev1992}; the rolling-window TWII and individual-stock rows use Newey--West HAC standard errors. The primary 9-stock average excludes both TSMC (2330.TW) and 0056.TW: TSMC is reported separately because of its dominant benchmark weight, and 0056.TW is a diversified ETF rather than an individual stock. The 10-security average adds 0056.TW back in for sensitivity, while still excluding TSMC.
\end{flushleft}
\end{table}
...
% DISPUTED PROVENANCE (2026-07-09, telegram-312): gamma=0.272,t=3.18 UNTRACEABLE. Canonical full-sample 1997-2026 re-estimate = gamma=0.105,t=5.31 (experiments/paper2_twii_fullsample_gamma_provenance), corroborated by K892 full_sample gamma=0.109,t=5.62. Amplification ratio (0.272/stock-avg) must be recomputed from 0.109. AWAITING OWNER SIGN-OFF before headline change.
0050.TW & 0.097 & 3.60 & 0.032 & 0.893 & 0.973 \\
SPY & 0.211 & 5.79 & 0.000 & 0.897 & 0.993 \\
\midrule
% ===================== PROVENANCE (2026-07-07) =====================
% The rolling-window (w=2000) individual-stock rows below (Hon Hai 2317,
% MediaTek 2454, Mega 2886), the ETF 0056 row, and the 9-stock /
% 10-security ROLLING averages (0.054 / 0.060) + rolling ratios (5.0x /
% 4.5x in the footnote) trace ONLY to knowledge entry N121 (from a
% since-deleted K530 run) with NO surviving source JSON.
% See paper/taiwan-vt/reviews/audit_step1_2.md.
% Reproducible re-estimation (documented K892 arch-MLE GJR(1,1) rolling
% last-window spec, all 10 securities, offline snapshots) is now the
% traceable source and shows the LEGACY VALUES ARE NON-REPRODUCIBLE:
%   % source: experiments/paper2_taiwan_indiv_rolling_gamma
%   2317 legacy 0.052/1.14 -> reproducible 0.032/0.92
%   2454 legacy 0.044/0.96 -> reproducible 0.045/1.54
%   2886 legacy 0.179/2.42 -> reproducible 0.054/1.41  (3x off)
%   0056 legacy 0.112/1.87 -> reproducible 0.310/3.92  (flips "second-highest" -> HIGHEST, above index 0.272)
%   9-stock rolling avg 0.054 -> 0.037 ; ratio 5.0x -> 7.4x
%   10-security rolling avg 0.060 -> 0.064 ; ratio 4.5x -> 4.3x
% RENDERED NUMBERS ARE INTENTIONALLY NOT CHANGED IN THIS COMMIT: the 0056
% correction entangles a rewrite of the 0056-inclusion sensitivity
% paragraph, which must go through the main-thread paper revision flow
% with author sign-off (CLAUDE.md paper narrative state machine).
% Follow-up: paper_body task paper2_taiwan_vt_rolling_block_reestimate.
% Caveat: reproducible per-stock windows are NOT calendar-aligned (2886
% snapshot ends 2025-01) -> recompute on aligned snapshots before adopting
% as canonical. PRIMARY full-sample BW-robust claim (gamma_bar=0.027,
% ratio 4.3x, K1302+K1302b, VERIFIED) is UNAFFECTED.
% ===================================================================
...
Hon Hai (2317) & 0.052 & 1.14 & 0.028 & 0.939 & 0.985 \\ % source: N121 NON-REPRODUCIBLE; reproducible=0.032/0.92 -> experiments/paper2_taiwan_indiv_rolling_gamma
MediaTek (2454) & 0.044 & 0.96 & 0.033 & 0.935 & 0.984 \\ % source: N121 NON-REPRODUCIBLE; reproducible=0.045/1.54 -> experiments/paper2_taiwan_indiv_rolling_gamma
Mega Financial (2886) & 0.179 & 2.42 & 0.015 & 0.901 & 0.977 \\ % source: N121 NON-REPRODUCIBLE; reproducible=0.054/1.41 -> experiments/paper2_taiwan_indiv_rolling_gamma
\midrule
\multicolumn{6}{l}{\textit{ETF (excluded from individual-stock average)}} \\
0056.TW (Yuanta High Div.\ ETF) & 0.112 & 1.87 & 0.021 & 0.922 & 0.982 \\ % source: N121 NON-REPRODUCIBLE; reproducible=0.310/3.92 (HIGHEST, contradicts prose) -> experiments/paper2_taiwan_indiv_rolling_gamma
\midrule
9-stock average (excl.\ 2330 \& 0056) & 0.054 & --- & 0.028 & 0.935 & 0.984 \\ % source: N121 NON-REPRODUCIBLE; reproducible rolling avg=0.037 -> experiments/paper2_taiwan_indiv_rolling_gamma
10-security average (incl.\ 0056; excl.\ 2330) & 0.060 & --- & 0.027 & 0.933 & 0.984 \\ % source: N121 NON-REPRODUCIBLE; reproducible rolling avg=0.064 -> experiments/paper2_taiwan_indiv_rolling_gamma
\bottomrule
\end{tabular}
\begin{flushleft}
\small \textit{Notes:} GJR-GARCH(1,1) with rolling window $w = 2000$ except where noted. 0050.TW and TSMC use full-sample MLE estimation, and their $t$-statistics are based on Bollerslev--Wooldridge robust maximum-likelihood standard errors \citep{bollerslev1992}; the rolling-window TWII and individual-stock rows use Newey--West HAC standard errors. Persistence $= \alpha + \beta + \gamma/2$. Individual stocks use the full available sample (2008--2026). The primary 9-stock average excludes both TSMC (2330.TW) and 0056.TW; the former is reported separately because of its dominant benchmark role, and the latter is a diversified ETF rather than an individual stock. TSMC's leverage parameter is robust to the mean specification: on the canonical full sample ($N=6{,}525$), the constant-mean estimate is $\gamma = 0.052$ ($t = 3.98$) and the zero-mean estimate is $\gamma = 0.059$ ($t = 4.25$), both significant (source: \texttt{experiments/paper2\_tsmc\_gamma\_zeromean\_sens}); the constant-mean spec was therefore not chosen to manufacture significance.
\end{flushleft}
\end{table}

\subsection{Diversification Amplification}

A notable finding is that the average $\gamma$ across the nine individual Taiwanese stocks (excluding 0056.TW, which is itself a diversified ETF) is approximately 0.027 under the canonical full-sample Bollerslev--Wooldridge robust GJR-GARCH(1,1) specification \citep{bollerslev1992}, while the TAIEX index $\gamma$ is 0.114---a point estimate ratio of approximately 4.3$\times$.\footnote{Under the rolling-window specification ($w=2000$, Newey-West HAC SE), the corresponding estimates are $\bar{\gamma}_{\text{indiv}}=0.054$, $\gamma_{\text{TAIEX}}=0.272$, and ratio $=5.0\times$. The canonical full-sample BW-robust specification (2008--2024 sample, $n=4{,}160$ observations) is the primary specification for the block-bootstrap inference reported in the paragraph below.} If we include 0056.TW in the rolling-window average (ten securities, $\bar{\gamma} = 0.060$), the ratio is 4.5$\times$ under rolling-window specification. We term this the \textit{diversification amplification} of the leverage effect: the process of aggregating individual stocks into an index magnifies the asymmetric volatility response. For comparison, the analogous ratio for U.S. equities is 2.8$\times$ (SPY $\gamma = 0.211$ versus an average individual stock $\gamma$ of 0.079, based on the 50 largest S\&P 500 constituents).
...
\paragraph{Sensitivity to 0056.TW inclusion.} Including 0056.TW (itself a diversified ETF with 30+ constituents and $\gamma = 0.112$, the second-highest among the ten securities under rolling-window specification) raises the average individual-security $\gamma$ from 0.054 to 0.060, thereby reducing the amplification ratio from 5.0$\times$ to 4.5$\times$ (rolling-window figures). Because 0056.TW already exhibits some diversification amplification internally, its inclusion biases the ``individual stock'' average upward and the amplification ratio downward. The direction of this bias is conservative (it makes the amplification claim \textit{smaller}), but the classification is factually incorrect. We therefore use the 9-stock average as the primary estimate; the canonical full-sample estimate is $\bar{\gamma} = 0.027$ (rolling-window: $\bar{\gamma} = 0.054$), and we report rolling-window sensitivity for comparability across specifications.
