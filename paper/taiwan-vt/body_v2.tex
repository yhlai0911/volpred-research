...
\end{tabular}
\begin{flushleft}
\small \textit{Notes:} $\gamma_{\text{GJR}}$ is the asymmetric leverage parameter from GJR-GARCH(1,1). For 0050.TW and TSMC, this reports the K892 canonical Constant-mean full-sample estimate; for all other rows, the Zero-mean GJR-GARCH(1,1) rolling window $w = 2000$ specification is used. $t(\gamma)$ uses maximum-likelihood standard errors for the K892 canonical rows and Newey-West HAC for others. 0056.TW is itself a diversified ETF (30+ constituents) and is therefore excluded from the primary 9-stock average used in the amplification ratio calculation (Section~\ref{sec:leverage}). The 10-security average including 0056 is reported for completeness.
\end{flushleft}
\end{table}
...
\midrule
TWII (TAIEX)$^{\P}$ & 0.272 & 3.18 & 0.012 & 0.870 & 0.990 \\
0050.TW$^{\S}$ & 0.097 & 3.60 & 0.032 & 0.893 & 0.973 \\
SPY & 0.211 & 5.79 & 0.000 & 0.897 & 0.993 \\
\midrule
\multicolumn{6}{l}{\textit{Individual Taiwan Stocks}} \\
TSMC (2330)$^{\ddagger}$ & 0.052 & 3.98 & 0.032 & 0.930 & 0.989 \\
...
\end{tabular}
\begin{flushleft}
\small \textit{Notes:} Persistence $= \alpha + \beta + \gamma/2$. Individual stocks use the full available sample (2008--2026 unless otherwise noted). 0056.TW is itself a diversified ETF (30+ constituents); it is reported separately and excluded from the primary 9-stock individual-stock average used for the amplification ratio. The 9-stock average ($\bar{\gamma} = 0.054$) is computed from the Zero-mean GJR-GARCH(1,1) specification with rolling window $w = 2000$ and reflects the spec used for the remaining individual stock rows; it is not recalculated to incorporate the K892 canonical values for TSMC alone. $^{\ddagger}$The TSMC (2330) row reports the K892 canonical estimate: $\gamma = 0.052$ ($t = 3.98$) from Constant-mean GJR-GARCH(1,1) MLE on the full 2004--2026 sample ($N = 6{,}525$); source: \texttt{experiments/k892\_*\_results.json}. An earlier rolling-window Constant-mean estimate yielded $\gamma = 0.054$ ($t = 1.07$), which was sensitive to window initialization; the full-sample canonical estimator supersedes it. $^{\S}$The 0050.TW row reports the K892 canonical estimate: $\gamma = 0.097$ ($t = 3.60$) from Constant-mean GJR-GARCH(1,1) MLE on the full 2008--2026 sample ($N = 4{,}219$); source: \texttt{experiments/k892\_*\_results.json}. An earlier Zero-mean GJR specification yielded $\gamma = 0.087$ ($t = 2.20$); both confirm statistical significance. $^{\P}$The TWII (TAIEX) row reports $\gamma = 0.272$ ($t = 3.18$) from the long-sample 1997--2026 specification (capturing the Asian Financial Crisis and Dot-Com sample windows where TAIEX leverage effects are strongest). The K892 canonical re-estimate restricted to the 2008--2026 subset yields $\gamma = 0.109$ ($t = 5.62$), confirming that the long-sample 0.272 estimate is driven substantively by pre-2008 crisis episodes. The long-sample window is retained as the primary TAIEX estimate because it includes the two largest Taiwan equity market shocks on record.
\end{flushleft}
\end{table}
...
An important caveat is that the amplification ratio depends on which index measure is used. The 5.0$\times$ ratio (or 4.5$\times$ including 0056) reflects the \textit{broad} TAIEX ($\sim$900 stocks) relative to individual stocks. When using the investable 0050.TW ETF (50 stocks, $\gamma = 0.097$ under the K892 canonical Constant-mean specification) as the index measure, the amplification ratio is a more modest $0.097 / 0.054 \approx 1.8\times$ (or $0.097 / 0.060 \approx 1.6\times$ including 0056; note the 9-stock average uses the Zero-mean specification so the comparison is approximate). The difference between the TAIEX-based and 0050-based ratios reflects the broader TAIEX's inclusion of small- and mid-cap stocks whose correlations exhibit stronger asymmetry during downturns. For practitioners implementing VT on the investable 0050.TW, the effective leverage amplification is therefore substantially lower than the headline figure, and GJR-GARCH's advantage over symmetric GARCH is correspondingly attenuated (consistent with the insignificant DM test reported in Section~\ref{sec:vt}).
...
An important finding from this decomposition concerns the leverage effect. Using the K892 canonical re-estimation (Constant-mean GJR-GARCH(1,1) on the full available sample; source: \texttt{experiments/k892\_*\_results.json}), the 0050.TW index exhibits $\gamma = 0.097$ ($t = 3.60$), while TSMC alone shows $\gamma = 0.052$ ($t = 3.98$). Both estimates are statistically significant. The higher index leverage (ratio $0.097/0.052 \approx 1.9\times$) reinforces the diversification amplification mechanism documented in Section~\ref{sec:leverage}: aggregating stocks amplifies the asymmetric volatility response beyond any individual constituent. Notably, TSMC exhibits a significant leverage effect under the full-sample canonical estimator---an earlier rolling-window Constant-mean estimate of $\gamma = 0.054$ ($t = 1.07$) had been sensitive to window initialization and sample length, causing a false appearance of insignificance. Under the full-sample specification, TSMC's volatility dynamics contain a meaningful asymmetric component alongside the earnings and semiconductor cycle surprises that characterize its return process \citep{black1976}, with the index-level amplification ($\approx 1.9\times$) reflecting the additional correlation asymmetry that emerges upon aggregation.
