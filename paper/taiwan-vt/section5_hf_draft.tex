...
Slot~B (pre-U.S.) is surprisingly important, contributing 35.6\% of the mean gap return despite preceding U.S.\ market opening. This suggests significant price discovery occurs in the TAIFEX night session even before the primary information source (U.S.\ equities) begins trading, consistent with Asian traders processing overnight news and global futures movements. These findings challenge the conventional view that overnight gaps are untradable risk and suggest that institutional investors should consider TAIFEX night session futures as a tool for real-time risk management.
...
GJR-GARCH and EWMA forecasts are computed using the same specifications as in Section~\ref{sec:vt}, with rolling 500-day estimation windows and refitting every 63 trading days. Crucially, GARCH produces a $\hat{\sigma}_t^2$ forecast that targets the conditional variance of daily returns; when evaluated against 5-minute RV, any systematic scale mismatch is absorbed by QLIKE's scale-invariant property \citep{patton2011}.
...
This explains the ``GARCH ceiling'' documented in Section~\ref{sec:vt}: GJR-GARCH's apparent invincibility is not a property of the Taiwan market's volatility dynamics but rather an artifact of the evaluation proxy. When the proxy is upgraded from $r_t^2$ to 5-minute RV, the GARCH ceiling shatters: HAR-RV outperforms GJR-GARCH with $t > 11$ on Track A and $t > 5$ on Track B. The practical lesson is that model comparisons conducted using daily $r_t^2$ are subject to a \textit{proxy ceiling}---a bound on the DM test power imposed by the noise in the proxy---and researchers should interpret insignificant DM results under $r_t^2$ with appropriate caution.
...
Table~\ref{tab:var_paradox} reports VaR backtest results at the 1\% level over the common out-of-sample period 2023--2024 ($N = 481$ for GARCH-based methods; $N = 450$ for HAR-based methods, reflecting the 22-day burn-in for the monthly RV component). GJR-GARCH combined with the Cornish-Fisher expansion (GJR+CF) passes the full ``Trinity'' of VaR backtests: Kupiec unconditional coverage ($p = 0.14$), Christoffersen independence ($p = 1.00$), and Basel Green Zone (2 violations in 250 days). It produces only 2 violations in 481 days (0.42\%), with an average 1\% VaR of $-3.83\%$.
...
\midrule
GJR+CF & 0.220 & 0.42\% & 2/481 & 0.14 & Green & PASS \\
...
This result---HAR-RV QLIKE 54\% better than GJR-GARCH, yet HAR produces 8.5$\times$ more VaR violations---constitutes a prediction--VaR paradox. The resolution lies in two distinct mechanisms:
...
The practical implication is unambiguous: for VaR applications in the Taiwan market, GJR-GARCH with Cornish-Fisher remains the preferred method despite its inferior volatility prediction accuracy. Volatility forecasting and risk management are related but distinct objectives that reward different model properties.
...
These caveats notwithstanding, the Realized GARCH results demonstrate that the prediction--VaR paradox documented in Section~\ref{sec:hf_paradox} is not an inherent limitation of high-frequency data, but rather a consequence of applying RV-based models (HAR-RV) without the recursive variance structure needed for reliable tail-risk estimation. When intraday information is channeled through a GARCH-compatible framework, both prediction accuracy and VaR reliability improve.
