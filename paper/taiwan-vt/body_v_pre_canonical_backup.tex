...
The fact that the same Bayesian methodology produces diametrically opposite conclusions for Taiwan (SPY return is indispensable) and the U.S. (no exogenous variable is needed) % Codex-R6-fix: tone down "formal statistical confirmation" → "statistical evidence"
provides statistical evidence consistent with Taiwan's role as an \textit{information receiver} in the global equity market. This asymmetry---unidirectional information flow from the U.S. to Taiwan with no reverse channel---reinforces the Granger causality evidence above and motivates the time-zone information transmission analysis in Section~\ref{sec:tz}.
...
\end{tabular}
\begin{flushleft}
% Codex-R6-fix: clarify per-strategy periods and add comparability caveat
\small \textit{Notes:} Evaluation periods differ across strategies: Buy \& Hold and EWMA VT cover 2010--2026; GARCH VT and GJR VT cover 2020--2026 (the 2000-day rolling window, starting from 2008 data, first produces forecasts in 2016, with an additional burn-in to 2020); 8.63/VIX covers 2016--2026; Student-$t$ VaR covers 2020--2026. Because evaluation periods are not identical, cross-strategy Sharpe ratio comparisons should be interpreted with caution. 8.63/VIX uses monthly rebalancing with 0.186\% round-trip transaction cost. VaR violations are the empirical 1\% VaR exceedance rate using Student-$t$(df$= 5$) distribution. MDD: maximum drawdown.
\end{flushleft}
\end{table}
...
\textbf{Return measurement caveat.} A critical distinction arises between close-to-close (c2c) and open-to-open (o2o) return measurement. The standard c2c return includes the overnight opening gap, which in Asian markets mechanically reflects the previous day's U.S. session. This gap alone accounts for approximately 78\% of the c2c strategy alpha ($R^2_{\text{gap}} = 0.35$ versus $R^2_{\text{c2c}} = 0.18$, with intraday reversal). Since the opening gap is realized before a trader can act on the signal, the c2c Sharpe ratio overstates implementable performance. % Codex-R6-fix: explicitly label c2c as non-implementable benchmark vs. o2o as implementable
We therefore report both c2c and o2o results throughout this section: c2c results constitute a \textit{non-implementable} benchmark that measures the total information transmission across markets, while o2o results measure the \textit{implementable} portion. The c2c results serve as an upper bound; the implementable o2o returns are substantially lower. The gap between them---which we interpret as evidence of efficient price discovery by the opening auction---constitutes a finding in its own right.
...
The c2c results confirm a strong and persistent information transmission channel from U.S. to Asian markets ($r = 0.376$), with six Asia-Pacific markets exceeding the Harvey threshold on a c2c basis (Hong Kong $t = 4.12$, Australia $t = 4.04$, Singapore $t = 4.03$, Korea $t = 3.83$, Taiwan $t = 3.76$, Japan $t = 3.69$). The failure of all U.S.-listed ETF controls confirms that the information is priced with a lag in local markets. Rather than interpreting the c2c--o2o gap as a failure, % Codex-R6-fix: tone down "highly efficient" → "efficient"
we view it as evidence of an efficient price discovery mechanism: the Asian opening auction---a call auction that aggregates pre-market orders accumulated overnight---incorporates approximately 78\% of overnight U.S. information within the first transaction of the trading day. % Codex-R6-fix: tone down "among the most efficient documented" → neutral phrasing
This finding is consistent with efficient cross-market information aggregation by the call auction mechanism, and has implications for market design in emerging economies where call auction efficiency is an active policy concern.
...
This gap-dominance finding has direct implications for strategy implementability. % Codex-R6-fix: tone down "fatal timing problem" → "significant timing constraint"
An ETF-based strategy that attempts to trade on the SPY signal faces a significant timing constraint: by the time the Taiwan market opens and the trader can place an order, the opening gap has already absorbed the signal, imposing an estimated cost of 38.5 bp per round-trip (the average absolute gap). However, a futures-based implementation using TAIEX futures (TX), which trade in a pre-market session beginning at 8:45 (15 minutes before the equity market opens), faces a substantially smaller gap cost of approximately 5 bp, suggesting a potentially viable Sharpe ratio of 0.93 for the futures channel. We leave the full analysis of futures-based implementation---including margin requirements, roll costs, and intraday liquidity---to future work.

Figure~\ref{fig:overnight_vix} visualizes the two key mechanisms underlying the time-zone information transmission channel. Panel~(a) shows the distribution of 0050.TW overnight gaps conditioned on the previous day's SPY return direction. The distributions are clearly separated: following SPY up-days, the gap distribution shifts rightward (mean $\approx +0.11\%$), while following SPY down-days it shifts leftward (mean $\approx -0.09\%$). The asymmetry confirms that cross-market information is incorporated into Taiwan prices primarily through the opening auction, before continuous trading begins. Panel~(b) documents the positive relationship between VIX levels and next-day 0050.TW absolute returns, with the binned means (red circles) showing a monotonically increasing pattern. The Spearman correlation of approximately 0.60 confirms the VIX-to-Taiwan volatility transmission channel that underpins the $8.63/\text{VIX}$ strategy.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig3_overnight_vix.pdf}
\caption{Information transmission from U.S.\ to Taiwan equity markets, 2010--2026. Panel~(a): Distribution of 0050.TW overnight gaps (log open$_t$ $-$ log close$_{t-1}$) conditioned on the sign of the previous day's SPY close-to-close return. Dashed vertical lines indicate conditional means. Panel~(b): Scatter plot of VIX level (previous U.S.\ close) versus next-day 0050.TW absolute return. Red circles show binned means (4-point VIX bins). The positive relationship confirms VIX as a valid volatility proxy for Taiwan.}
...
% Corrected: ETF round-trip = 0.186% (0.1% ETF tax + 0.04275%×2 commission at 3折)
\textbf{Transaction cost sensitivity.} At the observed 29 switches per year and a round-trip cost of 0.186\% per switch (the ETF-specific securities tax of 0.1\% plus discounted brokerage commissions), annual transaction costs are approximately 5.4\%. For the c2c strategy the breakeven cost is over 3\% per switch; for the o2o strategy the margin is substantially narrower.
...
% Corrected: ETF round-trip = 0.186% (0.1% ETF tax + 0.04275%×2 commission at 3折)
\textbf{Time-zone momentum.} Check whether the trailing 10-day SPY cumulative return is positive. If yes, hold 0050.TW; if no, hold cash. This requires daily monitoring and generates approximately 29 switches per year (transaction cost approximately 5.4\% per year at 0.186\% per switch). \textit{Caveat:} The c2c Sharpe of 1.47 overstates implementable returns; the o2o Sharpe is 0.87 (see Section~\ref{sec:tz}). The efficient incorporation of overnight U.S. information by Taiwan's opening auction means that most of the signal value is already priced in before trading begins. This strategy is best viewed as an information-transmission indicator rather than a standalone trading strategy.
...
\textbf{VIX sufficiency generalizes.} The U.S.\ finding that VIX serves as a ``sufficient statistic'' for volatility forecasting extends to Taiwan: adding 0050.TW's own lagged realized volatility to a VIX-only model improves $R^2$ by only $+0.003$, confirming that VIX captures nearly all relevant volatility information for 0050.TW. This result is consistent with the SSVS evidence in Section~\ref{sec:spillover} and supports the practical viability of VIX-only VT strategies in markets with strong U.S.\ linkages.

\textbf{Optimal allocation does not generalize.} The U.S.\ finding that a 50/50 equity/gold allocation is approximately optimal fails for Taiwan. A grid search over 0050.TW/SPY allocations reveals an optimal mix of approximately 20\% 0050.TW and 80\% SPY, reflecting the substantially higher volatility of the Taiwanese ETF (annualized vol $\approx$~27\%) relative to SPY ($\approx$~16\%). The heavy SPY tilt captures U.S.\ equity returns at lower volatility cost, and the diversification benefit of adding 0050.TW is limited by its high correlation with SPY conditional on U.S.\ information. This implies that Taiwanese investors seeking a two-asset VT portfolio should allocate predominantly to U.S.\ equities, with only a modest home-bias component.
