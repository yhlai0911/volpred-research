#!/usr/bin/env python3
"""
K886: PRG on 0050.TW (Daily Frequency)
========================================

Research Question (EMPIRICAL):
  K880v2: PRG does NOT beat GJR for SPY (DM t=-0.57 NS)
  K881:   PRG DOES beat GJR for QQQ (t=4.26), GLD (t=6.12), EEM (t=6.63)
  K886:   Test PRG on 0050.TW — Taiwan's main ETF, MISSING from daily analysis.

  0050.TW is interesting because:
  - High amplification factor (4.6x vs US markets)
  - US VIX leads Taiwan vol (Granger F=58.8)
  - Different microstructure from US ETFs
  - Different trading hours → overnight gap captures US session
...
Data: yfinance — 0050.TW (with clean_tw50_data fix for 2014 split artifact)
  Period: 2003-01-01 to 2026-04-05
  IS: first 70%, OOS: last 30%

CRITICAL: 0050.TW data requires clean_tw50_data() to fix Yahoo Finance's
incomplete 1:4 split adjustment (only back to 2014-01-02).
...
Error log rules applied:
  ...
  - 0050.TW: MUST call clean_tw50_data to fix split issue
  - Model-Target matching: all models evaluated on σ²_fullday (common target)
...
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "k886_prg_0050tw_results.json")
CHARTS_DIR = os.path.join(SCRIPT_DIR, "k886_charts")
os.makedirs(CHARTS_DIR, exist_ok=True)

np.random.seed(42)

# 0050.TW configuration
TICKER = '0050.TW'
START_DATE = '2003-01-01'
END_DATE = '2026-04-05'
DESCRIPTION = 'Taiwan Top-50 ETF (high amplification 4.6x, US lead-lag)'
...
REFIT_FREQ_PRG = 252         # annual for PRG (heavy MLE)
PRG_N_STARTS = 3             # multi-start optimization
...
def _prg_forecast_day(omega0, alpha0, beta0, omega1, alpha1, beta1,
    ...
    """Forecast sigma2_fullday for day t given state h from intraday_{t-1}."""
    omega = np.array([omega0, omega1])
    alpha = np.array([alpha0, alpha1])
    beta_p = np.array([beta0, beta1])
    gamma = np.array([gamma0, gamma1])
...
# ============================================================
# DATA LOADING — 0050.TW with clean_tw50_data fix
# ============================================================
def load_0050tw_data(start_date, end_date):
    """Load 0050.TW OHLC from yfinance with split fix, compute session decomposition."""
    import yfinance as yf

    print(f"  Downloading 0050.TW from yfinance...")
    data = yf.download(TICKER, start=start_date, end=end_date, auto_adjust=True, progress=False)
    print(f"    Raw data: {len(data)} days, {data.index[0].date()} to {data.index[-1].date()}")

    # Flatten MultiIndex columns if present
    if isinstance(data.columns, pd.MultiIndex):
        data.columns = data.columns.get_level_values(0)

    # CRITICAL: Apply clean_tw50_data to fix the 2014 split artifact
    print(f"    Applying clean_tw50_data() to fix 2014 1:4 split artifact...")
    close_raw = data['Close'].squeeze()
    clean_close, clean_returns = clean_tw50_data(close_raw)
    print(f"    Split fix applied. Clean close range: {clean_close.min():.2f} to {clean_close.max():.2f}")

    # Also fix Open, High, Low using the same ratio
    # Check if pre-2014 prices need adjustment
    split_date = pd.Timestamp("2014-01-02")
    pre_split_mask = data.index < split_date
    if pre_split_mask.any():
        # Check ratio on Close to determine if fix was applied
        last_pre_raw = data.loc[pre_split_mask, 'Close'].iloc[-1]
        first_post_raw = data.loc[split_date, 'Close'] if split_date in data.index else None
        if first_post_raw is not None:
            if isinstance(first_post_raw, pd.Series):
                first_post_raw = first_post_raw.iloc[0]
            if isinstance(last_pre_raw, pd.Series):
                last_pre_raw = last_pre_raw.iloc[0]
            ratio = last_pre_raw / first_post_raw
            if 3.5 < ratio < 4.5:
                print(f"    Fixing OHLC pre-2014 prices (ratio={ratio:.2f})...")
                for col in ['Open', 'High', 'Low', 'Close']:
                    data.loc[pre_split_mask, col] = data.loc[pre_split_mask, col] / 4.0

    # Use the cleaned close for session decomposition
    ...
    # Handle any remaining extreme price jumps (safety net from clean_tw50_data)
    # Override close with the cleaned version
    # But we need OHLC to be consistent, so we use the ratio-fixed data above
    # and just verify close matches
    pct_change = df['close'].pct_change()
    extreme_days = pct_change.abs() > 0.50
    if extreme_days.sum() > 0:
        print(f"    WARNING: {extreme_days.sum()} days with >50% price change detected after fix.")
        print(f"    Removing these days...")
        df = df[~extreme_days]
    ...
    # Remove extreme outliers — 0050.TW can have larger moves than US ETFs
    # Use 25% threshold (more lenient for Taiwan market)
    mask = (np.abs(df['r_c2c']) < 0.25) & (df['sigma2_fullday'] < 0.0625)
    n_removed = len(df) - mask.sum()
    if n_removed > 0:
        print(f"    Removed {n_removed} extreme outliers (>25% daily move)")
    df = df[mask]

    print(f"    After processing: {len(df)} days ({df.index[0].date()} to {df.index[-1].date()})")
    return df
...
# ============================================================
# MODEL 2: HAR on sigma2_fullday (log-space)
# ============================================================
def har_oos_forecast(sigma2_series, is_end, refit_freq=63):
    """HAR on log(sigma2_fullday) with daily/5d/22d lags."""
    eps_val = 1e-12
...
def prg_oos_forecast(r_overnight, r_intra, r2_overnight, r2_intra,
    ...
    Predicts sigma2_fullday = h_overnight + h_intraday.
...
def separate_garch_oos(r_overnight, r_intra, r2_overnight, r2_intra,
    ...
    sigma2_fullday = h_overnight + h_intraday.
    """
    n_days = len(r_overnight)
    forecasts = np.full(n_days, np.nan)
...
# CHARTS
# ============================================================
def make_charts(asset_result, charts_dir):
    """Generate charts for 0050.TW PRG analysis."""
    import matplotlib
    matplotlib.use('Agg')
    ...
    for m in model_names:
        q = asset_result['layer1_loss_functions'].get(m, {}).get('QLIKE', np.nan)
        if np.isfinite(q):
            qlikes.append(q)
            names.append(m)
    ...
    if qlikes:
        ...
        ax.set_title('K886: QLIKE Comparison for 0050.TW (OOS)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(charts_dir, 'qlike_0050tw.png'), dpi=150, bbox_inches='tight')
    plt.close()

    # Chart 2: DM t-stat bar chart (PRG_Extended vs others)
    fig, ax = plt.subplots(figsize=(10, 6))
    comparisons = ['GJR', 'HAR', 'PRG_Basic', 'Separate']
    dm_t_values = []
    dm_labels = []
    dm_results = asset_result.get('layer5_dm_tests', {})
    ...
    for comp in comparisons:
        ...
        if key1 in dm_results:
            t_val = dm_results[key1]['t_stat']
        elif key2 in dm_results:
            t_val = -dm_results[key2]['t_stat']

        if np.isfinite(t_val):
            dm_t_values.append(t_val)
            dm_labels.append(f"PRG_Ext\nvs {comp}")

    if dm_t_values:
        colors = ['#27ae60' if t < -3 else '#e74c3c' if t > 3 else '#95a5a6'
                  for t in dm_t_values]
        bars = ax.bar(dm_labels, dm_t_values, color=colors, edgecolor='black', linewidth=0.5)
        ax.axhline(3.0, color='red', linestyle='--', linewidth=1, alpha=0.7, label='Harvey |t|=3')
        ax.axhline(-3.0, color='red', linestyle='--', linewidth=1, alpha=0.7)
        ax.axhline(0, color='gray', linestyle='-', linewidth=0.5)
        ax.set_ylabel('DM t-statistic', fontsize=12)
        ax.set_title('K886: DM Tests — PRG Extended vs Others (0050.TW)\n'
                      'Negative = PRG Extended wins', fontsize=13)
        for bar, t_val in zip(bars, dm_t_values):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.1,
                    f'{t_val:.2f}', ha='center', va='bottom', fontsize=10, fontweight='bold')
        ax.legend(fontsize=10)

    plt.tight_layout()
    plt.savefig(os.path.join(charts_dir, 'dm_tests_0050tw.png'), dpi=150, bbox_inches='tight')
    plt.close()

    # Chart 3: Spearman correlation comparison
    fig, ax = plt.subplots(figsize=(10, 6))
    rhos = []
    ci_los = []
    ci_his = []
    sp_names = []
    for m in model_names:
        sp = asset_result.get('layer3_spearman', {}).get(m, {})
        rho = sp.get('rho', np.nan)
        if np.isfinite(rho):
            rhos.append(rho)
            ci_los.append(sp.get('ci_lo', rho))
            ci_his.append(sp.get('ci_hi', rho))
            sp_names.append(m)

    if rhos:
        x_pos = np.arange(len(sp_names))
        colors_sp = ['#e74c3c' if r == max(rhos) else '#3498db' for r in rhos]
        bars = ax.bar(x_pos, rhos, color=colors_sp, edgecolor='black', linewidth=0.5)
        # Error bars
        err_lo = [r - cl for r, cl in zip(rhos, ci_los)]
        err_hi = [ch - r for r, ch in zip(rhos, ci_his)]
        ax.errorbar(x_pos, rhos, yerr=[err_lo, err_hi], fmt='none', color='black',
                    capsize=5, linewidth=1.5)
        ax.set_xticks(x_pos)
        ax.set_xticklabels(sp_names, fontsize=11)
        ax.set_ylabel('Spearman rho', fontsize=12)
        ax.set_title('K886: Spearman Rank Correlation (0050.TW OOS)\n'
                      '95% Bootstrap CI shown', fontsize=13)
        for bar, r in zip(bars, rhos):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.005,
                    f'{r:.3f}', ha='center', va='bottom', fontsize=10)

    plt.tight_layout()
    plt.savefig(os.path.join(charts_dir, 'spearman_0050tw.png'), dpi=150, bbox_inches='tight')
    plt.close()

    # Chart 4: VaR violation rates comparison
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    for ax_idx, level in enumerate(['VaR_1pct', 'VaR_5pct']):
        ax = axes[ax_idx]
        vrs = []
        var_names = []
        expected = None
        for m in model_names:
            var_data = asset_result.get('layer4_var', {}).get(m, {}).get(level, {})
            vr = var_data.get('violation_rate', np.nan)
            if np.isfinite(vr):
                vrs.append(vr)
                var_names.append(m)
                if expected is None:
                    expected = var_data.get('expected_rate', 0.01 if '1' in level else 0.05)

        if vrs:
            colors_var = ['#27ae60' if abs(v - expected) < expected * 0.5 else '#e74c3c'
                          for v in vrs]
            bars = ax.bar(var_names, vrs, color=colors_var, edgecolor='black', linewidth=0.5)
            ax.axhline(expected, color='red', linestyle='--', linewidth=1.5, label=f'Expected ({expected:.0%})')
            ax.set_ylabel('Violation Rate', fontsize=11)
            ax.set_title(f'{level.replace("_", " ")}', fontsize=13, fontweight='bold')
            ax.legend(fontsize=10)
    ...
    plt.suptitle('K886: VaR Backtesting — 0050.TW', fontsize=14, y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(charts_dir, 'var_backtest_0050tw.png'), dpi=150, bbox_inches='tight')
    plt.close()

    # Chart 5: Cross-market comparison with K880/K881 reference
    fig, ax = plt.subplots(figsize=(10, 7))
    # Reference data from K880v2 and K881
    ref_data = {
        'SPY': {'ov_share': 14.0, 'improvement': -1.0, 'dm_t': -0.57},
        'QQQ': {'ov_share': 16.5, 'improvement': 3.5, 'dm_t': 4.26},
        'GLD': {'ov_share': 38.0, 'improvement': 15.0, 'dm_t': 6.12},
        'EEM': {'ov_share': 45.0, 'improvement': 10.0, 'dm_t': 6.63},
        'TAIFEX': {'ov_share': 38.0, 'improvement': 55.8, 'dm_t': 5.10},
    }

    # 0050.TW from this experiment
    ov_share_tw = asset_result['session_decomposition']['overnight_var_share_pct']
    prg_q = asset_result['layer1_loss_functions']['PRG_Extended']['QLIKE']
    gjr_q = asset_result['layer1_loss_functions']['GJR']['QLIKE']
    improvement_tw = (gjr_q - prg_q) / gjr_q * 100

    # Find DM t for PRG_Extended vs GJR
    dm_t_tw = np.nan
    for key, val in asset_result['layer5_dm_tests'].items():
    ...
    for name, d in ref_data.items():
        marker = 'D' if name == 'TAIFEX' else 'o'
        color = 'orange' if name == 'TAIFEX' else '#3498db'
        ax.scatter(d['ov_share'], d['improvement'], s=120, c=color,
                   edgecolors='black', linewidth=1.5, marker=marker, zorder=5)
        ax.annotate(f"{name}\n(t={d['dm_t']:.1f})",
                    (d['ov_share'], d['improvement']),
                    textcoords="offset points", xytext=(10, 5), fontsize=9)

    # Plot 0050.TW (highlight)
    ax.scatter(ov_share_tw, improvement_tw, s=200, c='#e74c3c',
               edgecolors='black', linewidth=2, marker='*', zorder=6)
    ax.annotate(f"0050.TW\n(t={dm_t_tw:.1f})",
                (ov_share_tw, improvement_tw),
    ...
    ax.set_title('K886: Cross-Market PRG Advantage\n'
                 '0050.TW placed in context of K880/K881/K874 results',
                 fontsize=13)
    plt.tight_layout()
    plt.savefig(os.path.join(charts_dir, 'cross_market_context.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    flush_print("=" * 70)
    flush_print("K886: PRG on 0050.TW (Daily Frequency)")
    flush_print("=" * 70)
    flush_print(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    flush_print(f"Common target: sigma2_fullday = r2_overnight + r2_intraday")
    flush_print(f"Models: GJR, HAR, PRG Basic, PRG Extended, Separate GARCH")
    flush_print(f"Asset: {TICKER} — {DESCRIPTION}")
    flush_print(f"CRITICAL: Using clean_tw50_data() to fix 2014 split artifact")
    ...
    df = load_0050tw_data(START_DATE, END_DATE)

    # Descriptive stats
    ov_share = df['r2_overnight'].mean() / df['sigma2_fullday'].mean() * 100
    in_share = 100 - ov_share
    flush_print(f"  Overnight var share: {ov_share:.1f}%")
    flush_print(f"  Intraday var share: {in_share:.1f}%")
    flush_print(f"  c2c return: mean={df['r_c2c'].mean():.6f}, std={df['r_c2c'].std():.4f}")
    flush_print(f"  Skewness: {sp_stats.skew(df['r_c2c'].values):.3f}")
    flush_print(f"  Kurtosis: {sp_stats.kurtosis(df['r_c2c'].values):.3f}")
    ...
    flush_print(f"  IS: {is_end} days ({df.index[0].date()} to {is_end_date})")
    flush_print(f"  OOS: {n_oos} days ({oos_start_date} to {df.index[-1].date()})")

    if is_end < 2000:
        flush_print(f"  NOTE: IS={is_end} < 2000 recommended minimum, but proceeding "
                     f"with 70/30 split as specified.")

    if n_oos < 252:
        flush_print(f"  WARNING: OOS < 252 days ({n_oos}). Results may be unreliable.")
    ...
    flush_print(f"      {n_prg_b} valid forecasts")
    ...
    flush_print(f"      {n_prg_e} valid forecasts")
    ...
    flush_print(f"      {n_sep} valid forecasts")
    ...
    for name in model_names_list:
        ...
        if len(valid_fc) > 0:
            flush_print(f"      {name}: min={valid_fc.min():.2e}, mean={valid_fc.mean():.2e}, max={valid_fc.max():.2e}")
            if valid_fc.min() <= 0:
                flush_print(f"      WARNING: {name} has non-positive forecasts!")
    ...
    for name in model_names_list:
        ...
        flush_print(f"    {name}: QLIKE={losses['QLIKE']:.4f}, MSE={losses['MSE']:.2e}, "
    ...
    flush_print(f"    Surviving (alpha=0.10): {surviving}")
    flush_print(f"    Eliminated: {eliminated}")
    ...
    for name in model_names_list:
        ...
        layer3[name] = sp
        flush_print(f"    {name}: rho={sp['rho']:.3f} [{sp['ci_lo']:.3f}, {sp['ci_hi']:.3f}]")
    ...
    for name in model_names_list:
        ...
        for level in ['VaR_1pct', 'VaR_5pct']:
            ...
            flush_print(f"    {name} {level}: VR={v['violation_rate']:.3f} "
                         f"(expect {v['expected_rate']:.2f}), "
                         f"Kupiec {kp}, CC {cp}, Basel {v['basel']}")
    ...
    for pair, result in dm_results.items():
        flush_print(f"    {pair}: t={result['t_stat']:.2f}, "
    ...
    for key in dm_results:
        if 'PRG' in key and 'Separate' in key:
            r = dm_results[key]
            flush_print(f"    {key}: t={r['t_stat']:.2f} -> {r['interpretation']}")
    ...
    make_charts(asset_result, CHARTS_DIR)

    elapsed = (datetime.now() - t0_global).total_seconds()
    ...
    final_results = {
        'experiment_id': 'K886',
        'title': 'PRG on 0050.TW (Daily Frequency)',
        'type': 'empirical',
        'data_source': 'yfinance (0050.TW with clean_tw50_data fix)',
        'asset': TICKER,
        'description': DESCRIPTION,
        'common_target': 'sigma2_fullday = r2_overnight + r2_intraday',
        ...
        'data_cleaning': {
            'method': 'clean_tw50_data from volpred.utils',
            'reason': 'Yahoo Finance 1:4 split only adjusted from 2014-01-02; pre-2014 prices 4x too high',
            'outlier_threshold': '25% daily move (more lenient than US 20%)',
        },
        'session_decomposition': {
            'overnight_var_share_pct': float(ov_share),
            'intraday_var_share_pct': float(in_share),
        ...
        'cross_market_reference': {
            'SPY_K880v2': {'dm_t': -0.57, 'result': 'PRG NS vs GJR'},
            'QQQ_K881': {'dm_t': 4.26, 'result': 'PRG beats GJR (Harvey PASS)'},
            'GLD_K881': {'dm_t': 6.12, 'result': 'PRG beats GJR (Harvey PASS)'},
            'EEM_K881': {'dm_t': 6.63, 'result': 'PRG beats GJR (Harvey PASS)'},
            'TAIFEX_K874e': {'dm_t': 5.10, 'result': 'PRG beats GJR (Harvey PASS)'},
        },
    ...
    flush_print(f"  Charts saved to {CHARTS_DIR}/")
    ...
    flush_print(f"  SPY (K880v2):  DM t=-0.57 (NS)     — PRG does NOT beat GJR")
    flush_print(f"  QQQ (K881):    DM t=4.26 (PASS)    — PRG beats GJR")
    flush_print(f"  GLD (K881):    DM t=6.12 (PASS)    — PRG beats GJR")
    flush_print(f"  EEM (K881):    DM t=6.63 (PASS)    — PRG beats GJR")
    flush_print(f"  TAIFEX (K874e):DM t=5.10 (PASS)    — PRG beats GJR")
    prg_vs_gjr_t = np.nan
    for key, val in dm_results.items():
        if 'PRG_Extended' in key and 'GJR' in key:
            prg_vs_gjr_t = val['t_stat']
    flush_print(f"  0050.TW (K886):DM t={prg_vs_gjr_t:.2f} {'(PASS)' if abs(prg_vs_gjr_t) > 3 else '(NS)'}    — THIS EXPERIMENT")
    flush_print("=" * 70)
