...
1. **Regime-specific negative result documented and reconciled with literature.** We document (K1133/K1133b) that the GAS-t reversal on Bitcoin is concentrated in the pre-institutional period (2017-2020: DM-HLN = -4.67, 9.92% QLIKE deficit), while the FTX-Luna (2021-2023) and spot-ETF eras show no GAS-t deficit. The companion cross-asset experiment K1129 — whose out-of-sample window starts in 2021 — independently finds a GAS-t deficit on BTC (DM-HLN ≈ -4.6 on its own window), flagging BTC as the cross-asset anomaly that motivated this period decomposition. Together these reconcile a single anomalous asset-level finding with the broader GAS literature where GAS-t typically wins on equity indices and commodities.
...
3. **Regime-switching rescues to parity, not superiority.** A Markov-Switching GAS-t extension provides a +5.97 DM-HLN improvement over single-state GAS-t, bringing it to statistical parity with GJR-Normal (point estimate marginally favourable, DM-HLN = +0.28, not significant) at the cost of roughly doubling the parameter count. The rescue-to-parity outcome rules out simple regime-confounding and indicates that the t-innovation mismatch is structural to pre-institutional BTC return distributions, not an artifact of unmodeled volatility regimes — but regime flexibility buys no net forecasting edge over the simpler Normal benchmark.
...
- Puzzle: K1129 (OOS 2021 onward) documents that GAS-t reverses on BTC (DM-HLN ≈ -4.6 vs GJR-N on its window), the lone anomaly in its cross-asset panel; our own K1133b decomposition locates the deficit in the pre-institutional period (2017-2020: DM-HLN = -4.67). Both contradict the broader GAS literature.
...
- **Three-period split** (institutional structure-based, pre-registered):
  ...
  - **Period 3 (spot-ETF era)**: 2026-01-05 → 2026-04-14 (n_OOS = 100, preliminary; the 2024-01 calendar start plus the 750-day estimation warm-up places the OOS window in 2026Q1, i.e., two years into the spot-ETF institutional regime). Spot ETF approvals 2024-01-10.
...
- **MS-GAS-t**: Two-state Markov-switching GAS-t with a hybrid Gray (1996)/Klaassen (2002) recursion, matching the archived implementation.
...
- **Evaluation**: QLIKE loss (Patton 2011); Diebold-Mariano-HLN t-statistic; conventional 5% significance for primary pairwise contrasts, with the more conservative Harvey-Liu-Zhu (2016) `|t| > 3` bar reported separately as a robustness tier rather than as the main gate.
...
- **Period 3 — Spot-ETF era (OOS window 2026-01-05 → 2026-04-14, n_OOS = 100 days, preliminary).** The period's calendar anchor is the 10 January 2024 SEC approval of US-listed spot Bitcoin ETFs (BlackRock IBIT, Fidelity FBTC, and others); after the 750-day rolling estimation warm-up, the evaluable out-of-sample window falls in 2026Q1, i.e., two years into the institutionalized regime, by which point creation-redemption arbitrage had fully linked Bitcoin's price to large-asset-manager flow and traditional brokerage rails. This window therefore measures the mature spot-ETF regime rather than the immediate post-approval transition. Sample size is acknowledged as preliminary; conclusions for Period 3 are reported but not used to support the paper's primary diagnostic claim, which is established on Period 1.
...
To test whether a regime-switching extension can rescue GAS-t on Period 1, we estimate a two-state Markov-switching GAS-t (MS-GAS-t) using a *hybrid* Gray (1996)/Klaassen (2002) construction: the implementation maintains per-state log-variance paths and updates them with Gray-style filtered state probabilities, rather than the pure Klaassen recursion that collapses the lagged variance via the expectation conditional on the *current* state (the archived implementation notes in `experiments/k1133b/README.md` document this choice). Both constructions avoid the Hamilton (1989) path-dependence problem that renders exact likelihood evaluation intractable; they coincide in the limit where the regimes are highly persistent ($p_{00}, p_{11} \to 1$). We caution that our fitted transition probabilities ($p_{00} \in [0.42, 0.85]$, $p_{11} \in [0.25, 0.75]$ across refits) are well away from that limit, so the hybrid filter is a genuinely different estimator from pure Klaassen in our sample, and replication should follow the archived code rather than the textbook Klaassen recursion. <!-- AUDIT FIX 2026-06-10: earlier draft described the estimator as pure Klaassen; corrected to match the archived implementation. Gray (1996) added to references. -->

For reference, the pure Klaassen formulation collapses the lagged variance via the *expected* lagged variance conditional on the current state and the full history:
...
The three-period partition described in Section 3 is institutionally motivated and pre-registered: the cut points correspond to identifiable structural events in the Bitcoin market microstructure rather than to data-driven changepoint detection. The pre-institutional period ends on 31 December 2020, before MicroStrategy's first treasury allocation began the institutional adoption wave; the FTX-Luna period brackets the major centralized exchange and stablecoin failures of 2022-2023; and the spot-ETF regime is evaluated through a preliminary 2026Q1 OOS window that arrives only after the 750-day rolling warm-up has elapsed. The pre-registered cut points eliminate the in-sample search bias that would inflate the apparent significance of any cross-period contrast and align the analysis with the structural-break testing tradition of Andrews (1993) and Bai and Perron (1998), albeit with the cut points specified ex ante from institutional history rather than estimated.

Table 1 reports the headline Diebold-Mariano-Harvey-Lin-Newey (DM-HLN) test statistics for models M2-M5 against the M1 GJR-Normal benchmark across the three institutionally motivated periods. The signs follow the standard QLIKE convention: a negative statistic indicates that the alternative model produces a higher (worse) QLIKE loss than the benchmark. We report conventional two-sided 5% significance for the pairwise DM-HLN comparisons and treat the more conservative Harvey-Liu-Zhu (2016) `|t| > 3` bar as a robustness tier for especially strong sub-period evidence.
...
*Notes: Negative statistics indicate that the alternative model produces a higher QLIKE loss than the M1 GJR-Normal benchmark. Bold entries denote conventional two-sided 5% significance; the stricter Harvey-Liu-Zhu (2016) `|t| > 3` bar is discussed separately as a robustness tier. The DM test uses the HLN small-sample correction (Harvey, Leybourne, and Newbold, 1997). Sample sizes refer to OOS one-step-ahead daily forecasts.*
...
The innovation contrast compares M4 GAS-Normal to M3 GAS-Student-t, holding the score-driven dynamics fixed at GAS and varying only the innovation distribution. The QLIKE drops from 2.1904 under M3 to 2.0402 under M4, and DM-HLN equals +2.67. The positive sign supports an inference that the Normal innovation is statistically preferred to the Student-t innovation within the GAS framework on pre-institutional Bitcoin at conventional levels, though it does not clear the stricter Harvey-Liu-Zhu `|t| > 3` bar. The two-sided p-value is approximately 0.008. The economic magnitude of 6.9% in QLIKE is non-trivial relative to the 9.92% deterioration of M3 against M1, accounting for roughly three-quarters of the reversal that the factorial design is asked to explain.
...
Read jointly, the innovation contrast and the dynamics contrast suggest a simple decomposition of the M3-versus-M1 deficit. The 9.92% QLIKE deterioration of GAS-t against GJR-Normal can be apportioned approximately as 76% attributable to the Student-t innovation factor (the M3-to-M4 step, which moves from 2.1904 to 2.0402 in QLIKE) and 24% attributable to the GAS dynamics factor (the M4-to-M1 step, which moves from 2.0402 to 1.9926). The factor with the larger share is statistically stronger while the smaller share does not. The asymmetric statistical resolution is consistent with the QLIKE point estimates and does not require the imposition of additional structure; it falls out of the orthogonal factorial design.
...
A complementary perspective comes from considering what the regime-switching technology actually accomplishes. MS-GAS-t separates the pre-institutional sample into two latent states whose tail parameters and score-driven coefficients differ. The improvement over single-state GAS-t implies that there exists a partitioning of the sample for which a state-specific Student-t calibration outperforms a pooled Student-t calibration. The failure to surpass GJR-Normal implies that even the best state-specific Student-t calibration matches but does not beat a Normal-innovation alternative without any regime flexibility. The two implications are jointly consistent if the optimal state-specific tail parameters move toward thinner tails than the pooled estimate in at least one state. The archived MLE refit log shows partial de-fattening rather than convergence to the Normal limit: across the six refits, the higher-$\nu$ state ranges over $\nu \approx 7.1$--$15.5$ while the lower-$\nu$ state ranges over $\nu \approx 2.8$--$6.9$ (ms_fit_log, K1133b). A $\nu$ of 9--15 is still moderately fat-tailed, so the regime-switching technology rescues part of the GAS-t deficit by *partially* de-fattening the tail in one regime — a movement in the direction of the Normal-innovation alternative, but not a wholesale revealed preference for it. We therefore read the MS evidence as consistent with, though weaker than, the innovation-attribution story from the factorial decomposition.

Figure 3 plots the filtered state probability path for MS-GAS-t over the full sample, with vertical markers at the FTX collapse (November 2022), the Luna collapse (May 2022), and the U.S. spot ETF approvals (January 2024). The filtered probabilities exhibit the regime persistence characteristic of two-state mixture models, with day-scale persistence that sometimes clusters into longer stretches but is not yet backed by an audited duration table. The event markers serve to anchor the regime trajectory to the institutional history of the asset and do not enter the estimation. The Period 2 and Period 3 comparisons of MS-GAS-t against M3 and against M1 produce DM-HLN statistics within the noise band (|t| < 1.0 in both periods), consistent with the Section 4 conclusion that no GAS-t deficit exists in those eras and consequently no rescue is required. The MS-GAS-t result therefore reinforces rather than complicates the Section 4 narrative: regime-switching matters where there is a deficit to rescue, contributes nothing where the single-state specification is already adequate, and reaches only parity with the Normal-innovation benchmark in the period where the rescue is needed most.
...
## Section 8: Robustness — implemented safeguards and planned extensions

<!-- AUDIT FIX 2026-06-10: the earlier draft of this section reported point estimates for five
robustness exercises (shift-removal contrast, multi-start dispersion shares, ±60-day boundary
shifts, alternative-loss DM statistics, leave-one-year-out DM range, ETH/BNB replication) that
do not exist in any archived results file (k1133b / k1133 / K1129 JSONs, READMEs, run logs all
grep-negative). Per the research-integrity rule those numbers are removed; this section now
distinguishes safeguards that ARE archived from robustness checks that are PLANNED. The planned
items must be run and landed in a k1133b_robustness_results.json before this draft converts to
LaTeX. -->

**Implemented safeguards (archived).** Two threats to inference are addressed in the archived
K1133b pipeline itself. First, look-ahead bias: all five candidate models and the
Markov-Switching extension generate one-step-ahead variance forecasts using only information
dated $t-1$, with an explicit `realized.shift(1)` step applied before any loss differential is
computed; the K1133b run was subjected to an independent code review (Codex, 2026-04-17) that
verified the absence of forward-looking leakage in both the single-state and Markov-Switching
estimation paths. Second, degenerate-regime filtering in the Markov-Switching estimation: runs
in which the inferred state-probability series collapses to a single regime are discarded by
the estimation code.

**Planned robustness checks (not yet run).** The following exercises are specified but their
results are not yet computed; no quantitative claims are made for them in this draft.
(i) *Optimiser stability*: multi-start MLE over the rolling windows with a documented
cross-seed log-likelihood dispersion diagnostic, per the K1213 methodology rule.
(ii) *Period-cut sensitivity*: re-running the headline factorial with the Period 1/2 and
Period 2/3 boundaries shifted by ±60 days around the FTX-Luna and spot-ETF anchors.
(iii) *Loss-function specificity*: re-evaluating the factorial under MSE-on-RV and the robust
loss family of Patton (2011).
(iv) *Leave-one-year-out*: re-estimating Period 1 contrasts excluding 2017, 2018, and 2020 in
turn.
(v) *Cross-asset generalisability*: replicating the Period 1 factorial on ETH and BNB.
Until these are run and archived, the headline result should be read with the caveat that its
robustness battery is incomplete; the implemented safeguards above cover the two highest-risk
threats (look-ahead and degenerate regimes).

## Section 9: Conclusion and Future Directions

This paper has documented and decomposed a previously unreported reversal in Generalized Autoregressive Score volatility models with Student-t innovations on pre-institutional Bitcoin. Three findings stand out. First, the reversal is period-specific: GAS-t underperforms GJR-Normal by 9.92\% in QLIKE during 2017--2020 (DM-HLN $= -4.67$), but the deficit disappears in both the FTX-Luna and spot-ETF eras, where no specification contrast carries a Diebold-Mariano statistic above $|1.1|$. Second, the reversal is driven by the Student-t innovation rather than by score-driven dynamics: an orthogonal factorial decomposition shows that swapping the innovation to a Normal density restores GAS to statistical parity with the GJR-Normal benchmark, while swapping the dynamics from GJR to GAS within the Normal family produces no significant degradation. Third, a Markov-Switching GAS-t extension provides a substantial $+5.97$ DM-HLN improvement over the single-state specification, reaching statistical parity with GJR-Normal (point estimate marginally favourable, DM-HLN $= +0.28$, not significant) at a substantial parameter cost --- ruling out regime-confounding while showing that regime flexibility buys no net edge over the simpler Normal benchmark; the t-innovation mismatch is a structural feature of the pre-institutional return-generating process.
