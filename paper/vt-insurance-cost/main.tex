...
Two features of this accounting deserve emphasis. First, $\text{IP}_{\text{opp}}$ is a \emph{net} quantity: because it is computed as a full-sample return gap, the returns VT avoids during drawdowns---the insurance payout---are already netted against the upside it forgoes. It measures foregone participation net of protection delivered, not gross foregone upside. Second, the opportunity-cost share $s = \text{IP}_{\text{opp}} / \text{IP}_{\text{total}}$ is a ratio of two sample means and therefore carries sampling uncertainty that a point estimate conceals. We quantify it with a stationary bootstrap \citep{politis1994} (circular, expected block length 21 trading days, 10{,}000 replications, seed 42), resampling the joint daily return-and-weight path so that both components are drawn on identical index paths. Because $s = R/(1+R)$ with $R = \text{IP}_{\text{opp}} / \text{IP}_{\text{direct}}$ is unbounded when the total premium approaches zero, we report a bounded percentile interval for $s$ only when the resampled total premium stays away from zero, and fall back on the total-premium interval otherwise.
...
\paragraph{Sampling uncertainty.} The point estimates above are imprecise, and the two strategies are imprecise in different ways. For unconditional VT (S1), the stationary bootstrap puts the opportunity-cost share at 90.8\% with a 95\% interval of $[50.0\%,\ 94.8\%]$; the total premium itself is bounded away from zero ($[0.87\%,\ 8.13\%]$ per annum). The qualitative claim that survives resampling is therefore the \emph{ordering}---opportunity cost is at least as large as direct cost, with the lower bound sitting essentially at parity---rather than the 91\% figure itself, which should be read as a point estimate with a wide upper tail, not a precisely identified share. For VoV-conditional VT (S2), \emph{no} bounded interval for the share exists: the resampled total premium straddles zero (2{,}295 of 10{,}000 replications produce a total premium $\leq 0$; interval $[-2.63\%,\ 3.99\%]$ per annum), so the share's denominator is not signed and the ratio has no bounded connected interval. This is a substantive result, not a numerical nuisance: once sampling uncertainty is admitted, S2's premium cannot be distinguished from zero, which independently corroborates the cross-OOS evidence in §\ref{sec:crossoos} and is the reason we treat VoV conditioning as hypothesis-generating rather than as a validated cost-reduction rule.
...
Figure~\ref{fig:wealth} makes the accounting visible as a wealth path. The S1--S0 gap widens gradually through the long unshaded stretches and narrows at crash troughs, where the protection pays out; it does not open in discrete episodes. This is the visual counterpart of the decomposition: the premium is paid continuously in forgone participation, not at the moments the hedge is needed.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig1_wealth_regimes.pdf}
\caption{Cumulative wealth (log scale) of buy-and-hold SPY (S0), unconditional VT (S1), VoV-conditional VT (S2), and the 50/50 SPY/GLD benchmark (S4), 2012--2024, with VoV-regime shading. Strategy rules, price convention (raw Close), and the CAGR convention are those of the replication package; the plotting script asserts each curve's CAGR against Table~\ref{tab:performance} before writing the figure. Source: K811v2 (\texttt{k811v2\_fig1\_wealth\_regimes.py}).}
\label{fig:wealth}
\end{figure}
