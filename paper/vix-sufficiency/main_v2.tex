...
Our main finding is strongly negative but highly informative. Across 11 signal families, more than 30 VIX sufficiency tests, and five market eras totaling 8,325 trading days, we find:
...
\begin{itemize}
    \item \textbf{Zero parameters}: The numerator 12 represents a target annualized volatility of 12\%, chosen as a round number near the long-run average of equity realized volatility.
...
Vol momentum 20/60 & 0.0001 & 0.0001 & \textbf{0.0216}$^{*}$ & 0.0002 & \textbf{0.0372}$^{*}$ & 2/5 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes}: Incremental $R^2$ from adding the signal to a VIX-only regression of 22-day forward realized volatility. Bold values with $^{*}$ indicate the signal passes the Harvey (2016) $|t|>3.0$ threshold in that era. During the GFC (Era~3), all three signals show economically meaningful incremental power, and during the COVID/inflation period (Era~5), volatility momentum achieves the largest incremental $R^2$ (0.037). These era-specific exceptions do not undermine overall VIX sufficiency---they occur precisely during extreme-stress transitions when the VIX level alone temporarily underestimates the speed of volatility change.
\end{tablenotes}
\end{threeparttable}
\end{table}

The pattern reveals an important nuance: VIX sufficiency holds firmly in normal regimes (Eras 1, 2, 4) where incremental $R^2$ values are uniformly below 0.001. However, during extreme-stress transitions---the GFC (Era~3) and COVID onset (Era~5)---competing signals show economically meaningful incremental power, with all three signals passing the Harvey threshold in Era~3. This suggests that VIX sufficiency has \emph{era-specific exceptions} during regime shifts, when the speed of volatility change temporarily outpaces the option market's ability to reprice. Nevertheless, the full-sample result (Table~3) overwhelmingly supports VIX sufficiency: the transient era-specific gains do not compound into reliable long-run forecasting improvement.
...
We conclude with a broader observation. In financial econometrics, null results are underreported relative to their information content. Our systematic horse race, with its pre-specified signal families and stringent statistical controls, provides stronger evidence than any individual positive finding could. The more than 30 null results accumulated across this research program are not failures; they are independent confirmations that the option market efficiently aggregates volatility-relevant information.
