...
% ─── Title ───────────────────────────────────────────────────
\title{Can Anything Beat VIX? A Systematic Out-of-Sample Evaluation of Eleven Signal Families for Equity Volatility Forecasting and Volatility Timing\thanks{All errors are my own. Data and replication code are available upon request.}}
...
\begin{abstract}
\noindent
We conduct a systematic out-of-sample horse race among eleven pre-specified signal families---cross-asset volatility momentum, VIX term structure, behavioral sentiment, the variance risk premium, multi-asset portfolio optimization, equal risk contribution, Bitcoin volatility, the yield curve slope, Google Trends fear proxies, overnight VIX changes, and calendar anomalies---evaluating whether any can improve upon VIX alone for equity volatility forecasting and volatility-timing portfolio construction. Using daily S\&P~500 data spanning 1993--2026, we apply a unified forecast pipeline with strict lag enforcement, transaction-cost accounting, the \citet{harvey2016} multiple-testing threshold ($|t|>3.0$), formal Holm-Bonferroni correction, and proxy-robust evaluation following \citet{patton2011}. The main finding is strongly negative: not a single signal family produces a statistically significant out-of-sample improvement---whether measured by the standard \citet{campbell2008} $R^2_{\text{OOS}}$ relative to the historical mean, by incremental $R^2$ relative to VIX-only forecasts, or by Sharpe ratio gains for volatility-timing strategies---and all results survive Holm-Bonferroni family-wise error rate correction. The VIX--realized volatility $R^2$ ranges from 0.24 to 0.64 across five non-overlapping eras spanning 33 years (coefficient of variation = 0.37), demonstrating time-invariance. Volatility model rankings are criterion-dependent---GJR-GARCH dominates under proxy-robust QLIKE on $r_t^2$ (Model Confidence Set sole member) while AMEM dominates for VaR/ES risk management (composite score 1.94 vs.\ 1.63)---yet VIX sufficiency holds regardless of which model or criterion is used. We frame VIX sufficiency as a rational outcome of option-market information aggregation and show that volatility timing functions as drawdown insurance---costly in Sharpe terms (average drag 3.49\%/year for 12/VIX) but welfare-improving for investors with CRRA risk aversion $\gamma \geq 4.5$. The aggregate null conceals structured channel-specific heterogeneity: HAR+VIX achieves regime-invariant gains on Parkinson realized volatility for large-cap equity (QQQ/IWM, all three VIX regimes), GARCH-MIDAS long-run factors are universally null (0 conditional passes across 54 cells), and GAS-t score-driven models are architecturally harmful on equity (DM $t=-3.27$) yet provide validated tail-risk estimation on commodity (ES passes 4/4 assets). A seven-asset cross-asset Publication-In-Time (PIT) panorama spanning equities, commodities, fixed income, and cryptocurrency (SPY, GLD, TLT, BTC-USD, QQQ, USO, EEM) delivers UNIVERSAL\_NULL\_7/7: of 28 specification cells, only one isolated outlier (TLT $\times$ financial-stress, DM $t=+3.74$) reaches the Harvey threshold, collapsing to $t=+2.00$ under a one-week extra lag and failing the QLIKE materiality gate, confirming native-IV sufficiency as the broadest cross-asset finding available at daily frequency. The informative null sharpens the research frontier: future gains require either higher-frequency realized measures or genuinely exogenous information not already embedded in option prices.
...
\begin{itemize}
    ...
    \item VIX's forecasting power is time-invariant: the VIX--realized volatility $R^2$ ranges from 0.24 (low-volatility QE era, 2012--2020) to 0.64 (post-dot-com era, 2000--2007), with a coefficient of variation of only 0.37.
    ...
    \item The aggregate null conceals structured channel-specific heterogeneity: HAR+VIX achieves regime-invariant Parkinson-volatility gains on large-cap equity (QQQ/IWM, all three VIX regimes); GARCH-MIDAS long-run factors are universally null (0 of 54 conditional passes); GAS-t is architecturally harmful on equity (DM $t = -3.27$ for SPY) but provides validated tail-risk estimation on negatively-skewed commodities (ES backtest passes 4/4 assets at 1\% level).
    \item Native-IV sufficiency extends universally across seven asset classes: of 28 cross-asset PIT specification cells (SPY, GLD, TLT, BTC-USD, QQQ, USO, EEM $\times$ four alt-data specs), only one cell (TLT $\times$ financial-stress, DM $t = +3.74$) crosses the Harvey threshold under the primary specification, and it fails every robustness check (lag-sensitive: collapses to $t = +2.00$ under one-week extra lag; QLIKE improvement $= +0.50\%$, below the 5\% materiality gate).
\end{itemize}
...
Our third contribution concerns the research frontier. The informative null delineates what \emph{cannot} work (daily signals already in the option-market information set) from what \emph{might} work: genuinely exogenous shocks (e.g., geopolitical events, pandemic declarations) that arrive between option-market pricing updates, and possibly higher-frequency realized measures (5-minute data) whose relative performance is as yet unresolved. A small-sample pilot (K745, $N = 37$) directly comparing daily HAR-ABS ($\text{QLIKE} = 0.077$) with 5-minute HAR-RV ($\text{QLIKE} = 0.109$) found that the daily model \emph{outperformed} the 5-minute model by 41.8\% on this limited sample---the reverse of what the high-frequency literature conventionally predicts. This preliminary result is not powered to establish that 5-minute data cannot improve forecasts; it shows only that under the K745 specification and sample, 5-minute HAR-RV did not beat daily HAR-ABS. A larger-scale experiment (K1139, planned) will use an extended 5-minute SPY dataset to resolve whether 5-minute information carries incremental predictive content beyond daily closing-price VIX. We therefore flag the 5-minute frontier as open but not yet empirically established.
...
%=================================================================
\section{Data and the Eleven Signal Families}
\label{sec:data}
%=================================================================
...
\subsection{The Eleven Signal Families}

We pre-specify eleven signal families spanning six broad categories: cross-asset, derivatives-based, behavioral, macroeconomic, alternative data, and calendar. Table~\ref{tab:signals} summarizes each family.

% ─── Table 1: Signal Families ────────────────────────────────
\begin{table}[H]
\centering
\small
\caption{The Eleven Signal Families}
\label{tab:signals}
...
Table~\ref{tab:main_results} presents the main results for all eleven signal families. The key finding is unanimous: no signal produces a statistically significant out-of-sample improvement over VIX.

% ─── Table 2: Main Results ───────────────────────────────────
\begin{table}[H]
\centering
\small
\caption{Volatility Forecasting: Eleven Signal Families vs.\ VIX}
...
% source: experiments/k731/k731_vix_term_structure_results.json .full_sample_strategies."BH 50/50"
--- & Buy-and-hold 50/50 SPY/GLD & 0.827 & $-0.043$ & $-32.5$ & 0.0 & --- \\
% source: experiments/k731/k731_vix_term_structure_results.json .full_sample_strategies."12/VIX"
...
Evaluated over the canonical 2008--2026 sample (K731), the 12/VIX benchmark (Sharpe = 0.870, MDD $= -32.2\%$) marginally outperforms the static 50/50 SPY/GLD allocation (Sharpe = 0.827, MDD $= -32.5\%$), with near-identical drawdown profiles. Under period-aligned measurement, VT does not trade return for tail protection: 12/VIX delivers slightly higher Sharpe \emph{and} marginally smaller MDD than buy-and-hold. The substantive conclusion of this section is therefore unchanged at the signal level---no signal-based strategy clears Harvey's $|t| > 3.0$ threshold against 12/VIX---but the comparison between the two benchmarks is more nuanced than previously stated, with VT showing modest dominance over passive allocation on this sample.
...
We further test whether financial-stress alt-data (NFCI, STLFSI, USEPU, WLEMU) can improve a daily regime-switching allocation between SPY and GLD. The matched-pair design (K1123, $N = 1{,}819$ trading days, 2019--2026) tests five alt-data allocation rules against the static 50/50 baseline (Sharpe $= 1.31$, OOS Sharpe $= 1.98$). All five alt-data strategies underperform the baseline on full-sample Sharpe (best: $1.16$) and on out-of-sample Sharpe. This extends the allocation null from the portfolio-optimization domain (Table~\ref{tab:multiasset}) to the regime-switching domain: VIX sufficiency holds regardless of whether the application is volatility forecasting, volatility-timing strategy, multi-asset optimization, or alt-data regime allocation.
% source: experiments/k1123/k1123_results.json .verdict .headline_table .n_days
...
\midrule
\multicolumn{3}{l}{Cross-era statistics:} & \multicolumn{2}{l}{Mean $R^2 = 0.446$} & \multicolumn{3}{l}{CV = 0.37 (time-invariant)} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
The VIX--RV relationship is remarkably stable. The coefficient of variation of $R^2$ across eras is 0.37---well below the 0.50 threshold we adopt for time-invariance. Importantly, the low $R^2$ in the QE era (0.244) does not indicate VIX failure; rather, it reflects the mechanical compression of both VIX and RV during this period (VIX stuck near 15, RV near 12), which reduces regression $R^2$ without impairing VIX's relative forecasting accuracy.
...
\midrule
% source: experiments/k752_vix_sufficiency_eras_results.json .part_d_competing_signals_by_era.Era<k>.signals.<signal>.incremental_R2
Overnight VIX (abs) & 0.0002 & 0.0001 & 0.0039$^{\dagger}$ & 0.0006 & 0.0032 & 1/5 \\
VRP proxy & 0.0005 & 0.0002 & 0.0160$^{\dagger}$ & 0.0008 & 0.0005 & 1/5 \\
Vol momentum 20/60 & 0.0001 & 0.0004 & 0.0216$^{\dagger}$ & 0.0001 & 0.0372$^{\dagger}$ & 2/5 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes}: Incremental $R^2$ from adding the signal to a VIX-only regression of 22-day forward realized volatility. Values are near zero in non-crisis eras (Era 1 DotCom, Era 2 PostDotCom, Era 4 LowVol/QE), confirming VIX sufficiency in normal regimes. $^{\dagger}$Denotes an in-sample Harvey $|t|>3.0$ threshold crossing in the indicated era; four such crossings occur---three in Era 3 (GFC: all three signals) and one in Era 5 (COVID/Inflation: Vol momentum 20/60). These in-sample crisis-era exceptions do not survive out-of-sample Diebold--Mariano testing with Holm--Bonferroni correction (Table~\ref{tab:horse_race}), consistent with the paper's core VIX-sufficiency finding on average while identifying well-defined crisis-regime boundary conditions.
\end{tablenotes}
\end{threeparttable}
\end{table}

The pattern is nuanced: during the three non-crisis eras (DotCom, PostDotCom, LowVol/QE), incremental $R^2$ values are uniformly tiny (all below 0.001) across all signals, confirming VIX sufficiency in normal market regimes. However, during the GFC (Era 3) and COVID/Inflation (Era 5) eras, three signals---Overnight VIX absolute change, VRP proxy, and Vol momentum 20/60---exhibit materially larger incremental $R^2$ values (up to 0.037 for Vol momentum in Era 5), with four in-sample cells crossing the Harvey threshold: all three signals in Era 3 GFC, and Vol momentum 20/60 in Era 5. This in-sample crisis-era exception is consistent with the paper's core out-of-sample finding, however: all era-pooled OOS Diebold--Mariano tests remain $|t|<3$ even after Holm--Bonferroni correction (Table~\ref{tab:horse_race}). The refined reading is that VIX sufficiency is robust on average but has identified, well-defined crisis-regime boundary conditions where the option-market information set may be partially incomplete during extreme volatility episodes.

A complementary matched-pair design directly tests whether alt-data signals improve forecasting specifically during VIX jump days (K1117, $N_{\text{jump}} = 181$ days at the VIX 2$\sigma$ threshold, $N_{\text{total}} = 4{,}022$). Six alt-data variables (VVIX, USEPU, NFCI, ANFCI, STLFSI, WLEMU) are tested for incremental forecasting content on jump-day windows versus control windows. The verdict is FULL\_NULL: no signal achieves DM-HLN $|t|>2$ in any specification, and no hypothesis ($H_1$, $H_2$, $H_3$) passes BH correction (VVIX: $H_1$ DM $t = 1.35$, $p_{\text{BH}} = 0.69$). VIX sufficiency extends even to extreme volatility episodes where option-market information updating is fastest.
% source: experiments/k1117/k1117_results.json .verdict .jump_counts .tests.vvix
...
The aggregate null across eleven signal families conceals a structured heterogeneity in \emph{how} different model architectures fail. We characterize three channels with qualitatively distinct failure patterns, providing sharper guidance for future research beyond "all signals fail."
...
\begin{enumerate}
    \item \textbf{Higher-frequency information}: Intraday 5-minute realized volatility \emph{may} capture microstructure dynamics (e.g., opening auctions, lunch-hour liquidity, closing imbalances) that closing-price VIX does not fully reflect. The direct evidence from our K745 pilot ($N = 37$), however, runs in the opposite direction: daily HAR-ABS ($\text{QLIKE} = 0.077$) outperformed 5-minute HAR-RV ($\text{QLIKE} = 0.109$) by 41.8\% in this small sample. A larger-scale replication (K1139, planned) is required to determine whether the small-sample direction reverses at scale; until then, we regard the intraday frontier as theoretically plausible but empirically unconfirmed.
...
Moreover, the robustness of VIX sufficiency is underscored by three additional findings. First, volatility model rankings are criterion-dependent: GJR-GARCH dominates under proxy-robust QLIKE on $r_t^2$, but AMEM dominates under $|r_t|$ targets. This instability in model rankings makes the stability of VIX sufficiency across all criteria all the more striking. Second, forecast accuracy and risk management quality are distinct dimensions---AMEM is the best risk manager (VaR/ES composite score 1.94) while GJR is the best forecaster---yet both operate entirely within the VIX information set, confirming that the relevant optimization is over model specification and application domain, not over information sources. Third, a seven-asset cross-asset PIT panorama (Table~\ref{tab:panorama_7asset}) delivers UNIVERSAL\_NULL\_7/7: of 28 specification cells spanning equities, commodities, fixed income, and cryptocurrency, only one isolated outlier reaches the Harvey threshold and it fails every robustness check, establishing native-IV sufficiency as the broadest cross-asset finding at daily frequency.

The practical implications are clear:

\begin{enumerate}
    \item \textbf{For researchers}: The daily-frequency, closing-price frontier is exhausted. Gains require genuinely exogenous information outside the option-market information set; the 5-minute realized-measure frontier is theoretically plausible but is not yet empirically confirmed (our K745 pilot with $N = 37$ found daily HAR-ABS \emph{outperformed} 5-minute HAR-RV by 41.8\%, a result that should be tested at scale before positing 5-minute data as a reliable improvement channel).
    ...
    \item \textbf{For regulators and risk managers}: VIX sufficiency is time-invariant (CV = 0.37 across 33 years). Models based on VIX do not need frequent recalibration, and regulatory frameworks (e.g., Basel III/IV) that reference VIX for market risk are on solid empirical ground.
\end{enumerate}
