...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Strict lag enforcement}: Every signal is lagged one trading day (\texttt{signal.shift(1)}) before being applied to returns, preventing look-ahead bias---a surprisingly common error that we document has inflated Sharpe ratios by up to 373\% in prior work.\footnote{In separate robustness analysis, careful code review identified look-ahead bias in four backtests, one of which produced a Sharpe ratio of 1.68 that collapsed to 0.355 after lag correction.}
    ...
    \item \textbf{Multiple-testing control}: We adopt the approximate conservative threshold of $|t| > 3.0$ motivated by \citet{harvey2016} and implement formal Holm-Bonferroni sequential correction across all Diebold-Mariano tests in the full family set (including the two revision-added families), controlling the family-wise error rate at $\alpha = 0.05$.
...
Our main finding is strongly negative but highly informative. Across 13 signal families and the full set of VIX sufficiency tests reported in Section~\ref{sec:results}, spanning five market eras totaling 8,325 trading days, we find:
...
\begin{itemize}
    ...
    \item No signal-based strategy statistically outperforms the simple 12/VIX benchmark (maximum Sharpe improvement = +0.030, behavioral sentiment, DM $|t| = 0.72$, not significant).
...
\begin{tablenotes}
\small
\item \textit{Notes}: All signals are computed using information available at market close on day $t-1$ and applied to day $t$ returns. Google Trends data are weekly and converted to daily by forward-filling. Sample sizes vary by data availability. VIX3M begins in 2008; Bitcoin reliable pricing begins in 2015; Google Trends via pytrends begins in 2004 but we use 2010+ for stability. $^\dagger$Families 12 and 13 operate on weekly observations (Friday-aligned realized vol target) due to NFCI's weekly release frequency; $N=431$ refers to the weekly count over 2018--2026 (260 IS + 171 OOS). Publication delays for the FRED inputs follow the release-calendar convention of \S\ref{sec:pub_delay}: the daily Economic Policy Uncertainty series (USEPU, WLEMU) clear their one-business-day release lag well before the next weekly forecast and enter at $\text{shift}(1)$, whereas the weekly financial-stress series (NFCI, ANFCI, STLFSI4), released five to six calendar days after their Friday reference date, enter at $\text{shift}(2)$; both are aligned to the weekly W-FRI evaluation grid. This is the ``corrected'' timing whose robustness against the uniform $\text{shift}(1)$ and uniform $\text{shift}(2)$ alternatives is reported in \S\ref{subsec:pub_delay_robust}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Overnight price changes reflect the processing of after-hours news, earnings announcements, and geopolitical events \citep{lou2019}. We use the absolute overnight VIX change ($|\text{VIX}_{\text{open},t} - \text{VIX}_{\text{close},t-1}|$) as a proxy for the information content of overnight news. This family is the sole exception to the close-of-$t{-}1$ information-set convention used by Families 1--9 and 11: its forecast origin is the day-$t$ opening auction (9:30 ET), at which $\text{VIX}_{\text{open},t}$ is realized, and the target is the forward realized volatility measured from day $t$ onward. Because both inputs ($\text{VIX}_{\text{open},t}$ and $\text{VIX}_{\text{close},t-1}$) are observed no later than the day-$t$ open while the target accumulates strictly after it, the signal contains no look-ahead; the intraday origin is a legitimate timing convention rather than a leak, and it is disclosed here so that the one-day-shift enforcement of the other daily families is not misread as applying to this one.
...
\citet{baker2016} construct the Economic Policy Uncertainty (EPU) index from newspaper coverage, tax-code expiration counts, and forecaster disagreement, and document its association with elevated volatility in equity and macroeconomic series. The daily U.S.\ EPU index (FRED: \texttt{USEPUINDXD}) and the weekly World Leading Economic Monetary Uncertainty index (\texttt{WLEMUINDXD}) provide canonical text-based uncertainty proxies that are widely cited in subsequent volatility-forecasting work. We test whether their composite carries information about future realized volatility orthogonal to VIX. Because the daily EPU series is publicly released with a one-business-day lag relative to the underlying news sample, the prior week's value is available before each weekly forecast, and all EPU inputs enter at $\text{shift}(1)$ on the weekly W-FRI evaluation grid shared by Families 12--13 (the release-calendar convention of \S\ref{sec:pub_delay}, verified in the robustness audit of \S\ref{subsec:pub_delay_robust}). Under this convention the alt-data-only EPU composite (Model M3, which \emph{replaces} VIX) is dominated by the VIX baseline with Diebold--Mariano $t=-2.54$ in out-of-sample QLIKE comparison; when EPU is instead \emph{added} to VIX (the nested increment), it contributes no incremental accuracy under the Clark-West test (Section~\ref{sec:results})---redundant with, rather than harmful to, the option-implied signal.

\subsubsection{Family 13: Financial-Stress Indices}

The Chicago Fed's National Financial Conditions Index \citep[NFCI;][]{brave2011} and the St.\ Louis Fed's Financial Stress Index \citep[STLFSI;][]{kliesen2010} aggregate prices, spreads, and balance-sheet quantities into composite financial-conditions readings that have been shown to lead recession dates and credit cycles. The Adjusted NFCI (\texttt{ANFCI}) removes the macroeconomic component to isolate the residual financial signal. These three indices---released weekly each Wednesday with a one-week lag for NFCI and STLFSI4---constitute the most academically-canonical set of financial-stress proxies and are obvious candidates for delivering incremental information beyond VIX during stress episodes. We test their composite against the VIX baseline under publication-lag-corrected alignment ($\text{shift}(2)$ for NFCI and ANFCI weekly-to-daily forward-fill; $\text{shift}(1)$ for STLFSI4); the alt-data-only composite (Model M4, which \emph{replaces} VIX) loses to VIX with $t=-3.00$ under the original convention and $t=-3.61$ under the corrected convention. As with EPU, the standalone stress composite underperforms VIX; and when the stress block is instead \emph{added} to VIX, the nested Clark-West increment is statistically zero (Section~\ref{sec:results})---the indices are redundant given VIX rather than a source of information that VIX omits.
...
\midrule
12 (EPU) & USEPUINDXD & Daily & $t+1$ business day & shift(1) (weekly grid) \\
12 (EPU) & WLEMUINDXD & Daily & $t+1$ business day & shift(1) (weekly grid) \\
13 (FinStress) & NFCI & Weekly (Fri obs) & $t+5$ calendar days & shift(2) (weekly grid) \\
13 (FinStress) & ANFCI & Weekly (Fri obs) & $t+5$ calendar days & shift(2) (weekly grid) \\
13 (FinStress) & STLFSI4 & Weekly (Fri obs) & $t+6$ calendar days & shift(2) (weekly grid) \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
\end{equation}
which collapses to the standard $\mathcal{F}_t = \{X_{j,t}\}_{j=1}^J$ only when $\delta_j = 1$ for all $j$. For Family 13 (the weekly financial-stress series, released five to six calendar days after their Friday reference date) we set $\delta = 2$ at weekly frequency; for Family 12 (the daily EPU series, which clear their release lag within one business day) we set $\delta = 1$. Equivalently, all primary results in this paper use \texttt{Signal.shift(1)} for the EPU inputs and \texttt{Signal.shift(2)} for the financial-stress inputs---the ``corrected'' timing whose sensitivity is examined in \S\ref{subsec:pub_delay_robust}.

\paragraph{Why this matters.} Our companion robustness experiment re-estimates every Family 12--13 specification under both the conventional shift(1) and the publication-delay-corrected shift(2). For SPY the null finding strengthens: the M4 financial-stress specification moves from DM $t=-3.00$ under shift(1) to $t=-3.61$ under shift(2), pushing it further past the Harvey threshold of $|t|>3.0$ in the wrong direction. The single \emph{apparent} cross-asset niche reported in an earlier draft---TLT volatility forecast with the financial-stress block, with shift(1) DM $t=+3.74$---collapses to $t=+1.96$ under shift(2), which fails the Harvey threshold and is no longer significant after Holm-Bonferroni correction. All other 14 of the 16 cells are stable across the two shift conventions. The convention adopted in the main text---the ``corrected'' timing that applies shift(1) to the daily EPU series and shift(2) to the weekly financial-stress series---is therefore the release-calendar-respecting one, and a reader applying the conventional uniform shift(1) would obtain a single weakly-significant cell whose collapse under shift(2) is informative on its own; we report both in Section~\ref{sec:results} for completeness.
...
\subsection{Nested-Model Correction: Clark-West and Small-Sample Adjustment}
\label{sec:clark_west}

A subset of our comparisons is \emph{nested}: the VIX-only forecast is the special case of the VIX-plus-signal forecast (Equation~\ref{eq:incremental}) obtained by restricting the signal coefficient to zero. \citet{clark2007} show that when nested models are compared by mean squared prediction error, the standard Diebold-Mariano statistic is biased \emph{against} the larger model, because under the null that the extra predictor is useless the additional estimated parameter injects pure estimation noise that inflates the larger model's out-of-sample loss. For a paper whose central claim is a \emph{null}---that no signal improves upon VIX---this bias is self-serving: it can make the augmented model look actively worse when it is merely redundant, and it makes the null appear stronger than the evidence warrants. We therefore report, for every nested comparison, the Clark-West MSPE-adjusted statistic based on
\begin{equation}
\hat{f}_t = \big(RV_t - \hat{RV}_t^{\text{VIX}}\big)^2 - \Big[\big(RV_t - \hat{RV}_t^{\text{VIX+sig}}\big)^2 - \big(\hat{RV}_t^{\text{VIX}} - \hat{RV}_t^{\text{VIX+sig}}\big)^2\Big],
\label{eq:clark_west}
\end{equation}
tested one-sided against $H_0: \mathbb{E}[\hat{f}_t] \le 0$ (``the signal does not improve on VIX''). The bracketed correction term $(\hat{RV}_t^{\text{VIX}} - \hat{RV}_t^{\text{VIX+sig}})^2$ removes the estimation-noise penalty, so a genuinely informative signal produces a large positive statistic. Because Clark-West is strictly more powerful than the standard test at detecting a true incremental predictor, a null that survives it is robust rather than an artifact of the standard test's conservativeness. We additionally apply the \citet{harvey1997} small-sample correction, multiplying each statistic by $\sqrt{(T+1-2h+T^{-1}h(h-1))/T}$ and referring it to the $t_{T-1}$ distribution; for the one-step weekly forecasts ($h=1$) this factor equals 0.997 and is immaterial. We note that comparisons in which the alt-data specification \emph{replaces} VIX rather than augmenting it (the pure alt-data models M3 and M4 of Section~\ref{sec:results}) are \emph{non-nested}, so the standard Diebold-Mariano test applies to them directly and the Clark-West adjustment is neither needed nor appropriate.

\subsection{Multiple-Testing Corrections}
\label{sec:multiple_testing}

When many predictors are tested, conventional $|t| > 1.96$ thresholds produce excessive false discoveries. \citet{harvey2016} address this problem in the context of asset pricing, presenting a framework in which the appropriate significance threshold depends on the number of tested strategies and the assumed prior probability of a true discovery. Their analysis---applied to roughly 300 factors in the cross-section literature---yields an approximate threshold of $|t| > 3.0$ under conservative priors. We adopt $|t| > 3.0$ as an approximate conservative threshold motivated by their analysis. Of our 13 signal families, 10 are evaluated via DM regression (families 1--4 and 8--13); families 5--6 are portfolio strategies tested via Sharpe-difference bootstrap and family 7 is tested via Granger causality, both reported separately in Section~\ref{sec:results}.

To provide formal multiple-testing control across these 10 primary regression tests, we implement the Holm-Bonferroni sequential procedure \citep{holm1979}. The procedure works as follows: (i) rank the 10 raw $p$-values from smallest ($p_{(1)}$) to largest ($p_{(10)}$); (ii) for rank $k$, reject $H_0$ if $p_{(k)} \leq \alpha / (10 - k + 1)$; (iii) stop at the first non-rejection. We report both raw and Holm-Bonferroni adjusted $p$-values for all tests. This procedure controls the family-wise error rate (FWER) at $\alpha = 0.05$ while being strictly more powerful than Bonferroni. As we show in Section~\ref{sec:results}, the Holm-Bonferroni correction does not change any conclusion---all 10 testable signal families fail both the raw and corrected tests under the directional ``signal beats VIX'' null---which strengthens the robustness of our null finding.
...
\item Families 1--11 use signals lagged one trading day. Families 12--13 follow the publication-delay convention of Section~\ref{sec:pub_delay} (\texttt{shift(1)} for the daily EPU series and \texttt{shift(2)} for the weekly financial-stress series, both on the weekly W-FRI frequency). Families 12--13 are evaluated on the AR(1)+VIX+signal specification on a 2018--2026 weekly sample; the partial-$r$ and IS $t$-stat columns are left blank because the 22-day-forward horizon used for the daily families does not align with the weekly evaluation horizon.
\item $^{\ddagger}$ Signal direction: signed DM $t$ is \emph{negative}, indicating that the VIX-only baseline forecasts \emph{better} than the standalone alt-data specification (M3/M4, which replace VIX). The directional one-sided null---``signal beats VIX''---is therefore not rejected (one-sided $p>0.99$). Under the standalone (non-nested) reading the two-sided rejection means the alt-data-only model forecasts worse than VIX; under the thesis-relevant nested reading (alt-data \emph{added} to VIX) the Clark-West increment is statistically zero (\S\ref{sec:clark_west}), so the correct interpretation is redundancy, not harm. We follow \citet{harvey2016} and treat these as null findings for the purpose of model selection.
\item $^{\dagger}$ Family 12 (EPU): combined USEPU+WLEMU specification (M3). $^{\dagger\dagger}$ Family 13 (Financial Stress): combined NFCI+ANFCI+STLFSI4 specification reported under the publication-delay-corrected shift(2). The corresponding shift(1) DM $|t|$ is 3.00, which strengthens to 3.61 under shift(2)---both significantly negative, reinforcing the null. The pooled M5 (EPU+FinStress jointly) yields DM $|t|=1.00$ (raw $p=0.319$, indistinguishable from VIX) under the same corrected timing.
...
Second, the Holm-Bonferroni correction reinforces the null. Among the ten testable DM tests, families 1--4 and 8--11 produce raw $p$-values of 0.147 or above and adjusted $p$-values of 1.000. Families 12--13 are two-sided significant in the raw tests (raw $p = 0.012$ and $p < 0.001$ respectively) but in the \emph{harmful} direction---their signed DM statistics are negative, meaning the alt-data augmentations significantly \emph{underperform} the VIX baseline (see footnote $\ddagger$)---so the one-sided ``beats VIX'' null is not rejected for any family. The multiplicity correction leaves no family significant in the beneficial direction, demonstrating that our conclusions are robust to formal multiple-testing control.

The negative signed statistics for the weekly alt-data families warrant a nested-model check, since a self-serving reading of a null result could rest on the standard Diebold-Mariano test's known conservativeness against a larger model (Section~\ref{sec:clark_west}). We distinguish two questions. The main-table entries for Families 12 and 13 evaluate the pure alt-data specifications (M3 and M4), which \emph{replace} VIX and are therefore \emph{non-nested} against the VIX baseline; there the standard test is valid and the alt-data-only forecasts genuinely lose to VIX. The thesis-relevant \emph{incremental} question---whether alt-data \emph{adds} to VIX---is a nested comparison (VIX+signal vs.\ VIX), for which we compute the Clark-West MSPE-adjusted statistic on the weekly panel (corrected shift convention, $n_{\text{OOS}} = 170$). The result confirms rather than overturns the null: adding EPU to VIX yields Clark-West $t = +0.12$ (one-sided $p = 0.45$), adding the financial-stress block yields $t = -0.56$ ($p = 0.71$), and the kitchen-sink VIX+all-alt-data specification yields $t = +0.14$ ($p = 0.45$)---none approaching the Harvey threshold, and all well short of conventional significance. Notably, the standard DM statistics on the same nested increments are significantly \emph{negative} ($t = -2.51$, $-2.82$, $-2.62$), but the Clark-West adjustment collapses them to essentially zero, revealing that the apparent underperformance is estimation noise from the extra parameters rather than active harm. The incremental content of alt-data beyond VIX is therefore not negative but simply \emph{absent}: VIX sufficiency holds even under the more powerful nested test, and the Harvey-Leybourne-Newbold small-sample correction (factor 0.997 at $h=1$) leaves these verdicts unchanged.

% source: daily nested Clark-West increments from experiments/k1116e (F2, F4) and experiments/k1116g (F1, F8, F11); fixed 2019--2026 OOS split, IS through 2018, n_OOS=1861, NW lag 21
The same nested-model logic extends to the daily signal families. Although the main-table Diebold-Mariano statistics for Families~1--11 already fall well below the Harvey threshold, several of these comparisons are nested---the augmented forecast $\text{RV} = a + b_1\text{VIX} + b_2\,\text{signal}_j$ collapses to the VIX-only benchmark under $b_2 = 0$ (Equation~\ref{eq:incremental})---so we verify that the daily null, like the weekly one, is not an artifact of the standard test's conservativeness against the larger model. For the five daily families whose signal admits a well-defined pinned-data nested specification (Family~1 cross-asset volatility momentum, Family~2 VIX term structure, Family~4 variance risk premium, Family~8 yield-curve slope, and Family~11 calendar anomaly), we compute the Clark-West MSPE-adjusted increment on a fixed 2019--2026 out-of-sample split ($n_{\text{OOS}} = 1{,}861$; in-sample estimation through 2018) with Newey-West lag 21 matched to the 22-day-forward horizon. The verdict is unanimous: every daily increment remains far short of significance---Family~2 is the largest at Clark-West $t = +1.69$ (one-sided $p = 0.045$), followed by Family~1 ($t = +0.69$), Family~8 ($t = +0.11$), Family~4 ($t = -0.22$), and Family~11 ($t = -1.50$)---and none approaches the Harvey threshold of $|t| > 3.0$. The more powerful nested test thus confirms VIX sufficiency for the daily families exactly as it does for the weekly alt-data block, and the Harvey-Leybourne-Newbold small-sample correction (factor 0.99 at the 22-day horizon) leaves each verdict unchanged. This fixed-split evaluation deliberately uses a different out-of-sample window than the expanding-window Diebold-Mariano statistics of Table~\ref{tab:main_results}; we report it separately rather than as a table column precisely so that the two window conventions are not conflated, but they agree in sign and non-significance for every family (a directional cross-check), confirming that the Clark-West robustness is not an artifact of the split choice. The three remaining daily families---Family~3 (behavioral put-call ratio), Family~9 (Google Trends fear), and Family~10 (overnight VIX change)---depend on external series (CBOE put-call volume, Google Trends queries, and the intraday VIX opening print) that are not yet pinned in the replication snapshot; their nested Clark-West increments are deferred to a data-provisioning follow-up and are expected, from their main-table Diebold-Mariano statistics ($|t| = 0.52$, $0.67$, and $1.12$, respectively), to be equally immaterial.
...
% source: experiments/k731/ canonical (errata_table3_bh_sharpe_canonical_fix.md 2026-05-30, applied 2026-06-10): BH 50/50 Sharpe 0.8268 over aligned 2008--2026 sample
--- & 12/VIX (SPY/GLD) & 0.870 & --- & $-32.2$ & 8.9$\times$ & --- \\
--- & Buy-and-hold 50/50 SPY/GLD & 0.827 & $-0.043$ & $-32.5$ & 0.0 & --- \\
\midrule
...
Under the canonical period-aligned benchmark (2008--2026 common sample), the 12/VIX strategy (Sharpe = 0.870) modestly outperforms the static 50/50 SPY/GLD allocation (Sharpe = 0.827, $\Delta = -0.043$), reversing the conclusion of an earlier draft whose buy-and-hold Sharpe (0.947) was computed over a non-aligned sample (see the 2026-05-30 erratum in the replication package). The margin is small and not statistically significant; the substantive conclusion is that no \emph{signal-augmented} strategy improves on the plain 12/VIX rule, while VT continues to deliver its drawdown-protection benefit relative to buy-and-hold.

Among signal-based strategies, the behavioral sentiment (fear hedge) variant comes closest to beating 12/VIX, with the largest Sharpe improvement (+0.030, Sharpe 0.900), but this is far from statistical significance (DM $|t| = 0.72$). The VIX term structure ``contango boost'' variant is the only other strategy with a positive improvement (+0.010, DM $|t| = 0.31$); all remaining strategies underperform the benchmark.
...
Table~\ref{tab:competing_eras} presents the \emph{in-sample} incremental $R^2$ of three representative competing signals across the five eras, obtained by adding each signal to a VIX-only regression of 22-day forward realized volatility within each era window. Unlike the out-of-sample horse race that constitutes our main results (Section~\ref{sec:results}), these are within-sample fits and therefore represent an upper bound on any signal's usable content.
...
Signal & Era 1 & Era 2 & Era 3 & Era 4 & Era 5 & Harvey Pass? \\
 & Dot-Com & Post-DC & GFC & QE & COVID & ($|t|>3.0$) \\
\midrule
Overnight VIX (abs) & 0.0002 & 0.0001 & \textbf{0.0039} & 0.0006 & 0.0032 & 1/5 \\
VRP proxy & 0.0005 & 0.0002 & \textbf{0.0160} & 0.0008 & 0.0005 & 1/5 \\
Vol momentum 20/60 & 0.0001 & 0.0004 & \textbf{0.0216} & 0.0001 & \textbf{0.0372} & 2/5 \\
\bottomrule
\end{tabular}
% source: experiments/k752/k752_vix_sufficiency_eras_results.json .part_d_competing_signals_by_era.{Era1_DotCom,Era2_PostDotCom,Era3_GFC,Era4_LowVol_QE,Era5_COVID_Inflation}.signals.{Overnight_VIX_Abs,VRP_Proxy,Vol_Momentum_20_60}.incremental_R2 ; Harvey pass = count of eras with |signal_t|>3.0 (GFC: overnight t=-3.15, VRP t=-6.51, vol-mom t=7.60; COVID: vol-mom t=9.30)
\begin{tablenotes}
\small
\item \textit{Notes}: In-sample incremental $R^2$ from adding each signal to a VIX-only regression of 22-day forward realized volatility, estimated within each era window. \textbf{Bold} cells cross the \citet{harvey2016} $|t|>3.0$ threshold. In the three calm eras (Dot-Com, Post-Dot-Com, QE) no signal is significant. Significance concentrates in the two crisis eras: all three signals add significant in-sample explanatory power during the GFC (2007--2012), and volatility momentum remains significant during COVID/Inflation (2020--2026), where it adds 3.7\% incremental $R^2$ ($t=9.30$). Source: experiment K752.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Asset & \texttt{base} & \texttt{epu} & \texttt{finstress} & \texttt{all} \\
\midrule
SPY     & $-3.021$ & $-2.603$ & $-3.001$           & $-2.537$ \\
GLD     & $-2.103$ & $-2.069$ & $-3.341$           & $-2.246$ \\
TLT     & $+1.433$ & $-2.477$ & $+3.743^{\dag}$    & $-5.666$ \\
BTC-USD & $-5.494$ & $-3.550$ & $+1.370$           & $+0.203$ \\
QQQ     & $-2.186$ & $-1.967$ & $-2.439$           & $-1.967$ \\
USO     & $-3.049$ & $-5.596$ & $-2.584$           & $-3.735$ \\
EEM     & $-2.596$ & $-3.539$ & $-1.434$           & $-0.999$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
Section~\ref{sec:pub_delay} introduced the publication-delay convention and adopted the ``corrected'' timing---\texttt{shift(1)} for the daily EPU series and \texttt{shift(2)} for the weekly financial-stress series---as the ex-ante specification for Families 12--13 in all main results. Because that decision rests on release calendars rather than on data, it is reasonable to ask whether the headline null would survive under the conventional uniform \texttt{shift(1)} that prior horse races in this literature have implicitly used, and whether a still-more-paranoid uniform \texttt{shift(2)} that also delays the daily EPU series would change the conclusions. A companion experiment addresses both questions by re-estimating the SPY weekly forecasting battery---and its cross-asset replication on GLD, TLT, and BTC---under three timing variants and comparing the resulting Diebold-Mariano statistics against the original specification.
...
\small
\begin{threeparttable}
\caption{Publication-Delay Robustness of DM-HLN Statistics}
...
\small
\begin{threeparttable}
\caption{Full-Sample Performance of Six SPY+GLD Allocation Rules}
...
\small
\begin{threeparttable}
\caption{Stationary-Bootstrap Sharpe-Difference Inference}
...
\small
\begin{threeparttable}
\caption{Stress-Period Average SPY Weight by Strategy}
...
This evidence therefore extends the universal-sufficiency claim from the forecasting paradigm to the allocation paradigm. Across nine bootstrapped Sharpe comparisons spanning two benchmarks and three single-signal alt-data series plus a hybrid, none of the alt-data strategies produces a statistically distinguishable improvement over either the 50/50 SPY+GLD moat or the vol-targeted scaler that uses no alt-data at all. The one cross-section in which an alt-data signal stands out is drawdown reduction, where NFCI ties the VIX-regime rule for the lowest MDD; the rest of the surface is statistically flat. Combined with the earlier forecasting nulls and the publication-delay robustness check, this allocation null closes the second of the two natural channels through which the academic literature has historically argued that EPU and financial-stress series should add value. Section~\ref{subsec:break_vix} accordingly removes regime-based allocation from the catalogue of plausible mechanisms that could break VIX sufficiency.
...
Two channels that prior literature has flagged as candidates for breaking VIX sufficiency are now explicitly ruled out by our evidence. The first is \textbf{alt-data macro-financial uncertainty proxies} (EPU, NFCI, financial-conditions indices, sentiment z-scores): one might conjecture that newspaper- or text-based uncertainty signals capture macro stress information that the VIX, anchored to S\&P~500 options, fails to internalize. Section~\ref{sec:criterion_dependent} and our forecasting horse race show that none of these proxies improves volatility forecasts beyond VIX, with Diebold--Mariano $p$-values failing to reject at any conventional level after Holm--Bonferroni correction. The forecasting channel is closed. The second is \textbf{regime-based volatility-timing allocation} (alt-data signals routed through portfolio weights rather than mean forecasts): one might further conjecture that even an uninformative \emph{forecast} signal could still add value if used as a \emph{regime indicator} for tilting allocations between risk-on and risk-off assets. Section~\ref{subsec:k1121_allocation} refutes this conjecture: across nine alt-data Sharpe comparisons against the 50/50 SPY/GLD baseline and a VIX-regime benchmark, no signal achieves statistical separation under a stationary-bootstrap test ($p \geq 0.10$ in every comparison), and the best alt-data signal (NFCI, Sharpe 1.312) is statistically indistinguishable from the static 50/50 benchmark (Sharpe 1.309, $p = 0.966$). The allocation channel is closed alongside the forecasting channel. The two surviving routes---higher-frequency realized volatility and genuinely exogenous shocks---do not involve alt-data text or macro proxies, and therefore do not constitute a counterexample to VIX sufficiency in the daily, closing-price information set.
...
We conclude with a broader observation. In financial econometrics, null results are underreported relative to their information content. Our systematic horse race, with its pre-specified signal families and stringent statistical controls, provides stronger evidence than any individual positive finding could. The 42 null results accumulated across this research program (the original 36 plus 6 from the alt-data forecasting and allocation extensions) are not 42 failures; they are 42 independent confirmations that the option market efficiently aggregates volatility-relevant information.
...
Harvey, D., Leybourne, S., \& Newbold, P. (1997). Testing the equality of prediction mean squared errors. \textit{International Journal of Forecasting}, 13(2), 281--291.
...
\bibitem[Huang et al.(2019)]{luo2019}
Huang, Z., Tong, C., \& Wang, T. (2019). VIX term structure and VIX futures pricing with realized volatility. \textit{Journal of Futures Markets}, 39(1), 72--93.
