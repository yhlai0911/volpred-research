...
- ✅ **superseded detector 三表移 appendix**（2026-07-14 18:xx 主線程，hourly-18）——見下方「2026-07-14 P0-2 appendix 收斂」條
...
- 2026-07-14 18:xx | **P0-2 appendix 收斂落地（主線程 hourly-18）**：DoD 雖已標 P0-2 ✅，但規格中「superseded detector 表移 appendix」那一項從未執行 —— 驗證：改動前 `main.tex` 全文 **無 `\appendix`**，三張 Sharpe-only detector 表（tab:cross_strategy_threshold / tab:scaling_window_matrix / tab:oat_robustness）仍坐在 Results 正文。本次補完：
  - 新增 `\appendix` §A「Legacy Sharpe-Only-Detector Results (Superseded)」，三表整批遷入，開頭明述為何 superseded（循環校準 + TF/MR 落在 gate 外）與「no claim in this paper rests on them」。
  - **§sec:oat 重寫而非刪除**：原本 robustness 論證掛在 legacy OAT 表（Sharpe-only detector），現改掛 redesign layer 的 5-cell 結果（tab:matched_control_vt，已驗證數字）—— VT 5/5 degrade、RR_VT 0/5；λ 單調驅動幅度（−0.421 / −0.781 / −0.899），γ 幾乎不動（−0.715 / −0.792）。Limitations 的「λ 是 binding driver」引用同步從 legacy 表改掛此處。
  - **⚠️ 對 brief 的一處刻意偏離（供後續 reviewer 覆核）**：Fable brief 要求 knife-edge § 一併移 appendix。我改為**留在正文但重新接地**——該節原本有問題是因為它拿 superseded detector 當證據，現已全部改述於 redesign layer，B2 的實質缺陷（「一邊宣告 detector 作廢、一邊拿它輸出當貢獻」）即已消除；而 QF referee 必問 knife-edge，把已合法接地的反駁埋進 appendix 只會弱化論文。若覆核者不同意，整節搬移成本很低。
  - 驗證：xelatex 34pp、0 error、0 undefined ref。
