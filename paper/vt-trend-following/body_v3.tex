...
\begin{abstract}
\noindent
Volatility targeting (VT) strategies are often characterized as repackaged trend following, since VT alpha is largely absorbed by a time-series momentum (TSMOM) factor. We show that this characterization conflates statistical alpha absorption with economic value. The primary benefit of VT is not alpha generation but drawdown insurance: across 22 assets spanning equities, commodities, bonds, and international markets, VT's maximum drawdown (MDD) protection is fully retained---and in most cases modestly enhanced---after TSMOM removal (5-asset K1192 canonical point estimates: SPY 103.7\%, 50/50 SPY/GLD 95.6\%, DIA 106.2\%, QQQ 109.0\%, IWM 102.2\%), while the Sharpe ratio channel is only partially attributable to trend following. Block bootstrap confidence intervals (10,000 replications, block size = 252 days) confirm that MDD retention is statistically robust, with 90\% CI lower bounds of 76--93\% and point estimates clustering at or above 100\% for all five assets tested. The mechanism is a VIX-level-based position-sizing channel distinct from return-direction momentum. Cross-sectionally, the GJR-GARCH leverage effect parameter ($\gamma$) predicts VT's TSMOM loading ($r = 0.564$, $p = 0.006$, $N = 22$), providing an empirical regularity consistent with the leverage effect channel driving VT--TSMOM overlap, though this cross-sectional link ($N = 22$) operates only across asset classes, not within equity sectors ($r = 0.163$, NS, $N = 11$). A split-sample robustness check---estimating $\gamma$ from 2007--2016 and TSMOM loading from 2017--2026---yields $r = 0.793$ ($p < 0.001$, K1193 canonical), \emph{strengthening} rather than attenuating the full-sample relationship. Internationally, US VIX serves as a broad de-risking signal: 13 of 13 equity markets achieve significant MDD improvement (K1178 canonical: average 24.9 percentage points, $t = 10.25$), with VIX sensitivity predicting the magnitude of protection ($r = -0.806$, $p < 0.001$, Spearman $\rho = -0.835$). A direct test confirms that five canonical trend-following strategies (SMA, Faber, Golden Cross, Dual Momentum, MA+VT) all fail the \citet{harvey2016} $t > 3.0$ threshold when applied to SPY, while VT's MDD protection remains robust across sub-periods. These results reframe VT as drawdown insurance that is not priced at a Sharpe drag---once TSMOM is hedged out, the drawdown protection is preserved or enhanced rather than eroded, overturning earlier interpretations of VT as repackaged trend following.
...
\midrule
Point estimate & 103.7 & 95.6 & 106.2 & 109.0 & 102.2 \\
Bootstrap median & 115.4 & 103.6 & 110.9 & 119.4 & 108.2 \\
90\% CI lower (5th pctl) & 93.0 & 76.0 & 82.3 & 89.0 & 87.4 \\
90\% CI upper (95th pctl) & 182.3 & 189.9 & 154.3 & 210.0 & 184.2 \\
\midrule
$H_0$: Retention $\leq 80\%$ & Reject & --- & --- & Reject & --- \\
$H_0$: Retention $\leq 100\%$ & Fail to reject & Fail to reject & Fail to reject & Fail to reject & Fail to reject \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Revised to K1192 canonical (2026-04-17, commit \texttt{d4dfd7bd}) from block bootstrap with 10,000 replications, block size 252 trading days, on full-sample returns. MDD retention is defined as the fraction of buy-and-hold-to-VT drawdown improvement preserved after TSMOM hedging (Eq.~\ref{eq:mdd_retention_boot}). Earlier reported point estimates and CIs (SPY: 93\% [86, 97]; 50/50: 96\% [90, 99]; DIA: 91\% [83, 96]; QQQ: 90\% [82, 95]; IWM: 97\% [91, 100]) implied partial attenuation of protection; the canonical K1192 results show point estimates at or above 100\% with right-skewed distributions whose medians all exceed 100\%. The directional interpretation is that TSMOM hedging preserves or modestly enhances MDD protection rather than attenuating it. $H_0$: Retention $\leq 80\%$ is rejected only where the 5th percentile exceeds 80 (SPY, QQQ). $H_0$: Retention $\leq 100\%$ cannot be rejected at the 5\% level for any asset---consistent with the hedge fully preserving, rather than eroding, the drawdown channel. 50/50 denotes the SPY/GLD equal-weight blend.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
This paper demonstrates that VIX-based de-risking embeds a mechanical trend-following overlay through the leverage effect, but this is not the whole story. VT possesses a dual mechanism: the Sharpe ratio channel operates partially through implicit asset-specific TSMOM exposure, while the maximum drawdown channel---VT's primary economic value---is largely independent of trend following. Across five US equity assets, K1192 canonical block bootstrap (10,000 replications) yields point estimates of MDD retention at or slightly above 100\% (95.6--109.0\%) with medians above 100\% for every asset, indicating that TSMOM removal preserves or modestly enhances the drawdown channel rather than attenuating it. The leverage effect parameter ($\gamma$) predicts VT's trend-following loading cross-sectionally ($r = 0.564$, $p = 0.006$, $N = 22$), an empirical regularity consistent with the leverage effect mechanism; a split-sample robustness check ($r = 0.793$, $p < 0.001$, K1193 canonical) shows this relationship not only survives but \emph{strengthens} under temporal separation of the estimation windows, driven by a 2017--2026 regime shift in which international and safe-haven loadings flipped positive. We note that this cross-sectional association ($N = 22$, mixed asset classes) does not establish a causal link.
