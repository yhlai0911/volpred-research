...
\midrule
% source (audit fix 2026-06-10): K1192 columns = experiments/k1192/k1192_results.json assets.<a>.bootstrap_results.def_a_retention_fraction {lo, median}; K1417 columns = experiments/k1417/k1417_results.json assets.<a>.by_mean_block.{756,1260}.def_a_retention_fraction {lo, median}. Earlier-draft baseline (86/97 etc.) was the non-reproducible v2 Table 6 reference; earlier 50th columns mistakenly carried the 95th-percentile values.
...
\begin{tablenotes}
\small
\item \textit{Notes:} Comparison of K1192 fixed-block (252-day) MDD retention CI 5th and 50th percentiles against K1417 stationary-bootstrap re-checks using expected block lengths of 756 and 1260 trading days (approximately 3 and 5 calendar years). The K1417 stationary bootstrap is a longer-memory robustness check that allows block dependence to extend across multiple business cycles, addressing whether retention inference is sensitive to the 252-day block assumption. At the 1260-day specification, 5th-percentile lower-bound shifts relative to the K1192 canonical baseline are positive for all five assets (SPY $+4.7$, 50/50 $+13.8$, DIA $+11.1$, QQQ $+8.5$, IWM $+12.6$, in percentage points): longer-memory resampling \emph{tightens} the lower bounds upward. Median retention estimates move modestly downward (e.g., SPY $115.4 \to 103.7$), so the stationary bootstrap concentrates the retention distribution around 100\% rather than shifting it up wholesale; the economically relevant non-erosion conclusion---lower bounds at or above $\sim$90--100\%---strengthens. The K1417 task verdict (``H2 NOT SUPPORTED'') refers to the narrow hypothesis that longer-block resampling would \emph{materially weaken} the lower-bound conclusions in the direction of erosion; the comparison shows the opposite. Data: \texttt{experiments/k1192/k1192\_results.json} (canonical baseline) and \texttt{experiments/k1417/k1417\_results.json}; replication: \texttt{python experiments/k1417/k1417.py} (stationary bootstrap, seed = 42, 10{,}000 replications).
\end{tablenotes}
\end{threeparttable}
\end{table}
