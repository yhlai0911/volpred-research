...
We employ three asset samples with varying start dates determined by data availability, summarized in Table~\ref{tab:sample_periods}.

The \textit{primary sample} comprises 22 assets across equities (SPY, QQQ, IWM, XLF, XLE, DIA, EEM, EFA, FXI, EWZ, EWJ, EWG, EWU, EWA, INDA), commodities (GLD, SLV, USO), bonds (TLT, LQD, HYG), and real estate (VNQ). Cross-sectional analyses that require all 22 assets simultaneously (Tables~\ref{tab:alpha_decomp} and~\ref{tab:cross_section}) use the common start date of January 2007, when the latest-listing ETFs in the sample become available. Single-asset and two-asset analyses---including the dual mechanism decomposition (Table~\ref{tab:dual_mechanism}), block bootstrap inference (Table~\ref{tab:mdd_bootstrap}), and factor model regressions (Table~\ref{tab:ff5})---extend back to January 2005 for SPY, GLD, and the 50/50 SPY/GLD blend, exploiting the longer data availability to maximize statistical power.

The \textit{international sample} consists of 13 country-level equity ETFs: 7~developed markets (EFA, EWJ, EWG, EWU, EWA, EWC, VGK) and 6~emerging markets (EEM, FXI, EWZ, INDA, EWT, MCHI), with a common start date of January 2007. The \textit{sector sample} comprises 11 SPDR sector ETFs (XLB, XLE, XLF, XLI, XLK, XLP, XLU, XLV, XLY, XLRE, XLC) with data from December 1998 to March 2026, providing the longest time series for within-equity robustness tests. All daily return data are sourced from Yahoo Finance.

\begin{table}[H]
\centering
\small
\caption{Sample Periods by Analysis}
\label{tab:sample_periods}
\begin{tabular}{@{}llll@{}}
\toprule
Sample / Analysis & Assets & Period & Rationale \\
\midrule
Cross-sectional (22 assets) & Full primary sample & Jan 2007--Mar 2026 & Common start of all ETFs \\
Single-/two-asset decomposition & SPY, GLD, 50/50, DIA, QQQ, IWM & Jan 2005--Mar 2026 & Maximizes power for key tests \\
Factor models (FF5 + BAB) & SPY & Jan 2005--Mar 2026$^{\dagger}$ & BAB proxy from May 2011 \\
International (13 markets) & Country ETFs & Jan 2007--Mar 2026 & Common start of all ETFs \\
Sector (11 sectors) & SPDR sector ETFs & Dec 1998--Mar 2026 & Longest available history \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item $^{\dagger}$M5 (with BAB) uses the post-May 2011 subsample when SPLV/SPHB become available.
\end{tablenotes}
\end{table}

VIX data are obtained from the CBOE (available from January 1990, predating all ETF samples). The risk-free rate is the daily 13-week Treasury bill rate (IRX), time-varying throughout the sample. Fama--French five-factor data (MKT, SMB, HML, RMW, CMA), the momentum factor (MOM), and the betting-against-beta factor (BAB) are obtained from Kenneth French's data library and AQR, respectively.
...
To provide statistical inference for MDD retention---a single extreme-value statistic with high sampling variability---we employ a block bootstrap procedure. For each of the five assets with MDD decomposition results (SPY, 50/50 SPY/GLD, DIA, QQQ, IWM), we:

\begin{enumerate}
\item Draw 10,000 block bootstrap samples from the daily return series, using a block size of 252 trading days (one year) to preserve the autocorrelation structure relevant for drawdown dynamics.
...
This block bootstrap approach accounts for the serial dependence in daily returns and the path-dependent nature of maximum drawdown, providing robust inference for an extreme-value statistic that standard asymptotic theory does not easily cover.
...
\begin{tablenotes}
\small
\item \textit{Notes:} This table reports alpha decomposition regressions for 22 assets over January 2007--March 2026. $\gamma$ is the GJR-GARCH leverage effect parameter. M1 regresses VT excess returns on MKT; M2 adds orthogonalized TSMOM$^{\perp}$ (252-day lookback). $\alpha$ is annualized (\%). $t_{\text{NW}}$ denotes Newey--West $t$-statistics with automatic lag selection. $\Delta\alpha = (\alpha_{\text{M1}} - \alpha_{\text{M2}})/\alpha_{\text{M1}} \times 100\%$, i.e., the percentage of M1 alpha absorbed by TSMOM (reported only when M1 alpha is positive; ``--'' indicates negative M1 alpha where percentage reduction is undefined). Note that $\Delta\alpha$ reflects compositional reallocation within the factor model and does not directly correspond to the TSMOM contribution to total strategy Sharpe, which averages 1.4\% across 22 assets (ranging from 7.1\% for SPY to $-5.0\%$ for EFA). $^{***}$, $^{**}$, $^{*}$ denote significance at the 1\%, 5\%, and 10\% levels, respectively.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
GJR $\gamma$ vs.\ $\beta_{\text{TSMOM}}^{\perp}$ & 0.487 & 0.021 & 0.461 & 0.031 \\
\multicolumn{5}{@{}l}{Bootstrap 95\% CI for Pearson $r$: $[0.114, 0.737]$} \\
\midrule
\multicolumn{5}{@{}l}{\textit{Cross-sectional regression (full sample): $\beta_{\text{TSMOM},i}^{\perp} = \gamma_0 + \gamma_1 \cdot \text{GJR}_{\gamma,i} + \varepsilon_i$}} \\
...
To confirm that this MDD preservation is not SPY-specific, we repeat the decomposition for DIA, QQQ, and IWM. The MDD retention rates are 91\% (DIA), 90\% (QQQ), and 97\% (IWM), spanning a range of 90--97\% across all five assets tested. This consistency confirms that VT's drawdown protection operates through the VIX-level channel regardless of the specific equity asset. The TSMOM contribution to the total strategy Sharpe ratio varies across assets: it is 7.1\% for SPY but turns negative for some emerging-market assets (e.g., EEM: $-0.6\%$, EFA: $-5.0\%$), yielding a cross-asset average of approximately 1.4\%. This heterogeneity indicates that the alpha reduction percentages in Table~\ref{tab:alpha_decomp} reflect compositional reallocation within the factor model rather than a uniform trend-following premium.

\paragraph{Bootstrap inference for MDD retention.} Because maximum drawdown is a single extreme-value statistic with high sampling variability, we provide formal statistical inference via block bootstrap (10,000 replications, block size = 252 days). Table~\ref{tab:mdd_bootstrap} reports the results. The point estimates of MDD retention range from 90\% (QQQ) to 97\% (IWM), with 90\% confidence intervals of [82\%, 95\%] (QQQ) to [91\%, 100\%] (IWM). Critically, all five lower confidence bounds exceed 80\%, confirming that the high MDD retention is not an artifact of sampling variability. Even under conservative inference, at least four-fifths of VT's drawdown protection survives TSMOM removal for every asset tested.

\begin{table}[H]
\centering
\begin{threeparttable}
\caption{Block Bootstrap Confidence Intervals for MDD Retention (\%)}
...
90\% CI lower (5th pctl) & 86 & 90 & 83 & 82 & 91 \\
90\% CI upper (95th pctl) & 97 & 99 & 96 & 95 & 100 \\
\midrule
$H_0$: Retention $\leq 80\%$ & Reject & Reject & Reject & Reject & Reject \\
Bootstrap $p$-value & $< 0.01$ & $< 0.01$ & $< 0.01$ & $< 0.01$ & $< 0.01$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Block bootstrap with 10,000 replications and block size of 252 trading days (one year). MDD retention is defined as the fraction of buy-and-hold-to-VT drawdown improvement that survives TSMOM hedging (Eq.~\ref{eq:mdd_retention_boot}). The one-sided test rejects $H_0$: Retention $\leq 80\%$ at the 1\% level for all five assets. 50/50 denotes the SPY/GLD equal-weight blend.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
This paper demonstrates that VIX-based de-risking embeds a mechanical trend-following overlay through the leverage effect, but this is not the whole story. VT possesses a dual mechanism: the Sharpe ratio channel operates partially through implicit asset-specific TSMOM exposure, while the maximum drawdown channel---VT's primary economic value---is largely independent of trend following (90--97\% retained across five US equity assets after TSMOM removal, with block bootstrap 90\% confidence intervals excluding retention below 80\% for all five assets; though generalization to a broader asset universe remains an open question). The leverage effect parameter ($\gamma$) predicts VT's trend-following loading cross-sectionally ($r = 0.564$, $p = 0.006$, $N = 22$), an empirical regularity consistent with the leverage effect mechanism; a split-sample robustness check ($r = 0.487$, $p = 0.021$) confirms this relationship survives temporal separation of the estimation windows, though we note that this cross-sectional association ($N = 22$, mixed asset classes) does not establish a causal link.
