...
\begin{abstract}
\noindent
Volatility targeting (VT) strategies are often characterized as repackaged trend following, since VT alpha is largely absorbed by a time-series momentum (TSMOM) factor. We show that this characterization conflates statistical alpha absorption with economic value. The primary benefit of VT is not alpha generation but drawdown insurance: across 22 assets spanning equities, commodities, bonds, and international markets, VT's maximum drawdown (MDD) protection is preserved after TSMOM removal (5-asset canonical point estimates: SPY 103.7\%, 50/50 SPY/GLD 95.6\%, DIA 106.2\%, QQQ 109.0\%, IWM 102.2\%), while the Sharpe ratio channel is only partially attributable to trend following. A 252-day moving-block bootstrap delivers 90\% CI lower bounds of 76--93\% across the five-asset canonical table; a stationary-bootstrap robustness check with 3--5 year expected block lengths yields lower bounds of 90--100\%, \emph{strengthening} the non-erosion conclusion under longer-memory resampling. Point estimates above 100\% should be interpreted cautiously: they indicate that hedging does not damage the drawdown channel, but part of the incremental improvement can arise mechanically when the hedge trims exposure during momentum-crash rebound windows. The mechanism is therefore best described as VIX-level-based position sizing that remains distinct from return-direction momentum in net economic effect. Cross-sectionally, the GJR-GARCH leverage effect parameter ($\gamma$) predicts VT's TSMOM loading ($r = 0.564$, $p = 0.006$, $N = 22$), providing an empirical regularity consistent with the leverage effect channel driving VT--TSMOM overlap, though this cross-sectional link ($N = 22$) operates only across asset classes, not within equity sectors ($r = 0.163$, NS, $N = 11$). A split-sample robustness check---estimating $\gamma$ from 2007--2016 and TSMOM loading from 2017--2026---yields $r = 0.793$ ($p < 0.001$, split-sample canonical), though part of this strengthening likely reflects asset-class composition and safe-haven loading shifts in the later subsample rather than cleaner causal identification. Internationally, a 12/VIX overlay reduces raw maximum drawdown in all 13 country equity ETFs studied (inception-aware average 27.5 percentage points; common-sample average 12.6 pp, joint stationary-bootstrap 90\% CI $[4.2, 19.3]$ pp), but an exposure-matched decomposition shows that this headline measures genuine risk reduction through de-levering, not volatility timing: the overlay realizes only 0.52--0.68 times buy-and-hold volatility, a constant-weight benchmark with the same average exposure and no VIX input reproduces most of the raw improvement, and the exposure-matched drawdown gap does not reject a circular-shift no-timing null in either sample (common-sample $-0.9$ pp, one-sided $p = 0.56$; inception-aware $+5.0$ pp, $p = 0.21$; 0 of 13 markets survive Holm correction). A direct test confirms that five canonical trend-following strategies (SMA, Faber, Golden Cross, Dual Momentum, MA+VT) all fail the \citet{harvey2016} $t > 3.0$ threshold when applied to SPY, while VT's MDD protection remains robust across sub-periods. These results reframe VT as drawdown insurance whose net protection survives TSMOM removal, overturning earlier interpretations of VT as repackaged trend following.
...
We argue the answer is no. While VT and TSMOM share a common channel through the leverage effect, VT possesses a distinct economic mechanism that trend following does not replicate. Specifically, we decompose VT's benefits into two channels: (i)~a \textit{Sharpe ratio channel} that operates partially through implicit TSMOM exposure, and (ii)~a \textit{maximum drawdown (MDD) channel} that operates through VIX-level-based position sizing and is largely orthogonal to trend following in net outcome. This dual-mechanism framework resolves the tension between the statistical finding that TSMOM absorbs VT alpha and the practical observation that VT provides economically meaningful drawdown insurance even when alpha is not statistically significant. Our results also address the \citet{cederburg2020} critique: even if VT does not generate significant alpha, the preservation of MDD protection after TSMOM removal (canonical point estimates 95.6--109.0\% across five US equity assets, with block bootstrap 90\% CI lower bounds at 76--93\%) constitutes an economically distinct benefit that utility-based tests may underweight, especially for investors who value drawdown protection in the spirit of habit or surplus-consumption preferences~\citep{campbell1999}.

Our paper makes three contributions. First, we show that the leverage effect parameter from GJR-GARCH estimation ($\gamma$) predicts VT's TSMOM loading cross-sectionally across 22 assets ($r = 0.564$, $p = 0.006$), an empirical regularity consistent with the leverage effect channel through which VIX-based de-risking mimics trend following. We emphasize that this cross-sectional association ($N = 22$, mixed asset classes) establishes a statistical link rather than a causal mechanism. A potential endogeneity concern arises because both $\gamma$ and the TSMOM loading are estimated from the same return series: a split-sample robustness check (estimating $\gamma$ from 2007--2016 and TSMOM loading from 2017--2026) yields $r = 0.793$ ($p < 0.001$, split-sample canonical), but we interpret the increase conservatively because it partly reflects a 2017--2026 regime shift in which international and safe-haven loadings flipped from near-zero to positive. This link holds across asset classes (equities vs.\ commodities vs.\ bonds) but breaks down within equity sectors ($r = 0.163$, NS, $N = 11$), where gamma variation is insufficient to differentiate TSMOM exposure. This boundary condition is new to the literature.

Second, we decompose VT's benefits into Sharpe and MDD components using TSMOM-hedged portfolio construction. After removing TSMOM exposure, MDD protection is preserved across five assets (canonical point estimates: SPY 103.7\%, 50/50 SPY/GLD 95.6\%, DIA 106.2\%, QQQ 109.0\%, IWM 102.2\%), while the Sharpe ratio improvement is only modestly reduced. Block bootstrap inference (10,000 replications, block size = 252 days) confirms that MDD retention is statistically robust: the 90\% confidence intervals (fraction definition) are [93\%, 182\%] for SPY, [76\%, 190\%] for 50/50, [82\%, 154\%] for DIA, [89\%, 210\%] for QQQ, and [87\%, 184\%] for IWM, with lower bounds at 76--93\%. A stationary-bootstrap re-check with longer expected block lengths leaves this non-erosion conclusion unchanged. We treat $>100\%$ estimates as evidence that hedging does not damage the drawdown channel, not as proof of a separate dominant mechanism, because some enhancement can arise when the hedge reduces exposure during rebound phases of momentum crashes~\citep{daniel2016}. The TSMOM contribution to Sharpe improvement is small: the originally reported 1.4\% figure could not be reproduced, and we therefore use the back-calculated 5.3\% of total strategy Sharpe as the canonical value. This demonstrates that VT's primary economic value---drawdown insurance---operates through a channel distinct from trend following.

Third, we document broad international evidence on US VIX as a de-risking signal---and then show why its headline number cannot be read as drawdown protection. Across 13 international equity ETFs (7 developed, 6 emerging), a simple 12/VIX overlay computed on a pinned, cost-inclusive monthly protocol reduces raw maximum drawdown in all 13 markets (inception-aware average 27.5 percentage points; common-sample average 12.6 pp, joint stationary-bootstrap 90\% CI $[4.2, 19.3]$ pp), while only 1 of 13 shows a Sharpe improvement. However, the overlay realizes only 0.52--0.66 times buy-and-hold volatility on the inception-aware sample (all 13 markets exceed a 20\% exposure-mismatch threshold), and a constant-weight benchmark holding each market's \emph{average} 12/VIX weight---with no VIX information at all---reproduces most of the raw headline (common-sample $+10.7$ of $+12.6$ pp, 13/13 positive) while earning an exposure-matched gap of essentially zero. Once each buy-and-hold benchmark is scaled to the overlay's realized volatility, the exposure-matched drawdown gap is small and does not reject a circular-shift no-timing null in either sample (common-sample average $-0.9$ pp, one-sided $p = 0.56$; inception-aware $+5.0$ pp, $p = 0.21$; 0 of 13 markets survive Holm correction). The VIX signal itself is genuine: the realized weight path attains the lowest volatility among all 231 calendar phases of itself, so 12/VIX does de-lever into turbulent months. But its economic content is conditional risk reduction, not drawdown protection at constant risk---any leverage-adjusted investor could replicate the raw MDD improvement by scaling down exposure. This decomposition matters beyond our setting: raw MDD comparisons between strategies with materially different realized exposures systematically overstate timing skill, and the exposure-matched protocol applied here provides a portable correction.

Our paper contributes to several literatures. We extend \citet{hood2025} by decomposing the dual channel (Sharpe vs.\ MDD), which their framework does not distinguish. We clarify that while VT embeds asset-specific TSMOM exposure through the leverage effect, this is distinct from diversified managed-futures strategies that trade across dozens of uncorrelated markets~\citep{moskowitz2012, baltas2013}. Our finding that VT's embedded momentum exposure operates through the leverage effect channel complements \citet{daniel2016}, who show that momentum crashes are driven by precisely this leverage-effect-induced volatility clustering. We also revisit the ``volatility-as-insurance'' perspective of \citet{lai2026a} in a global setting---finding that the international version of the payout is primarily de-levering rather than same-risk protection---building on the foundational work of \citet{fleming2001}, who established the economic value of volatility timing in a portfolio context.
...
Following \citet{moskowitz2012}, we construct a time-series momentum factor using the sign of the past 252-day (12-month) return measured through day $t-1$:
\begin{equation}
\text{TSMOM}_t = \text{sign}(r_{t-253:t-1}) \times r_t,
\label{eq:tsmom}
\end{equation}
where $r_{t-253:t-1}$ is the cumulative return over the prior 252 trading days. This notation matches the implementation's explicit lagging of the signal before applying period-$t$ returns.
...
This moving-block bootstrap approach accounts for the serial dependence in daily returns and the path-dependent nature of maximum drawdown, providing baseline inference for an extreme-value statistic that standard asymptotic theory does not easily cover. Because drawdown dependence can extend beyond a single calendar year, we also check the main inference with a stationary bootstrap using multi-year expected block lengths ; that longer-memory resampling does not materially weaken the qualitative conclusion that the drawdown channel survives TSMOM hedging.
...
\begin{tablenotes}
\small
\item \textit{Notes:} This table reports alpha decomposition regressions for 22 assets over January 2005--March 2026. $\gamma$ is the GJR-GARCH leverage effect parameter. M1 regresses VT excess returns on MKT; M2 adds orthogonalized TSMOM$^{\perp}$ (252-day lookback). $\alpha$ is annualized (\%). $t_{\text{NW}}$ denotes Newey--West $t$-statistics with automatic lag selection. $\Delta\alpha = (\alpha_{\text{M1}} - \alpha_{\text{M2}})/\alpha_{\text{M1}} \times 100\%$, i.e., the percentage of M1 alpha absorbed by TSMOM (reported only when M1 alpha is positive; ``--'' indicates negative M1 alpha where percentage reduction is undefined). Note that $\Delta\alpha$ reflects compositional reallocation within the factor model and does not directly correspond to the TSMOM contribution to total strategy Sharpe, which is approximately 5.3\% ($\alpha_{\text{absorbed}}/\text{Sharpe}_{\text{total}} = 0.043/0.805$). $^{***}$, $^{**}$, $^{*}$ denote significance at the 1\%, 5\%, and 10\% levels, respectively.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
% source: (experiments/k1457/k1457_results.json), classical OLS SE, HC3 reported in parentheses; M0 reproduces body baseline
\multicolumn{5}{@{}l}{M0 ($\gamma$ only): $\hat{\gamma}_0 = 0.001$ ($t = 0.03$), $\hat{\gamma}_1 = 0.568$ ($t = 3.06$), $R^2 = 0.319$} \\
% source: — dummy-only OLS, classical SE; HC3 t=-1.84 reported alongside
\multicolumn{5}{@{}l}{M1a (dummy only): $\hat{\gamma}_2 = -0.075$ ($t = -2.27$, HC3 $t = -1.84$), $R^2 = 0.205$} \\
% source: — joint OLS β = γ0 + γ1·γ + γ2·D_nonEq + ε; γ subsumes asset-class effect
...
To confirm that this MDD preservation is not SPY-specific, we repeat the decomposition for DIA, QQQ, and IWM. The canonical MDD retention rates are 106.2\% (DIA), 109.0\% (QQQ), and 102.2\% (IWM)---with point estimates clustering at or slightly above 100\% across all five assets tested. This consistency confirms that VT's drawdown protection operates through the VIX-level channel regardless of the specific equity asset. We again avoid reading the $>100\%$ values as evidence that TSMOM hedging creates a superior standalone insurance strategy, because those values can also arise mechanically when the hedge reduces exposure around rebound phases inside momentum-crash episodes. A trough-window decomposition across the two major crisis bottoms in the five-asset canonical set (10 asset-trough observations) is consistent with this caution: around the March 2020 trough, the TSMOM hedge contributes positively on negative-signal days (median $+30.0$ percentage points), whereas at the March 2009 trough three of five assets show zero hedge contribution and the remaining two are small and of mixed sign (50/50 SPY/GLD $+2.1$ pp, QQQ $-3.5$ pp). The mechanical hedge channel is therefore not universal across crisis episodes, but neither can it be declared entirely absent in 2009; we read this trough-window arithmetic as descriptive evidence rather than a direct PureVT-versus-buy-and-hold drawdown-path attribution. The TSMOM contribution to the total strategy Sharpe ratio is correspondingly small---the originally reported 1.4\% figure could not be reproduced, and we therefore use the back-calculated 5.3\% of full VT Sharpe as canonical (see forensic note in Section~\ref{sec:discussion})---indicating that the alpha reduction percentages in Table~\ref{tab:alpha_decomp} reflect compositional reallocation within the factor model rather than economically large value transfer.

\paragraph{Bootstrap inference for MDD retention.} Because maximum drawdown is a single extreme-value statistic with high sampling variability, we provide formal statistical inference via block bootstrap (10,000 replications, block size = 252 days) and verify the sign of the inference under a stationary-bootstrap robustness check with multi-year expected block lengths. Table~\ref{tab:mdd_bootstrap} reports the moving-block results for all 22 assets plus the 50/50 composite using the retention-fraction definition (Eq.~\ref{eq:mdd_retention_boot}). Point estimates are at or above 100\% for 20 of 23 rows (range: 83.8\% for LQD to 223.4\% for XLE). The 90\% confidence intervals are asymmetric---all distributions are right-skewed---and 22 of 23 assets have CI lower bounds above zero (SLV: $-$2.5\%, marginally negative). Eight assets reject $H_0$: Retention $\leq 80\%$ at the 5th percentile (SPY, QQQ, DIA, IWM, EFA, INDA, FXI, VNQ). No asset rejects $H_0$: Retention $\leq 100\%$, consistent with TSMOM hedging preserving rather than eroding the drawdown channel across asset classes. The stationary-bootstrap re-check does not reverse any of the five canonical non-erosion conclusions (see Table~\ref{tab:k1417_stationary_compare}), so the main claim does not depend on the 252-day resampling choice.

% source: (experiments/k1376/k1376_results.json), block-bootstrap B=10000 block=252 seed=42, 90% CI
\begin{table}[H]
\centering
\begin{threeparttable}
...
\centering
\begin{threeparttable}
\caption{Stationary-Bootstrap Robustness Check: MDD Retention CI Comparison Against the Canonical Five-Asset Set}
\label{tab:k1417_stationary_compare}
\begin{tabular}{lcccccc}
\toprule
& \multicolumn{2}{c}{fixed-block 252d} & \multicolumn{2}{c}{stationary 756d} & \multicolumn{2}{c}{stationary 1260d} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
Asset & 5th\,(\%) & 50th\,(\%) & 5th\,(\%) & 50th\,(\%) & 5th\,(\%) & 50th\,(\%) \\
\midrule
% source (audit fix 2026-06-10): columns = experiments/k1192/k1192_results.json assets.<a>.bootstrap_results.def_a_retention_fraction {lo, median}; columns = experiments/k1417/k1417_results.json assets.<a>.by_mean_block.{756,1260}.def_a_retention_fraction {lo, median}. Earlier-draft baseline (86/97 etc.) was the non-reproducible v2 Table 6 reference; earlier 50th columns mistakenly carried the 95th-percentile values.
...
\midrule
\multicolumn{6}{l}{\textit{5th-percentile lower-bound shift (1260-day stationary vs.\ 252-day fixed):}} & \\
\multicolumn{6}{l}{\quad Mean shift across five assets} & $+10.1$\,pp \\ % recomputed 2026-06-10 from canonical columns above: (4.7+13.8+11.1+8.5+12.6)/5
\multicolumn{6}{l}{\quad Median shift across five assets} & $+11.1$\,pp \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Comparison of fixed-block (252-day) MDD retention CI 5th and 50th percentiles against stationary-bootstrap re-checks using expected block lengths of 756 and 1260 trading days (approximately 3 and 5 calendar years). The stationary bootstrap is a longer-memory robustness check that allows block dependence to extend across multiple business cycles, addressing whether retention inference is sensitive to the 252-day block assumption. At the 1260-day specification, 5th-percentile lower-bound shifts relative to the canonical baseline are positive for all five assets (SPY $+4.7$, 50/50 $+13.8$, DIA $+11.1$, QQQ $+8.5$, IWM $+12.6$, in percentage points): longer-memory resampling \emph{tightens} the lower bounds upward. Median retention estimates move modestly downward (e.g., SPY $115.4 \to 103.7$), so the stationary bootstrap concentrates the retention distribution around 100\% rather than shifting it up wholesale; the economically relevant non-erosion conclusion---lower bounds at or above $\sim$90--100\%---strengthens. The stationary-bootstrap task verdict (``H2 NOT SUPPORTED'') refers to the narrow hypothesis that longer-block resampling would \emph{materially weaken} the lower-bound conclusions in the direction of erosion; the comparison shows the opposite. Data: \texttt{experiments/k1192/k1192\_results.json} (canonical baseline) and \texttt{experiments/k1417/k1417\_results.json}; replication: \texttt{python experiments/k1417/k1417.py} (stationary bootstrap, seed = 42, 10{,}000 replications).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\midrule
% source (audit fix 2026-06-10): 50/50 MDD column updated to the canonical (experiments/k1192/k1192_results.json assets."50/50".bootstrap_results.point_estimates: bh -32.49, vt -16.84, hedged -17.53); earlier draft carried -12.4/-13.1 from a superseded vintage. 50/50 Calmar cells retain the earlier vintage pending the reproduce-gate recomputation batch (annualized-return series not archived in) and are marked (v).
Buy \& Hold & 0.611 & $-$55.2 & 0.214 & 0.865 & $-$32.5 & 0.364\textsuperscript{(v)} \\
12/VIX VT & 0.797 & $-$26.3 & 0.283 & 0.982 & $-$16.8 & 0.624\textsuperscript{(v)} \\
TSMOM-Hedged VT & 0.737 & $-$25.3 & 0.281 & 0.937 & $-$17.5 & 0.501\textsuperscript{(v)} \\
Pure TSMOM & 0.172 & $-$27.5 & 0.092 & 0.232 & $-$31.8 & 0.075\textsuperscript{(v)} \\
\midrule
...
MDD protection (VT) & \multicolumn{3}{c}{28.9 pp} & \multicolumn{3}{c}{15.7 pp} \\
MDD protection retained & \multicolumn{3}{c}{29.9 pp (103.7\%)} & \multicolumn{3}{c}{15.0 pp (95.6\%)} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
Figure~\ref{fig:return_decomp} visualizes this dual-channel decomposition. Panel~(a) shows the Sharpe ratio improvement from VT over buy-and-hold, decomposed into the TSMOM-attributable portion (32--39\%) and the VIX-level residual (61--68\%). Panel~(b) shows the MDD protection channel, where 95.6--109.0\% of the total drawdown reduction survives TSMOM removal across the five assets in the the canonical replication (values above 100\% indicate that the hedged strategy's drawdown was \emph{shallower} than unhedged VT's, so the TSMOM component is slightly negative in net drawdown terms). The stark asymmetry between the two panels---moderate TSMOM contribution to Sharpe but at-or-below-zero contribution to MDD protection---confirms that VT's primary economic value operates through a channel distinct from trend following.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig1_return_decomposition.pdf}
\caption{VT Return Decomposition: Sharpe Ratio vs.\ Maximum Drawdown Channels. Panel~(a) decomposes the Sharpe ratio improvement (VT minus buy-and-hold) into the TSMOM-attributable portion and the VIX-level residual for SPY and 50/50 SPY/GLD. Panel~(b) decomposes the maximum drawdown protection (percentage points) into TSMOM-attributable and VIX-level components for five assets using the the canonical point estimates. Percentages indicate the share attributable to each channel. Data from Table~\ref{tab:dual_mechanism}, and Section~3.3. The key finding is the asymmetry: TSMOM explains 32--39\% of Sharpe improvement but does not contribute positively to net MDD protection (retention 95.6--109.0\% after TSMOM removal, with negative TSMOM shares when retention exceeds 100\%).}
...
\subsection{International Evidence: Raw Drawdown Reduction Is De-Levering, Not Timing}
\label{sec:international}

Table~\ref{tab:international} extends our analysis to 13 international equity markets, using a pinned-data, monthly-rebalanced, cost-inclusive protocol (experiment K1695; see table notes). The raw results are striking: all 13 markets see a positive MDD improvement under the 12/VIX overlay, with a common-sample average reduction of 12.6 percentage points (joint stationary-bootstrap 90\% CI $[4.2, 19.3]$ pp on $N = 3{,}557$ shared trading dates, $\Pr(\Delta\text{MDD} \le 0) = 0.0006$) and a larger inception-aware average of 27.5 pp (the common sample excludes the 2008--2009 crisis for most markets). Only 1 of 13 markets shows a Sharpe improvement (EWJ, marginal at $+0.009$); the average Sharpe change is $-$0.044 and the average annual return cost is 1.2 pp after 10 bps turnover costs.

% source (v4 retraction rewrite): experiments/k1695/k1695_results.json .decision + .inference.{primary,no_timing_reference,circular_shift_null} + .samples.*.summary
These raw numbers, however, compare strategies with materially different risk. The 12/VIX overlay realizes only 0.52--0.66 times buy-and-hold volatility on the inception-aware sample (Table~\ref{tab:international}, Vol.\ ratio column; all 13 markets exceed a 20\% exposure-mismatch threshold), so the raw $\Delta$MDD conflates two effects: holding less risk on average, and holding less risk at the right times. Two diagnostics separate them. First, a no-timing benchmark that holds each market's \emph{average} 12/VIX weight constant---rebalanced monthly with the same cash sleeve and cost model but no VIX input---reproduces most of the raw headline: $+10.7$ pp of the $+12.6$ pp common-sample average, with the same 13/13 positive-improvement count as the overlay, while its own exposure-matched gap is essentially zero ($+0.01$ pp). Second, scaling each buy-and-hold benchmark by a constant leverage $\lambda$ to match the overlay's realized volatility (an exposure-matched comparison) shrinks the average drawdown gap to $-0.9$ pp on the common sample and $+5.0$ pp on the inception-aware sample. Neither gap rejects a circular-shift no-timing null that re-times the overlay's own weight path across all calendar phases (one-sided $p = 0.56$ and $p = 0.21$, respectively), and no individual market survives a Holm correction in either sample (0/13).

What survives is a statement about risk reduction, not timing. On the inception-aware sample the \emph{raw} statistic does reject its phase null ($+27.5$ pp observed vs.\ a null mean of $+15.2$ pp, $p = 0.039$), and the reason is measurable: the realized weight path attains the lowest volatility ratio among all 231 calendar phases of itself (0.560 vs.\ a phase-null mean of 0.698). VIX forecasts volatility, so 12/VIX de-levers into genuinely turbulent months---a real property of the signal that reduces realized risk. But reducing risk is not the same as drawing down less than a benchmark carrying the same risk, and any leverage-adjusted investor could replicate the raw MDD improvement by scaling down constant exposure. The cross-sectional correlation between VIX sensitivity (the correlation between an asset's daily returns and VIX changes) and raw $\Delta$MDD---Pearson $r = -0.817$ ($p < 0.001$), Spearman $\rho = -0.775$ ($p = 0.002$)---should be read in the same light: markets more negatively correlated with VIX are de-levered more aggressively by the overlay, so this descriptive association primarily tracks the size of the exposure reduction rather than timing skill. Developed markets' larger inception-aware raw improvements relative to emerging markets (31.3 vs.\ 23.0 pp) carry the same caveat; the exposure-matched gaps preserve the ordering ($+7.5$ vs.\ $+2.0$ pp) but neither group's gap is statistically distinguishable from the no-timing null.


\begin{table}[H]
\centering
\begin{threeparttable}
\caption{International VT: Raw MDD Reduction and Exposure-Matched Decomposition ($N = 13$)}
\label{tab:international}
\footnotesize
\begin{tabular}{@{}lccccccccc@{}}
\toprule
& & \multicolumn{2}{c}{Buy \& Hold} & \multicolumn{2}{c}{12/VIX VT} & & & & Matched \\
\cmidrule(lr){3-4} \cmidrule(lr){5-6}
Market & VIX Sens. & Sharpe & MDD (\%) & Sharpe & MDD (\%) & $\Delta$Sharpe & $\Delta$MDD (pp) & Vol.\ ratio & $\Delta$MDD (pp) \\
\midrule
\multicolumn{10}{@{}l}{\textit{Panel A: Developed Markets}} \\
% source: experiments/k1695/table5_rows.csv (inception-aware rows; pinned snapshot; vol_ratio_vt_over_bh + exposure_matched_delta_mdd_pp)
EFA (EAFE) & $-$0.654 & 0.243 & $-$61.0 & 0.211 & $-$28.3 & $-$0.032 & 32.8 & 0.52 & 8.5 \\
EWJ (Japan) & $-$0.576 & 0.215 & $-$53.5 & 0.224 & $-$24.0 & 0.009 & 29.5 & 0.57 & 9.6 \\
EWG (Germany) & $-$0.606 & 0.240 & $-$63.1 & 0.205 & $-$33.5 & $-$0.034 & 29.6 & 0.53 & 5.5 \\
EWU (UK) & $-$0.610 & 0.210 & $-$64.0 & 0.160 & $-$30.5 & $-$0.050 & 33.5 & 0.52 & 8.7 \\
EWA (Australia) & $-$0.587 & 0.275 & $-$67.0 & 0.176 & $-$34.0 & $-$0.098 & 33.0 & 0.52 & 7.5 \\
EWC (Canada) & $-$0.588 & 0.321 & $-$60.8 & 0.295 & $-$33.6 & $-$0.026 & 27.1 & 0.52 & 3.0 \\
VGK (Europe) & $-$0.640 & 0.252 & $-$63.7 & 0.212 & $-$29.9 & $-$0.040 & 33.8 & 0.53 & 9.7 \\
\midrule
\multicolumn{10}{@{}l}{\textit{Panel B: Emerging Markets}} \\
EEM (EM Broad) & $-$0.600 & 0.233 & $-$66.4 & 0.174 & $-$32.1 & $-$0.059 & 34.3 & 0.52 & 8.1 \\
FXI (China) & $-$0.493 & 0.179 & $-$72.7 & 0.131 & $-$47.4 & $-$0.048 & 25.3 & 0.57 & 1.7 \\
EWZ (Brazil) & $-$0.505 & 0.234 & $-$77.3 & 0.173 & $-$67.4 & $-$0.061 & 9.9 & 0.58 & $-$14.7 \\
INDA (India) & $-$0.467 & 0.267 & $-$45.1 & 0.191 & $-$26.1 & $-$0.076 & 19.0 & 0.66 & 5.0 \\
EWT (Taiwan) & $-$0.569 & 0.411 & $-$62.9 & 0.375 & $-$31.7 & $-$0.036 & 31.2 & 0.57 & 9.5 \\
MCHI (China Broad) & $-$0.468 & 0.169 & $-$62.8 & 0.148 & $-$44.3 & $-$0.021 & 18.5 & 0.66 & 2.4 \\
\midrule
% source: experiments/k1695/k1695_results.json .samples.inception_aware.summary (average_vol_ratio, average_exposure_matched_delta_mdd_pp)
\multicolumn{2}{@{}l}{Average (inception-aware)} & 0.250 & $-$63.1 & 0.206 & $-$35.6 & $-$0.044 & 27.5 & 0.56 & 5.0 \\
\midrule
\multicolumn{10}{@{}l}{\textit{Formal inference (13-market common sample, $N = 3{,}557$ shared dates; joint stationary bootstrap, mean block 252 days)}} \\
% source: experiments/k1695/k1695_results.json .inference.primary.average_delta_mdd_pp
\multicolumn{4}{@{}l}{Raw average $\Delta$MDD:} & \multicolumn{6}{l}{12.6 pp; 90\% CI $[4.2,\ 19.3]$ pp; $\Pr(\le 0) = 0.0006$} \\
% source: experiments/k1695/k1695_results.json .inference.primary.average_exposure_matched_delta_mdd_pp
\multicolumn{4}{@{}l}{Exposure-matched average $\Delta$MDD:} & \multicolumn{6}{l}{$-$0.9 pp; 90\% CI $[-7.0,\ +3.6]$ pp; $\Pr(\le 0) = 0.7075$} \\
% source: experiments/k1695/k1695_results.json .inference.circular_shift_null.{common_period,inception_aware} + .decision.per_sample
\multicolumn{4}{@{}l}{Circular-shift no-timing null (matched gap):} & \multicolumn{6}{l}{common $p = 0.559$; inception-aware $p = 0.212$; Holm survivors 0/13 in both samples} \\
% source: experiments/k1695/k1695_results.json .inference.no_timing_reference.common_period
\multicolumn{4}{@{}l}{No-timing constant-weight benchmark (common):} & \multicolumn{6}{l}{raw $+10.7$ pp (13/13 positive); exposure-matched $+0.01$ pp} \\
\midrule
\multicolumn{10}{@{}l}{\textit{Cross-sectional predictors of raw $\Delta$MDD (descriptive; primarily tracks exposure reduction)}} \\
% source: experiments/k1695/k1695_results.json .samples.inception_aware.summary.vix_sensitivity_vs_delta_mdd
\multicolumn{4}{@{}l}{VIX Sensitivity vs.\ raw $\Delta$MDD:} & \multicolumn{6}{l}{$r = -0.817$ ($p < 0.001$), $\rho = -0.775$ ($p = 0.002$)} \\
\multicolumn{4}{@{}l}{GJR $\gamma$ vs.\ $\Delta$Sharpe:} & \multicolumn{6}{l}{\emph{Removed in v3:} replication did not reproduce the originally reported Spearman $\rho = 0.830$ (canonical: $\rho = 0.187$, NS); see forensic note in Section~\ref{sec:discussion}.} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Inception-aware full-sample results for 13 international equity ETFs (most from January 2007; INDA from February 2012, MCHI from April 2011; all through March 2026), computed from a pinned data snapshot (\texttt{auto\_adjust=False} with explicit distribution reinvestment; experiment K1695). VT applies the prior calendar month-end 12/VIX weight, rebalanced monthly, with 10 bps one-way turnover costs; Sharpe ratios use a daily prior-day \texttt{\^{}IRX} risk-free proxy. VIX Sensitivity is the correlation between daily asset returns and daily VIX changes. $\Delta$MDD is the reduction in maximum drawdown (positive = improvement); per-market positive $\Delta$MDD is descriptive, not individual significance. Vol.\ ratio is the overlay's realized volatility divided by buy-and-hold's on the same sample; all 13 markets exceed a 20\% exposure mismatch, so raw $\Delta$MDD is not reported alone. Matched $\Delta$MDD scales each buy-and-hold benchmark by a constant leverage $\lambda$ equal to the vol.\ ratio, so both legs carry the same realized volatility; the residual gap is the timing component. Formal inference uses the joint circular stationary bootstrap (mean block 252 days) on the common sample, resampling all 26 BH/VT return vectors on shared date indices; the circular-shift null re-times the overlay's own monthly weight path across all calendar phases and asks whether the observed exposure-matched gap is distinguishable from phase-random de-levering. Developed markets average raw $\Delta$MDD = 31.3 pp (matched $+7.5$); emerging markets 23.0 pp (matched $+2.0$); neither matched gap rejects the null. Only 1 of 13 markets (Japan) shows a Sharpe improvement; the average $\Delta$Sharpe is $-$0.044.
\end{tablenotes}
\end{threeparttable}
\end{table}

Figure~\ref{fig:cross_asset} presents the cross-sectional relationship between Sharpe ratio change and raw MDD improvement. Twelve of the 13 markets fall in the quadrant of negative $\Delta$Sharpe and positive raw $\Delta$MDD: the overlay gives up return while reducing drawdowns. Both axes are raw (unmatched) quantities, so the vertical dispersion primarily reflects how aggressively each market is de-levered rather than timing skill (Section~\ref{sec:international}); the exposure-matched gaps in Table~\ref{tab:international} are an order of magnitude smaller. The developed markets (blue circles) cluster at higher raw improvement values (average 31.3~pp) than emerging markets (red squares, average 23.0~pp). Brazil (EWZ) shows the smallest raw improvement (9.9~pp) and the only strongly negative exposure-matched gap ($-$14.7~pp), consistent with idiosyncratic risk sources that are less responsive to the US VIX signal.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig2_cross_asset_scatter.pdf}
\caption{Cross-Asset VT: Sharpe Ratio Change vs.\ Raw MDD Improvement for 13 International Markets. Each point represents a country ETF (inception-aware sample, pinned snapshot; experiment K1695). The $x$-axis shows the change in Sharpe ratio from applying 12/VIX VT relative to buy-and-hold; the $y$-axis shows the raw improvement in maximum drawdown (percentage points). Blue circles denote developed markets ($N = 7$); red squares denote emerging markets ($N = 6$). Dotted lines indicate cross-sectional averages ($\Delta$Sharpe $= -0.044$; raw $\Delta$MDD $= 27.5$~pp). Both axes are unmatched quantities: because the overlay realizes only 0.52--0.66 times buy-and-hold volatility, the raw drawdown improvements primarily reflect de-levering; exposure-matched gaps (Table~\ref{tab:international}) average $+5.0$~pp and do not reject a no-timing null. Data from Table~\ref{tab:international}.}
...
% : Prediction accuracy ≠ allocation optimality --- strengthens core thesis
...
%, : 12/VIX irreducible --- the formula itself is a continuous mean-reversion trade
...
% : 12/VIX is return-OPTIMAL among 427 configurations
...
\subsection{Insurance Pricing Reconsidered: What the Premium Actually Buys}

Our international results sharpen---and partly retract---the ``insurance pricing'' interpretation of VT~\citep{lai2026a}. Across 13 markets, the overlay extracts a consistent ${\sim}4\%$/year Sharpe drag (the ``premium'') and delivers broad raw MDD reduction (the apparent ``payout''). The exposure-matched decomposition of Section~\ref{sec:international}, however, shows that most of this payout is delivered by holding roughly half the volatility of buy-and-hold on average, not by timing: a constant-weight position with the same average exposure collects nearly the same raw drawdown reduction at similar cost. The insurance analogy therefore needs restating. What the premium buys, in the international panel, is a mechanically smaller risk budget whose size co-moves with VIX---genuine conditional risk reduction, but replicable by any investor who can scale exposure directly. The breadth of the raw result---13 of 13 markets across developed and emerging economies---remains consistent with the global financial cycle framework of \citet{miranda2020}, in the limited sense that a single US signal successfully proxies for global risk conditions; it is not evidence of same-risk drawdown protection.

The cross-sectional correlation between VIX sensitivity and raw $\Delta$MDD ($r = -0.817$) should be reinterpreted in the same way. Markets more sensitive to US-driven risk-off episodes are de-levered more aggressively by the overlay, so the association primarily prices the size of the exposure reduction rather than a protection payout. We also do not interpret VIX here as a pure expected-volatility state variable: VIX embeds the variance risk premium~\citep{bollerslev2009} and investor demand for downside protection~\citep{bondarenko2019}. As a cost description for the single-asset US case, the insurance arithmetic remains informative: the breakeven VIX level---above which VT's cash drag is more than offset by crisis-period savings---is approximately 26.6 for SPY. Below this threshold, VT underperforms buy-and-hold (the ``premium'' regime); above it, VT outperforms (the ``payout'' regime). In the low-VIX regime (VIX $< 15$), VT costs roughly 4.0\%/year in foregone Sharpe, while in the crisis regime (VIX $> 30$), it delivers approximately 12\%/year of excess protection.

% : 50/50 allocation also irreducible --- dynamic allocation fails Harvey
...
Sixth, the international equity analysis (13 markets in Table~\ref{tab:international}) is now stated in exposure-matched terms: the raw drawdown improvements are attributable primarily to de-levering, and the exposure-matched gaps do not reject a no-timing null in either sample. A remaining limitation is sample-driven: the common window (post-2012) excludes the 2008--2009 crisis, while the inception-aware sample contains only that single crisis episode. The inception-aware exposure-matched gap ($+5.0$~pp, positive in 12 of 13 markets, $p = 0.21$, 0/13 after Holm) is therefore neither confirmed nor ruled out as a modest crisis-window timing effect; one crisis is not a test.
...
This paper demonstrates that VIX-based de-risking embeds a mechanical trend-following overlay through the leverage effect, but this is not the whole story. VT possesses a dual mechanism: the Sharpe ratio channel operates partially through implicit asset-specific TSMOM exposure, while the maximum drawdown channel---VT's primary economic value---is largely independent of trend following. Across all 22 primary assets plus the 50/50 composite, block bootstrap (10,000 replications) finds that 22 of 23 rows have positive CI lower bounds (90\% level), with point estimates at or above 83.8\% and medians above 100\% for 21 of 23 rows. The the canonical five-asset results (SPY 103.7\%, 50/50 95.6\%, DIA 106.2\%, QQQ 109.0\%, IWM 102.2\%) are confirmed and generalized: TSMOM removal preserves the drawdown channel across asset classes, not just US equities, while some $>100\%$ estimates can arise mechanically during rebound phases of momentum crashes. The leverage effect parameter ($\gamma$) predicts VT's trend-following loading cross-sectionally ($r = 0.564$, $p = 0.006$, $N = 22$), an empirical regularity consistent with the leverage effect mechanism; a split-sample robustness check ($r = 0.793$, $p < 0.001$, split-sample canonical) survives temporal separation of the estimation windows but is partly shaped by a 2017--2026 regime shift in which international and safe-haven loadings flipped positive. We note that this cross-sectional association ($N = 22$, mixed asset classes) does not establish a causal link.

Four practical implications emerge. First, VT investors already hold embedded asset-specific trend-following exposure as a mechanical byproduct of the leverage effect. Second, a single US VIX signal reduces realized risk and raw drawdowns across global equity allocations (13/13 markets with positive raw MDD reduction; inception-aware average 27.5 pp, common-sample average 12.6 pp), but this benefit is de-levering rather than same-risk protection: a constant-weight position with the same average exposure replicates most of it, and the exposure-matched gap does not reject a no-timing null, so leverage-flexible investors should not read the raw drawdown reduction as timing skill. Third, within-equity diversification (sector rotation) does not require sector-specific VT tuning, as the gamma mechanism operates only at the cross-asset-class level. Fourth, within the specifications tested, the $12/\text{VIX}$ formula and the 50/50 equity/gold allocation are both highly robust: no optimization---dynamic allocation, alternative thresholds, or higher rebalancing frequency---survives the \citet{harvey2016} threshold, confirming that VT's value resides in its parsimonious risk-scaling mechanism rather than in parameter tuning.
