...
We establish this through a pre-specified out-of-sample design applied to seven primary assets spanning equities (SPY, QQQ, EEM), precious metals (GLD, SLV), government bonds (TLT), and cryptocurrency (BTC-USD), with validation extended to up to twenty-six assets. Forecasts are generated on rolling windows with explicit lagging, evaluated with the QLIKE loss and Diebold--Mariano tests, and taken through to a volatility-targeting (VT) allocation to measure decision-level, not merely statistical, value. The forecast-level comparison is run on the same assets under a single common out-of-sample protocol; the allocation-level evidence is then reported over each asset's native evaluation window and read as descriptive corroboration rather than as a matched-window comparison.
...
Our design observes the pass-through from volatility \emph{measurement} to portfolio \emph{allocation} in two linked stages: the forecast-level comparison uses the same assets under a single common out-of-sample protocol, while the allocation-level evidence (Table~\ref{tab:vt}) is reported over each asset's native evaluation window and is treated as descriptive corroboration rather than as a matched-window horse race. We describe, in order, the data and a single canonical sample partition for the forecast-level comparison, the two competing volatility measurements, the rolling out-of-sample forecasting scheme and its timing, the forecast-level evaluation criteria, and the allocation-level testbed.
...
We now ask whether the forecast-level gains of Section~\ref{sec:forecast_results} survive to the volatility-targeting allocation of Section~\ref{sec:vt_methodology}, using for each asset the GARCH specification prescribed by the selection rule above. Table~\ref{tab:vt} reports buy-and-hold versus VT performance for the five primary assets with sufficient history; because each asset is evaluated over its own native window (see table notes), we read this allocation-level evidence as descriptive corroboration of the level-driven pattern rather than as a matched-window comparison.% source: experiments/k903 VT cross-asset replication
