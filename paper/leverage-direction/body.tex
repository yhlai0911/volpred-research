...
\textit{First}, we employ Newey-West HAC standard errors \citep{newey1987} with 8 lags (chosen as $\lceil 0.75 \times T^{1/3} \rceil$ where $T$ is the number of quarterly gamma estimates), which accounts for the moving-average structure induced by the overlap. All $t$-statistics in Table~\ref{tab:gamma} are HAC-corrected; for SPY, the correction reduces the naive statistic substantially while preserving strong significance ($t = +8.30$ after correction), and for GLD the corrected statistic ($t = +0.15$) confirms that the unconditional mean carries no significance to begin with.
...
\end{itemize}
The $t$-statistic of $\gamma$ is directly available from standard GARCH estimation output, and is our \emph{primary} formulation because it is self-adaptive to estimation precision. Two distinct statistics must not be conflated here: the \emph{single-window} estimation $t$ (the rule's input, available in real time from the current window's GARCH output) and the \emph{quarterly-mean} HAC $t$ reported in Table~\ref{tab:gamma} (a retrospective cross-window summary). Bitcoin is the borderline case that illustrates the distinction: BTC's quarterly-mean HAC $t = +1.83$ sits marginally above 1.65, while its high cross-window instability (std $= 0.136$, 28\% negative windows) means individual estimation windows frequently fail the significance test. Empirically the stakes are low: Table~\ref{tab:qlike} shows the two models are statistically indistinguishable for BTC ($\Delta = -0.06\%$, $p = 0.848$), so either prescription is defensible---we label BTC ``GJR/Borderline'' in Table~\ref{tab:gamma} accordingly. Under the $t > 1.65$ rule applied to the quarterly-mean statistic, the prescribed model is never significantly beaten in any of the nine DM comparisons in Table~\ref{tab:qlike}: SPY's two cells show significant GJR superiority, GLD's 2023--2024 cell shows significant GARCH superiority (consistent with its insignificant mean $\gamma$), and the remaining cells show no significant difference. The equivalent magnitude threshold within our sample lies in the range $[0.12, 0.17]$ (i.e., between BTC's 0.117 and QQQ-2025's 0.17), which confirms the rule is not tied to a single numerical cutoff.

\textbf{In-sample versus out-of-sample classification.} An important caveat is that both the magnitude band ($|\gamma| \in [0.12, 0.17]$) and the significance-based formulation ($t > 1.65$) are calibrated on the same sample used for evaluation; the in-sample classification consistency is therefore partly mechanical and should not be interpreted as a measure of predictive accuracy. In the genuinely out-of-sample test---using 2023 $\gamma$ estimates to predict 2024--2025 model choice for $N = 6$ assets---the multi-window average achieves 6/6 correct classifications, and the single-point rule achieves 5/6 (83\%). However, with $N = 6$, even random classification would achieve 100\% accuracy with probability $2^{-6} \approx 1.6\%$, so the OOS evidence, while supportive, has limited statistical power. Independent validation on a larger, separate asset universe is needed to confirm generalizability of both the threshold level and the classification accuracy.
