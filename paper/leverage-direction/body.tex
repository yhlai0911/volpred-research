...
The insurance cost can be partially offset through VIX-conditional leverage. Decomposing the 12/VIX strategy's cost by VIX regime reveals that the equity reduction cost is concentrated in low-VIX periods ($-3.47\%$/yr when $\text{VIX} < 15$), while high-VIX periods actually generate positive excess returns ($+8.17\%$/yr when $\text{VIX} > 25$), yielding a breakeven VIX of approximately 26.6. A regime-conditional leverage rule---applying 1.5$\times$ leverage when $\text{VIX} < 15$ and 1.0$\times$ when $\text{VIX} > 25$---exploits this asymmetry by capturing additional equity premium during calm markets while maintaining full downside protection during stress. The strategy passes the \citet{harvey2016} threshold ($t = 7.90$) and all 11 of 11 cross-validated out-of-sample periods, improving the Sharpe ratio by $+0.11$ and CAGR by $+5.3$ percentage points with only modest MDD deterioration ($+2.7$pp). The insurance efficiency---defined as MDD improvement per unit of return cost---is $5.7\times$ higher for VIX-conditional leverage than for standard 12/VIX targeting. This strategy requires a margin account and applies only to U.S.\ equity markets where VIX regime identification is reliable.
...
\subsection{The HAR Paradox: Universal Prediction Superiority Without Economic Improvement}
\label{sec:har_paradox}

The complexity ceiling documented above contains one notable exception that sharpens rather than refutes it. When the Heterogeneous Autoregressive (HAR) model of \citet{corsi2009} is estimated using absolute returns $|r_t|$ as the volatility proxy---rather than squared returns $r^2_t$---it achieves a dramatic improvement in statistical forecast accuracy. HAR-ABS produces a Diebold-Mariano statistic of $-15.45$ ($p < 0.001$) against GJR-GARCH for SPY, with QLIKE of 0.49 versus 1.51. This result is universal across asset classes: in cross-asset validation, HAR-ABS outperforms GJR-GARCH in all seven markets tested (SPY, QQQ, EFA, EWZ, GLD, TLT, 0050.TW), with DM statistics ranging from $-11.11$ to $-21.74$, all significant at $p < 0.001$. The proxy choice is decisive: HAR with squared returns ($r^2_t$) produces a QLIKE of 1.57---\textit{worse} than GJR-GARCH---while HAR with absolute returns achieves 0.49, a threefold improvement from the same model structure.

However, this prediction superiority does not translate into economic improvement. When HAR-ABS is used as the scaling signal for volatility targeting, it produces the \textit{lowest} mean Sharpe ratio (1.12) among all VT variants tested, compared to 1.75 for 12/VIX and 0.97 for buy-and-hold, across three cross-validated out-of-sample periods. HAR-ABS generates 1.9 times higher daily turnover (0.059 vs.\ 0.032) and its weight correlation with 12/VIX is only 0.51, indicating fundamentally different information content. The hybrid HAR-VIX variant achieves a Sharpe of 1.69---nearly matching 12/VIX---because VIX contributes the bulk of the position-sizing signal. This is the fifth independent confirmation of the prediction$\neq$application paradox documented in our research program: the statistically best predictor is the economically worst strategy signal.

The resolution lies in the temporal orientation of each model's information. HAR is backward-looking: it optimally aggregates historical return magnitudes at daily, weekly, and monthly horizons ($\beta_5$ dominates in equity assets, $\beta_{22}$ in commodities and bonds). VIX is forward-looking: it embeds the options market's consensus variance expectation, including the variance risk premium. For \textit{measurement}---ex post evaluation of how well the forecast matches realized variance---the richer backward-looking aggregation of HAR is superior. For \textit{allocation}---determining how much risk to take tomorrow---the forward-looking risk pricing embedded in VIX is irreplaceable. This distinction reinforces the complexity ceiling: the binding constraint on investment performance is not forecast accuracy but the forward-looking risk information that only implied volatility provides.
