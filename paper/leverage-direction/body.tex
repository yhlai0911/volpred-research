...
\textbf{Third}, we document a persistent time-zone arbitrage channel in which U.S.\ market information spills over to Asian equity markets with a one-day delay. A simple momentum strategy---holding the local index when trailing SPY returns are positive, otherwise cash---achieves $t$-statistics exceeding the \citet{harvey2016} multiple-testing threshold of 3.0 in six Asia-Pacific markets: Hong Kong ($t = 4.12$), Australia ($t = 4.04$), Singapore ($t = 4.03$), Korea ($t = 3.83$), Taiwan ($t = 3.75$), and Japan ($t = 3.69$), after transaction costs. A global portfolio combining U.S.\ volatility targeting and Asian time-zone momentum achieves Sharpe 1.61 ($t = 4.31$) with maximum drawdown of $-8.4\%$, benefiting from near-zero correlation ($\rho = 0.14$) between the two alpha sources.
...
The Spearman rank correlation between $\gamma$ and $\beta^{\text{trend}}$ is $\rho_s = 1.000$ ($p < 0.001$), indicating a perfect monotonic relationship across the seven primary assets. The Pearson correlation is $r = 0.993$ ($p < 0.001$), confirming near-perfect linearity. To assess robustness beyond this small cross-section, we extend the analysis to 17 assets (adding commodity ETFs from Appendix~A) and 26 assets (further adding individual equities and international ETFs). The Spearman correlation degrades gracefully: $\rho = 0.874$ ($p < 0.001$, $N = 17$) and $\rho = 0.753$ ($p < 0.001$, $N = 26$), confirming that the gamma-mechanism mapping is a genuine cross-asset regularity rather than an artifact of small sample size.
...
Third, we identify a time-zone arbitrage channel in which U.S.\ market information spills over to Asian equity markets with a one-day delay, generating statistically significant alpha exceeding the \citet{harvey2016} threshold of $t > 3.0$ across six Asia-Pacific markets ($t = 3.69$--$4.12$) after transaction costs. A global portfolio combining U.S.\ volatility targeting and Asian time-zone momentum achieves Sharpe 1.61 ($t = 4.31$) with maximum drawdown of $-8.4\%$, benefiting from near-zero correlation ($\rho = 0.14$) between two economically distinct alpha sources.
