...
The model comparison framework reveals that rankings depend critically on the evaluation criterion. In a comprehensive comparison across seven volatility models (K778/K780), GJR-GARCH dominates on QLIKE($r^2$)---the \citet{patton2011} proxy-robust metric---while AMEM (Asymmetric Multiplicative Error Model) with Gamma-distributed innovations dominates on VaR calibration, achieving a 1\% violation rate of 1.09\% versus the 1\% target. This asymmetry between forecast accuracy and risk management quality has practical implications: practitioners selecting models for VaR compliance should prefer AMEM's Gamma-distributed innovations, which naturally accommodate the right-skewed distribution of volatility, while researchers evaluating point forecast accuracy should use GJR-GARCH as the benchmark. The finding reinforces the three-domain separation in Table~\ref{tab:complexity_ceiling}: the GARCH equation governs QLIKE, the distributional assumption governs VaR, and neither domain's ranking transfers to the other.
