...
\item \textbf{Historical Simulation:} $\text{VaR}_\alpha = Q_\alpha(\hat{z}_{1:T}) \cdot \hat{\sigma}_t$, where $\hat{z}_s = r_s / \hat{\sigma}_s$ are in-sample standardized residuals and $Q_\alpha(\cdot)$ denotes the empirical $\alpha$-quantile

\end{itemize}
where $\nu$ is the degrees of freedom (fixed at 5 in our baseline, consistent with typical empirical estimates for financial returns). The choice of df = 5 is validated out-of-sample: Section~4.4 demonstrates that Basel III Green Zone compliance is achieved for all df $\leq$ 6, and that jointly estimating df produces \textit{worse} performance by over-adapting to quiet markets. The fixed df = 5 is thus a conservative choice that is robust across the empirical range df $\in$ [4, 7]. Historical simulation avoids this parameter choice entirely by using the empirical quantile of standardized residuals, which captures any non-Gaussian features (skewness, excess kurtosis, asymmetry) present in the data.
...
The $t$-statistic of $\gamma$ is directly available from standard GARCH estimation output. This significance-based criterion is equivalent to an absolute threshold of approximately $\gamma > 0.08$ in our sample and yields classifications consistent with all nine DM test outcomes across six assets and two OOS periods (Table~3). For the threshold $\gamma > 0.08$, all four asset-period combinations above the threshold show GJR superiority (positive DM statistic), while all five combinations below show no significant difference or GARCH superiority---consistently classifying each case correctly. The threshold is robust to perturbation: any value in $[0.06, 0.12]$ produces identical classifications for all nine asset-period combinations, and the significance-based formulation ($t > 1.65$) is inherently adaptive to estimation precision, avoiding a fixed numerical cutoff.
...
% v2-NEW-H1: Orthogonality of GARCH equation and distributional assumption (K799/K802/K824)
A sharper demonstration of domain orthogonality emerges when we cross the GARCH equation choice with the distributional assumption. Table~\ref{tab:var_ortho} reports VaR 1\% backtesting results for SPY over 2023--2024 (502 trading days) under four configurations. GARCH(1,1) with Normal innovations produces 7 violations (1.39\%, Kupiec $p = 0.40$, Basel Green Zone)---a comfortable pass. Switching to GJR-GARCH---which dominates on QLIKE ($-3.8\%$, DM $p = 0.001$)---raises violations to 10 (1.99\%, Kupiec $p = 0.049$), a borderline failure. The mechanism is straightforward: GJR's asymmetric response to negative returns generates higher conditional variance during stress periods; under Normal quantiles, the fatter conditional tails produce excess VaR exceedances that the thin-tailed Normal $z$-score cannot accommodate.

Crucially, replacing the Normal with Student-$t$ (df=5) innovations \textit{under the same GJR-GARCH equation} reduces violations to 6 (1.20\%, Kupiec $p = 0.67$, Basel Green Zone). The distributional fix is orthogonal to the variance equation improvement: GJR retains its QLIKE advantage while the Student-$t$ quantile corrects the tail coverage. A nonparametric alternative---applying GJR forecasts to the empirical distribution of standardized residuals (historical simulation)---achieves the lowest violation rate of all: 4 violations (0.80\%, Kupiec $p = 0.64$, Basel Green Zone) without any distributional assumption. Historical simulation lets the data determine the tail shape rather than imposing a parametric form, and its zero-parameter design makes it immune to the quiet-market over-fitting that degrades jointly estimated Student-$t$ degrees of freedom (Section~\ref{sec:robustness_checks}).

This confirms that \textbf{the GARCH equation and the distributional assumption constitute independent, non-transferable optimization domains}. Improving one dimension (variance dynamics via GJR) can degrade another (tail quantiles under Normal) unless both are jointly calibrated. The practical implication is clear: practitioners who adopt GJR-GARCH for its forecast superiority \textit{must} simultaneously upgrade from Normal innovations---and historical simulation of standardized residuals represents the most robust choice, requiring no degrees-of-freedom selection while achieving the best empirical coverage.
...
\end{equation}
For the baseline 12/VIX strategy over the full 2005--2026 sample, we obtain $\hat{\gamma}_{HM} = -0.035$ ($t = -0.39$, $p = 0.70$), indicating no directional timing ability.
...
Our VaR attribution analysis carries a practical message for risk managers: the largest improvements in Basel III compliance come from the simplest adjustment---moving away from Normal quantiles. This pushes back against the trend toward increasingly complex tail-risk models. Both Student-$t$ and historical simulation address the fundamental problem (fat tails) directly, while more sophisticated approaches capture second-order effects that are empirically negligible. Indeed, the best-performing method requires \textit{no} distributional parameters at all: historical simulation of GJR-GARCH standardized residuals achieves 4/502 violations (0.80\%, Basel Green Zone) versus Student-$t$'s 6/502 (1.20\%) and Normal's 10/502 (1.99\%, borderline failure). The nonparametric approach dominates because it lets the empirical tail shape---including any skewness, excess kurtosis, or asymmetry not captured by Student-$t$---determine the VaR threshold directly.

A further null result clarifies the role of distributional assumptions: changing the innovation distribution materially affects VaR performance but does not affect QLIKE rankings (DM $p = 0.56$). This separation is economically intuitive: QLIKE evaluates second-moment forecasting, whereas VaR depends on tail quantiles. The orthogonality is starkly demonstrated when GJR-GARCH---the QLIKE-optimal equation---produces a borderline VaR failure under Normal innovations (10/502 violations, Kupiec $p = 0.049$), while symmetric GARCH passes comfortably (7/502, $p = 0.40$). Restoring compliance requires only a distributional upgrade: GJR-GARCH with historical simulation achieves the best coverage (4/502 violations, $p = 0.64$), while Student-$t$(df=5) also passes (6/502, $p = 0.67$), both retaining the forecast advantage while satisfying risk management constraints (Table~\ref{tab:var_ortho}). The lesson is that optimizing the variance equation and the distributional assumption are independent tasks that must be pursued jointly---and that the optimal distributional choice is the one with the fewest assumptions.
...
We repeat the \citet{henriksson1981} timing test specifically for the Hybrid VT strategy on 2014--2026 ($N \approx 3{,}100$ daily observations). Hybrid VT generates statistically significant alpha of 5.77\% annualized ($t = 3.99$, $p < 0.001$) with negative $\gamma_{HM} = -0.043$ ($t = -4.06$). The significant negative $\gamma_{HM}$ for Hybrid VT---contrasting with the insignificant $-0.035$ for the baseline 12/VIX strategy---reflects Hybrid VT's more aggressive volatility scaling in the shorter, higher-volatility 2014--2026 subsample. Both results confirm that VT alpha originates from \textit{variance management}, not directional prediction.
...
Normal $\to$ Hist.\ Sim. & VaR (4/502, 0.80\%, Green) & QLIKE (unchanged) & Zero parameters; Tab.~\ref{tab:var_ortho} \\
...
Three orthogonal domains emerge: (i) the \textit{GARCH equation} determines volatility forecast accuracy (QLIKE), where GJR-GARCH defines a daily-frequency floor; (ii) the \textit{distributional assumption} determines tail risk coverage (VaR), where historical simulation of standardized residuals is optimal (0.80\% violation rate, zero distributional parameters), followed by Skewed Student-$t$ and FHS; (iii) the \textit{signal source} determines strategy performance, where VIX-implied volatility dominates via the embedded variance risk premium. The orthogonality between domains (i) and (ii) is directly quantified in Table~\ref{tab:var_ortho}: GJR-GARCH wins QLIKE but \textit{fails} VaR under Normal innovations, while both historical simulation and Student-$t$ restore compliance without affecting forecast accuracy. Gains in one domain do not transfer---and can actively harm---performance in another.
