...
\paragraph{Deferred robustness work.} Three additional exercises considered in earlier drafts are intentionally not carried as active evidence here: a uniform-lag re-estimation of the subperiod Granger tests, a dedicated pre-/post-ETF split, and a cross-asset VXN/QQQ replication. The first two are not currently written back into the experiment JSON that underlies this manuscript, and the third still relies on an older, uncorrected methodology rather than the corrected v2 specification. They remain natural next steps, but they are not treated as completed robustness results in this draft.
