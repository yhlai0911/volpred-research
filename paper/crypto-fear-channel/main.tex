...
\end{tabular}
\begin{flushleft}
\footnotesize VIX min/max are 9.14 / 82.69. $\text{RV}^{(20)}_{\text{btc}}$ min/max are 0.098 / 1.701 (annualized). Source: \texttt{experiments/k1025/k1025\_v2\_results.json .descriptive\_statistics}.
\end{flushleft}
\end{table}

Augmented Dickey-Fuller (ADF) stationarity tests reject the unit-root null for all three modeling series at conventional levels: $\text{RV}^{(20)}_{\text{btc}}$ ($t = -4.90$, $p < 0.001$, lags 20), VIX ($t = -5.71$, $p < 10^{-6}$, lags 9), and $\text{RV}^{(20)}_{\text{spy}}$ ($t = -4.46$, $p < 0.001$, lags 20).% source: experiments/k1025/k1025_v2_results.json .adf_tests
...
We split the 2015--2026 sample into five non-overlapping subperiods --- pre-mania (2015--2017), crypto-winter (2018--2019), COVID (2020), bull-bear (2021--2022), and recovery-plus-ETF (2023--2026) --- and re-estimate the BTC-to-VIX Granger test within each subperiod with AIC-selected lag length. Table~\ref{tab:subperiod} reports the result. Granger causality is statistically significant only during 2020 ($F = 12.31$, $p = 1.6 \times 10^{-7}$, lag 3); each of the other four subperiods fails to reject the non-causality null at any conventional level.% source: experiments/k1025/k1025_v2_results.json .subperiod_granger
