...
Lag & $F$ & $p$ & $F$ & $p$ \\
\midrule
1 & 2.04 & 0.153 & 21.78 & $3.0 \times 10^{-6}$ \\
2 & 0.21 & 0.812 & 19.57 & $< 10^{-8}$ \\
3 & 0.20 & 0.896 & 13.11 & $< 10^{-8}$ \\
4 & 0.17 & 0.951 & 9.49  & $< 10^{-7}$ \\
...
This section reports four robustness checks on the headline findings of \S\ref{sec:results}: the time-series stability of the Diebold-Yilmaz spillover index, the lag-length sensitivity of the subperiod Granger structure, and a pre-ETF vs.\ post-ETF microstructure comparison.

\subsection{Spillover-index stability across rolling windows}
\label{subsec:r_spillover}

The Diebold-Yilmaz total spillover index is computed on each of 512 rolling 252-trading-day windows. The mean is 90.11\% with a standard deviation of only 0.21~percentage points (i.e., the rolling spillover ranges from 89.79\% to 90.81\%, a peak-to-trough swing of about 1~percentage point across the entire sample).% source: experiments/k1025/k1025_v2_results.json .spillover_index
...
Table~\ref{tab:multiasset} reports the headline parameters of each finding under the two channel specifications. The qualitative pattern is preserved across the BTC$\to$VIX (S\&P) and BTC$\to$VXN (NASDAQ) channels: BTC downside vol Granger-causes the equity fear gauge in 5/5 lags under both specifications; the QR coefficient sign-reverses between lower and upper quantiles in both; the COVID-2020 subperiod is the only window with significant Granger predictability in both; the Diebold-Yilmaz net-receiver decomposition for BTC is essentially identical ($-74.41$pp vs $-76.64$pp); and the OOS DM test fails the Harvey threshold under both specifications. Quantitative magnitudes shift modestly: the upper-tail amplification ratio is $7.04\times$ for VIX versus $5.76\times$ for VXN (both well above unity, with VIX exhibiting the stronger sign-reversing tail amplification of the two), the OOS DM statistic is $-1.14$ vs $-0.43$ (both non-significant), and the 2020 in-sample Granger $F$ is $12.31$ vs $13.41$. The broader-market S\&P 500 fear gauge thus displays a stronger upper-tail response to BTC volatility than the more sector-concentrated NASDAQ-100 fear gauge --- a finding consistent with the broad-market index's wider range of constituents responding to coordinated risk-off flows. These shifts are realistic empirical variation across the two equity-fear gauges and do not reverse the qualitative direction of any of the five stylized facts.
...
\midrule
Asymmetric Granger BTC$^-$ best $F$ (lags 1--5)        & 21.78 (lag 1) & 24.31 (lag 1) \\
Asymmetric Granger BTC$^+$ p (all lags 1--5)            & 0.16--0.95 (NS) & 0.14+ (NS) \\
QR $\beta_{0.05}$                                       & $-2.78$ & $-1.46$ \\
QR $\beta_{0.5}$                                        & $+2.82$ & $+2.83$ \\
QR $\beta_{0.95}$                                       & $+19.85$ & $+16.29$ \\
QR upper-tail amplification ($\beta_{0.95}/\beta_{0.5}$) & $7.04\times$ & $5.76\times$ \\
Sub-period 2020 Granger $F$                              & 12.31 & 13.41 \\
DY total spillover (\%)                                  & 90.11 & 90.09 \\
DY net BTC (pp)                                          & $-74.41$ & $-76.64$ \\
OOS DM stat (Harvey)                                     & $-1.14$ (NS) & $-0.43$ (NS) \\
\bottomrule
\end{tabular}
\begin{flushleft}
