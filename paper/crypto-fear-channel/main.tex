...
Finally, the Diebold-Yilmaz-style spillover analysis quantifies directionality. Across the 512 rolling 252-day windows in the sample, the total spillover index averages 90.1\% with very low variation (standard deviation 0.22~percentage points; min 89.73\%, max 90.84\%), indicating that volatility shocks among $\{$VIX, $\text{RV}^{(20)}_{\text{spy}}$, $\text{RV}^{(20)}_{\text{btc}}\}$ are persistently and almost completely cross-transmitted. The decomposition reveals, however, that BTC is a \emph{net receiver}: the average shock contribution \emph{from} BTC to the system is 23.7\%, while BTC's average net spillover is $-74.4$ percentage points.% source: experiments/k1025/k1025_v2_results.json .spillover_index
...
The Diebold-Yilmaz-style total spillover index is computed on each of 512 rolling 252-trading-day windows. The mean is 90.11\% with a standard deviation of only 0.22~percentage points (i.e., the rolling spillover ranges from 89.73\% to 90.84\%, a peak-to-trough swing of about 1.11~percentage points across the entire sample).% source: experiments/k1025/k1025_v2_results.json .spillover_index
 The remarkable stability of the total index --- a swing of just 1.0~percentage point across an 11-year period that includes COVID-2020, the 2022 inflation regime, and the 2024 ETF approval --- indicates that the \emph{cross-asset variance-decomposition geometry} of the BTC--SPY--VIX system is essentially constant, even as individual pairwise correlations and Granger structures shift dramatically. The corollary is that the regime-dependence we document in \S\ref{subsec:r_regime} is \emph{not} an artifact of structural breaks in the underlying VAR system: the variance-decomposition shares move by less than 1~percentage point even as the within-period Granger $F$-statistic ranges from a minimum of 0.16 (2023--2026 recovery+ETF) to a maximum of 12.31 (COVID-2020) --- a 48-fold spread. The spillover index and the Granger test are measuring different objects --- aggregate connectedness vs.\ directed predictability --- and the regime story belongs to the latter.

The directional decomposition is similarly stable. BTC's average ``from-system'' spillover (the share of BTC RV variance attributable to VIX and SPY shocks) is 23.7\% across the rolling windows; the average net spillover (to minus from) is $-74.4$ percentage points. BTC remains a net receiver in every window: in no rolling 252-day subsample does the variance-decomposition flip BTC into a net-sender position. The ``crypto as fear amplifier conditional on equity stress'' interpretation is therefore a structural feature of this particular rolling-VAR decomposition, not a window-specific finding.

\paragraph{Deferred robustness work.} Three additional exercises considered in earlier drafts are intentionally not carried as active evidence here: a uniform-lag re-estimation of the subperiod Granger tests, a dedicated pre-/post-ETF split, and a cross-asset VXN/QQQ replication. The first two are not currently written back into the experiment JSON that underlies this manuscript, and the third still relies on the older K1025b methodology rather than the corrected v2 specification. They remain natural next steps, but they are not treated as completed robustness results in this draft.
