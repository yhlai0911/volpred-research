"""Owner-fenced formal caller for one immutable publisher reconcile batch.

The external interface is one ``reconcile`` call. Durable WorkItem creation,
...
_OWNER_FAMILY = "publisher.article.supabase.reconcile"
_PRIMARY_AUTHORITY_KEY = "publisher:article.supabase.reconcile"
_OPERATIONS_CORE_OWNER = "operations_core"
_LEGACY_OWNER = "legacy"
_WORKER_ID = "effect-worker:publisher-article-reconcile"
_RECEIPT_SCHEMA = "owned-publisher-reconcile-receipt.v1"


class PublisherArticleReconcileOwnershipLost(RuntimeError):
    """The caller no longer owns the publisher reconcile family generation."""


@dataclass(frozen=True)
class PublisherArticleReconcileOwner:
    schema_version: str
    effect_family: str
    owner: str
    generation: int
...
@dataclass(frozen=True)
class OwnedPublisherReconcileCommand:
    idempotency_key: str
...
@dataclass(frozen=True)
class OwnedPublisherReconcileRequest:
    owner_generation: int
    work_id: str
    effect_id: str
    request_sha256: str
    terminal_receipt: OwnedPublisherReconcileReceipt | None = None


@dataclass(frozen=True)
class OwnedPublisherReconcileAttempt:
    owner_generation: int
    work_id: str
    work_version: int
...
@dataclass(frozen=True)
class OwnedPublisherReconcileReceipt:
    schema_version: str
    owner_generation: int
    work_id: str
    work_status: str
...
class OwnedPublisherArticleReconcile:
    """Converge one immutable safe batch through the durable owner."""
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(worker_id, str) or not worker_id.strip():
            raise ValueError("owned publisher reconcile worker_id is required")
        if (
            isinstance(lease_seconds, bool)
            or not isinstance(lease_seconds, int)
            or lease_seconds <= 0
        ):
            raise ValueError("owned publisher reconcile lease_seconds must be positive")
        self._store = store
        ...
        self._token_factory = token_factory or (
            lambda: f"owned_reconcile_{uuid4().hex}"
        )
    ...
    def _token(self, kind: str) -> str:
        ...
        if not isinstance(token, str) or not token.strip():
            raise ValueError(
                f"owned publisher reconcile {kind} token is required"
            )
        return token.strip()
    ...
    def _current_primary_lease(
        ...
        *,
        ...
        if not isinstance(lease, PrimaryLease):
            raise TypeError(
                "owned publisher reconcile keepalive returned no typed "
                "PrimaryLease"
            )
        if (
            ...
            raise PublisherArticleReconcileOwnershipLost(
                "owned publisher reconcile keepalive lease identity mismatch"
            )
        if expected is not None and (
            ...
            raise PublisherArticleReconcileOwnershipLost(
                "owned publisher reconcile keepalive lease was replaced"
            )
        return lease
    ...
    def _validate_settlement_receipt(
        receipt: OwnedPublisherReconcileReceipt,
        *,
        attempt: OwnedPublisherReconcileAttempt,
        outcome: EffectAttemptOutcome,
    ) -> None:
        if not isinstance(receipt, OwnedPublisherReconcileReceipt):
            raise TypeError(
...
class SupabaseOwnedPublisherReconcileStore:
    ...
    @classmethod
    def from_environment(cls) -> SupabaseOwnedPublisherReconcileStore:
    ...
    def read_owner(self) -> PublisherArticleReconcileOwner:
        return _owner_from_payload(
            self._rpc("volpred_read_publisher_article_reconcile_owner", {})
        )
    ...
    def request(
        ...
        *,
        ...
        payload_bytes = encode_publisher_article_reconcile_payload(
            canonical_feed_sha256=command.canonical_feed_sha256,
            articles=command.articles,
        )
        payload = _mapping(
            json.loads(payload_bytes),
            field="publisher reconcile payload",
        )
        response = self._rpc(
        ...
        return OwnedPublisherReconcileRequest(
    ...
    def settle(
        ...
        if isinstance(outcome, AcknowledgedEffect):
            ...
            raise TypeError(
                "owned publisher reconcile provider returned invalid outcome"
            )
        response = self._rpc(
...
def _normalize_command(
    ...
    if not isinstance(command, OwnedPublisherReconcileCommand):
        raise TypeError("OwnedPublisherReconcileCommand is required")
    ...
    canonical_feed_sha256 = _required_text(
        decoded.get("canonical_feed_sha256"),
        field="publisher reconcile canonical feed hash",
    )
    articles = decoded.get("articles")
    if not isinstance(articles, list):
        raise RuntimeError("publisher reconcile articles must be a list")
    normalized_articles = tuple(
        dict(_mapping(article, field="publisher reconcile article"))
        for article in articles
    )
    return OwnedPublisherReconcileCommand(
        idempotency_key=_required_text(
            command.idempotency_key,
            field="owned publisher reconcile idempotency_key",
        ),
        canonical_feed_sha256=canonical_feed_sha256,
        articles=normalized_articles,
        actor_ref=_required_text(
            command.actor_ref,
            field="owned publisher reconcile actor_ref",
        ),
    )
...
def _owner_from_payload(
    ...
    owner = _required_text(
        payload.get("owner"),
        field="publisher reconcile owner",
    )
    if owner not in {_LEGACY_OWNER, _OPERATIONS_CORE_OWNER}:
        raise RuntimeError(f"unsupported publisher reconcile owner: {owner}")
    return PublisherArticleReconcileOwner(
        schema_version=_required_text(
            payload.get("schema_version"),
            field="publisher reconcile owner schema_version",
        ),
        effect_family=_required_text(
            payload.get("effect_family"),
            field="publisher reconcile effect_family",
        ),
        owner=owner,
        generation=_positive_integer(
            payload.get("generation"),
            field="publisher reconcile owner generation",
        ),
        changed_at=_required_text(
            payload.get("changed_at"),
            field="publisher reconcile owner changed_at",
        ),
        changed_by=_required_text(
            payload.get("changed_by"),
            field="publisher reconcile owner changed_by",
        ),
        change_reason=_required_text(
            payload.get("change_reason"),
            field="publisher reconcile owner change_reason",
        ),
    )
