...
_CAPABILITY = "incident.lifecycle"
_CONTRACT_REF = "contract://issue-13/durable-incident-owner"
...
class IncidentOwnerAttestation:
    schema_version: str
    capability: str
    owner: str
    generation: int
...
def _text(payload: Mapping[str, Any], field: str) -> str:
    ...
    if (
        ...
        raise ValueError(
            f"Incident owner RPC returned an invalid {field}"
        )
    return value
...
def _positive_integer(payload: Mapping[str, Any], field: str) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
        ...
        raise ValueError(
            f"Incident owner RPC returned an invalid {readable_field}"
        )
    return value
...
class SupabaseIncidentOwnerStore:
    """Read the durable incident-lifecycle owner without mutation authority."""
    ...
    @classmethod
    def from_environment(cls) -> SupabaseIncidentOwnerStore:
    ...
    def read_owner(self) -> IncidentOwnerAttestation:
        try:
            ...
            raise RuntimeError(
                f"incident ownership RPC failed: {exc}"
            ) from None
        if not isinstance(decoded, Mapping):
            raise TypeError(
                "incident ownership RPC returned a non-object response"
            )
        if set(decoded) != _EXPECTED_FIELDS:
            raise ValueError("Incident owner RPC returned invalid fields")
        if (
            decoded.get("schema_version")
            != "incident-owner-attestation.v1"
        ):
            raise ValueError("Incident owner RPC returned an invalid schema")

        capability = _text(decoded, "capability")
        if capability != _CAPABILITY:
            raise ValueError(
                "Incident owner RPC returned another capability"
            )
        owner = _text(decoded, "owner")
        if owner != "legacy":
            raise ValueError("Incident owner RPC returned an invalid owner")
        generation = _positive_integer(decoded, "generation")
        if generation != 1:
            raise ValueError(
                "Incident owner RPC returned an invalid generation"
            )
        contract_ref = _text(decoded, "contract_ref")
        if contract_ref != _CONTRACT_REF:
            raise ValueError(
                "Incident owner RPC returned an invalid contract"
            )
        ...
        if receipt_capability != capability:
            raise ValueError(
                "Incident owner RPC receipt capability drifted"
            )
        if receipt_owner != owner:
            raise ValueError("Incident owner RPC receipt owner drifted")
        if receipt_generation != generation:
            raise ValueError(
                "Incident owner RPC receipt generation drifted"
            )
        if receipt_contract_ref != contract_ref:
            raise ValueError(
                "Incident owner RPC receipt contract drifted"
            )
        if receipt_changed_at != changed_at:
            raise ValueError(
                "Incident owner RPC receipt changed_at drifted"
            )
        if receipt_actor_ref != changed_by:
            raise ValueError("Incident owner RPC receipt actor drifted")
        if receipt_reason != change_reason:
            raise ValueError("Incident owner RPC receipt reason drifted")
        if datetime.fromisoformat(changed_at) > datetime.fromisoformat(
            attested_at
        ):
            raise ValueError("Incident owner RPC chronology drifted")

        return IncidentOwnerAttestation(
