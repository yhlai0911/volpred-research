"""Keep first-party CI actions on their supported Node 24 major releases."""
...
def test_supported_node24_action_majors_are_used_everywhere() -> None:
    stale: list[str] = []
    ...
    assert not stale, "Deprecated GitHub Action runtime(s):\n" + "\n".join(stale)


def test_setup_uv_cache_pruning_is_explicit() -> None:
    missing: list[str] = []
    for path, job_name, index, step in _action_steps():
        if step.get("uses") != "astral-sh/setup-uv@v9":
            continue
