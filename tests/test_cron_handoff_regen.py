...
def test_handoff_regen_prefers_project_venv_python() -> None:
    ...
    assert 'PYTHON_BIN="/Users/yhlai0911/volpred-research/.venv/bin/python"' in text
...
def test_handoff_regen_uses_shared_python_runner_for_both_subcommands() -> None:
    ...
    assert (
        '"${PYTHON_RUN[@]}" '
        "/Users/yhlai0911/volpred-research/scripts/generate_handoff.py"
    ) in text
    assert (
        '"${PYTHON_RUN[@]}" '
        "/Users/yhlai0911/volpred-research/scripts/task_pool_claim.py cleanup --stale-hours 2"
    ) in text
