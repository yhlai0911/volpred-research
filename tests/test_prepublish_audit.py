...
# ── image-URL gate (2026-06-08 缺圖 incident) ───────────────────────────────
from volpred.publisher.prepublish_audit import audit_image_urls


def test_experiments_path_image_blocked():
    c = "![圖1](https://volpred.zeabur.app/experiments/k674/k674_dd_heatmap.png)"
...
def test_supabase_article_images_passes():
    c = "![圖1](https://qxhfgdfzazwpkdgesavm.supabase.co/storage/v1/object/public/article-images/k674_dd_heatmap.png)"
    r = audit_image_urls(c)
    assert r["total"] == 1
    assert r["broken"] == []
...
def test_no_images_is_clean():
    r = audit_image_urls("純文字內容，完全沒有任何圖片")
    assert r["total"] == 0
    assert r["broken"] == []


# ── non-stat label exclusions (2026-06-08 K1423 false-positive fix) ─────────
def test_tw_ticker_not_flagged_as_stat():
    # 0050 / 2330.TW next to 報酬/迴歸 must NOT be a provenance claim.
    from volpred.publisher.prepublish_audit import extract_numeric_claims
    c = "元大 0050 的迴歸 alpha 並不顯著；台積電 2330.TW 年化報酬率另計。"
...
def test_index_name_500_not_flagged():
    from volpred.publisher.prepublish_audit import extract_numeric_claims
    c = "標普 500、SPY 的迴歸顯示年化 alpha 為 9.2%。"
    raws = {x["raw"] for x in extract_numeric_claims(c)}
...
def test_methodology_constants_not_flagged():
    from volpred.publisher.prepublish_audit import extract_numeric_claims
    c = "t 檢定採 Newey-West HAC（lag=5）；年化 alpha = 日 alpha × 252。"
    raws = {x["raw"] for x in extract_numeric_claims(c)}
