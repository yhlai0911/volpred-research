"""Tests for `scripts/dreaming_review.py` â€” the loop-engineering slow loop.
...
def test_detect_stale_knowledge_skips_broad_review_false_positives(tmp_path):
    ...
    without repeating any specific reversed claim â€” that is a keyword-overlap
...
def test_detect_missing_retry_strategy_flags_orphaned_failure(tmp_path):
    ...
    _write(
        ...
        [
            ...
            # parked: has controlled blocked_reason â†’ not flagged
            {"id": "T2", "k_id": "K2", "status": "failed", "blocked_reason": "awaiting_external_data", "completed_at": _iso(2)},
            # has follow-up: K3 also has a succeeded sibling â†’ not flagged
...
def test_detect_semantic_concentration_flags_high_rehash(tmp_path, monkeypatch):
    # High semantic rehash rate â†’ finding (boss email-12139 semantic directive).
    monkeypatch.setattr(
        "volpred.ops.topic_similarity.semantic_concentration_report",
        lambda *a, **k: {
            "status": "concentrated", "sample": 20, "rehash_count": 8, "rehash_rate": 0.4,
            "near_twin_pairs": [{"title": "RECH-X è·¨å¸‚å ´", "twin": "AI æ³¢å‹•çŽ‡æ¨¡åž‹ RECH-X", "similarity": 0.77}],
        },
    )
...
def test_detect_semantic_concentration_fail_open(tmp_path, monkeypatch):
    # Embeddings down â†’ no finding (never breaks the run).
    monkeypatch.setattr(
        "volpred.ops.topic_similarity.semantic_concentration_report",
...
def test_detect_persistent_alerts_flags_recurring_root_cause(tmp_path):
    """Same alert_key fired N times across M days â†’ root cause unhandled."""
    storage = _storage(tmp_path)
    _write_alert_dedup(
        storage,
        {
            "abc123def456" + "0" * 52: {
                ...
                "last_sent_at": _iso(0.5),  # last fire 12h ago â€” still active
            }
        },
    )
    ...
    _write_alert_dedup(
        storage,
        {
            "k" * 64: {
                ...
                "last_sent_at": _iso(3),  # 3d ago â€” beyond 48h recovered cutoff
            }
        },
    )
    findings = dr.detect_persistent_alerts(str(storage), {}, NOW)
...
def test_detect_persistent_alerts_skips_short_burst(tmp_path):
    """5 fires in one hour â‰  multi-day persistence â€” needs M-day span."""
    storage = _storage(tmp_path)
    _write_alert_dedup(
        storage,
        {
            "b" * 64: {
                ...
                "send_count": 5,
                "first_sent_at": _iso(0.04),  # â‰ˆ1h ago
...
def test_reconcile_new_then_persisting(tmp_path):
    ...
    # second run: same signature â†’ not new, strike increments
...
def test_quiescent_activity_marker_holds_strike_no_escalation(tmp_path):
    ...
    strikes to critical â€” a decaying alert is resolving itself, not persisting."""
    baseline: dict = {}
    esc_final = []
...
def test_legacy_baseline_entry_no_marker_stays_conservative(tmp_path):
    ...
    the missing marker is treated as 'advance unknown' â†’ quiescent â†’ hold."""
    baseline = {
        "persistent_alert:legacy": {
            ...
            # NOTE: no "activity_marker" key â€” legacy schema
        }
    }
    f = dr.DreamFinding(
        pattern_type="persistent_alert",
...
def test_main_non_dry_writes_baseline_and_emails(tmp_path, monkeypatch):
    ...
    assert len(sent) == 1  # new findings â†’ one boss email
...
def test_a_warn_level_orphaned_failure_still_gets_queued(tmp_path):
    ...
    times. Critical-only therefore made auto_dispatch unreachable in practice â€” the
