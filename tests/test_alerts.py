...
def test_parse_loop_health_breaches_on_task_outcome_with_stable_title(tmp_path: Path):
    ...
    # Mostly-failed terminal tasks â†’ task_outcome degrading â†’ breach.
    _write_json(
...
def test_parse_content_quality_breaches_on_lazypack_gap(tmp_path: Path):
    ...
    good = (
        "æœ¬æ–‡å¼•ç”¨ K123ï¼Œè³‡æ–™ä¾†æºï¼šyfinanceã€‚\n\n"
        "![ä¸»åœ–](https://example.com/chart.png)\n\n"
        "## æ‡¶äººåŒ…åœ–çµ„\n\n"
        "![æ¦‚å¿µ](https://example.com/lazypack.png)\n"
    )
    missing = "æœ¬æ–‡å¼•ç”¨ K124ï¼Œè³‡æ–™ä¾†æºï¼šyfinanceã€‚\n\n![ä¸»åœ–](https://example.com/chart.png)\n"
    _write_json(
...
def test_build_alert_condition_report_flags_required_breaches(tmp_path: Path):
    ...
    # Per control-plane rule (v12): host_cron_fail åªçœ‹ storage/logs/cron/*.log æœ€æ–°
    # "=== exit N ===" é 0ã€‚scheduler_state staleness ä¸å† count (advisory only).
    _write_text(
...
def test_check_alert_conditions_sends_each_breached_condition_once(tmp_path: Path, monkeypatch):
    ...
    # disk usage forced low. (paper_trading absent â†’ gap check is ok.)
    ...
    # separately â€” surfaced by the 2026-06-29 loop-health work.)
...
def test_paper_stale_severity_and_isolation(tmp_path: Path):
    """M3 paper-line staleness (2026-06-21 boss email-11851/11854 å°ç¨±è£œå¼·): the whole
    ...
    # empty paper line (no .tex/.md anywhere) â†’ critical
