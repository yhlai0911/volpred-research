...
**規則 4 — 會被自己 kill 的程序，不可用 `finally` 當釋放外部資源的唯一手段（2026-08-04 補）**：selfreload 會 SIGTERM 自己、custody 失效會 SIGKILL worker，**兩者都不跑 `finally`**。因此凡是取得**跨程序可見、且別人會卡在上面**的資源（git `index.lock`、pid 檔、具名 lock），釋放路徑不能只有 in-process cleanup —— 必須讓「持有者已死」可以被**後續程序證明**：寫 owner sidecar（pid + `ps lstart` 指紋 + host + 時間戳），下一個 tick 自行回收自己的殘骸。**回收必須 fail closed，而且「沒有 sidecar 的鎖一律不准碰」** —— 那是真的 git 正在寫，刪掉就是毀掉一次 index 更新（這正是天真的 stale-lock cleaner 會犯的錯）。liveness 只認 `IDENTITY_DEAD` / `IDENTITY_MISMATCH`，**`unverified` 代表探測本身失敗、什麼都沒證明，不可放行**；另加最小年齡避免撞上 adopt 中途。**機械 owner**：`phase_z.reclaim_leaked_index_lock()`（liveness 判定沿用 `procutil.check_identity`，不另立第二套）。
...
**規則 5 — 長命 daemon 的 fail-closed provider gate 必須配對 spawn 前 fail-safe strip（2026-08-04 補）**：provider child 根本不該收到的 forbidden auth 變數，spawn 路徑在 authorize 前用 `sanitize_provider_spawn_environment()` strip 掉（只 log key 名），不是讓 gate 把整條 backbone 停擺。對偶規則：library constructor **禁止 bulk-load .env 進 `os.environ`**（EmailNotifier 曾把 OPENAI_API_KEY 灌進 daemon → 全派工停擺而 ops_snapshot 全綠）；`ps eww` 只能排除 boot-time env 來源，對 runtime 注入無效。**機械 owner**：registry sanitize + authorize（同 owner）+ `tests/test_email_notifier.py` 的 allowlist∩forbidden==∅ cross-gate。
**代表 incident**（全文見 archive）：
- 2026-08-04 **EmailNotifier 把 .env（含 OPENAI_API_KEY）灌進 daemon os.environ，provider registry fail-closed 全派工停擺**：in-process alert 鏈（phase_z→alerts→owned_email→EmailNotifier）runtime 注入；`ps eww` 乾淨造成前一輪誤排除。修：投遞域 allowlist + spawn 前 strip（3da9b93f4）— Q3
- 2026-08-05 13:32 **3-STRIKE×6 FIXED：leader 乾淨退出後的殘留子程序被當成失敗，六班已成功的工作被丟棄**：`_run_one_attempt` 在 `proc.wait()` 返回後檢查 producer cohort，發現殘留就**只 log 然後放棄**（`TIMEOUT_SURVIVED_SENTINEL` → `outcome=kill_failed_orphan` → task re-pend → 下一班重做已完成的工作）。六次全同形（8/4 ci-root-30875762101、ci-red-30884267057×2、ci-red-30911746339×2；8/5 ci-red-30973884810），每次 agent 都**已寫完 final text**，殺死它們的是沒收乾淨的 MCP server / 背景 shell。**不對稱是 bug 不是設計**：timeout 分支早就 killpg 整棵樹（§A 規則本體），乾淨退出卻不殺 —— worker 擁有該 process group，收孤兒是它的責任，不是丟棄 agent 成果的理由。修：leader 退出後對可證明屬於自己的 cohort 主動 `_kill_pgid`（reason=`orphan_reap_after_leader_exit`）+ settle 後重讀；**收不掉仍 fail closed**、`None`（無法證明歸屬）一律不碰。雙向回歸測試各一，398 dispatch 測試綠。三張被 block 的 CI 單已 unblock。**流程教訓**：發現第 6 次時我先 block + 補證據到既有單（= 止血 + 排隊），被老闆當場糾正「為什麼不立即處理」—— 這正是 3-strike 明令禁止的 reaction；看見結構性根因就該當場三層重構 — Q3
- 2026-08-05 11:35 **3-STRIKE×91：selfreload 被 monitored-source fail-closed 永久封鎖，daemon 跑舊碼 17 小時**：`release_image._committed_entries` 在任何 monitored source 未 commit 時拒建 release；01:47 起連續失敗 **274 次**、`supervisor_started_at` 停在 08-04T10:24Z。**矛盾**：monitored source（`src/volpred/ops/**`、`scripts/dispatch_supervisor/**`）同時是 Codex loop / phase_z / 主線程三方活躍寫入區，穩態幾乎永遠有 dirty 檔 → fail-closed = selfreload 永久失效。本次連環卡：`task_signature.py` 孤兒改動 15h（主線程 adopt commit 解除）→ `state.py` 立刻接手擋住（mtime 距嘗試 1 分鐘，活躍寫入不可碰）。**兩個衍生教訓**：(a) 274 次失敗**只寫 log、零告警**（`dispatch_supervisor_stale_code` 未觸發）— fail-closed + silent retry 是 no-silent-fallback 規則的變體違反；(b) **我因此誤宣稱「refractory 全面攔截已驗證生效」** — 實際 daemon 跑舊碼，只驗到舊碼本就有的 e2 攔截，e1 攔截從未上線。跑舊碼的 daemon 與健康的 daemon 觀測上同形，這正是 §A 早已寫過的坑，這次是**驗證者自己**踩進去：驗證「新行為生效」必須先證明「新碼已載入」（比對 `supervisor_started_at` vs commit 時間），不可只看行為面。修復單已入池 — Q3
- 2026-08-04 18:30 **foreign incident backlog 歸零（44 條 blocking → 0、9 張 open → 0、derate 解除 cap 2→4）**：收尾三步 = (1) refractory：episode ≥2 不再 out-of-band request_fire（dirty 集合每變一次就插班燒 slot 的回路，今日燒 4 班；`repeat_episode_refractory` + test）；(2) 「產生 vs 落地」兩 owner 分離的首次實走：唯讀 fire 5374936c 產出 15 條 per-file 裁決（每條附 live 驗證），主線程以 `foreign_disposition.py --apply` 落地（PRECONDITION：依賴閉包三檔先 quarantine-checkpoint 再入 disposition，防 ImportError 上 main）；(3) 最後一條 K1342 README errata 假宣稱 — working tree 已有讓宣稱為真的 INTERVAL 修正（agent 只查 HEAD/quarantine/worktrees 漏了 main working tree），驗證後兩檔同 commit。教訓：incident 補救的 durable 通道（裁決 JSON 交接）目前只有 agent final text 一條路，sandbox 擋掉 receipt — 通道制度化屬修復單 scope — Q3
- 2026-08-04 17:10 **phase-z foreign incident「不死單」三重根因與清場**：(1) `_git_lines` 不解 git C-quoting → CJK 檔名永遠判不出 quarantine 覆蓋 → 2 張 incident 數學上關不掉 → derate 取最舊 open 單為由把 slot cap 永久釘在 2/4（93f727e5 班機械診斷；修 `-c core.quotePath=false` + CJK regression test）；(2) `incident_closeable` 對已 commit 進 HEAD 的乾淨路徑仍要求 quarantine ref → landed 路徑永遠 uncovered（修 ls-files landed 出口 + test）；(3) compute queue `source_task_terminal:succeeded` 自動取消把 erratum 的 follow-up review gate silent 砍掉 — 「成功後才跑的 gate」被「成功」取消，研究產物差點永久卡 working tree（止血：K1259 fallback 重審 CONDITIONAL_PASS 後落地；契約 bug 單已入池）。同場清出機器產物半場（gitignore diagnostics/graphify + lazypack 38 檔按慣例 commit + foo.jsonl 退場），incident 阻塞路徑 44→16，剩餘全為需 per-file 收養決策的真人工作檔（D1 不猜 bytes，交修復單）— Q3
- 2026-08-04 15:43 **3-STRIKE TRIGGER：連續三班 fire 以 `kill_failed_orphan` 收場（hang 到 timeout 被殺 + producer pgid 殘留）**：14:22 job 3175c52f（ci-root-30875762101, 1150s）、15:13 job 1675be8d（ci-red-30884267057, 931s，log 僅一句「I'll wait for the monitor」= agent 在 fire 內無界等 CI）、15:43 job 592ef34e（同張 ci-red 第二次, 1197s，agent 其實完成 CI 修復報告但 8 個子程序不退 + fire_receipt 撞 sandbox EPERM）。共同形狀 = **CI 修復任務的 fire 執行契約缺三件事**：禁止 fire 內等 CI 綠燈（等待屬 ci watcher 收口）、子程序生命週期收束（agent 收工 ≠ pgid 淨空）、receipt 寫入路徑與 sandbox 相容。止血：ci-red-30884267057 已 block（prior_attempts_failed）；重構單已入池（見 assign 2026-08-04 07:5xZ）— Q3
- 2026-08-04 13:49 **index.lock class 第 4 次：無 sidecar 的原生 lock 落在 reclaim 盲區**：sidecar 機制（d6fedf171）只蓋 phase_z 自建 CAS lock；某個 git 原生操作在 13:49:47 留下 0-byte 無 sidecar lock → drain 的 shared-index refresh fail-closed（`committed_recovery_index_failed`），commit 3e642ea0b 成功但 shared index 落後 HEAD（staged-D+untracked 假象）。主線程人工回收（pgrep 無 git、>5min、drain 已棄；`git_writer_lock run` 內 rm+reset）。來源未定 → followup task 已入池（assign 2026-08-04 05:59Z，P2）；fail-closed 行為本身正確，別因此放寬「無 sidecar 不碰」
- 2026-08-04 **PHASE-Z 洩漏 `.git/index.lock`，卡死全 repo 的 writer**：`_refresh_shared_index_cas` 不是「等」git 的 index lock，而是**自己建**（`os.open(O_CREAT|O_EXCL)`）、把 index 複製進去當 scratch、最後 `os.replace` 採用。釋放只靠 `finally`，但 selfreload 的 SIGTERM 與 custody SIGKILL 都不會執行它。一旦洩漏，`git write-tree` 失敗 → `git_writer_lock.py commit` 全數回 `cannot snapshot current index`；而 git 的 `index.lock` **不帶任何持有者身分**（存在即宣告），所以沒人能區分「持有中」與「已棄置」。2026-08-04 單一 session 內洩漏三次（43 分 / 80 秒 / 43 分），每次都靠人工判斷「這鎖看起來夠舊了」才敢刪 —— 那是紀律，不是機制。修法：acquire 時寫 owner sidecar，**adopt 與 abort 兩條出口都清**（`os.replace` 會吃掉 lock 但留下 sidecar，殘留的 sidecar 會去描述**別人**之後建立的鎖，比沒有更危險）；`run_phase_z` 進入時先自我回收。實測驗證：真的 spawn 一個子程序持鎖 → 存活時回 `holder_match` 不動它，SIGKILL 後才回收並留 receipt。8 條新測試、146 條 phase_z 測試通過 — Q3
...
**代表 incident**：
- 2026-08-02 **ROOT_CAUSE_FIXED_AND_VERIFIED** NFP 2026-08-07 T-7 孤兒產物把「發佈前 7 個日曆日」的文章排程槽、「第 7 個交易日收盤」的研究資訊集與最新 T-6 snapshot 混成同一天，又對重疊 controls 使用 iid Welch，導致線上文章的 7/30 VIX=17.09、n=1,485／p=0.342 與「事件日穩定」敘事不成立；正式化為 pinned network-free experiment，改用 7/29 VIX=20.66、exact-overlap controls n=2,062 與 HAC(22) p=0.583，經 canonical update gateway 四次 errata 後 live URL=200，舊 claim 已不再承載讀者面結論、只於 errata／audit trail 留存。locked commit `ea336b823`、committed-tree 172 tests／artifact／methodology／certify、clean-clone 198/198 scalar reproduction、orphan reaper 與 task succeeded receipt 均已回讀；完整五步證據：`docs/error_log_archive/2026-Q3-nfp-t7-information-set.md`
...
**代表 incident**：
- 2026-08-02 **root_cause_fixed_and_verified**（`assign_6383dffb`）：被拒絕的 `codex/k1707-pseudo-stress` branch 有可泛用的 exact label permutation／Holm 邏輯，但 main 的 K1380 又手寫第二份，且全庫實際已有 41 個歷史 Holm-family helper；根因是 canonical seam 不存在，也沒有 forward ratchet。新增 `volpred.stats.inference` 深模組、K1380 改走 canonical，normalized scientific payload 完全不變，並以 tracked-only baseline 阻擋新副本、stale active 與 retired resurrection。雙軸 review 先後抓出 live-filesystem 汙染、絕對容差吞 tiny effects、未置中造成 translation drift、baseline resurrection、branch GC、事後手寫 reproduce spec、network-enabled clean clone、refresh provenance 誤標與 multi-output partial generation 等缺口，全部補回歸後 PASS。canonical finalizer 現 hash-bind result/spec/declared outputs 並最後寫 completion receipt；K1707 的 401 MB raw acquisition 已與 network-denied analysis 分離。兩個 committed disposable-clone rerun 分別 184/184 與 132/132 scalar match、皆 `pass_tolerated`、canonical source 回讀 unchanged。拒絕 branch 的科學結果未採用；5 個 commits 存成 74 KiB Git bundle，fresh bare repo 還原 tip/result SHA 成功，bundle 先進 commit `3fa0e2550`、核心實作進 `ee8d9b4cd`、artifact contract 進 `1c96138ea` 後才精確刪除 local branch，刪後再驗 bundle PASS 與 branch absence。
...
**規則（可終止性延伸到 `warn()`，2026-08-03 補）**：上條「每個 alert 條件都要有它怎麼停的答案」**同樣適用於每一則 `warn()`**。週期性流程若在「無事可做」的分支只 warn 不寫終止狀態，它下一輪必然再走同一條路 —— 那不是通知，是空轉。寫 `warn()` 前先問：**呼叫端下一輪還會不會再看到同一件事？會 → 缺的是終止邊界，不是一行警告。** 且 `warn()` 預設只進 stderr，在排程 wrapper 裡等於進了沒人讀的 per-job log；`.claude/rules/no-silent-fallback.md` 要求的可觀測性到此為止會斷掉。**機械 owner**：排程程序由 `scripts/cron_lib.sh` 開啟 `VOLPRED_DIAGNOSTICS_PERSIST`（有 size cap + 單代輪替），`alerts.py::_parse_recurring_diagnostic_warning_state` 把「同一則警告持續重複且仍在燒」升級成 alert → 走既有預設處置變成 task。
...
**代表 incident**：
- 2026-08-03 **root_cause_fixed_and_verified（控制面）** release pool 真實 `eligible=0`、7 張 daily_article 已在 queue 且多數已升 P1，但「乾池偵測／補池」只存在於被動 dispatch report，Operations Core 每分鐘 tick 沒有把 release stock 接到 worker admission；單一安全 slot 又被 recurring CI preempt 連續取得，因此「任務存在」不等於「文章會被執行」。修成 model-free article-continuity actuator：每分鐘用 release path 自己的 preview 判斷，乾池時批次升級但只 nominate 最老一張 continuity preempt，durable `request_fire` 在 slot 忙碌時保留；supervisor 對 scheduled preempt 使用顯式 rank，human urgent／time-critical 仍在外層；已有文章 in-flight 禁止並行 nomination，池恢復後只清自己 marker。live 回讀已見 `fire_request_reason=article_continuity:K1706_article_general` 且當下 CI job 仍 running，證明 request 未被忙碌 slot 吃掉。另修 Operations Core `schedule_receipts.json` 無限增長（25,171 fires / 27.6MB）為保留全部 actionable、terminal/shadow 有界壓縮，避免 atomic rewrite 再次耗盡磁碟。全文：`docs/error_log_archive/2026-Q3-article-continuity.md`
