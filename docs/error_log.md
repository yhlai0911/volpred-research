...
| 2026-04-13 | IS-based regime cutoffs degenerate when OOS 含 unprecedented volatility（K1128 教訓，K1131 2026-04-17 補充驗證） | K1128 VIX tertile split: IS 2017-2019 VIX 9-37 vs OOS 2020-2021 VIX 15-82 (COVID)，IS quantile cutoff 套 OOS 變 low tertile=0 bars + mid 854 + high 20060 | IS quantile 邊界在 unprecedented event 下失效 — 所有 IS-based threshold 都有此風險 | (1) IS 擴含 prior crises (2008/2011/2015) → K1132 待測 (2) Expanding-window adaptive quantile → K1133 待測 (3) ~~連續 VIX-dependent β via spline~~ → **K1131 (2026-04-17) empirically INVALIDATED**：spline OOS DM t=-3.94 **反向**（顯著劣於 tertile），IS 外推到 OOS VIX=82 爆炸（f_abs range [-0.58, +20.43]），AUC=0.4965 below chance。結論：K1128 degeneracy 是**結構性問題**（OOS regime disjoint IS），不是 functional form 問題。Discrete/smooth 皆 fail (4) Rolling quantile → K1134 待測。診斷：套 cutoff 前先 `assert OOS_low_count > 0 and OOS_mid_count > 0`。影響範圍：regime-switching GARCH、HMM、K1121 NFCI threshold（需回查）。已記 E064 |
