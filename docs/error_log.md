...
| 2026-04-06 | LaTeX 方程式渲染壞掉（`au_t imes g_t`） | 前端 KaTeX 應渲染 `\tau_t \times g_t` 但顯示 `au_t imes g_t` | `publisher.py` L204: `description.replace('\\t', '\t')` 把 JSON 中的 `\tau` → TAB+au、`\times` → TAB+imes。`\t` 是 JSON 的 TAB 轉義，被盲目替換成 TAB 字元後 KaTeX 無法解析 | 根本修正三層：(1) `publisher.py` 移除 `\\t`→TAB 替換，`\\n`→newline 改為 `re.sub(r'\\n(?![a-zA-Z])', '\n', ...)` 保護 `\nu`/`\nabla` (2) 二進制修復 9 篇文章的 JSON（確保 `\\tau` 雙反斜線）(3) Supabase sync 更新。教訓：**不可盲目做字串替換——`\t` 在 JSON/LaTeX/Python 各有不同意義，替換前必須考慮上下文** |
