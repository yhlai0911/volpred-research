...
**代表 incident**：
- 2026-08-02 **ROOT_CAUSE_FIXED_AND_VERIFIED** `publisher_arc_dedup` 已是 warn-only，NFP T-7 也在 warning 後 6 分鐘成功發佈，control-gate lifecycle 卻因 `candidate_or_feed` 未把 event key/slot 納入 feed identity 而誤記 `unjoined` harm；統一 feed identity 現涵蓋事件 stage，回放改判 `published=1, unjoined=0`，不復活內容鎖 — `docs/error_log_archive/2026-Q3-publisher-arc-event-join.md`
