...
**代表 incident**：
- 2026-08-02 **contained**（`assign_6383dffb`）：被拒絕的 `codex/k1707-pseudo-stress` branch 有可泛用的 exact label permutation／Holm 邏輯，但 main 的 K1380 又手寫第二份，且全庫實際已有 41 個歷史 Holm-family helper；根因是 canonical seam 不存在，也沒有 forward ratchet。新增 `volpred.stats.inference` 深模組、K1380 改走 canonical 且完整重跑 result SHA 不變，並以 tracked-only baseline 阻擋新副本、stale active 與 retired resurrection。拒絕 branch 的科學結果仍未採用；5 個 commits 已存成可驗證／可還原 Git bundle。待 bundle 先 commit、精確刪除 local branch、terminal receipt 回讀後才可升 `root_cause_fixed_and_verified`。
