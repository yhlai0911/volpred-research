...
## 2026-07-16 22:27 — 宣告「完成」後 41 分鐘同一條通道再次靜默：responder 無重試，且它自稱的兜底是假的

**現象**：20:50 我向老闆宣告 Telegram 回覆端「已修好並驗證」（jq FATAL 修掉、responder 實跑
exit=0、訊息確實回出去）。**21:46 老闆再傳一則，又沒動作**；22:27 他只問了兩個字「完成了？」。
查下去：`telegram-877` 已 pending 41.4 分鐘 —— 上一輪才加的 `telegram_reply_backlog` alert
**正確報了 warn**（這條 alert 上線 2 小時就抓到它要抓的東西），但那不能抵銷「我宣告完成時，
它其實沒完成」。

**根因（第三層，比 jq 更深）**：
1. **21:46 撞的是 session limit（10:20pm reset），不是 weekly limit** — responder 三輪 exit=1。
2. **responder 是純 event-driven**：`_spawn_responder()` **只**從 `handle_message()` 呼叫 ——
   只有「新訊息進來」會 spawn 它。它一失敗，**沒有任何東西會重試**。22:20 額度恢復了也沒用：
   訊息就躺著，直到老闆**再傳一則**，才被下一個 responder 的 drain loop 順便清掉。
   → 於是「老闆問為什麼沒回應」本身成了唯一的復原機制。
3. **它自稱的兜底是虛構的**：`telegram_poll.py` 有三處註解寫「hourly dispatch 兜底，最壞 ~1h」，
   但 `.claude/rules/task-routing.md:40` 明確把 `telegram_reply` 標為「**不進一般 hourly/Codex
   claim**」。**那個安全網不存在** —— 所以失敗完全沒有地板。三處註解讓每個讀者（包括我，上一輪）
   都以為最壞情況有界。
4. **附帶的謊**：`_spawn_responder()` 在 `Popen` 成功時 return True —— 那只說明「行程起來了」，
   完全不說明「訊息被回了」。fallback ack 只在 `Popen` 自己拋例外時才發（幾乎不可能）。
   所以「spawn 成功 → 立刻 exit=1」這條路徑上，老闆收到全然的沉默，daemon 卻記錄成功。

**修正**：poll daemon 本來就在 `while True` 裡跑，**讓它兼任 retry driver**（不新增排程層）：
每個 pass 在收訊之後跑 `_retry_stuck_replies()` —— 有 `telegram_reply` pending 超過 2 分鐘就
重新 spawn responder。5 分鐘 backoff：額度耗盡時 responder 是**秒退**的，而 poll pass 只有 ~25s，
沒有 backoff 會在「額度正是問題」的時候每 25 秒重燒一次 opus。只重試 `pending`；`claimed` 不碰
（那是 responder 正在做，或卡死 → 屬 backlog alert 的 critical>2h 面）。三處假註解一併改掉。

**驗證**：telegram-877 22:30:32 succeeded、alert 回 info/0；daemon 經 kickstart 重載，
code 22:33:41 / daemon start 22:34:35 → 跑的是新碼不是 7/10 的記憶體副本。
Tests：9 cases（該重試時重試 / 新鮮·已claim·已回時不動 / backoff 收斂重複 pass / backoff 會過期 /
壞掉的佇列不能弄死 daemon）。

**教訓**：
1. **「我驗證過它能動」不等於「它壞掉時能自己回來」**。上一輪我實跑 responder → exit=0 →
   宣告完成。那只證明**修好的那條路徑在健康狀態下可走**，完全沒測「它再壞一次會怎樣」——
   而它 56 分鐘後就再壞一次。修復的驗收標準必須含**失敗態的復原**，不只成功態的重跑。
2. **註解裡的安全網要當成宣稱來查證，不是當成事實來相信**。「hourly dispatch 兜底」白紙黑字
   寫在三個地方、跨越好幾次修改都沒人質疑，但 routing rule 從頭到尾就排除了它。
   **交叉查證 owner rule，不要相信被保護方自己的說法。**
3. **event-driven + 無重試 = 把復原責任外包給老闆**。凡是「只在外部事件到達時才執行」的處理器，
   失敗後若無獨立 driver 重試，它的復原條件就是「同一個外部事件再來一次」——
   對聊天通道而言，那就是老闆再問一次。這與 §J「探針要架在 outcome 上」同源：
   **這次是 recovery 也必須架在 outcome 上**（佇列殘留），不是架在事件到達上。
4. **不要用自己造的假環境當儀器**（同回合第二次犯）：我用 `env -i PATH=/usr/bin:/bin` 測
   `codex --version` 失敗，差點宣稱 Codex failover 壞了。實際上 `codex_failover.py:120` 早就
   處理了 `env node` shebang，且 supervisor daemon 的真實 PATH 首項就是 nvm bin ——
   **failover 是健康的**。測試環境不等於執行環境，這跟本案主根因（拿 ingress 心跳當 outcome 儀器）
   是同一個錯誤模式。

## 2026-07-19 17:xx — K1623 rev2：Codex 判「no CRITICAL/HIGH」的那份 code，四週後被判 FAIL（7 條）；漏掉的全部是「宣稱 vs 證據」不是「程式對不對」

**Class**: §L（裁決 / review-cert）主因 + §G（DM-HAC 硬規則）連帶
**觸發**: `dreaming_orphaned_experiment_k1623` → task `assign_5aa9d5f5`（P2, experiment）

### 事實

同一份 `experiments/k1623/k1623.py`，兩次獨立 review 直接衝突：

| 輪次 | Reviewer | 判定 |
|---|---|---|
| 2026-07-04 | codex-cli 0.142.3, **gpt-5.5** | **no CRITICAL / HIGH** |
| 2026-07-17 | codex **gpt-5.6-sol**, reasoning=high | **FAIL（7 條理由）** |

**算術完全一致 —— 兩輪沒有任何一個數字對不上。** 分歧 100% 落在「README 對人類說的話，
有沒有被程式產生的數字支撐」。

### 前輪 review 抓到什麼、漏掉什麼

**抓到（rev2 複驗，全部仍然成立）**：無 lookahead（HAR `Xall[i+1]` / ARFIMA one-step /
`latest_break(logrv[:i+1])` / EWMA 遞迴全部只用 ≤ i 資料）、QLIKE orientation 正確（actual/pred）、
GPH 正負號、fracdiff 權重、Bai-Perron DP+BIC、ARFIMA one-step 索引。
它甚至抓到並修掉了兩個 minor（ARFIMA d 的 silent except fallback、forecast clip 未 instrument）。

**漏掉（全部由 rev2 撤回或修正）**：
1. **識別宣稱無定理支撐** —— 扣掉 ≤5（permissive ≤15）個**決定性** Bai-Perron mean break，
   在數學上扣不掉 Diebold-Inoue 所設想的**隨機／密集** shift 過程。故「demean 後 d̂ 仍 >0」
   與「Diebold-Inoue 為真」**完全相容**，不是它的反證。README 卻寫「純假象假說被拒絕」。
2. **DM 只跑了 2 個 loss 中的 1 個，而被省掉的那個方向相反**。§4.5 寫「QLIKE + MSE ... DM + HLN」，
   實際 `run_oos` 的 `dm_vs_HAR` 只吃 QLIKE 向量；MSE 只存 summary mean 從未檢定。
   補跑後 ARFIMA 的 MSE 在 5 資產中有 4 個**低於** HAR（11–16%），5 個資產有 3 個排序反轉。
3. **`for lag in range(1, h)` 在 h=1 是空迴圈 → 40 個 DM 全部沒做 HAC**（§G 老坑再現）。
4. **「多處反而顯著更差」與結果表不符** —— 10 個 focal QLIKE 比較實際只有 **1 個**名目顯著，
   且 BH FDR 後 **0 個**存活。20 個 DM 比較**完全沒做多重比較修正**。
5. **「不可交易」是純修辭** —— 任何一輪都不存在策略／成本／效用測試。
6. **ELW 方法描述不實** —— 稱「Shimotsu-Phillips 可估非平穩 d」，實作只做 sample-mean demeaning，
   非 SP 的 μ̂(d) 加權；對 VIX（d̂=0.723>0.5）樣本平均根本不是有效 level 估計量。
   且 `FD_MAXK=2000` 對 n=2,565–4,655 **每個資產都 binding**。
7. **BreakRobustHAR 描述與 code 相反** —— README 稱「只用 latest-break 之後樣本」，
   `k1623.py:471` 的 `wstart = max(0, min(brk_start, i-22-60))` 在斷點太近時**刻意把窗口推回斷點之前**。
8. **VIX permissive 斷點選到 15/15 = cap binding** → 宣稱的 20.3%「上界」不構成上界。

### 根因

**code review 與 claim review 是兩個不同的檢查面，而我們只機械化了前者。**

一個 code reviewer 的預設問題是「**這支程式有沒有照它想的跑**」。
它天生不會去問「**README 第 3 行那句話，是這份 JSON 撐得起來的嗎**」——
除非 brief **明確**把 README 宣稱逐條列為受審對象。前輪 brief 沒有，所以前輪沒查。
這不是 gpt-5.5 比較笨，是**被問的問題不一樣**。

這與 §G 的 K1709 是同一個 class 的兩面：K1709 是「自帶測試全過但違反 repo 硬規則」，
本案是「code review 全過但 README 超譯」。共同結構 = **通過的那道 gate，量的不是會出事的那個維度。**

### 已做的修正

- **rev2 撤回 3 條 + 修正 5 條**，逐條 before/after/basis 進 `k1623_rev2_results.json.retracted_claims[8]`。
- **補齊 40 個 DM**（5 資產 × 4 對照 × 2 loss），全改 canonical `dm_test`（Newey-West, lag=10），
  加 BH FDR + Bonferroni，family 明確定義（`within_loss_20` / `focal_10` / `pooled_40`）。
- **HAC 是雙向的**：31 格 |t| 縮小、9 格放大（4 個 acf(1)<0 的**全部**放大），3 格 5% 判定翻面。
- **資料 vintage pin 回第一輪**，避免把「檢定被修好」與「樣本移動」混淆；
  4 資產 1e-9 完全重現，TW0050 因上游快取**修訂**（非延長）無法精確重現，**如實揭露**（偏差 2.51e-3）而非強行對齊。
- **補 generated-regressor Monte Carlo**（500 reps, seed=42）量化 SE 低估：真實抽樣 sd 是漸近 SE 的
  1.21–1.33 倍。**但如實報告與批評方向相反的發現** —— 斷點估計本身只貢獻 1.01–1.10×，
  低估主因是 1/(2√m) 漸近公式本身過度樂觀，不是 generated-regressor。
- 依 K1259 規則**不寫入 `knowledge.json`**（FAIL 的實驗不得收編）。

### 教訓（可執行）

1. **「Codex 判 no CRITICAL/HIGH」≠「宣稱與證據相符」。** 前者是 code 的性質，後者是 README 的性質。
   引用前輪 verdict 當 closure 前，先問「那輪到底被要求查什麼」。
2. **審查 brief 必須把 README 宣稱逐條列為受審對象。** `experiment_gates.py` 的 claim surface
   已經包含 `README.md`（「overclaim 是透過 README 抵達人類的」）—— 但 **surface 涵蓋 ≠ reviewer 會去看**。
   派 review 時要明寫「逐條列出 README 的每個 headline 宣稱，並指出支撐它的 JSON 欄位；找不到就報 FAIL」。
3. **「我們報了 A 和 B 兩個 loss」要查的是「有沒有對 A 和 B 都做檢定」，不是「有沒有算 A 和 B」。**
   本案 MSE 一直都在 JSON 裡，只是從未被檢定，而它方向相反。**存了 ≠ 檢定了。**
4. **null 結果不是安全區。** 「反正結論是 null，做不做 HAC 都一樣」在本案是錯的：
   9 格 |t| 反而放大。§G 已記 k621 同型教訓，本案是第二次坐實。
5. **修「宣稱 vs 證據」時不要順手改 code。** 本輪刻意選「改描述對齊 code」而非反之
   （brief 選項 (i)）—— 改 code 會讓所有已驗證數字失效，把一個 claim 修復任務變成重跑任務。

## 2026-07-21 07:xx — K1623 arm C：review subagent 執行了它被指派「審查」的實驗碼，先審後跑被繞過

**Class**: C（審查認證）。**嚴重度**：MEDIUM（未污染最終結論，但把 review 的獨立性打穿）。

**發生什麼**：K1623 arm C 的 `k1623_rev3_armc_mc.py` 派給 `feature-dev:code-reviewer` subagent 做
fresh-context review（Codex usage limit 到 2026-07-25，走 fallback path）。該 subagent **不只讀 code，
還把它跑了**，於 2026-07-21 00:32 台灣時間產出一份 artifact —— 用的是**尚未套用 wording fix 的 bytes**、
**不在 compute queue 內**、**沒有經過 runner 的 post-run experiment gate**。

**為什麼是問題（不是「反正數字一樣」就沒事）**：
1. **先審後跑的順序本身就是控制項**。審查者跑出結果後，它對 code 的判斷就不再獨立於它看到的輸出 ——
   「跑得出漂亮數字」會回頭汙染「這段 code 對不對」的判斷。
2. **繞過 gate 的 artifact 會被誤當成結果**。該檔若留在 `experiments/k1623/`，下一個收件人分不出
   它和排隊跑出來的正式產物。
3. 這是 K1259 教訓（subagent fallback PASS ≠ primary-path PASS）的鄰居：**fallback path 的紀律
   比 primary path 更容易鬆**，因為它是臨時繞道。

**當時的處置（正確，記錄備查）**：該檔已移出實驗目錄到
`/tmp/k1623_quarantine/armc_prefix_unauthorized_run.json`、**未 commit**，且其數值以
`preview_numbers_not_citable` 明標 **NOT CANONICAL — DO NOT CITE** 寫進 `k1623_arm_c_decision.json`，
只作為「排隊跑出來後要對帳」的對照。

**本輪收件的對帳結果**：正式 run 與該 preview **完全一致**（bias 分解 4dp 逐格相符、sd factor ranges
4dp 相符、dominant factor 5/5 相同）。**這證明當時的隔離處置沒有掩蓋數值問題，但不改變流程違規的定性。**

**根因**：review brief 只寫了「審查這份 code」，**沒有明寫「禁止執行」**。subagent 把「跑一次看看」
當成盡責的審查手段 —— 在多數 code review 情境這是對的，在**實驗碼**情境不是，因為實驗碼的執行
本身就是受管制動作（要進 queue、要過 gate、要有 provenance）。

**修正方向（建議，尚未落地 —— 本輪 worktree agent 不改 `.claude/` 共享面）**：
`.claude/skills/autonomous-research/references/agent-brief-template.md` 的 review brief 段落應加一條
硬性 scope 限制：「**只讀不跑**：禁止執行受審實驗腳本；需要看輸出就引用既有 artifact，
或把執行需求回報給主線程走 compute queue」。這條要放進 brief template 而不是只靠 reviewer 自覺，
理由同 anti-stacking —— 規範要在**派工當下**送達，不是事後靠 prose 提醒。

**追蹤**：`experiments/k1623/k1623_arm_c_collection_verdict.json` →
`process_gap` / `recommended_hardening` 欄位。

## 2026-08-04 — 長命 daemon 的 runtime os.environ 被 library import side effect 汙染，provider registry fail-closed 造成全派工停擺（ops_snapshot 全綠）

**症狀**：dispatch supervisor 每次 fire 都被 provider registry 拒絕：
`provider environment contains forbidden API-key/alternate-auth variables ['OPENAI_API_KEY']`
→ `outcome=provider_policy_denied`，`current_job` 永遠 None。同時 heartbeat 新鮮、
fire 照排程觸發、ops_snapshot 全綠 —— 「看起來健康」的零執行。3 張 P1 卡死
（assign_ec2a9ee7 本單、assign_ce6097bf erratum 重跑、assign_078bd8a2 tombstone 缺口）。

**為什麼前一輪查不到（矛盾點）**：`ps eww -p <pid>` 顯示 supervisor 環境只有 11 個乾淨變數
—— 但 `ps eww` 看到的是 **boot-time** env；被驗證的 `child_env` base 是
`external_child_environment()` 預設的 **runtime `os.environ`**。兩者在 daemon 啟動當下相等，
任何 runtime 注入後就分岔。「process 環境乾淨」的排除證據對 runtime 注入類 bug 無效。

**根因鏈（機械證實）**：phase_z 在 daemon **in-process** 發 alert →
`volpred.ops.alerts.send_alert` → `volpred.ops.delivery.owned_email` →
`EmailNotifier.__init__` → `_prime_project_env()` **把整份 `.env` / `.env.local` /
frontend `.env.local`（23 個 key，含 OPENAI_API_KEY、SMTP_PASSWORD、TELEGRAM_BOT_TOKEN、
GOOGLE_CLOUD_API_KEY、SUPABASE_SERVICE_ROLE_KEY）灌進 process-global `os.environ`**。
乾淨 env 下 `EmailNotifier(storage_dir=tmp)` 一行即可復現注入。時間線吻合：12:00 CLI
自動升級觸發 pin 拒絕 → 前一輪修 pin 的同時 alert 已在 daemon 內把 env 汙染 →
12:38 pin 修好後拒絕理由換成本單這個。

**修法（3da9b93f4，兩層、各一個 enforcement owner）**：
1. **Injector 根治**：`email_notifier._load_env_file` 只准 prime `_DELIVERY_ENV_ALLOWLIST`
   （SMTP/EMAIL/IMAP/site-url 等投遞域 15 key）；第三方 auth secret 永不進 process env。
   非投遞消費者本就自載 scoped env（`supabase_rpc.runtime_environment`、`volpred.ops.telegram`）。
2. **Spawn 邊界 fail-safe strip**：registry 新增 `sanitize_provider_spawn_environment()`
   —— spawn 路徑在 authorize **之前**按該 contract 的 forbidden 清單 strip（只 log key 名），
   worker.py（claude）與 codex_failover._authorize_codex（codex probe+work）都走它。
   authorize gate 對漏網者照樣 fail-closed；forbidden 清單**沒有放寬**，registry 仍是唯一 owner。
   短命 CLI spawn site（gen_lazypack_*、run_agent_job 等）**刻意維持純 fail-closed**：
   它們的 env 來自 fresh shell，被拒是即時可見的錯誤，不是 silent outage。

**驗證**：reload daemon（deferred release 80db06a7 completed）→ request_fire → worker 真 spawn
（見本 entry 對應 fire receipt）；`provider_policy_denied` 停止增加。回歸 gate：
worker strip-before-authorize+Popen、codex preflight 真 sanitize、sanitize 單元測試、
EmailNotifier allowlist 測試、以及機械 cross-gate（delivery allowlist ∩ 任一 provider
forbidden_env == ∅，`tests/test_email_notifier.py`）。相關測試 100+ 全綠。

**教訓（class 級）**：
- **fail-closed gate 在長命 daemon 內需要配對的 fail-safe 出口**：gate 擋的變數若 child
  根本不該收到，正解是 spawn 前 strip，不是讓整條 backbone 停擺等人來修。
- **library constructor 禁止 bulk-load .env 進 os.environ**：process-global 秘密注入
  = 對同 process 內所有其他子系統的隱形供應鏈。scoped mapping（回傳 dict）才是正途。
- **`ps eww` 只能排除 boot-time 來源**；查 runtime env 類 bug 要在驗證點 log key 名單
  （不印 value），或直接在乾淨 subprocess 內 import 可疑模組做注入 A/B。
