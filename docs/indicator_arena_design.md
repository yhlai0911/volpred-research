...
### 3.0 投資實用性 gate（2026-06-11 boss 指示，優先於其他上架條件）

指標必須具**投資實用價值**，不是學術練習。檢驗問題：「一般投資人看到今天的訊號，知道該做什麼嗎？」答不出來就不上架。卡片必附「投資上怎麼用」教育性說明（通用用法，非個人化建議）。首例：C6（q95 RV 校準）2026-06-11 editorial 下架 — 統計校準練習，投資人無法據此操作；歷史訊號永久保留。純方法學互比類指標需重新框架出投資意涵才可上架（例：C4 框架為「與 GARCH 警戒線打擂台 — 兩線分歧 = 模型不確定性高」）。
