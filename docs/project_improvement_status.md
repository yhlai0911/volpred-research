...
- ⚠️ **K1259 review-cycle closure 部分 RETRACTED 2026-04-29**（Codex primary-path FAIL；2026-04-28 subagent fallback 不充分）：(1) commit `d4c2faf1` MAJOR-3 `load_ledger` docstring 仍 OK；(2) commit `53c1d559` MAJOR-1 Phase 1.5 backfill scripted — 仍 OK；(3) commit `aff7b4a5` MAJOR-2 NON_DM_PATH_TOKENS filter — **closure RETRACTED**：blacklist 只含 5 token (ttest/mcnemar/wilcoxon/kstest/kruskal)，但 Codex re-review 確認 ledger 仍含 ~12 殘留 non-DM rows from `statistical_tests`×10 (K528/K594/K990/K1006) + `stat_test`×1 (K658) + `welch`×1 (K975) + `vs_zero`×3 (K1006)，全 keyed via `t_stat` 欄位（不在 audit generic-key 子群）；(4) knowledge `c4db347a` confidence **0.88→0.75 RETRACTED**；(5) commit `9b9951fd` research_program 同步 — also RETRACTED 同 slot。**Codex review v2 doc**：`experiments/k1259/codex_review_v2.md`（task `task-moiszyai-2sik97`, session `019dd4c9`, 5m 5s, FAIL verdict）。**Pending fix（next slot）**：選擇 (a) 擴展 NON_DM_PATH_TOKENS 加 `welch`/`stat_test`/`statistical_test`/`vs_zero` OR (b) Codex-recommended positive DM gate (path must contain `dm`/`harvey`/`hln`)；re-run pipeline；MED finding (asset semantic) + MINOR (comment row count).
