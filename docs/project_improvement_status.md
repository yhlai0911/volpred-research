...
- ✅ 2026-07-16 依使用者新選擇恢復「每次完成自動語音」，但不把 prose 塞回 always-loaded guide：收編進既有 `enforce_final_text.py` Stop owner，以 argv 直呼 `/usr/bin/say`；只限 canonical main 互動 session、有完成型 final text 且同 turn 未播過，worktree/background/blocker 全跳過。`/task-done` 移除手動 `say`，避免雙播。
