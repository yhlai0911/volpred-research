...
| Program commit 15：**immutable safe reconcile production ownership** | 🟡 contained 2026-07-25（safe slice ✅；delete intent + recovery graph + execution adapter ✅） | Hourly `feed_sync`依`publisher.article.supabase.reconcile` generation在legacy逐篇caller與Operations Core immutable batch之間路由；private payload、WorkItem、EffectRequest/outbox、primary lease與typed settlement由單一adapter擁有。Live rehearsal完成`legacy/1 → operations_core/2 → legacy/3 rollback → operations_core/4`，回讀work/effect/outbox/attempt全acknowledged、local/Supabase 14/14、drift 0；最終tracked-snapshot selected suite 91案通過（含完整47-case PostgreSQL檔）。後續failure injection再封住single與batch provider read-back阻塞期間authority換代的write window：每筆真正upsert前精確重驗原epoch，漂移時projection write=0、settlement=0，相鄰82案通過。Delete另有兩段式零I/O contract；production catalog再校正為六張cascade table／七條FK edge，legacy apply以service-role-only RPC在delete前核對live graph，完整保存article/child rows、feed SHA與fsync read-back後dump SHA。Destructive adapter再要求durable approval active、全批exact preflight、每筆mutation前candidate/approval/authority重驗，以及具evidence的typed absence acknowledgement；相鄰127案通過。Recovery與worker execution-contract根因皆已`root_cause_fixed_and_verified`；production Supabase projection、delete owner CAS、restore與live rollback仍缺，因此program commit 15整體維持`contained` |
