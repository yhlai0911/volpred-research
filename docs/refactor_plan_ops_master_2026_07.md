...
| Program commit 15：**immutable safe reconcile production ownership** | 🟡 contained 2026-07-25（safe slice ✅；destructive contract／owner／projection／restore ✅） | Hourly `feed_sync`依`publisher.article.supabase.reconcile` generation在legacy逐篇caller與Operations Core immutable batch之間路由；private payload、WorkItem、EffectRequest/outbox、primary lease與typed settlement由單一adapter擁有。Live rehearsal完成`legacy/1 → operations_core/2 → legacy/3 rollback → operations_core/4`，回讀work/effect/outbox/attempt全acknowledged、local/Supabase 14/14、drift 0。Destructive delete另有獨立owner CAS、durable approval、六表／七edge recovery、atomic compare-delete、exact restore executor與service-role-only atomic restore RPC；各切片皆`root_cause_fixed_and_verified`。整體仍缺manual-only live synthetic delete→restore→feed convergence rehearsal與physical two-Mac authority receipt pair，因此program commit 15維持`contained`。 |
...
| **WS-B producer-scoped isolation（本計畫最大架構件）** | ✅ 試點上線 2026-07-20；觀察窗至 08-03 | worktree `5d0cc4fb0`+`4f95639c5` → merge `c38b3a1f1`；platform_ops lane 隔離 workspace 機械配發（agent 無法自選 ownership namespace）+ targeted merge gate + gate 紅自動開 remediation 單（無死局）+ 成本上限與 disk floor；PHASE-Z 猜作者降級 fallback（隔離 cohort 零介入、recognizer 保留一個月後另案退役）；main 複驗 186 tests 綠；`wsb_isolation_pilot` 已入觀察帳本（deadline 08-03：零 incident → 擴 governance lane + 開退役案；有 incident → mode=off + root-cause 單）；daemon selfreload 自動生效 |
...
| Program commit 15：**destructive delete production owner／approval／projection** | 🟡 contained 2026-07-25（本切片 ✅） | 獨立family `publisher.article.supabase.delete`；generation-CAS owner、private WorkItem/EffectRequest/outbox、durable scope approval、attempt-bound provider及transaction-fenced compare-delete已部署。Live ACL/RLS回讀、approval record→read→revoke、六表candidate read-back及`legacy/1 → operations_core/2 → legacy/3 rollback`皆通過；故意compare-delete被拒且article evidence不變；相鄰222 tests綠。此切片 `root_cause_fixed_and_verified`；exact restore executor、manual live delete→rollback→convergence與physical two-Mac receipt pair未完成，umbrella仍`contained`。 |
...
| Program commit 15：**destructive production atomic restore projection** | 🟡 contained 2026-07-25（projection slice ✅） | `SupabasePublisherArticleDeleteRestoreProjection`＋service-role-only batch RPC已部署；七edge catalog／row shape／全批absent-exact preflight後依parent→六child恢復，relation雙edge去重，exact post-readback，例外全批rollback，replay零write。隔離PG17 6案（含scope drift、nullable child binding與mid-batch failure）通過；production migrations `20260725020432`＋forward-only NULL-safe wrapper `20260725020935`與owner／ACL／14 RLS policies已回讀，舊v1不對service role開放，advisors無新增告警，零live article mutation。此slice `root_cause_fixed_and_verified`；manual live synthetic delete→restore→feed convergence與physical two-Mac receipt pair仍缺，umbrella維持`contained`。 |
