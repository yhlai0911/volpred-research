...
| Program commit 15：**immutable safe reconcile production ownership** | 🟡 contained 2026-07-25（safe slice ✅） | Hourly `feed_sync`依`publisher.article.supabase.reconcile` generation在legacy逐篇caller與Operations Core immutable batch之間路由；private payload、WorkItem、EffectRequest/outbox、primary lease與typed settlement由單一adapter擁有。Live rehearsal完成`legacy/1 → operations_core/2 → legacy/3 rollback → operations_core/4`，回讀work/effect/outbox/attempt全acknowledged、local/Supabase 14/14、drift 0；最終tracked-snapshot selected suite 91案通過（含完整47-case PostgreSQL檔）。Safe ownership切片為`root_cause_fixed_and_verified`；destructive delete仍缺獨立formal effect／owner／rollback，因此program commit 15整體維持`contained` |
