...
| **T03 Queue Ownership Cutover（GitHub #9）** | 🟡 contained 2026-07-27 | Canonical queue 已安全回到 `queued_execution`；23:37 UTC 後 scheduled v4 receipts 的 reconciliation issues 為 0，selector winner 差異是有 oracle/evidence 綁定的 `coordinator_capability_contract` policy change。04:15 UTC 最新 receipt 仍有 10 個 blocking dimension mismatch：2 筆 active legacy claim 各缺 readiness／claim ownership／durable lease expiry（共 6），另有 4 筆 main-thread lane 未映射到 Coordinator capability，所以 clean suffix 尚未開始。Closure audit 另發現 assessor 會讓同 owner 的歷史 content blocker、incomplete evidence、observation gap 或 replay-clock violation 永久污染未來窗口；公開 assessment seam 現以不可向歷史移動的 `max(recorded_at, observed_at)` 排序 owner epoch 與最新 clean segment，只有內容乾淨、時間連續且 live 的 segment 真正滿七日才取代舊窗口，append-only 歷史不刪，近期或大幅 backdated blocker／owner mismatch 仍立即 fail closed。RED→GREEN 與兩輪 reviewer remediation 後 assessment 檔 37 tests、相鄰 104 tests 綠。正式 owner 仍為 `legacy/1`，未 stage gate／transfer／rollback，故不得結案。 |
