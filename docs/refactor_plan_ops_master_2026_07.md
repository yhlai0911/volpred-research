...
| **T05 follow-up：Boss Report owned delivery／canonical read model（GitHub #39）** | 🟡 contained 2026-07-26 | 20:10 production fire 證實 Operations Core scheduler 仍呼叫 legacy direct-SMTP Boss Report，且內容讀 2026-05-19 pseudo-living files。實作已以 canonical generation／activation／cron slot／fire digest 綁 immutable payload 與 owned-email idempotency、共用 durable owner router／terminal replay、wrapper pre-effect owner gate、typed §7＋current task-pool mode read model與小寫 `z` 正規化。跨主機先以 service-role-only RPC 讀 immutable command／terminal receipt；core request 不可被 legacy supersede，明確 legacy rollback 亦由 Primary Authority＋deterministic Message-ID Gmail Sent read-back 防重送；DB owner transfer 與 PA acquire 共用 advisory lock、有效 lease 時 transfer fail closed，legacy 取得 fence 後再核對 request／generation，關閉 transition TOCTOU。Scoped 回歸 **154 案綠**，包含實際本機 PostgreSQL migration／shared-read／terminal-replay／雙 connection advisory-race transaction，dry render 已確認舊 cycle／舊 action／假 warning 消失。待 production migration、實際 WorkItem／EffectRequest／outbox／Gmail Sent read-back及下一個 schedule receipt 回讀後，才可升為 `root_cause_fixed_and_verified`。 |
