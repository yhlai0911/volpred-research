...
| Program commit 15：**immutable safe reconcile production ownership** | 🟡 contained 2026-07-26（safe／destructive contract／owner／projection／restore／live rehearsal slices ✅） | Hourly `feed_sync`依`publisher.article.supabase.reconcile` generation在legacy逐篇caller與Operations Core immutable batch之間路由；private payload、WorkItem、EffectRequest/outbox、primary lease與typed settlement由單一adapter擁有。Live rehearsal完成`legacy/1 → operations_core/2 → legacy/3 rollback → operations_core/4`，回讀work/effect/outbox/attempt全acknowledged、local/Supabase 14/14、drift 0。Destructive delete另有獨立owner CAS、durable approval、六表／七edge recovery、atomic compare-delete、exact restore executor與service-role-only atomic restore RPC；`live-20260726-0503`已完成synthetic delete→exact restore→cleanup delete及standing convergence零mismatch，各切片皆`root_cause_fixed_and_verified`。Operations-core umbrella現在只缺physical two-Mac authority receipt pair；雙端readiness gate、readiness→role→final receipt SHA鏈及standby對primary receipt的pre-mutation gate皆已落地，epoch與holder identity不再由caller填寫，program commit 15維持`contained`直到實體pair完成。 |
