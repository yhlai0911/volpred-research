...
| **T03 Queue Ownership Cutover（GitHub #9）** | 🟡 contained 2026-07-27 | Canonical queue 已安全回到 `queued_execution`；scheduled v4 receipts 的 reconciliation issues 為 0，selector winner 差異是有 oracle/evidence 綁定的 `coordinator_capability_contract` policy change。`9eb9845f1` 已讓新 claim 持久化兩小時 expiry，並把 main-thread lane 映射成 Coordinator exclusive capability；141 tests 綠。04:31 UTC live read-back 的 3 個 blocking dimension mismatch 全來自修復落地前已在執行的 `assign_8262c921`；不人工釋放而待其自然結束後，04:56:14 UTC 正式 receipt `scheduled_20260727T045614581709Z_def2f814b885` 已比對 3,389／3,389 rows、0 reconciliation、0 blocking dimensions，252 個差異全為有 evidence 的 registered policy change，七日 continuous-clean suffix 由此刻啟動。Closure audit 另發現 assessor 會讓同 owner 的歷史 content blocker、incomplete evidence、observation gap 或 replay-clock violation永久污染未來窗口；公開 assessment seam 現以不可向歷史移動的 `max(recorded_at, observed_at)` 排序 owner epoch 與最新 clean segment，只有內容乾淨、時間連續且 live 的 segment 真正滿七日才取代舊窗口，append-only 歷史不刪，近期或大幅 backdated blocker／owner mismatch 仍立即 fail closed。其後再修正 expiry lifecycle：`blocked_until` 只作 not-before，allowlisted `work_shadow_cutover_ready_v1` gate 會在 expiry 後重跑同一 canonical assessment，未 ready／未知 gate／讀取失敗皆保持 blocked，避免舊日期造成提早 cutover；parent task 已綁定正確 `2026-08-03T04:56:16.743114Z`。Reviewer找出的manual unblock／release bypass、re-block gate removal、whole-writer schema／adapter coverage與mark CLI分離讀寫鎖缺口亦已補上：任何gated row無法經claim/start/handoff/preassign/release進入dispatchable狀態，非terminal re-block保留gate，canonical writer拒絕非法組合，mark CLI以同descriptor `LOCK_EX`完成交易，真owner evidence fixtures覆蓋ready／short／mismatch／unreadable。RED→GREEN 與 reviewer remediation 後 assessment 檔 37 tests、queue lifecycle／assessment相鄰 188 tests 綠；先前全專案回歸 5,721 passed、1 skipped。正式 owner 仍為 `legacy/1`，七日未滿且未 stage gate／transfer／downstream acknowledgement／rollback，故不得結案。 |
