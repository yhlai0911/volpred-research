...
| **T12 Guided Migration／Parity Manifest（GitHub #17）** | 🟡 contained 2026-07-29 | Commits `15fcd6c85`,`0586e114c`建立四角色獨立Ed25519簽章、one-time challenge、兩端clean immutable Git、artifact/tool exact-SHA、secret-reference／permission與formal RPO/RTO fail-closed gate；plan固定`authorizes_primary_lease=false`。Live gate抓出並修正uv-managed Python `home` origin假陰性，以及Telegram state writer每次把secret reference mode退回0644的根因；後者現以0600同目錄temp、file fsync、atomic replace、parent-directory fsync寫入，live同內容寫回SHA不變且mode=0600。104個相鄰測試與post-commit 51案全綠，Matt Spec／Standards最終雙PASS。Studio capture已回讀四組secret preflight全綠，並正確拒絕仍存在的`GOOGLE_CLOUD_API_KEY`／`OPENAI_API_KEY`；不得靠刪警報繞過，須由#12 subscription replacement退役。已產生21,693,118-byte、SHA-256=`89b044640c61c71b8834be872ea77a31d9ccc23e1965a55c9404343c492779b2`的clean target dry-run bundle，檔名掃描0個`.env*`／私鑰。MacBook Tailscale仍offline，fresh target signature／formal-effect RPO=0 pair尚缺；#17被#12→#9七日gate阻塞，維持OPEN／`contained`。 |
| **T13 Primary Lease／Fencing（GitHub #18）** | ✅ root_cause_fixed_and_verified 2026-07-27 | 五步 closure audit推翻先前 umbrella 誤結案：two-Mac receipt只驗 isolated key 且 effect/provider=0；production capability-scoped keys可形成系統級雙主，renew/expiry/reject亦無 append-only receipt。底層 contract現收斂所有 formal builders/adapters至唯一`operations-core-primary`，DB grant trigger拒絕非canonical key；migration`20260727080000`新增acquire/renew/expiry/demote/reject token-redacted ledger與typed rejection，DB-clock read/reconcile會materialize無takeover自然expiry，release backend outage則由fsync local demotion intent在下次acquire前重播，僅於舊lease已失效後收成remote receipt。Migration已精確部署並回讀owner／FORCE RLS／ACL、legacy SQL authority boundaries=0；global-key live canary完成epoch 1 acquire／renew／double-primary reject／authorize／release，相關測試467 passed且Matt雙審PASS。Git formal commit另補上緊貼writer subprocess前的第二次durable authority transaction，同時重驗WorkLease、commit owner generation與全域Primary Authority，grant漂移或partition時保持HEAD不變；Git actuator 26案、相鄰authority／delivery 107案與PostgreSQL transaction 8案全綠。Email adapter亦在Sent-mail不存在後、真正SMTP前重驗同一Primary Lease；normal／recovery／legacy共用邊界，相關範圍206案與Matt雙審全綠。乾淨commit archive的owner-only live canary由`operations_core/4`寄出並獲Gmail Sent exact read-back，同key重播attempt仍為1；authority-loss案在真實IMAP後demote epoch 35，SMTP log=0且Message-ID不存在，去敏receipt=`storage/ops/email_mutation_boundary_canary_latest.json`。T13五步Gate與Issue #18 acceptance scope均完成；legacy writer retirement分屬#24/#46。 |
| **T14 Warm Standby／接手（GitHub #21）** | 🟡 contained 2026-07-29 | 最新physical two-Mac no-effect rehearsal `issue21-warm-standby-20260727-1615`已證明Mac Studio→MacBook Pro exact-next epoch `1→2`、DB-clock handoff `0.528014s`、duplicate/effect/provider皆0，但它不是持續warm standby。Closure audit依Issue blocking edges確認#21仍被#16/#17阻塞，而兩者又被#12→#9七日ownership gate阻塞。MacBook（`IvanLaideMacBook-Pro.local`）以專用SSH identity部署最小tracked runtime（`src/volpred`、`scripts`、`config`、lockfiles），沿用MacBook本機既有mode-600 authority環境及已驗證的Python `3.12.10`／OpenSSL `3.0.16` venv；沒有複製Studio secret，也沒有安裝VolPred LaunchAgent／cron。Operations Core source/dependency/runtime identity兩端exact match `9db5479f…e401`，readiness在任何lease mutation前回讀相同Supabase backend與publisher owner `operations_core/8`。Studio transport partition後`9.635672s`關閉local gate，MacBook等待`29.034066s`後才於舊lease的DB-clock expiry後取得epoch 2，RTO遠低於5分鐘。Commit `66b884aef`再補齊原本完全缺席的signed cold-restore actuator：只從source snapshot綁定的clean immutable Git objects建deterministic bundle，排除secret／runtime state，target以no-clobber staging原子落地並產生mode-0600 signed receipt，固定不安裝schedule、不做effect、不授權lease。43個回歸與正式CLI live rehearsal全綠；同一bundle還原兩個全新temp target的payload tree SHA皆為`6064cb7ac218186a4c6efdc7c7357c8ecf9c6b84331965a8298459bd441a8b4b`。這完成可重複的isolated no-effect takeover與cold-restore execution兩個slice，但仍不能證明formal effect的RPO=0、完整guided parity／TCC capability或持續warm process；上述責任分屬#16/#17/#24。#21維持OPEN／`contained`；先完成#9→#12→#16/#17後才可正式promotion closure。 |
| **T05 follow-up：Boss Report owned delivery／canonical read model（GitHub #39）** | ✅ 2026-07-27 | 20:10 production fire 證實 Operations Core scheduler 仍呼叫 legacy direct-SMTP Boss Report，且內容讀 2026-05-19 pseudo-living files。實作已以 canonical generation／activation／cron slot／fire digest 綁 immutable payload 與 owned-email idempotency、共用 durable owner router／terminal replay、wrapper pre-effect owner gate、typed §7＋current task-pool mode read model與小寫 `z` 正規化。跨主機先以 service-role-only RPC 讀 immutable command／terminal receipt；core request 不可被 legacy supersede，明確 legacy rollback 亦由 Primary Authority＋deterministic Message-ID Gmail Sent read-back 防重送；DB owner transfer 與 PA acquire 共用 advisory lock、有效 lease 時 transfer fail closed，legacy 取得 fence 後再核對 request／generation，關閉 transition TOCTOU。Scoped 回歸 **154 案綠**，包含實際本機 PostgreSQL migration／shared-read／terminal-replay／雙 connection advisory-race transaction；production migration `20260726134809` 已部署並回讀 owner／ACL／fence。2026-07-27 08:10 台灣時間的下一個自然 fire `operations-core-v1:boss_report_4h:35208392b85064c22499c492` 由 `operations-core-scheduler` 一次成功；canonical RPC 回讀 owner=`operations_core/4`、WorkItem=`succeeded`、Effect=`delivered`、attempt_count=`1`、Primary Authority epoch `211`，Gmail Sent evidence 與 scheduler receipt 完全一致，無 direct SMTP、重送或 owner drift。closure scoped regression **103 passed**，狀態為 **`root_cause_fixed_and_verified`**。 |
...
| **T03 Queue Ownership Cutover（GitHub #9）** | 🟡 contained 2026-07-27 | Canonical queue 已安全回到 `queued_execution`；scheduled v4 receipts 的 reconciliation issues 為 0，selector winner 差異是有 oracle/evidence 綁定的 `coordinator_capability_contract` policy change。`9eb9845f1` 已讓新 claim 持久化兩小時 expiry，並把 main-thread lane 映射成 Coordinator exclusive capability；141 tests 綠。04:31 UTC live read-back 的 3 個 blocking dimension mismatch 全來自修復落地前已在執行的 `assign_8262c921`；不人工釋放而待其自然結束後，正式 clean suffix 曾開始，但 2026-07-27 12:38 UTC 的 provenance breach 使 assessor 依法重置窗口。最新 canonical queue gate `work_shadow_cutover_ready_v1` 的 authoritative `blocked_until`／`next_eligible_at` 為 **`2026-08-03T12:40:16.244030Z`**；expiry 只是 not-before，屆時仍必須重跑 assessment，未 ready／未知 gate／讀取失敗皆保持 blocked。Closure audit 已防止歷史 blocker 永久污染，也防止 manual unblock／release、re-block gate removal、whole-writer schema與分離讀寫鎖繞過。正式 owner 仍為 `legacy/1`，七日未滿且未 stage gate／transfer／downstream acknowledgement／rollback，故不得結案。 |
...
| **Incident machine-self actuator amendment**（owner 08-01「通知列錯卻未主動修」） | 🟡 implementation verified / live read-back pending | Production audit 證實五個 shipped machine-self kinds 被設成 notification-only，導致 detector 有信件、task pool 無 actuator。依既有 incident identity 修正為 episode 1 恰一張 isolated P2 repair，綁 `repo_patch`／有限 output scope／exact deterministic task id／2h deadline；未派工 task 先 queue CAS terminal 再推進 incident，claimed／running 則等待 supervisor 正式 custody-drain receipt，避免雙 owner。terminal 後仍 breached 才進 episode 2 單一 main-thread 根因升級，不再自修。既有 durable rows會帶 `policy_history` 前向遷移；完成 tests、Matt 雙審與 production natural-fire 回讀後才升為 `root_cause_fixed_and_verified`。 |
| **H4 starvation admission amendment**（owner 08-01「通知提出錯誤卻未主動修」） | 🟡 natural fire verified / sustained clean pending | 真實 queue/read-only report 已把 `K1221_article_general_v2` 判為 P1 starved 49h，但 supervisor 在 generic menu 之前持續 preassign CI/platform task，故診斷 verdict 從未擁有執行席位，`draft_pool_low` 累積 31 次且 pool=0。修正把 starvation threshold／age／rank 移入 `volpred.ops.task_pool_selection` 單一 owner，report 與 `dispatch-preassign` 共用 main-thread／unknown-lane／collision／urgency eligibility；第一個 runnable starved row 是 generic task 時 supervisor yield 並把 exact id 帶進 worker environment，claim CLI 在該 row terminal 前拒絕其他 identity、terminal 後才允許 batch-drain。contractless mutating rows先剔除、不能當幽靈 blocker。2026-08-01 natural fire 先認領 K1221，依 evidence gate 誠實裁決為不可寫，隨後同班認領真正可寫的 K1450，完成 draft `mile_5378daa1` 並回讀 feed draft count `0→1`；claim／dispatcher／supervisor／Codex failover class sweep **514 passed、1 skipped**。仍須把 draft pool 拉回門檻並 sustained-clean，才可升為 `root_cause_fixed_and_verified`。 |
...
| **T09 Scheduler Ownership Cutover** | 🟡 live / contained 2026-07-29 | 原 `system_crontab` owner audit 已轉給單一 Operations Core daemon，但 07-29 class sweep 發現 `cron_jobs[id=volpred-compute-worker]`／`com.volpred.compute-worker` 不在該 inventory，故先前「legacy per-job LaunchAgents=0」只覆蓋已列入的 jobs。Issue #46 tracer slice已讓 active legacy `cron_jobs` row fail closed，並把 compute cadence／parallelism移入 `system_crontab`；07:45 natural fire attempt 1 exit 0與priority-0 queue smoke共用exact fire key，receipt-gated reconcile後LaunchAgent not found、targeted audit零conflict/dormant，故此漏網slice為`root_cause_fixed_and_verified`。T09 umbrella仍受sustained-clean長窗約束，只維持contained。 |
...
| **T21 Shared Frontend Core（GitHub #19）** | ✅ root_cause_fixed_and_verified 2026-07-28 | `frontend-core`以client-safe contract＋server-only production adapter提供article data／related data、request auth、SEO、analytics identity與typed error seam；article與auth拆為獨立深模組。原版與v3 `/reports/[id]`已移除各自的primary article/related/SEO/error處理，統一走`articleFrontendCore`＋`requireArticlePage`（只有authoritative missing→404，provider outage→5xx）；`/api/me/summary`改走`authFrontendCore`，credential rejection→401、provider/missing-config→5xx；`ProductAnalytics`改用相同analytics identity。Nested frontend commit=`2f02266`；canonical contract tests **8 passed**、相鄰original/v3/analytics tests **27 passed**、production build **88 routes**、route parity **29 passed／134 routes／0 blocker**，Matt Standards／Spec最終雙PASS。Zeabur deployment `6a6893e4eac99cc636f23d58`為RUNNING；live同篇原版/v3均200且canonical exact相同，v3回讀真article title，兩個missing route均404，匿名member summary=401，feed=1,692、strategies=11。Migrated callers的舊direct `getArticle`／`getRelatedArticles`／request-auth／analytics-normalizer路徑已退役；原版與v3 renderer本身保留為同core上的兩個presentation adapter。完整`npm check`仍被fixed-point既有`data-server.ts:1971` row-cap gate擋，未冒稱全域CI綠，另行追蹤。 |
| **T39 Termination Intent／Checkpoint Resume（GitHub #45）** | 🟡 contained 2026-07-30 | Formal operator termination與observe-only bounded slice已完成：dispatch明示`write_intent=observe_only`／空outputs，task-session-job-attempt CAS覆蓋admission replay與settlement；custody-backed producer只能PGID coalition `TERM_KILL`，且以bounded PID／PGID drain acknowledgement收口，ambiguous `ps` probe一律fail closed。Live job `7394003c…`／intent `a639722c…`回讀`custody_drained → system_terminated/143 → task re-pend → workspace empty_removed`；commits `e9397f62b`、`abd77f9dd`、`43efe2a44`、`bfd6bc2cd`、`04da4f679`、`98159061d`，clean **419 passed、1 skipped**，Matt Spec／Standards雙PASS。此bounded root class為`root_cause_fixed_and_verified`；完整checkpoint resume、host restart、lease expiry與duplicate commit/effect acceptance仍被#9/#24阻塞，T39 umbrella維持OPEN／contained。 |
| **T40 Legacy Execution Retirement（GitHub #46）** | 🟡 contained 2026-07-30 | Formal owner census與atomic retirement preflight已落地，四個canonical evidence producer與hourly recorder均由Operations Core運行。2026-07-30 已完成 legacy hourly dispatcher 的 physical retirement：正式 shell exact bytes移入 `scripts/_legacy/`、canonical/user plist與live TCC copy移除、runtime row只保留 schedule/pregate policy、owner/manifest references移除，live `launchctl`回讀 label absent；158 tests綠，retirement assessor已沒有physical blocker。此 dispatcher slice為`root_cause_fixed_and_verified`。但 fresh formal census仍有五個non-schedule legacy owners（`work.coordinate`、`git.commit`、`publisher.article.supabase.delete`、`incident.lifecycle`、`provider.execution`），而14日 sustained-clean尚未開始；Issue #46 direct blocking edges與formal transfers未完成，所以umbrella仍維持`contained`，不得宣稱全域舊架構已退役。 |
| **T21.1 Papers Row-cap Prerequisite（GitHub #48）** | ✅ root_cause_fixed_and_verified 2026-07-28 | T21 完成後的 canonical frontend check 暴露 `getPapers()` 直接 `papers.select('*')`，會在 PostgREST row cap 靜默截斷。Nested frontend commit=`0ad9bb6` 把既有 `fetchAllRows` 抽成單一 `postgrest-pagination` contract，paper catalog 以 `display_order ASC, updated_at DESC, id ASC` 穩定跨頁；provider error、malformed page、非法 page size 全部 fail closed。RED→GREEN 後 row-cap guard 3 + contract 4、frontend-core 8、article-views 7、typecheck 與 production build 88 routes全綠；Matt Standards／Spec最終雙PASS。Zeabur deployment `6a689964eac99cc636f23e9c`為RUNNING；production `/api/papers`回讀12列／12 unique／排序契約成立，原版`/paper`與`/v3/paper`皆200。Active checkout既有question-dedup、三份PDF與刪除中的chronology test均未被commit收走。 |
| **T22 vNext Public／Editorial 體驗（GitHub #22；blockers #49/#50/#51）** | ✅ root_cause_fixed_and_verified 2026-07-28 | 原版與v3保留為同一authoritative snapshot／frontend core上的獨立presentation adapters；current risk、interpretation、methodology／provenance、immutable history、mobile ARIA／responsive、SEO／canonical、analytics identity與效能預算納入`npm run check:vnext-experience`及release-bound schema-v3 `verify:vnext-live`。Active nested frontend依序整合`330c231`、`bcf6d31`，最終candidate=`63dfd71f24242d0257e434aee91c067abfce51bd`；live gate實際抓出conditional footer proof gap與持續analytics/SWR使`networkidle`永不收斂兩個根因，均以RED→底層修正→對抗回歸收斂。最終vNext **28/28**、完整`npm run check`／typecheck／88-page production build全綠，route parity **134 routes／25 rules／7 scenarios／0 blocker**（4個single-mode gap保留為explicit governed gaps），Matt Spec／Standards雙PASS。Zeabur deployment `6a68bffeeac99cc636f24959`為RUNNING；production computed-browser receipt `schema_version=3,outcome=pass,errors=[]`，原版／v3／about／paper／questions／history全200，release=`vnext-experience-20260728.1`，兩首頁回讀相同feed=`mile_eaacd072`、experiments=115、VIX=18.67、sigma=13.4，`/api/health`=200。#51與#22均已關閉。 |
| **T23 Member Continuity／Privacy（GitHub #23）** | ✅ root_cause_fixed_and_verified 2026-07-29 | 原版／v3共用browser anonymous identity、登入後receipt-backed merge、收藏／追蹤／提醒／提問與可解釋read/delete seam；未送出的提問草稿只留browser，analytics與member delete均使用HMAC subject digest，介面明示研究內容非個人化投資建議。Private `volpred_member`與`volpred_analytics` tables皆FORCE RLS，五個public RPC由no-login/non-bypass worker持有、`SECURITY DEFINER`、空`search_path`且只給service role。PG17 managed runner只在function create/replace window建立`INHERIT FALSE/SET TRUE` self-grant，最後以`GRANTED BY CURRENT_USER`精確撤回，不動Supabase bootstrap grant，重跑仍冪等。Parent commits=`ff46f9fbe`,`dc43c4b1e`,`c2bbcbf61`，nested frontend=`a15bc14`,`b264eb5`；PostgreSQL integration **8 passed**，member gate **19 passed**且已納入canonical `npm run check`，frontend full check／89-page production build與Matt Spec／Standards雙PASS。Production receipts=`20260728190500`,`20260728191149`；service-role transaction live E2E通過save/follow/reminder/question/read、exact replay、identity merge及雙delete後完整rollback。Zeabur deployment `6a6913edeac99cc636f25a5d`為RUNNING；原版／v3首頁、文章、questions、member頁皆200，continuity GET/POST/DELETE匿名皆401。candidate-only clean worktree部署未夾帶active checkout WIP。 |
...
`5deb97fa7`把`host_authority/operations-core-primary`從硬編碼`unresolved`
