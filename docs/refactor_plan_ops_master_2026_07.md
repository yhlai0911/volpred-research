...
| Program commit 15：**immutable safe reconcile EffectRequest（shadow）** | 🟡 contained 2026-07-25 | `prepare_publisher_article_reconcile()`以單一interface產生payload-bound batch EffectRequest；完整articles內嵌且唯一排序，worker不重讀mutable feed。Provider先讀、只寫mismatch、再exact read-back；invalid contract durable dead-letter、retry與replay已鎖。新8案＋相鄰共36 passed；production adapter read-only回讀`mile_30b22ca5`相符。尚缺hourly production payload/outbox owner、delete family與rollback rehearsal，不宣稱commit 15 cutover |
