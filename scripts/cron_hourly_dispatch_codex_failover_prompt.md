...
5. 用 `uv run python scripts/git_writer_lock.py commit --actor codex-failover --message '<[codex] ASCII message>' -- <exact paths>` 提交；**不 push**（本機 git_push_backup cron 會處理）。
6. 若上一輪有未完事項，先收尾再挑新工。
