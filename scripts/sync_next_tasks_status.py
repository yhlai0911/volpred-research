...
def find_experiment_dir(task_id: str) -> Path | None:
    ...
    candidate = EXPERIMENTS / task_id.lower()
    if candidate.is_dir():
        return candidate
    return None
...
def main() -> int:
    ...
    for idx, task in enumerate(tasks):
        ...
        exp_dir = find_experiment_dir(tid)
        if exp_dir is None:
            continue
    ...
    for _, task, info, exp_dir in candidates:
        print(
            ...
            f"[exp_dir={exp_dir.relative_to(ROOT)}]"
        )
    ...
    if args.apply and candidates:
        ...
        print(f"[sync_next_tasks] APPLIED {len(candidates)} updates to {NEXT_TASKS.relative_to(ROOT)}")
    elif args.dry_run:
