    ...
    corrections = [
        {
            ...
            "content_replacements": [
                ...
                (
                    ...
                    f"但這個差距**談不上統計顯著**。無母數 Wilcoxon 檢定的顯著性為 {pa['wilcoxon_p_vs_all']:.5f}；兩種參數檢定分別為 {p_welch:.3f}（Welch）與 {p_student:.3f}（Student），再對「vs 全體」與「vs 週五」兩個檢定做 Holm 校正後為 {holm_overall:.3f}，沒有跨過 5% 門檻。四個 VIX 分組也無一通過 Holm 校正，描述方向仍有很大不確定性。",
                ),
                (
                    ...
                    "## 那事先避開 NFP 日划算嗎？\n\ncanonical 重跑只涵蓋波動比較，沒有重跑策略、產業分散或事件前後漂移。舊版「完全跳過 NFP」與條件減倉回測使用 first-Friday 替代日曆，不能繼續作為官方日曆下的策略證據。\n\n目前能確認的是 NFP 日平均絕對報酬較高，但多重比較校正後未達 5% 門檻。是否值得提前出場，需要用官方日期重跑同一套時間對齊、成本與基準；在那之前，本篇撤回相關風險調整後績效、最大回撤與報酬貢獻結論。",
                ),
                (
                ...
                (
                    ...
                    f"背景環境會改變絕對震幅；這次重跑尚未證明它會改變 NFP 的增量衝擊。低、高 VIX 比值的直接差異顯著性為 {regime_test['p_two_sided']:.3f}，95% 重抽樣區間包含零。",
                ),
                (
                    "*資料來源：yfinance（SPY, ^VIX），實驗 K741 canonical 重跑（官方 BLS 發布日曆），2010-01-01 至 2026-03-30*",
                    "*資料來源：yfinance（SPY, ^VIX），本次 canonical 重跑採官方 BLS 發布日曆，2010-01-01 至 2026-03-30*",
                ),
                (
                    "本文波動率數字來自實驗 K741 **canonical 重跑**（`experiments/k741/k741_nfp_event_study_canonical_results.json`，2026-07-20），改用官方 BLS 發布日曆與 forward-only 交易日對應。資料來源：yfinance（SPY, ^VIX），期間 2010-01-01 至 2026-03-30，共 194 次 NFP 事件 + 3,890 個非 NFP 交易日。策略回測與事件前後漂移數字沿用舊日曆版本，尚未重跑。策略回測含 10 bps 來回交易成本。相關前置研究：K528（NFP 參數 vs 非參數檢定）、K661（NFP 前一日波動）。",
                    "本文波動率數字來自這次 **canonical 重跑**（內部來源路徑已記在文章 metadata，2026-07-20），改用官方 BLS 發布日曆與只向前對應交易日。資料來源：yfinance（SPY, ^VIX），期間 2010-01-01 至 2026-03-30，共 194 次 NFP 事件 + 3,890 個非 NFP 交易日。策略回測與事件前後漂移數字沿用舊日曆版本，尚未重跑。策略回測含 10 bps 來回交易成本。相關前置研究包含 NFP 的參數與非參數比較，以及事件前一日波動分析；內部識別碼已移至文章 metadata。",
                ),
            ],
            "details_patch": {
                "experiment_refs": ["K741", "K528", "K661"],
