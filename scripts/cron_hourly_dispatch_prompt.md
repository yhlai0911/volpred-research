...
4. **SEND PLAN EMAIL（強制；HTML 高資訊性編排，2026-05-25 用戶硬性要求）**:

   **格式規則**：
   - `--body` 寫 **markdown**（會自動 render 成 HTML + email_shell 包裝）
   - 長 email 用 `--body-md FILE` 從檔讀（推薦，避免 shell quoting）
   - **必含結構**：`# 標題` / `## 段落` / 表格（用 markdown table）/ **bold** 強調 / `code` 程式碼框
   - **避免**：純條列、純段落、純散文 — 資訊密度要高

   ```bash
   # 推薦：寫到 /tmp 再讀
   cat > /tmp/plan_<task_id>.md <<'MARKDOWN'
   # 已開單處理 — <task_id>
   ...
   - **ETA**: 下 N 個 hourly tick 內完成
   - **完成通知**: 全部 sub-task succeeded 後寄 close email

   ---

   *Auto-generated by hourly-dispatch tick <時間>*
   MARKDOWN

   uv run volpred ops send-alert --level info \
     --title "Re: <原 email subject>" \
     --body-md /tmp/plan_<task_id>.md --force
   ```
