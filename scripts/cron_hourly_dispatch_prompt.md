...
2. **派工前先 claim**：先確認 `$VOLPRED_TASK_CLAIM_OWNER` 非空，再跑 `uv run python scripts/task_pool_claim.py claim --id <id> --owner "$VOLPRED_TASK_CLAIM_OWNER"`。這是 supervisor 依 slot_id + job_id 產生的唯一且 retry-stable ownership token；缺值必須停止並回報 dispatcher identity error，禁止退回日期/小時或自訂名稱。拒絕 `wrong_status` / `already_claimed` 時換另一 task，禁強推。若 task 帶 `issue_ref`，後續 Git writer 必須傳同一個 `--task-id <id>`。
...
4. 完工標 succeeded/failed：`uv run python scripts/task_pool_claim.py complete --id <id> --status succeeded --result "<摘要>"`。`trending_repost` / `event_article` 若摘要宣告 feed 已發佈 `mile_<id>`，同班必先產出 `storage/drafts/fb_mile_<id>.md`；缺稿時 complete 會 fail-closed，禁止用 `fb_repost_*` follow-up 代替完稿。
   若 task 帶 `repair_lane=self_optimization`，succeeded 前必須另傳
   `--repair-verification-json '{"method":"...","tests":"...","readback":"..."}'`；
   沒有三項真實證據不得結案，也不會寄修復成功通知。
...
2. 若有 entries → 優先處理最舊一條，依 `followup_mode` 分流：
   ...
   四種 mode 都在成功建出 next task 後跑 `uv run python scripts/compute_queue.py mark-followup-dispatched --id <id> --next-task-id <task_id>` 防重派。這個 settlement 與 source task 的 `complete/failed` 結算**順序無關且可重試**：source 已終局時保留終局，已由新 job 接手時保留新 owner；不可因 source 已先結算就跳過本命令。處理完本條 followup 後依 **Batch-drain 原則**：預算 ≥12 分鐘就回 A0 重新判斷、繼續下一張 — **不是本小時收工**。
...
5. 分流決策（token 節省）:
   ...
   - heavy compute (GARCH MLE / Bootstrap / data fetch / 全期 backtest / pooled-MLE multistart) → 改 `uv run python scripts/compute_queue.py enqueue --script <path> --title <T> --result-artifact <path> --output-path <exact-file-path> --followup-brief 'brief' --followup-task-type paper_review --timeout 3600`。`--result-artifact` 是成功後置條件；`--output-path` 是 reaper 可精確 commit 的 ownership（多檔就重複傳，禁傳目錄）。Compute worker cron */15 min 接手；下次 hourly 自動派 interpretation agent（省 60-70% tokens）。注意: 腳本必須完整已寫才能 enqueue。此規則凌駕 topology 預設 — 任何類型的 heavy compute 子步驟都走 compute_queue。
