...
- 結算日：每月第三個週三
- **⚠️ 轉倉處理**：不要直接用 TX1。正確做法：**用 TX（全合約），每日按成交量選最活躍的合約月份**。TX1 在結算日自動換約有 roll gap ~0.5-1.0%。用 TX 按量切換更平滑，在流動性自然轉移時換約而非硬切
- 集合競價：「*」標記
...
詳見 `taiwan-macro-data` skill（`{{SKILL_ROOT}}/taiwan-macro-data/SKILL.md`）。
