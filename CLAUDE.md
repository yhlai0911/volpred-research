...
- 研究續跑採 `slot-aware idle-driven continuation`，不是高頻 heartbeat
- **10 核硬體可並行 3 個 agent**。只要 concurrent agent < 3 就允許啟動下一個（discovery 亦可），不必等 queue 清空
- 單次觸發只啟動一個 agent，但多個 agent 可在不同 K 編號 / 不同 worktree 下**同時** running
