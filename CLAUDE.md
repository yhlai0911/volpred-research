...
- **重要事件前後必須安排研究與文章**：每月初用 WebSearch 查詢未來一個月的重要政治/經濟/金融事件（FOMC、CPI、NFP、GDP、央行決議、大型法說會）。**寫作時點明確規定**：
  - **預告文章**：事件**前 2 天**截止發佈（hard deadline，不可延）。文章寫好可以是 draft，由 cron 在事前 2 天前釋出
  - **解讀文章**：事件**公布後立即**寫並 `--status published`（不等次日，當天就要發）。CLAUDE.md 行 184 規則：事件驅動文章延遲 = 過期
  - 例：TSMC 法說 04/16 14:00 → 04/14 前發預告，04/16 14:00 後立即發解讀
