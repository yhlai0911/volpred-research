...
- **⚠️ 主題重複檢查必須在「決定主題後、啟動寫作 agent 前」完成（不是發佈時才檢查）：**
  1. `grep -i '關鍵詞' storage/reports/feed.json | grep title | head -10`
  2. 若找到同 audience 相似文章 → **不啟動 agent**，除非有明確的新觀點
  3. 若有部分重疊 → 在 agent prompt 中明確指出「已有 XXX 文章講過 Y，這次要從 Z 角度切入」
  4. 高頻重複主題（50/50 配置 10+篇、VT 保險 5+篇、隔夜波動 3+篇）→ **原則上不再寫，除非有新實驗數據**
  5. 浪費資源寫完才發現重複 = 流程失敗（2026-04-01 教訓：K791 隔夜波動文章與 K772 重複）
