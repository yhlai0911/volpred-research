...
- [ ] **價格剛性行業 credit spread 對 FOMC 衝擊敏感度差異作 vol 前哨** — 用 HYG/LQD/VCIT bond ETF + FRED FOMC 會議利率驚喜（Cleveland Fed 免費 surprise series），把 S&P 行業分高/低 price-setting power（NFIB 小企業調查 / 行業加成代理），event study FOMC 窗口 [-1,+5] spread response 差異 + rolling vol 比較，檢定 sticky-price 代理組 spread reaction 是否系統大於 flexible-price 組並對次月 SPY RV 有前哨意義（yfinance + FRED）；不做結構模型，只做截面 event study + vol 預測通道（來源：JFQA Dec 2025 Augustin/Cong/Corhay/Weber Price Rigidities and Credit Risk；sticky-price × credit vol 截面題尚未在 vol forecasting 框架驗證）
...
- [ ] **台股散戶占比 × 近期報酬 interaction 預測台股 RV** — TWSE 公開散戶日成交占比（每日免費）+ yfinance 0050.TW / 個股 daily，建散戶活躍度 z-score × 近 5 日 cumulative return interaction，檢定高散戶日後次日 RV 是否在負報酬時顯著放大（複製 Chordia 行為邏輯但用 TWSE 散戶占比替代 survey 資料）；台股版創新在 TWSE 散戶占比免費公開，台股行為 vol forecast 研究空白（來源：JFQA 2025 Chordia 行為 vol 邏輯；FAJ 2025 sentiment beta；台股 retail-driven vol 研究尚未用 free TWSE proxy 完整驗證）
