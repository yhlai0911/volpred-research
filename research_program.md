...
- [ ] **演算法定價反壟斷執法（RealPage 式租金演算法案件）事件窗對房東 REIT 與旅遊定價股 vol 的影響** — DOJ/FTC 公開反壟斷訴訟/和解公告日曆（RealPage、機票/飯店動態定價案件）+ yfinance 公寓 REIT（AVB/EQR/UDR/ESS）+ JETS/旅遊訂價股籃，檢定執法公告 t-1..t+10 RV、downside、超額報酬是否顯著；與 RL market-maker 流動性不對稱指紋題（line 1156）正交（那是證券市場微結構價差，這裡是租賃/旅遊業演算法定價的反壟斷法律風險）、與 M&A HSR 反壟斷 overhang 題正交（那是併購審查，這裡是既有業務定價演算法法律風險）（來源：JPE/AER 2025-26 algorithmic collusion/pricing 反壟斷法律經濟學熱潮，RealPage 案為指標性案例）。
