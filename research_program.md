...
|---|---|---|---|
| **P5** | **vt-crowding-abm** | 🟢 **K1261+K1262+K1262b evidence COMPREHENSIVE (2026-04-27)** — v1 reproduce GREEN 33/33 + v2 撤回 → **K1261 Phase 1** (10,500 sims H1+) + **K1262 Phase 2** (16,800 sims scaling × window, **H1+ STRONGLY SUPPORTED** 12/12 cells) + **K1262b OAT** (16,000 sims λ/γ ±50%, **H1+ confirmed robust** 5/5 cells, calibration EXACT VT=70% match P5 paper Table 2). 三輪獨立 code review CONDITIONAL/PASS 0 CRITICAL bug. **NotebookLM「knife-edge」critique 被 rebut — TF=30% in 5/5, MR≤VT in 5/5; QUALITATIVE ordering robust ±50% λ/γ; magnitude SHIFTS directionally {70%,100%,70%,70%,70%} mechanism-consistent with Kyle-λ (lower λ → longer VT survival)**. 強 evidence「positive-feedback crowding 是 generic family, VT 是 representative case」, 完整 robustness 三 phase (strategy-spec / market-microstructure / NoiseControl null). knowledge entries: f1d85a74 (K1261) + f3b9edd4 (K1262) + 81ebfe54 (K1262b). **P5 paper rewrite reviewer-resistant** — 等用戶 confirm narrative shift → v3 framing change | 不投稿；等用戶 confirm reframe → v3 paper rewrite (1-2 週 主線程 effort)。Reviewer response framing 已 ready: «qualitative TF/MR ≤ VT robust; VT magnitude 直線型 mechanism response, NOT artifact»。所有 robustness gates 已過 |
