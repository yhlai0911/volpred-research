...
| **P6** | **prg-periodic-garch** | 🟢 **v4.1 done, eligible for ready_for_submission (2026-04-27)** — v4 review 4.2★ + 5/6 gate → v4.1 主線程 batch 5 actions apply: §4.5 Table 5 移除 PRG-vs-GJR row 解 cross-section 6.00 vs 5.24 數值差 + cross-ref tab:main_results / Bollerslev1996→BollerslevWooldridge1992 (QML cite) + bibitem / §4.5 caption 16-init 澄清為 LR diagnostic internal grid / Bollerslev1996 title heteroskedasticity / Todorova Souček diacritic. PDF=15 頁 / bibitems 22 (+1 BW1992) / 0 undefined / paper-update sync ✅. **不跑 v5 review**（純 cosmetic + disambiguation 無新 narrative，token ROI 低）. **預期 6/6 gate PASS** (latex 4.4-4.6 + citation continue GREEN + criterion 4 desk-accept 35-45% PASS) → 下個 slot stage promote ready_for_submission | 不直投稿；下個 slot stage promote + 通知用戶 confirm 投稿 |
