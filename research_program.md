...
|---|---|---|---|
| **P5** | **vt-crowding-abm** | 🟡 **K1261 Phase 1 evidence in (2026-04-27)** — v1 reproduce GREEN 33/33 + v2 latex 4.4★ 撤回 → **K1261 Phase 1 main 10,500 sims (TF/MR/NoiseControl × 7 × 500 MC, 132s wall) provides H1+ evidence**: TF threshold @ 20%, MR @ 50% (real instability 20-30%), NoiseControl null, VT @ 100% (strict criterion vs P5 70% softer). 強 evidence「positive-feedback crowding is generic, NOT VT-specific」. 主線程 caveats: (1) strict detector ≠ P5 criterion (2) MR 30% price collapse legitimate finding (3) TF 50% extreme runaway vol×1500 (4) Codex review pending. v3 paper rewrite 等 Codex pass + Phase 2 robustness sweep (TF/MR scaling ∈ {1,3,5,10}) + 用戶 confirm narrative shift | 不投稿；K1261 evidence pending Codex + Phase 2 → 用戶 confirm reframe to 「positive-feedback crowding family, VT 是 representative case」 |
