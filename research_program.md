...
- [ ] **高維 factor timing 在波動 regime 下是否只是在過度換手** — 用 yfinance factor ETF（MTUM/VLUE/QUAL/USMV/RPV/IVE/IWF）與 FRED macro/vol proxies 建 shrinkage / elastic-net factor timing，與 equal-weight factor basket、12-1M momentum、vol-target baseline 比較；重點不是找最高 Sharpe，而是拆「vol regime 改善」是否被 turnover/cost 吃掉，OOS 用 DM + bootstrap Sharpe diff；與 Factor ETF flow crowding 題正交，這裡是 timing model 本身（來源：Financial Analysts Journal Q2 2025 "Optimal Factor Timing in a High-Dimensional Setting"）。
...
- [ ] **被動資金流對 mega-cap idiosyncratic vol 的放大：SPY/IVV/VOO 代理版** — 用 SPY/IVV/VOO 成交額/AUM proxy 與 top-10 mega-cap idio-vol（CAPM residual RV）建 monthly panel，檢定 passive-flow shock 是否主要放大最大權重股票的 residual RV，而非等比例提高所有成分股波動；與 AI HHI concentration 題不同，這裡識別的是 flow shock → idio-vol amplification 機制（來源：Review of Financial Studies 2025 "Passive Investing and the Rise of Mega-Firms"）。
