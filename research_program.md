...
- [ ] **VT / 動態 risk parity 的「turnover-cost dominance」門檻：何時 monthly rebalance 反勝 daily** — yfinance SPY/TLT/GLD/HYG/SHY 多頻率（daily / weekly / monthly）VT 與 dynamic risk parity 回測，明確套入 0.08-0.35 cost-to-turnover 區間，計算各 rebalance 頻率的 net-of-cost Sharpe / MDD / cost drag 百分比，找出 turnover cost 吞掉 risk-adjusted alpha 的 break-even；與既有 524 EM carry × VT、537 CDaR vs VT 框架題正交——這裡是 frequency × cost 的 net-vs-gross 落差驗證，回應「靜態 RP 低 turnover、動態 RP 高 OOS」的 tradeoff（來源：Journal of Investment Management / arXiv 2025-26 dynamic vs static risk parity；overlay 年化 turnover 420% 案例）
