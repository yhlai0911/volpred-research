...
- [ ] **Quadratic-variation estimator disagreement 作 forecast uncertainty：RV estimator risk band 是否預測下期 QLIKE error** — 用 yfinance 5-min 可得窗口 + daily OHLCV，對 SPY/QQQ/IWM/TLT/GLD/HYG 同時計算 5-min RV、subsampled RV、Parkinson/Garman-Klass/Yang-Zhang、realized-kernel-lite / noise-robust proxy；把 estimator dispersion 當 measurement-uncertainty 訊號，檢定它是否預測 HAR/GARCH 下期 QLIKE error、MCS set 大小、以及 vol-target position size 應否降槓桿。與 line 1211 MCS dispersion 不同，這裡是「波動率量測器本身的分歧」而非模型集合分歧（來源：JFEC 2025 high-frequency quadratic variation estimator comparison；noise-robust estimator vs 5-min RV）
