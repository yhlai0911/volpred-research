...
- [ ] ⚠️(medium-low, dispatch 前查期刊分量) **區間值（interval-valued / range-based）RV 預測 vs 點預測 HAR** — 把日 OHLC 的 [low,high] range 視為 interval-valued time series 物件直接預測整段區間，與傳統 point HAR-RV 比 interval-forecast accuracy（coverage + interval MSE）；interval-valued/fuzzy grep=0（來源：Journal of Forecasting 2026 fuzzy/interval RV；頂刊分量較弱 → ⚠️ dispatch 前確認是否值得投）。
