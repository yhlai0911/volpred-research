...
- [ ] **Firm-level labor-shortage exposure 對 wage-sensitive idio RV 的截面預測力** — 用 SEC 10-K / 10-Q labor-shortage mention、BLS JOLTS industry openings-quits、yfinance listed firms / sector ETF 建 firm-level shortage exposure proxy，檢定高曝險公司在 t+5/t+22 idio RV、downside semivariance、automation/AI-capex basket relative vol 是否放大；所有 textual / industry signal 必須 `groupby(ticker).shift(1)` 或用 filing-date release lag。與 line 1324 labor-force growth / immigration 題正交—— 那是 macro labor-supply sector shock，這裡是 firm-level textual/segment exposure 的 cross-section risk prior（來源：RFS 2026 "Firm-Level Labor-Shortage Exposure"；labor constraint + automation response 是新 corporate-finance 熱點）。
