...
- [ ] **回購靜默期（buyback blackout）覆蓋率作為市場層級 vol 日曆因子** — yfinance earnings_dates 建 S&P 500「處於 blackout 的市值占比」日序列（5 週前~2 日後窗），檢定高覆蓋期 SPY RV 是否系統性偏高、與 OPEX/月曆效應正交化後倖存（J. Accounting & Economics 2025 corporate calendar 與回購時點 + practitioner flow 線；「回購是最大單一買盤、blackout 同步消失」缺正式日頻檢定）。與 #174（盈餘 jump 截面）不同——aggregate 供需日曆弧。
