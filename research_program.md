...
- [ ] T+1 結算制度（2024-05-28 生效）對美股/ETF 隔夜 gap 與 realized vol 的結構斷點 — yfinance SPY/QQQ/IWM + 一籃 ADR（settlement gap 最敏感）+ 大型 ETF，以 2024-05-28 為斷點做 event study / regression discontinuity，檢定 T+1 後 (i) 隔夜 close→open gap variance、(ii) 月底/季底再平衡日 RV、(iii) ETF vs 成分股 RV 裂口是否系統性改變；明確分前後子樣本、對齊國際 calendar 避免 date drift（來源：SEC T+1 rule 2024；LSEG/Clearstream 2025 settlement-cycle market-quality briefs；學術實證仍空白 = 先行機會）
...
- [ ] **Gaming / sports-betting / esports 籃子的 vol spillover 與 risk-on 訊號** — yfinance ESPO/HERO/NERD/GAMR + DKNG/FLUT/HOOD + QQQ/ARKK/BTC，做 DCC/VAR/Diebold-Yilmaz spillover，檢定 gaming/betting basket 是否在 market turmoil 前後變成高 beta transmitter 或 diversification sink；與 meme/gamma-squeeze 題正交（來源：JAI Spring 2026 gaming volatility spillovers；gaming 作 alternative exposure 被重新評估）

- [ ] **optflowcrowd 總體選擇權買權需求差作 short-vol timing proxy** — 個別 order-flow 資料需付費；先用 CBOE put/call ratio、OCC aggregate volume、Cboe index/equity split 建免費 proxy，檢定 call-buying pressure 是否預測 SPY excess return、VIX change、short-vol strategy drawdown；與 VIX/SKEW level 題正交，聚焦「訂單流 crowd」（來源：JFQA 2026 forthcoming Betting Against the Crowd）
...
- [ ] **Fedspeak forecast-revision shock 作 equity/bond tail-vol prior** — 抓 Fed 官網公開演說全文與行事曆，先用 dictionary / FinBERT-lite 分 GDP、inflation、labor 三類 forecast-revision tone，全部 `shift(1)` 加入 SPY/TLT/IEF/QQQ HAR-RV 與 5d left-tail 分類；與 FOMC event / macro attention 題不同，這裡是 between-meeting central-bank speech 的資訊 shock，而非正式決議日或一般新聞量（來源：Journal of Econometrics 2025 central-bank-speech NLP；FOMC-member speech forecast revisions 可解釋 equity/bond vol 與 tail risk）

- [ ] **ETF sampling arbitrage 是否只放大 liquid constituents 的 vol/comovement** — 用 SPY/IVV/VTI/IWM/MDY/sector ETF 公開 holdings + yfinance 成分股流動性 proxy，建「ETF basket sampling tilt」與 tracking-error/volume shock，分 liquid/illiquid tercile 檢定 ETF shock 後成分股 RV、market beta、close-to-close comovement 是否異質；與 ETF macro-efficiency / heartbeat 題正交，聚焦 creation-redemption basket sampling 的截面異質性（來源：JFQA 2026 ETF Sampling and Index Arbitrage；AP arbitrage 對 liquid stocks 的 volatility / market-quality 影響更強）
...
- [ ] **Bond mutual fund demandable-equity run proxy 對信用 ETF vol 的領先性** — 用 ICI/FRED 可得 fund-flow 或退而求其次用 AGG/BND/LQD/HYG 成交額 shock、折溢價 proxy 與銀行存款 / MMF assets，建 open-end fixed-income fund liquidity-transformation 壓力指標，檢定是否領先 HYG/LQD/BKLN/TLT RV 與 credit-equity downside correlation；與既有 bond ETF liquidity 題不同，這裡從 mutual-fund equity / swing-pricing 設計角度測 run-like redemption 壓力（來源：RFS 2026 Bank Debt, Mutual Fund Equity, and Swing Pricing in Liquidity Provision；open-end funds 作非銀行流動性提供者與 run risk）
...
- [ ] **Structural VIRF shock library：把重大歷史衝擊轉成 volatility impulse response templates** — 用 SPY/TLT/UUP/GLD/HYG 多資產日報酬估簡化 BEKK/DCC 或可行 MGARCH，對 2018Q4、2020-03、2022 rate shock、2025 tariff shock 建 historical / scenario VIRF，量化衝擊後 vol 與 covariance persistence；與一般 VAR/Diebold-Yilmaz spillover 題不同，這裡是 second-moment impulse response 與 confidence band，服務風控情境庫（來源：Journal of Financial Econometrics 2025 Structural Volatility Impulse Response Analysis；VIRF 被擴展到可識別、可做 scenario 的 volatility shock response）

- [ ] **Climate-news duration 而非 climate-news level：green/brown 反應時間差是否預測 tail risk** — 用 RSS/GDELT climate-news clusters 建 event duration / decay proxy，搭配 ICLN/TAN/XLE/XOP/XLU 與可得 green/brown baskets，檢定同一 climate-news shock 下「反應時間較長」是否預測後續 RV、VaR/ES 與 green-brown correlation spike；與既有 green-minus-brown RV spread / physical climate event 題不同，這裡聚焦 news-attention duration 與價格反應速度（來源：JBF 2025 climate-news response-time model；daily 與 intraday duration 可改善 VaR/ES 風險統計）
...
- [ ] **biodiversity transition-risk 商品籃的 RV 與 tail repricing** — 用 JNCC/SEI Commodity Footprints 開源 biodiversity intensity（若欄位取得受阻，先用高曝險農產品/林業 proxy：CORN/SOYB/WEAT/CANE/JO/WOOD/DBA）+ yfinance commodity ETF/futures，圍繞 Kunming-Montreal / TNFD / deforestation policy 事件窗檢定高 biodiversity-footprint 籃是否有更高 RV、downside semivariance 與政策後負 abnormal return；與 green-minus-brown 氣候 vol 題正交，這是自然資本/生物多樣性 transition risk（來源：Review of Finance 2026 "The pricing of biodiversity risk in commodity markets"；高 biodiversity footprint 商品 risk premium / event repricing）

- [ ] **retail structured-product complexity 的免費 ETF proxy：複雜產品熱度是否預測 tail-risk mispricing** — 用 option-income、defined-outcome/buffer、single-stock leveraged/inverse ETF 籃（JEPI/JEPQ/QYLD/XYLD、PJUL/BUFF 類、TSLL/NVDL/CONL/TSLQ 等）建「complex retail payoff demand」成交額/資產價格熱度 proxy，檢定複雜產品成交額 shock 是否領先標的 RV、upside cap 後 reversal、或 VIX/VVIX change；若缺完整 launch/AUM，先做 yfinance volume diagnostic，不宣稱結構型票據原文 replication（來源：Review of Finance 2026 "Competition, complexity, and security design: evidence from retail investment products"；complexity 與 tail risk / investor comparison friction）

- [ ] **Friday / triple-witching closing-auction concentration 作隔日 RV 與 reversal 訊號** — yfinance 或本機 5-min SPY/QQQ/IWM + 大型股樣本，標 Friday、monthly options expiry、triple-witching dates，計算收盤 30min volume share / range-vol concentration，檢定當日收盤 auction crowding 是否預測隔日 open-to-close RV、reversal 與策略交易成本；與 Russell reconstitution 尾盤 dislocation 題正交，這裡是常態 options-expiry / auction mechanism 而非指數重組事件（來源：JFQA FirstView 2026 "Price Impact in Closing Auctions, Opening Auctions, and Continuous Markets"；closing auction cost/liquidity 與 Friday/triple-witching volume spikes）
