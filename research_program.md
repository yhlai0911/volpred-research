...
- [ ] 下行 VRP 與上行 VRP 反號分解 + 跨 horizon 預測力差異 — 用 ^VIX/realized variance 拆 upside/downside semivariance 算 downside-VRP vs upside-VRP（免期權版用 RV 半變異代理），檢定下行 VRP 是否顯著大於整體、對 SPY 1/3/6 個月報酬與 RV 的預測力是否集中在 3-5 個月；與既有「隔夜 vs 日內 VRP 反號」（時段切割）正交，這裡切上行/下行（來源：JFE/SSRN 2025 downside variance premium 主導；明確 lag）
...
- [ ] 商品 inventory/seasonality surprise 的「regime-conditional」預測力 — yfinance CL=F/NG=F/USO/UNG/DBA RV + FRED 季節 dummy，檢定供給中斷/低庫存 regime 下季節性對 commodity RV 的預測力是否被放大（供給彈性低時 surprise 衝擊更大）；與既有 inventory-surprise feature（全期 baseline）正交，聚焦 supply-tightness regime 交互（來源：Oxford Economics/BIS 2026 commodity supply-driven volatility；低 producer margin 放大農產敏感度）
...
- [ ] Conditional Expected Drawdown (CED) 作為 VT 策略風險目標 — 用 SPY/QQQ/IWM 日 OHLC 算固定 horizon (20d/60d) 的 drawdown 分佈 tail-mean，比較 CED-targeting 與傳統 vol-targeting 在 Sharpe / MDD / Calmar 的差異（yfinance，明確 signal lag）（來源：JPM 2025-26 drawdown control 系列；CED 易解釋且 practitioner-friendly，是 vol-target 升級候選）
- [ ] Intraday vs Overnight VRP 分解的跨資產差異 — SPY/QQQ/IWM/EFA 用 close-to-open vs open-to-close realized variance 拆 VRP，檢定 overnight VRP 是否承擔 majority of premium、是否預測 next-day RV（yfinance；無 options 用 GARCH-implied 代理 IV）（來源：J. Futures Markets 2025 Papagelis；overnight 時段研究是 2025-26 熱點）
- [ ] Risk-regime correlation breakdown 早期偵測 — 用 SPY/TLT 60d rolling correlation + DCC-GARCH，建 regime classifier（low-corr / decoupled / inverted）並檢驗 regime 轉換前 N 日 vol-of-correlation 是否異常上升（yfinance）（來源：CFA Institute "Risk Regimes and Portfolio Resilience" 2026 系列；2020/2022 都是 corr breakdown 觸發配置失敗）
...
- [ ] EM FX carry × FX vol regime 的雙閥門進出場 — 用 USDBRL/USDMXN/USDZAR/USDIDR daily 算 carry signal（rate diff via FRED），FX vol 用 60d realized；只在 carry > threshold 且 FX vol < threshold 時開倉，OOS Sharpe / MDD vs 純 carry baseline（yfinance + FRED）（來源：Bloomberg 2025-12 carry 17% 報酬 + ING 2026 outlook；2026 大議題但學術完整 OOS validation 缺）
- [ ] Volatility skew 作為股票借券費代理的 cross-section 預測 — 用 ETF skew（SPY put-call IV spread 從歷史 GARCH 推）的横截面排序，檢驗高 skew 個股次月 returns underperformance（小範圍 ETF universe，yfinance；無 options 用 ML-based skew proxy）（來源：JFE 2025-26 Why does options market info predict stock returns；borrow fee 作為 limit-to-arbitrage 新解釋）
...
- [ ] 收益率曲線 steepener regime 預測股市 vol 與 sector rotation — 用 ^TNX-^IRX 或 ^TNX-^FVX 曲線斜率變化 dV/dt（steepening rate）而非斜率水平，檢定 fast-steepening 期 SPY vol jump、XLF/XLU sector vol 差異（yfinance）（來源：Nuveen / LPL 2026 outlook 一致預期 steepener；多數研究只看 level 不看 dV/dt）
