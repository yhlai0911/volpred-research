...
|---|---|---|---|
| **P5** | **vt-crowding-abm** | 🟢 **K1261+K1262 evidence STRONG (2026-04-27)** — v1 reproduce GREEN 33/33 + v2 撤回 → **K1261 Phase 1** (10,500 sims TF/MR/NoiseControl/VT 132s) verdict H1+ + **K1262 Phase 2** (16,800 sims TF/MR × scaling {1,3,5,10} × window {10,22,60} × 7 × 100 MC, 600s) verdict **H1+ STRONGLY SUPPORTED**: TF threshold < VT in 12/12 cells under softer detector + MR < VT in 12/12, direction robust NOT knife-edge. **P5-style (Sharpe-only) detector calibrates EXACTLY to VT=70% matching P5 paper Table 2**; under same detector TF=20%/MR=20%/NoiseControl=null — both faster than VT. 兩輪獨立 code review (Codex CLI fallback to feature-dev:code-reviewer) CONDITIONAL PASS, 0 CRITICAL bug. 強 evidence「positive-feedback crowding 是 generic family, VT 是 representative case (largest AUM + real-world λ/γ amplification)」. **P5 paper rewrite STRONGLY SUPPORTED** — 等用戶 confirm narrative shift → v3 framing change | 不投稿；等用戶 confirm reframe → v3 paper rewrite (1-2 週 主線程 effort)，K1262b λ/γ OAT 改 confirmatory（非 essential）|
