...
- [ ] **地理型 investor attention / local information advantage 的波動效應** — 用公司 HQ 州、Google Trends state-level search、地方新聞量與 yfinance 個股資料，拆 local vs nonlocal attention shock，檢定是否預測個股 RV、gap risk、bid-ask proxy 與非本地 attention；先從大州/大市值公司做可行 pilot。與一般 attention 題正交——把 attention 變成 spatial channel，並直接測 local information advantage 對 volatility/liquidity 的傳導（來源：RCFS 2025 geography of investor attention；local attention predicts volatility and spreads 是可轉譯 VolPred angle）。
