...
- [ ] **Hedge-fund Treasury crowding 風險定價** — 用 Fed FEDS 2026 hedge-fund U.S. Treasury exposure（大戶 gross exposure 2023→2025 倍增）+ CFTC Treasury futures positioning、FRED rates、TLT/IEF/ZN=F 建 crowding proxy，所有 positioning/funding signals release-lag 後 `shift(1)`，檢定是否領先 Treasury RV、yield jump 與 SPY-TLT correlation regime；與 repo-basis funding stress 題正交——那是 SOFR/EFFR funding spread，這裡是持倉擁擠/forced-deleveraging channel（來源：Fed FEDS Notes 2026 hedge-fund Treasury exposures + IMF GFSR 2026 NBFI amplification risk）。
