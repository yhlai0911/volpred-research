...
- [ ] ML 強化的日頻流動性測度（CPQS proxy）作系統性流動性風險 → vol 通道 — yfinance 美股/ETF 日 OHLCV，建 Closing-Percent-Quoted-Spread proxy（非 Amihud，與 line 481/554 正交）+ boosting/NN 學非線性映射估日頻 spread，再把估出的系統性流動性風險因子加入 HAR-RV / 跨資產 vol 檢定其 OOS 增量；明確 lag（來源：Dai-Shi-Zhang JBF 2025「Estimating Market Liquidity from Daily Data: Marrying Microstructure Models and Machine Learning」；CPQS 為最佳 percent-cost daily proxy，1000-10000× 計算節省）
...
- [ ] **能源新聞 topic shock 對原油與能源股 RV 的 OOS 增量** — 用免費新聞/RSS/GDELT 標題建 oil-market topic/sentiment 指標（先做可重現 lexicon/topic baseline，不直接依賴封閉 Thomson Reuters），加入 CL=F/USO/XLE/XOP HAR-RV 與 EIA inventory controls，比較 QLIKE/DM 與 event-window RV；與 line 464 原油 vol spillover、line 557 inventory NULL 正交——這裡測「文字 topic flow 是否提供非價格/庫存資訊」（來源：Financial Analysts Journal 2026 "Big Data Meets the Turbulent Oil Market"；能源 NLP 指標對油價、vol、inventory、能源股 OOS 有增量的趨勢）

- [ ] **演算法做市失靈的 spread-asymmetry 指紋：對稱成本衝擊下流動性是否單邊惡化** — yfinance 日 OHLCV 建 Roll/Corwin-Schultz/CPQS proxy，選 SPY/QQQ/IWM + 高電子化 mega-cap basket，在 VIX/成交量對稱 shock 後檢定 bid-ask proxy 是否有非對稱擴張、且是否預測 t+1..t+5 RV；與 K1355 generic CPQS liquidity predictor 正交——這裡聚焦 RFS 2026 reinforcement-learning market maker 的可觀測「spread asymmetry signature」，不是再做流動性指標賽馬（來源：RFS 2026 Algorithmic Pricing and Liquidity in Securities Markets；AI/Q-learning 做市與市場穩定性成為新 microstructure 熱點）

- [ ] **AI 勞動收入曝險 × sector ETF vol：人力資本 shock 是否改變投組風險承擔** — 用 O*NET/免費 AI exposure score 對行業做 AI-labor exposure，映射到 sector ETF（XLK/XLC/XLY/XLI/XLV/XLF 等）與 BLS wage/employment surprise，檢定高 AI-exposure 行業在 AI adoption news / layoff shock 後 RV、downside semivariance、sector correlation 是否升高；與 AI capex/HHI/電力需求題正交——這裡是 household labor-income hedging channel（來源：JOIM 2026 Q2 "Adapting to AI: Rethinking Labor Income and Retirement Design"；AI 對 lifetime income/退休設計成為投資管理議題）

- [ ] **五種 realized skewness 估計法的 multi-asset tail premium 賽馬** — yfinance ETF/futures proxy（SPY/QQQ/TLT/GLD/USO/UUP/FXY/HYG/EEM）月頻，用 robust skew / downside-upside semivariance / trimmed tail 等 5 種 estimator 建 total-skewness signal，檢定跨資產下一月報酬、RV 與 left-tail exposure；與 line 1095 FX skewness premium 正交——這裡是 multi-asset estimator robustness，而非單一 FX 橫斷面（來源：JOIM 2026 Q2 "A Tail of Five Skews"；多資產 skewness risk premium 與 estimator choice 是 2026 實務題）

- [ ] **Prediction-market implied probability shock 作 macro event vol prior** — 用 Kalshi/Polymarket 公開 market price/API（先抓 FOMC、CPI、NFP、election/geopolitical categories）建 event-probability change，檢定是否領先 VIX9D/VIX、SPY RV 與 event-day tail move；所有 signal 用 `shift(1)`，並與既有 SOFR/FedWatch/FOMC prior 框架比較；與 FOMC 單事件 prior 正交——這裡測新興 event-contract market 是否是即時風險 prior（來源：Pew Research Center 2026 prediction-market volume surge；event contracts 交易量 2025H2-2026 快速擴大）

- [ ] **Gaming / sports-betting / esports 籃子的 vol spillover 與 risk-on 訊號** — yfinance ESPO/HERO/NERD/GAMR + DKNG/FLUT/HOOD + QQQ/ARKK/BTC，做 DCC/VAR/Diebold-Yilmaz spillover，檢定 gaming/betting basket 是否在 market turmoil 前後變成高 beta transmitter 或 diversification sink；與 meme/gamma-squeeze 題正交——這裡是 digital-entertainment/alternative-investment sector spillover，不是單股散戶 squeeze（來源：Journal of Alternative Investments Spring 2026 gaming volatility spillovers；gaming 作 alternative exposure 的角色被重新評估）

- [ ] **Aggregate option order imbalance 的 market-risk-premium 與 short-vol timing proxy** — 付費個別 options order imbalance blocked；先用 CBOE put/call ratio、OCC aggregate option volume、Cboe index/equity options split 建免費 proxy，檢定 call-buying imbalance 是否預測 SPY excess return、VIX change、short-vol strategy drawdown；與 VIX/SKEW level 題正交——這裡聚焦「訂單流 crowd」而非 implied level（來源：JFQA 2026 forthcoming "Betting Against the Crowd: Option Trading and Market Risk Premium"；option order imbalance 的市場風險溢酬訊號成為新前沿）
