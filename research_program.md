...
- [ ] **Anti-stockholder identity /「不持股」情緒作 retail-risk-off volatility proxy** — 用 Google Trends（stock market gambling / stocks are rigged / do not own stocks 類詞）、Gallup/Fed SCF stock-ownership low-frequency anchor、AAII sentiment、FINRA margin/retail proxy（若可得）建 anti-stock identity index，檢定其是否領先 meme/retail-heavy basket、ARKK、IWM、single-name high-idio-vol basket 的 t+5/t+22 RV、volume crash 與 downside semivariance；必須和 generic bearish sentiment / VIX level / recent return 控制分離。與 investor-memory、attention-utility、social-transmission 題正交—— 這裡是 non-participation identity 與股東形象，而非既有投資人注意力（來源：RFS 2026 "Proud to Not Own Stocks: How Identity Shapes Financial Decisions"）。
