...
- [ ] **Missing / stale market data 作 liquidity-vol prior：缺資料不是清洗問題，而是風險訊號嗎** — 用 yfinance 小型股 / microcap ETF / 低流動 ETF universe，記錄 missing row、zero volume、stale close、異常 corporate-action gap、以及資料恢復日，檢定這些 data-quality events 是否領先 next 5d/22d RV、gap risk、delisting-like drawdown；用大盤/成交額/價格 level 控制，避免把資料缺口誤當 alpha。與既有 liquidity proxy（Amihud/Corwin）正交，這裡研究「missingness pattern」本身是否是 liquidity stress proxy（來源：RFS 2025 Missing Financial Data；missingness 具系統性且影響 risk-premium / portfolio construction）
