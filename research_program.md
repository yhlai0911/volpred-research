...
- [ ] **CTA / 趨勢策略 crisis-alpha 基率重檢：免費 ETF proxy 下 trend-following 在 stress regime 真有 alpha 還是只是 mean-reverting noise** — yfinance KMLM/DBMF/CTA + SPY/AGG，建 12-month time-series momentum signal（Moskowitz-Ooi-Pedersen 標準）proxy，shift(1) 後在 VIX≥80%-ile 或 SPY drawdown≥10% 子樣本內計算 CTA-vs-SPY excess return、Sharpe、MDD、bootstrap Welch t；包含 2020/2022/2025-tariff 三個 crisis；嚴格 Harvey gate；與 line 1215 hedge-fund alpha-dispersion（dispersion angle, NULL closed）區別 — 這裡是 trend signal alpha existence test（來源：JAI 2026 trend following crisis alpha 持續主題；Greyserman-Kaminski 框架在 2022/2025 後需重檢）
