...
- [ ] **FX predictability complexity penalty：非線性模型是否只在小樣本看起來有效** — 用 FXE/FXY/FXB/FXA/UUP/CEW + FRED fundamentals，比較 random walk、linear ridge、Ridge-RFF/kernel features 對 FX return、RV、left-tail indicator 的 OOS；用 Clark-West / DM / bootstrap Sharpe 檢定 12/60/120 月訓練窗下 performance decay；與 FX skewness/carry 題正交（來源：Fed FEDS 2025 "Virtue or Mirage? Complexity in Exchange Rate Prediction"）

- [ ] **permanent-capital insurance platform integration 是否改變 alternative-manager vol beta** — 用 BX/KKR/APO/ARES/BAM/CG + insurer partners（MET/PRU/EQH/GL）公開併購/partnership dates、credit spread 與 insurer ETF proxy，檢定 integration 後 AAM basket 的 beta、RV、credit-spread sensitivity、downside correlation 是否 regime shift；與 private-fund LP cash-flow stress 正交（來源：JAI 2026 "Permanent Capital Meets Private Markets"）

- [ ] **低風險組合建構參數不確定性作 ex-ante model-risk band** — 用大型股 / ETF panel 建 low-vol / min-vol portfolio，系統改 covariance estimator、lookback、權重上限、long-only / limited-short、turnover cost，量化同一 signal 下 realized vol、tracking error、drawdown 與 net Sharpe 分散；與 USMV/SPLV 因子報酬題不同，測 implementation model risk（來源：JPM 2026 "Low Risk, High Variability"）
