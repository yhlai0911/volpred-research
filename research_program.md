...
- [ ] 已實現離散度（指數 vs 成分股）作為指數波動率與尾部的預測子 — 計算 0050.TW/SPY 指數 RV 與成分股平均 RV 的 realized dispersion，檢定高離散是否領先指數 vol spike / drawdown；yfinance 成分股日資料（免付費選擇權版）（來源：dispersion / implied-correlation risk premium 2025 實務熱點）。差異化：純 realized 版免資料成本，庫內 line 504/530 dispersion 多談 implied。
