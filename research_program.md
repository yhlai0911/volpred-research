...
- 狀態：✅ 初稿完成（2026-04-10）
- 核心實驗：K988/K988b（17 規格比較）、K994（跨資產）、K995（VaR/ES）、K997（本地 fear index）、K998（VRP 預測 null）、K1685（K1393-faithful 延長 OOS）
- 核心發現：A4f（VIX², free ω）對 GJR 的完整 OOS 點估計優勢在 K1685 延長至 2026-07-10 後仍存活（n=1,890；QLIKE 1.431558 vs 1.515680；canonical DM t=+3.9656），但 symmetric 12-start robustness 僅 t=+3.0098、lag-10 sensitivity=2.9901；裁決為 `GO_WITH_FRAGILITY_DISCLOSURE`，賣點只能是方向性優勢與簡約性，不可宣稱顯著性廣泛穩健。
- [x] **P0-2 實證 gate（K1685，2026-07-12）**：K1393 legacy anchor t=3.602900965 精確重現；post-hoc full-population parameter audit 覆蓋 primary 30 + multistart 30 refits，selected fits 全 finite/定態，Codex primary + 兩個 fresh-context reviewers PASS。另發現 Paper 9 舊 CSV 在 2026-05-04～05-15 有 10 個重複日期，K1391 reversal 同時混入 degraded spec 與資料污染；舊 CSV 不手改，collector race/de-dup 另修。手稿 endpoint 與 sensitivity disclosure 綁定留在 P0-1 主線程。
...
- [x] ~~台灣 5-min 數據 HAR-RV（0050.TW 47 天，ETA 2026 Q2）~~ — **K1664 completed 2026-07-10, pilot directional only**：本機 `data/intraday/0050_TW_5min_*.csv` 已累積 108 個有效 0050.TW 5-min 日檔（2026-01-20..2026-07-08；每日至少 53 rows），可重建 intraday RV。Lag-clean log-HAR OOS（`raw_signal.shift(1)`；22d lag + 60-row expanding train → OOS n=26）顯示 HAR_DW_log QLIKE 0.1333 vs persistence 0.1933（+31.0% improvement）但 DM t=-1.64，未過 Harvey |t|>3；HAR_DWM_log +18.4% 亦未顯著。結論：0050 5-min RV pipeline 已可用，HAR(d,w) 有方向性 edge，但樣本太短，只能記 pilot；需繼續累積到 ≥252 trading days 再做 formal HAR-RV gate。knowledge=06430647。
...
- [ ] Sentiment-Augmented GARCH-LSTM — Computational Economics 2025 · **⏸ DEFERRED (C14 2026-07-06, blocked_until 2026-09-04)**：架構=GARCH-LSTM 已 NULL (K1312)，新意僅情緒外生變數，落在「新 covariate→HAR OOS」飽和弧 + 免費情緒資料覆蓋風險。
- [ ] KAN for VIX Forecasting — Expert Systems with Applications 2025 · **⏸ DEFERRED (C14 2026-07-06, blocked_until 2026-09-04)**：KAN 已 NULL (K1263, QLIKE +23.7% worse than GJR, ML ceiling 第 7 次)，唯一新意 target=VIX 非 RV，但日頻 ML ceiling 9x-confirmed，prior 強 NULL。
...
> **📋 C14 diversity-rule 冷卻評估（2026-07-06）**：4 筆 auto-generated ML 架構實驗（K1383 PatchTST / K1385 Sentiment-GARCH-LSTM / K1388 HAR-GNN / K1389 KAN-for-VIX）自 5/27 起因 NULL-quartet diversity rule 凍結但 `blocked_until=null`（永久凍結治理 gap）。評估結論：**4 筆全在日頻波動率 ML ceiling 飽和弧（9x-confirmed，見下方 §核心結論 C），無一值得試水** —— KAN 已 NULL (K1263)、GARCH-LSTM 已 NULL (K1312)、GNN/Transformer 皆架構重複。研究誠實 > task 原本「解封 1 筆試水」的預設：試水任一筆 = diversity rule 要防的 ML treadmill。**行動**：4 筆全補 `blocked_until=2026-09-04`（≈季度 re-eval，非永久凍結）；未來 ML 探索budget應投**新問題類別**（如 intraday/high-freq、probabilistic quantile、regime-conditional）由 research-topic-discovery 挖掘，**不再對日頻點預測疊架構**。

### Rough Volatility & Hurst
- [x] **K1624 波動率長記憶「真假」識別（≠ path roughness，勿與下列 rough-vol 混淆）**（2026-07-04）：RV 表觀 long memory (d≈0.4) 主要是 **level-shift 假象、非真 fractional integration**。SPY(N=8413)/^GSPC(N=10455)/0050.TW(N=4282) daily OHLC + Garman-Klass range 與 |r| proxy；對 Bai-Perron/PELT 偵測 breaks demean 後 Local Whittle d_post 崩落，true-LM parametric bootstrap(B=200，已 method-validate break-test over-rejection) 判 **5 spurious + 1 mixed**（僅 SPY range 留弱真 LM），|r| proxy 三市場一致 spurious → 複現 **Granger-Hyung (2004)**。OOS(expanding, 月 refit, n=1000) **ARFIMA(0,d,0) 從不勝 HAR**（HLN-DM t=+10~+13 且數值脆弱），break-robust HAR 不系統性勝 plain HAR。方法論指引：**RV 建模用短記憶 HAR + 允許 level shifts，優於假設 fractional integration 的 ARFIMA/FIGARCH**（呼應 K442 FIGARCH d=0.61 無 OOS 增益）。Codex gpt-5.5 xhigh reviewed（1 HIGH lookahead fixed+rerun）。knowledge=faedf43c。
...
- [ ] **K1732 extension — IS_k regime 訊號的策略疊加測試**（2026-07-19 補，源自 K1732 台股 29 年回測 + 7/17 大跌事件）：K1732 已確認 TAIEX 峰態影響份額 IS_k 死亡交叉 regime 與未來 13 週崩跌起點有顯著關聯（17%→32%，circ-shift p=0.0138），但**未測任何部位規則**。延伸題：IS_k 警戒 regime 疊加 VT/減碼規則的淨績效（含交易成本、exposure-matched MDD gap + phase-randomization null，遵守 MDD scale artifact 硬規則）；次題：IS_k 訊號在 SPY/日經/其他亞洲市場的 out-of-market 驗證（論文 19 指數的 unconditional 版本）。相關：Lai & Chang (2409-662)、K836、K921。（2026-07-07；research_backlog all-covered fallback → hourly-20 派 general-purpose agent 掃 JPM/FAJ/JFM/JFEC/JBES/JoE/AER 2025-26 趨勢層級補料。刻意鎖 forecast-eval 方法學護城河（fluctuation test / Mincer-Zarnowitz / realized-cov / HARQ-contrarian）+ 淨成本 VT + realized-dispersion / VRP 時段結構，避開兩個飽和 NULL 弧。）
> 來源：journal-discovery agent（WebSearch 2025-26 趨勢層級，未捏造論文標題+作者+期號）。全免費資料 yfinance/FRED/TAIFEX 或平台自有 DM ledger。**主線程 grep 去重砍 5 條**：VT 報酬歸因（撞 Paper 3 + K46/K53/K79/K1044）、naïve 期貨尾部避險（arc-dup line 598 naive-hedge robustness）、crypto funding→RV regime（撞 line 1472 perp funding 清算循環）、台海 GPR 事件窗（撞 line 536 台灣 country-GPR）、氣候→商品 RV event-window（撞 line 482 飽和 NULL 弧「外生 shock→次日 RV event-window」~15 連 NULL）。存活 9 條真正正交。4 條零額外資料成本（純內部 DM ledger 升級 = 方法學護城河）。
- [ ] Net-of-cost 平滑波動率目標 + no-trade band — 對 TAIEX/SPY VT 加 21 日訊號平滑與再平衡帶寬，量測換手率下降幅度與扣除實際台股/美股交易成本後的淨 Sharpe（毛 vs 淨對照）；yfinance + 台股手續費/證交稅參數（來源：FMPM 2025 optimal rebalancing boundary、Man Group 2026；VT turnover 200%+ 侵蝕淨績效是 2025-26 實務熱點）。差異化：no-trade band 在庫內 0 覆蓋，line 76 K49 是機制非淨成本。
- [x] **K1701 已實現離散度（指數 vs 成分股）預測指數波動率與尾部** — `experiments/k1701/` completed 2026-07-13，正式 verdict **INCONCLUSIVE_NO_EXACT_NULL_CLAIM**。SPY/S&P 500 與 0050.TW/臺灣 50，2010-01-04 至 2026-07-10；primary 2 資產 × 3 horizon 改用 paired fixed-756 rolling Giacomini–White + Patton QLIKE，正向 gate 0/6，但 Holm 後能排除至少 1% QLIKE gain 亦為 0/6，故不可再沿用舊版 raw-DM「NULL 已證實」措辭。識別階梯仍支持較窄結論：`rdisp` 與成分股平均波動水準是同一 regressor span；控制水準後 `rho` / `csvd_rel` 的 MSE + Clark–West + BH family 0/12 通過，未發現真正離散度的穩健增量證據。raw expanding-window DM 全部降為 diagnostic-only；lookahead truncation、j+h<i embargo 與六格同訓練日 / 756 筆 audit 全通過。
- [ ] VRP 的交易時段 vs 非交易時段拆解（免付費代理版）— 用 realized vol 對 VIX/VIXTWN 的 gap 當 VRP proxy，拆成日盤段與隔夜段，檢定兩段 VRP 的可預測性與對後續波動的資訊含量差異；yfinance + FRED VIX（來源：JFM 2025「VRP over trading and nontrading periods」）。差異化：聚焦 VRP 時段結構而非 RV 時段動能（庫內 overnight/intraday 多談 RV 動能）。
- [ ] 時序基礎模型 × HAR 的預測組合誠實檢定（問組合是否進 MCS，不是問誰贏）— 對 SPY/0050.TW/TX，把免費開源 TSFM（TimesFM/TTM 權重）與 Log-HAR 做 equal-weight 與 bias-corrected 組合，用 MCS + DM-HLN 檢定組合是否穩定進 superior set；免費開源模型 + yfinance/TAIFEX（來源：2025-26 TSFM vs 計量 benchmark + corrected forecast combinations 熱潮）。差異化：庫內 ML ceiling 9 次確認 → 定位為 forecast-combination 方法學（庫內 combination 僅 3 hits），非再證 ML 單獨獲勝。
- [x] Giacomini-Rossi fluctuation test 回溯 VolPred 既有 DM ledger — `experiments/research_giacomini_rossi_fluctuation_test_volpred_dm_ledg/` completed 2026-07-08，verdict **METHOD_DIAGNOSIS_NULL**：K1259 的 2718-row DM ledger 是 pairwise DM summary，不是逐日 loss-differential path。可識別 A4f/HAR rows 20 筆（K1002/K1014/K1054/K1057/K1063/K1072），CF-Rolling rows 0，pair-level date-indexed raw `d_t` series 0；因此嚴格 Giacomini-Rossi fluctuation test **不可執行**，只能做 summary-stat coverage audit。後續若要做正式時變相對績效，需在 DM-producing experiments 儲存 `date, model_a_loss, model_b_loss, loss_diff` sidecar 或在未來 ledger 加 `loss_series_uri`。
- [x] **K1661 已完成；K1713 duplicate closure + HAC retrofit（2026-07-16）** — 日頻 OHLC HARQ contrarian 題已由 `experiments/k1661/` 完整覆蓋（SPY/0050.TW/TWII 作 TX free-data proxy；GK RV、range quarticity、rolling HAR/HARQ、QLIKE）。K1713 對 pinned K1661 bytes 重播 3 市場 × 4 模型與 30 組統計全吻合，並修補 K1661 舊 `h=1 → HAC lag 0` 推論：canonical Bartlett-HAC lag=15 後 SPY/0050.TW/TWII HARQ-vs-HAR t=0.475/0.752/0.473，仍全未達 Harvey |t|>3，故維持 `NULL`；此為重複任務關閉與方法 retrofit，不算獨立 replication、不新增第二筆 knowledge。
- [ ] 已實現共變異數 HAR → 最小變異組合的樣本外檢定 — 對多資產（TW0050/TSMC/金融股 或 SPY/QQQ/GLD/TLT）用 HAR 直接建模 realized covariance 矩陣（Cholesky/log-matrix 參數化），比對 shrinkage/EWMA 共變異數在 GMV 組合的 OOS 波動與換手；yfinance 日資料（來源：2024-26 HAR-type realized covariance forecasting）。差異化：庫內幾乎全單變量，realized-cov-matrix→組合是新面向，直接服務組合構建產品。
- [ ] Conditional（regime-gated）波動率目標：只在高波動 regime 才縮曝險 — 對比「永遠 VT」與「僅在偵測到高波動 regime 時啟動 VT」的淨成本績效，檢定關掉平靜期無謂換手能否改善淨 Sharpe/Calmar；yfinance + 既有 regime 偵測（來源：FAJ Conditional Volatility Targeting）。差異化：與庫內 CED（backward-looking scaler, NULL, line 611）不同——本題是 regime on/off 開關而非連續 tail scaler。
- [ ] VolPred 自家 HAR/GARCH 預測庫的 Mincer-Zarnowitz 校準審計 — 對現有模型輸出跑 MZ 迴歸（斜率=1、截距=0）與 forecast-efficiency 檢定，系統性找出哪些模型有系統性高估/低估偏誤並提出 bias-correction；純內部資料零額外成本（來源：Mincer-Zarnowitz 檢驗預測效率為 forecast-eval 標配）。差異化：庫內從未做 calibration 層審計，直接提升所有既有預測可信度。
...
- [x] **GARCH-MIDAS 雙成分分解：長期波動成分是否在長 horizon 打敗單成分 GARCH/HAR** — `experiments/research_garch_midas_horizon_garch_har/` completed 2026-07-04，verdict **NULL_NO_HARVEY_MIDAS_EDGE**：yfinance daily adjusted OHLCV（SPY/QQQ/GLD，2005-01-04 至 2026-07-02，各 5,407 daily returns），用 H=22/66 forward average close-to-close variance target + target-end embargo（train row `j+H<i`）比較 rolling GARCH(1,1)、HAR、Component_MIDAS、HAR_MIDAS。兩個 horizon pooled QLIKE 最佳皆為 HAR（H22=0.3326；H66=0.3681），但 HAR vs GARCH 也未過 Harvey（t=-1.57/-1.94）。MIDAS 增量為負：Component_MIDAS vs HAR 在 H22/H66 分別 -13.78% / -12.85% QLIKE improvement（t=+1.50/+1.75，未顯著且方向較差），HAR_MIDAS vs HAR 亦較差（-5.25% / -2.04%）。結論限縮：daily component-MIDAS proxy（非完整 GARCH-MIDAS MLE）沒有長 horizon OOS 增量；長 horizon 本身不等於 long-run component 有用。
- [x] HARQ 已實現四次矩測量誤差修正：本機 5-min 算 realized quarticity，HARQ（RV×√RQ 交互修正衰減偏誤）vs 標準 RV-HAR，OOS QLIKE/DM；與 line 1400 noise-robust kernel 正交（那是換 RV 估計量，這是回歸內修正）→ duplicate-closed 2026-07-05 by `experiments/research_harq_5_min_realized_quarticity_harq_rv_rq_vs_rv/`: already fully covered by K1582 (`experiments/k1582/`), which computes 5-min RQ, HARQ vs HAR, OOS QLIKE/DM/MCS. Canonical TX_active result n_oos=1,697: HARQ +1.94% QLIKE, DM t=-2.60; SHARK_like +2.05%, DM t=-1.77; **DIRECTIONAL_ONLY**, no Harvey pass. Future work requires new SPY/0050 5-min archive or a materially different HARQ-X estimator.
- [x] ~~Markov-switching multifractal（MSM）作 rough-vs-long-memory 辯論第三解~~ → **K1637 completed 2026-07-05，CONDITIONAL_NULL_MSM_BEATS_HAR_BUT_LOSES_TO_EWMA**：`experiments/k1637/` daily close-to-close variance proxy（本機 `price_cache.db`，SPY/QQQ/GLD/TLT/0050.TW；0050.TW 走 `clean_tw50_data`；OOS pooled rows=9,351、unique dates=1,948）。4-component `MSM_GMM`（GMM-lite moment matching + exact finite-state filter）確實勝 HAR（pooled QLIKE 1.7696 vs 2.2671，DM-HAC t=-7.32）與 MS-vol-lite（+4.02%，t=-3.34），但輸最簡單 EWMA(0.94)（QLIKE 1.5556；MSM vs EWMA t=+3.70，Harvey-pass against MSM）。結論：多頻 regime 機制可解釋 persistence，但免費日頻 proxy 未過實務模型升級 gate；不可宣稱 MSM 是 deployable superior forecaster。
- [x] ~~隨機波動率（threshold SV / realized-SV skew-t）模型類 vs GARCH 的 VaR/分位預測~~ → **K1648 v2 completed 2026-07-06，WEAK_SV_AVERAGE_QLIKE_EDGE_NO_HARVEY_WIN**：`experiments/K1648/` 用 yfinance daily adjusted OHLC（SPY/TLT/HYG，2010-01-01..2026-07-02，OOS 2018-01-02 起，n=2,136/asset）比較 EWMA、GARCH_N、GJR_T、SV_KF、TSV_KF、RSV_KF。Codex review 指出原 v1 用 `exp(h_pred)` 低估 SV variance 且缺 convergence audit；v2 改 Kalman log-normal mean `exp(h_pred + 0.5*p_pred)`，SV annual refit chosen success rate 81/81=100%，fallback=0。結果：RSV_KF 平均 QLIKE 1.3442，略勝 GJR_T 1.3445 / GARCH_N 1.3658；RSV 是 SPY/TLT winner，但 vs GARCH_N DM t=-1.65/-2.16，未達 Harvey |t|>3。GJR_T 仍在 HYG 有唯一 Harvey-significant win（t=-4.07）。結論限縮：免費日頻 OHLC + QML/Kalman RSV 有弱平均 edge，但尚非 publication-strength SV-class victory；完整 Bayesian realized-SV skew-t / intraday RV 仍是另題。
...
- [x] ~~EVT（POT/GPD 尾）× GARCH-filter 的 ES 估計 + e-backtesting 賽馬~~ → **research_evt_pot_gpd_garch_filter_es_e_backtesting completed 2026-07-07，EVT_GPD_COMPETITIVE_NOT_DOMINANT**：`experiments/research_evt_pot_gpd_garch_filter_es_e_backtesting/` 用 SPY/HYG adjusted-close log returns（2007-04-12..2026-07-02，OOS 2015-01-01，n=2,891/cell）比較 GJR-Normal、GJR-StudentT、GJR-FHS、GJR-EVT-GPD，1%/5% VaR+ES，annual expanding GJR refit + daily recursion；forecast at t 只用 `y.iloc[:pos]` 與 r[t-1]，無 same-day lookahead。結果：EVT-GPD 是 SPY 5%/1% FZ joint VaR/ES loss winner；HYG 5%/1% 則 Student-t winner。Sequential ES e-backtest size-2 detections：Normal 4/4、StudentT 3/4、FHS 1/4、EVT-GPD 1/4。結論：POT/GPD tail fitting 可降低 equity-index residual-tail underestimation pressure，但不是 universal ES upgrade；credit ETF tail 在本樣本由 Student-t 較佳。Codex review `CONDITIONAL_PASS`；限制：兩資產日頻診斷、固定 10% GPD threshold、非完整 FRTB desk implementation。
...
- [x] ~~CAViaR 直接動態分位數 VaR：不建波動模型也能報尾部~~ → **K1651 completed 2026-07-06，CONDITIONAL_CAVIAR_COMPETITIVE_NO_HARVEY_EDGE**：`experiments/k1651/` 正式化 SAV/AS/IG/Adaptive CAViaR vs GJR-GARCH skew-t / HS250，SPY/HYG daily adjusted-close log returns 2007-04-12..2026-07-02，OOS 2015-01-01 起，每 cell n=2,891，1%/5% VaR，annual expanding refit + daily recursion，所有 forecast at t 僅用 index<t 與 r[t-1]；CAViaR-IG 明確輸出負左尾 VaR `-sqrt(...)`。GJR-GARCH-SkewT 是 3/4 cells 的 pinball winner 與 3/4 cells 的完整 VaR+DQ gate winner；CAViaR-AS 在 SPY 1% 最佳且過 gate（pinball 0.000357 vs GARCH 0.000369），但 DM-HAC vs GARCH t=-1.78，未達 Harvey |t|>3。HYG 1% 無模型全 gate；Adaptive CAViaR 明顯過度違約。結論：direct-quantile CAViaR-AS 是競爭性 challenger，但本樣本不支持「CAViaR 系統性勝過 GARCH-VaR」；vanilla CAViaR 無原生 ES，joint VaR-ES 需另開 CAViaR-FZ / RES-CAViaR。
- [x] **K1655 Growth-at-Risk 搬到市場（true-PIT 更正 + VIX encompassing follow-up）— `NULL_NO_DOMINANCE_OR_INCREMENTAL_EVIDENCE`，post-run review `PASS`（2026-07-11）**：以 ALFRED `output_type=1` 完整 revision intervals 重建每個 Friday origin 當時可得 NFCI；首個 public vintage 2011-05-25 前 594 個 origins 全排除，最終 panel 788 週（2011-05-27～2026-06-26）。對 `^GSPC` 5% 左尾的 expanding OOS，NFCI 相對 unconditional empirical quantile 在 H=1/4/12 的 pinball 改善為 **−1.934% / −2.656% / −7.331%**，DM-HLN t=+0.898/+0.690/+1.819，無一過 `t < −3`；PIT 樣本內斜率亦不顯著（p=.151/.205/.871）。後續同 origins 配對比較顯示 VIX-only 相對 NFCI-only 點估改善 +13.242%/+5.894%/+1.725%，但 canonical DM t=−1.608/−1.538/−0.638，三格都未過 Harvey/Holm。固定 rolling R=400 的 VIX+NFCI 相對 VIX-only 反而惡化 −3.720%/−2.975%/−14.561%；CQFE 1,999-rep block-bootstrap 的 full-null Holm p=.822/.822/.723，`lambda_joint=0` Holm p 全為 1.000。雙重 null 只能寫「無穩健 VIX dominance、無 NFCI beyond-VIX 增量證據」，不能宣稱 VIX 已證明 subsume NFCI。舊 final-vintage `CONDITIONAL_PASS` 與 2008 敘事維持撤回。（來源：Adrian et al. AER 2019；Giacomini-Komunjer JBES 2005；ALFRED；K1655）
...
- [x] **Rough volatility / ARRV / fractional OU 進賽馬**（2026-07-01，`experiments/research_rough_volatility_arrv_fractional_ou_joe_2023_fou/`，knowledge `fae873b0`）：TAIFEX 5-min RV 確認 low-H roughness（frequency-ratio H=0.0545；variogram H=0.1428），但 ARRV/fOU-lite 不勝 HAR/HARQ（TAIFEX formal OOS n=488；HARQ QLIKE=0.2269 vs HAR 0.2444, DM t=-1.87 不過 Harvey；ARRV/fOU-lite 輸；high-VIX split 同樣 NULL）。SPY 2026 僅短樣本 diagnostic（114 raw files / 113 valid days；OOS n=40），不作正式 inference。結論：measurement-versus-allocation again；後續若要重開，必須做完整結構 fOU/ARRV MLE 或更長 SPY 5-min archive，不要再用 lite proxy 當 decisive test。
- [x] **HARQ/SHARK 測量誤差修正 HAR**（2026-06-30 → K1582，knowledge `05793297`；2026-07-02 補 Codex review CONDITIONAL_PASS + 寫入 knowledge，並打勾避免第三次重複派工——2026-07-02 00:59 一個 worktree agent 已因與此重複而正確拒絕重做）：TX_active（day-session, n_oos=1697）HARQ QLIKE +1.94%（DM t=-2.60）、SHARK_like +2.05%（DM t=-1.77），方向對但未過 Harvey `|t|>3`；SPY（n_oos=51）與 0050.TW（n_oos=38）皆 INSUFFICIENT_DATA（遠低於 252-OOS 門檻），不可作 cross-market 正式結論。**Verdict: DIRECTIONAL_ONLY** — 「夜盤噪音大的市場修正效果更顯著」假說未被證實。若要真正解決，需要擴充 SPY 5-min 歷史存檔（目前僅 2026 局部快照）到 ≥252 OOS，這是資料蒐集任務，不是重跑同一 pipeline。
...
- [x] **Agreed vs Disagreed Uncertainty 二維 regime** → **K1601 完成 2026-07-02，DIRECTIONAL_ONLY**：SPY 1994-2026，uncertainty level = VIX / FRED JLNUM3M，disagreement = Philadelphia Fed SPF RGDP D2(T+1)，所有 predictors `.shift(1)`，SPF next-quarter-start known lag，JLN two-month lag，21d forward RV OOS 用 `target_end_pos < forecast_pos` 防 overlap leak。描述性 regime 結果方向與 naive agreed-worse 敘事相反：high-VIX 中 high-disagreement forward vol 23.96% > low-disagreement 18.50%（agreed-minus-disagreed HAC t=-2.97）；high-JLN 中 22.33% > 13.89%（t=-4.71）。但 SPF/JLN 不過 VIX forecast gate：VIX_SPF_JLN QLIKE +0.84% vs VIX, DM t=-0.38；VIX_SPF 更差；JLN_SPF 明顯輸。結論：SPF disagreement 有 descriptive regime signal，但無 Harvey/MCS 強度的 VIX 之外 OOS forecast 增量；proxy-limited，非 Gambetti macro paper replication。
...
- [x] ~~ETF 宏觀效率 vs ETF fragility 的雙面檢定~~ → **已由 `experiments/k_etf_vs_etf_fragility_2026_06_14` 完成（2026-06-14，Codex PASS）**：shock day ETF basket abs return 2.09% vs 0.66%（MW p=9.83e-103）、next-day decay p=1.83e-22、H2 reversal interaction −0.183（clustered p=0.0449）；結論為 reduced-form support with caveats。2026-07-18 backlog 重複生成的 K1732 任務已標 duplicate 關閉（2026-07-19 撞號修復時發現）。
...
- [x] **黃金 safe-haven 的 lagged USD／實質利率 regime moderator** → **K1676 completed 2026-07-10，NULL_NO_ROBUST_LAGGED_MACRO_STATE_MODIFIER**：GLD/SPY/UUP + FRED DFII10，2016-07-06 至 2026-06-18、2,503 日（SPY≤−1% tail=272；−2% robustness=88）。各資產 native return 後再對齊，UUP regime `shift(1)`，DFII10 backward `merge_asof` + 額外 1 market-day lag。Joint tail-mean / hierarchical tail-beta 四個 partial interactions 的 HAC t=+0.34/−1.49/−1.15/−0.97，Holm p≥0.541，5,000-rep block CI 全跨 0，0/4 strict passes；USD weak primary cell n=47、−2% 多格稀疏亦擋 PASS。方向上 high-real-yield bucket 的 GLD tail mean/positive-rate 較差，但 bootstrap CI 跨 0，只能列 diagnostic。結論：K1628 的 episode dependence 不能被簡化成可重複的 lagged USD／real-yield moderator，更不能宣稱 causal「脫鉤」或可交易訊號。
...
**2026-07-12 P10 / K1025_v3 override（取代上列 v2 narrative state）**：深審發現 v1/v2
FEVD 均誤切 `decomp[-1]`；K1025_v3 已用 pinned snapshot（N=2,812）與 KPPS generalized
FEVD 重跑並經 Codex 語義審查 PASS。主 TCI=19.5153%、BTC net=−0.9478pp；Cholesky
排序使 BTC net 在 +10.5485/−8.1080pp 間翻號，一階差分又翻成 +1.9165pp，故原稿
「強淨接收者」方向敘事失效。Lagged-VIX QR 的 τ=.95 beta=+0.4175、95% CI
[−0.6133,+1.1413]，8.5×/7.04× tail-amplification 敘事不存活。共同樣本 AIC 選 OOS
AR(3)，full DM=−0.9950、CW=−0.1248，只能判未見改善證據。公開文章 `mile_113ce9d1`
...
- [x] ~~**Bond mutual fund demandable-equity run proxy 對信用 ETF vol 的領先性**~~ → **K1538 completed 2026-06-23，WEAK_DIRECTIONAL_PROXY / gate fail**：yfinance AGG/BND/LQD/HYG/BKLN/TLT/SPY/^VIX 日資料 + FRED deposits/MMF assets（2010-01-04 至 2026-06-23，4,143 daily rows；K1536 已占用，故使用 K1538。K1537 從未被使用 —— 舊版此處誤記為已占用，2026-07-14 更正）建 run-pressure proxy：bond ETF dollar-volume shock、負債券 basket return、HYG-vs-LQD underperformance、ETF illiquidity、cash migration。所有 predictive signal 明確 `signal_lag = run_pressure_index.shift(1)`。HAC regression 方向符合預期但未過 gate：HYG RV5 beta=+0.00284, t=2.04, raw p=0.041 但 BH q=0.191 / Bonferroni p=0.372；HYG RV21 t=1.68，BKLN RV5 t=1.65，HYG-SPY downside corr21 t=1.45。OOS MSE 改善方向正（HYG 5d +4.06%、LQD 5d +4.35%、BKLN 5d +4.20%）但 DM t 皆未過 Harvey（best BKLN 5d t=-1.88）。結論：只能作「免費 proxy 有弱方向線索」；不可發文宣稱 bond-fund run pressure robustly predicts credit ETF vol，若重開需 ICI fund-flow / fund NAV discount / TRACE。
...
- [x] **銀行 BHC AI-loss disclosure 是否預測銀行 RV** — `experiments/research_bhc_ai_loss_disclosure_rv/` completed 2026-06-24，verdict **MIXED_DISCLOSURE_SIGNAL**：SEC `data.sec.gov` + Archives 10-K 原文，JPM/BAC/WFC/C/GS/MS/USB/PNC/TFC/COF 2019-2025 共 70 份 filing，yfinance bank stocks + KBE/KRE/XLF/SPY，月頻 disclosure state 全部 `groupby("ticker").shift(1)`，final observed price month 丟掉避免 partial-month target。銀行個股 FE/year FE panel 738 obs 對 next-month RV 全部不過 Harvey（combined t=-0.45）；ETF aggregate 只有 XLF model-risk proxy 正向過門檻（coef 0.0242, t=3.65），KBE/KRE 與 combined proxy 不過。結論：公開 10-K disclosure proxy 不足以支持 bank-stock RV edge；XLF model-risk signal 只列 governance/sector-risk feature candidate，不上架策略。
...
- [x] **NBFI 流動性壓力的免費 proxy → 信用 / 銀行 ETF vol 領先性（EU FSB 2026 stress test 前哨）** — `experiments/research_nbfi_proxy_etf_vol_eu_fsb_2026_stress_test/` completed 2026-06-24，verdict **NULL_OR_WEAK_DIAGNOSTIC_NBFI_PROXY**：用 HYG/BKLN/BIZD 價量壓力、BIZD-HYG discount-style proxy、FRED WRMFNS/MMMFFAQ027S MMF flow、TOTBKCR bank-credit contraction、SOFR-IORB 建 free-data NBFI run-pressure index，所有 predictive use 走 `run_pressure_index.shift(1)`；KRE/KBE/XLF/HYG future 5d/22d RV + downside variance 共 16 個 variance OOS cells，positive Harvey-level QLIKE cells=0，median QLIKE improvement=-0.33%，mean=-0.62%。XLF 22d RV 最佳 QLIKE +2.11% 但 DM t=0.77 / HAC t=2.81 未過 gate；cross-sector corr diagnostic 亦無效。結論：public proxy 只能弱診斷，不支持 NBFI run-pressure robustly leads bank/credit ETF vol。
...
- [x] **Hedge-fund alpha-dispersion regime 的免費代理：strategy ETF dispersion 是否預測股票 cross-sectional vol** — `experiments/research_hedge_fund_alpha_dispersion_regime_strategy_etf/` completed 2026-06-24，verdict **NULL_OR_MIXED_NEGATIVE_STRATEGY_ETF_DISPERSION**：QAI/MNA/CTA/DBMF/MRGR/HFXI/RPAR 日報酬建 strategy-side cross-sectional dispersion（21d/63d smoothed，`shift(1)`），baseline 控制 own lagged target、SPY 22d return/RV、VIX level/change；S&P 500 current constituents next-22d cross-sectional return variance + avg individual RV、IWM/IWR yfinance top-holdings avg RV 共 4 個 OOS cells，positive Harvey-level QLIKE cells=0，median QLIKE improvement=-10.44%，mean=-9.81%。結論：免費 strategy ETF dispersion proxy 不支持 next-month stock cross-sectional risk leading signal；S&P universe 有 survivorship bias，IWM/IWR 只為 top-holdings diagnostics。
...
- [x] **Cornish-Fisher regime tail adjustment 作 Sector rotation timing：HMM-regime + CF 修正能否擇時 sector ETF** — `experiments/research_cornish_fisher_regime_tail_adjustment_sector_rot/` completed 2026-06-24，verdict **NULL_CF_HMM_ROTATION_NO_EDGE**：yfinance 11 sector ETF + SPY，2018-06-19 至 2026-06-23，OOS 2021-07-01 至 2026-06-23（1,249 trading days / 61 monthly rebalances）。每月用 trailing 756 日 SPY return fit 2-state Gaussian HMM，高 variance state=turbulent；各 sector 用該 regime 訓練窗的 Cornish-Fisher 5% VaR + empirical ES tail score 配 inverse-risk weight，turbulent 時 XLP/XLU 1.75x defensive boost，權重 next trading day 生效並收 10bps turnover cost。CF-HMM net Sharpe 0.718、MDD -19.0%，sector EW Sharpe 0.731、MDD -19.2%，sector EW VT Sharpe 0.782、MDD -16.2%；DM return loss vs EW t=+1.04、vs VT t=+0.05 不過，只有 downside vs raw EW t=-3.52 過但 vs VT t=+1.19 不過。結論：CF-HMM 主要變成 XLP/XLU defensive tilt，不能支持 sector-rotation edge；簡單 VT baseline 已吸收其風險改善。

- [x] **Index inclusion fast-entry mechanism 對被納入個股 + 同 sector 對手 RV 的事件研究** — `experiments/research_index_inclusion_fast_entry_mechanism_sector_rv/` completed 2026-06-24，verdict **WEAK_OR_MIXED_EVENT_DIAGNOSTIC_INDEX_INCLUSION_PROXY**：因 2026-06-24 前尚無可觀察 mega-IPO fast-entry 後驗樣本，且 S&P DJI 2026-06 已決定不採 mega-cap-only fast-track，本題改作近期 public proxy event study：13 個 S&P 500 additions + 3 個 Nasdaq-100 annual reconstitution additions（2024-03-18 至 2025-05-19），yfinance adjusted close 2023-01-04 至 2026-06-23，事件窗 [-30,+60]，pre-window-only normalization / jump threshold，無 same-day signal-return PnL。主要 stock-minus-peer post60/pre RV log-ratio differential mean=0.0107、t=0.07、bootstrap CI [-0.2753, 0.3396]、date-cluster CI [-0.1544, 0.2012]；included-stock jump-rate delta mean=0.0219/t=1.37、pairwise-corr delta mean=0.0568/t=1.64，皆未過 Harvey-style t>3 / CI>0 gate。結論：近期大型 index inclusion / reconstitution proxy 不支持「納入後被納入股相對同業 RV robust 上升」；結果不能外推為 2026 fast-entry rule 的因果 null。

- [x] **TIPS breakeven volatility 與 corporate bond return：BEI vol 是否預測下期 LQD/HYG RV 與 credit spread vol** — `experiments/research_tips_breakeven_volatility_corporate_bond_return/` completed 2026-06-24，verdict **NULL_OR_MIXED_NEGATIVE_BEI_VOL_CREDIT_RV**：FRED T5YIE/T10YIE 建 21d/63d realized BEI vol，baseline 控制 own lagged target variance、BEI level/change、VIX、SPY 22d RV，augmented 加 BEI-vol features；LQD/HYG/BKLN 5d/22d future RV/downside variance + HY/IG OAS spread-change variance 共 16 個 OOS cells，positive Harvey-level QLIKE cells=0，median QLIKE improvement=-3.17%，mean=-8.97%。HYG/BKLN 5d RV 有正向但 DM t 僅 1.6-1.8，不足以支持 robust edge；OAS diagnostics 因 FRED CSV 僅 2023-06-26 後樣本而保守解讀。
...
### 期刊主題挖掘 batch 2026-06-24b（journal_discovery_20260624_0 重生 fire；WebSearch JFM/JBES/JFutMkts/JFQA/Math.Finance/CFA/JAI/JFRA/Corp.Gov./Emerald/EFM 2025-26 趨勢層級；selectivity gate 後 13→11 條入單）
> 來源：journal-topic-discovery（task `journal_discovery_20260624_0` 第二次 fire，agent general-purpose / sonnet medium，duration 4 min, 25 tool uses, 103k tokens）。主線程 grep 既有 backlog + 06-24a batch 後剔除 2 條：(a) Hurricane NOAA event window（line 487 既有 NOAA hurricane × KIE/XLU 已覆蓋，per 06-17d selectivity gate）、(b) VVIX→SPY tail-jump（K43 VVIX/SKEW/VIX3M overlay 已 NULL closed，line 77）。本批刻意挑「微結構 free-data proxy（HFT spread/Amihud、AP-concentration、variance dispersion、short-squeeze）+ 方法學前沿（cojump asymmetric、rough/path-dependent/Markovian HAR 賽馬）+ 跨領域事件（ESG controversy、Russell reconstitution、stablecoin TVL→BTC tail）+ 台灣 retail 客群（多幣別 hedge）+ alt-asset crisis-alpha 重檢」五軸；趨勢層級來源採公開期刊頁面，不捏造論文標題+作者+期號。

- [ ] **Cojump 與 asymmetric jump 對 ^VIX 與 SPY 的雙向預測：VIX 跳上 vs 跳下哪邊更領先 RV** — yfinance SPY/^VIX 日資料 2010-2026，用 BNS bipower variation 在不可得時用 daily range/log-return 的 nonparametric jump proxy（Lee-Mykland 日級近似）拆 positive/negative jump 與 cojump 指標，shift(1) 後檢定 (a) 個別資產 jump 與 cojump 對另一資產 5/22d log-RV 的 OOS DM 增量、(b) asymmetric jump（VIX 正跳 vs SPY 負跳）哪邊預測力較強；只接 Harvey |t|≥3 + MCS survive；與既有 K1145 EAV 跨資產正交，這裡是日級 jump asymmetry channel（來源：Journal of Futures Markets 2025 VIX-stock cojump 雙向預測 + Journal of Forecasting 2025 jump-asymmetric spillover；HAR-RV + jump asymmetry 為 2025-26 RV 主流）

- [ ] **Rough vs path-dependent vs Markovian HAR 的 free-data 三方賽馬：1 週-3 月期限結構誰真贏** — yfinance SPY/QQQ/IWM/TLT 日 RV，建 (a) 標準 HAR、(b) HAR + power-law weights（rough proxy，H∈(0,0.5)）、(c) HAR + exponential-kernel superposition（path-dependent Markovian proxy），對 1/5/22d OOS QLIKE+DM+MCS 賽馬，誠實檢定論文「rough underperforms Markovian/path-dependent at 1w-3m」是否在日級 free-data RV 也成立；與 K1487 ML-ceiling、line 1203 tree-HAR 軸正交，這裡是 kernel-form 比較（來源：Mathematical Finance 2025 Abi Jaber et al. "Volatility Models in Practice: Rough, Path-Dependent, or Markovian?"；2026 Futures of Quantitative Finance Conference 主題）

- [ ] **HFT proxy 日內流動性指標：TAQ-free 條件下 effective spread + Amihud illiq 變動是否預測下一日 open-to-close RV** — yfinance daily OHLCV + 5-min bars 局部歷史建 Corwin-Schultz high-low spread proxy、Amihud illiq、close-bid-ask range 三 free-data HFT proxy，shift(1) 後檢定是否預測下一日 5d RV、open-to-close vol 與 intraday range；與 K1364 ETF sampling、line 1137 RLMM 軸正交，這裡是 HFT-style liquidity provision 變動 → 隔日 RV channel（來源：Journal of Financial Markets 2026 HFT-liquidity 主題；Adrian et al. JFM intraday market making）

- [x] **台灣投資人多幣別 hedge ratio 的 ex-post realized vol-minimization：靜態 vs DCC vs regime-switching** — `experiments/k1548/` completed 2026-06-24，verdict **STATIC_OR_FULL_HEDGE_DOMINATES_DYNAMIC_IN_FREE_DATA_OOS**：yfinance `TWD=X` + `SPY/EFA/EEM/QQQ`，training 2015-01-05→2019-12-31（1206 asset/FX intersection days），OOS 2020-01-03→2026-06-23（1564 days）；全避險、static MV、EWMA/DCC-lite、train-only HMM regime 四種 overlay 對 unhedged 的 daily squared-return reduction 均過 HAC t>3 + 3000-rep block-bootstrap CI>0 gate，但動態 proxy 沒有打敗簡單避險：平均 vol reduction full hedge 5.01%、static MV 4.91%、HMM 4.94%、EWMA/DCC-lite 3.97%。結論：支持台灣投資人 USD ETF currency hedge 可降 TWD realized vol；不支持此 free-data/no-cost 設定下為 DCC-lite/HMM 複雜度付費。注意：EWMA/DCC-lite 不是 full DCC-GARCH，無 forward points/cost/tax/roll model；knowledge promotion 留給 K1259 writer gate。

- [x] **Bond ETF AP-concentration 壓力代理：authorized participant 集中度高的 ETF 是否在 stress days 有更大 NAV deviation 與 RV gap** — `experiments/k1551/` completed 2026-06-24，verdict **PARTIAL_GROUP_SUPPORT_MIXED_ETF_RANKING**：yfinance `institutional_holders` 對 AGG/BND/LQD/HYG/MUB/EMB/TLT/IEF 全為空/404，故 K1551 沒有宣稱 true AP concentration；改做 data-limited free-data AP-fragility proxy diagnostic（credit/category complexity + turnover + high-low spread + inverse dollar-volume depth）與 rolling t-1 OLS fair-value residual（非真 NAV premium/discount）。VIX>25 or MOVE>120 stress days 下，高 fragility group（EMB/HYG/LQD/MUB）相對低 group（AGG/BND/IEF/TLT）的 abs residual DID=0.000860，Welch t=8.34，bootstrap CI [0.000672,0.001069]；fwd5 RV DID=0.000107，t=4.88，CI [0.000067,0.000153]。但 ETF-level ranking weak：fragility vs residual lift Spearman rho=0.262 p=0.531、vs fwd5 RV rho=0.214 p=0.610，且 TLT duration shock 是反例。結論：支持「group-level bond ETF stress fragility proxy」診斷，不支持可發佈的 AP concentration ranking；若要真正 AP concentration，需要 Form N-CEN/AP activity、CRSP/issuer NAV premium-discount 與 creation/redemption basket data。

- [ ] **Stablecoin TVL regime-switch 與 BTC tail VaR：USDT/USDC supply 收縮是否預示 BTC 8d-21d 高 tail-loss regime** — DefiLlama 公開 stablecoin TVL daily series + yfinance BTC-USD/ETH-USD（2018-2026），用 2-state HMM 在 stablecoin TVL 7d growth rate 上 fit regime，shift(1) 後檢定低/收縮 regime 是否預測 BTC/ETH 下 8d/21d VaR/ES 5%、left-tail loss、IV-RV gap；與 K523/K1532 stablecoin→FX channel、line 1213 SHY/SGOV 軸正交，這裡是 stablecoin contraction → crypto tail regime（來源：HKMA Research Memorandum 02/2026 stablecoin spillover；2026 Q1 stablecoin TVL record + 區域監管壓力 → 收縮 regime 增加）

- [x] **CTA / 趨勢策略 crisis-alpha 基率重檢：免費 ETF proxy 下 trend-following 在 stress regime 真有 alpha 還是只是 mean-reverting noise** — `experiments/k1547/` completed 2026-06-24，verdict **NULL_OR_NEGATIVE_STRESS_ALPHA_FOR_FREE_ETF_PROXY**：yfinance DBMF/KMLM/CTA + SPY/AGG/^VIX，CTA equal-weight proxy 與 252d TSMOM timing overlay 均用 `shift(1)`；stress_union（lagged VIX rolling 80%-ile or lagged SPY drawdown≤-10%）中 CTA_EW vs SPY 年化 mean excess `-26.0%`、paired t=-1.13、bootstrap CI crosses 0，TSMOM 更弱（`-35.7%`, t=-1.60）。2022 inflation subperiod positive（+77.2% ann excess, t=2.12, CI>0）但未過 Harvey t>3，2025 tariff stress negative，2020 對 252d timing 無足夠 pre-history。結論：免費 ETF proxy 支持「特定 bear market 可幫忙」但不支持一般化 crisis-alpha / timing overlay claim。

- [ ] **Index-vs-single-stock realized variance dispersion 作 SPY 隔日 RV 訊號：低成本 dispersion proxy 是否預測 vol-of-vol** — yfinance SPY + top-30 S&P 500 個股 daily RV，建 average single-stock RV ÷ index RV 的 dispersion ratio（free-data implied correlation 反代理），shift(1) 後檢定是否預測 SPY 隔日 RV、5d vol-of-vol、^VIX change；與 K731 VIX term-structure、line 1217 VXX expected return 軸正交，這裡是 cross-section dispersion channel（來源：Journal of Derivatives 經典 variance dispersion swap 文獻 + 2026 RF dispersion trade 持續實務題）

- [x] **Short interest 急升 + borrow rate proxy：squeeze-risk 是否預測 small-cap 個股下 5d 高 RV** — `experiments/k1550/` completed 2026-06-24，verdict **NULL_NO_ROBUST_SQUEEZE_RISK_VOL_SIGNAL**：因免費資料源無 true borrow-rate / stock-loan tape，K1550 改成 K1502 cache-backed public FINRA short-volume squeeze-pressure proxy（21 檔 small-cap / meme-risk basket，2023-01-03→2026-06-12，FINRA rows 18,144，panel rows 18,123）；score = rolling z(21d FINRA short-ratio change) + rolling z(21d FINRA short-volume flow ÷ avg yfinance volume)，event threshold 用 rolling 252d 90%-ile 並 `shift(1)`，targets 為 t+1..t+5 log RV / jump / left-tail。結果：positive log-RV 10/21，median log-RV diff -0.0583，mean -0.0412，bootstrap 95% CI [-0.1760, 0.1039]，sign-test p=1.0；jump 與 left-tail aggregate CI 也跨 0。結論：公開 FINRA short-volume pressure 不能當作穩健 squeeze-risk vol predictor；真正 semi-monthly short interest + borrow-rate / utilization version 仍需另找資料源，不可用 K1550 宣稱 borrow-rate evidence。

- [ ] **ESG controversy event 公開日曆 → 公司級 RV 與 sector ETF spillover** — 用 SEC enforcement actions + DOJ press release scrape + Wikipedia controversy timeline 建 verified ESG-controversy event date set，對涉事公司 + ESG ETF（ESGU/SUSL/USSG）做 [-5,+10] event window CAR、abnormal RV、left-tail；明確標 proxy diagnostic（與 K1487 GDELT keyword NULL 不同，這裡用 verified event timeline）；與 06-23 batch climate green-brown 軸正交（來源：FAJ 2025-26 + Corporate Governance 2026 ESG rating crash-risk 持續主題；JFRA 2025 ESG controversies 顯著推升 firm-specific tail risk）

- [ ] **Russell reconstitution 1-month rebalance window 對 small-cap RV 與 IWM/IWO 流動性的 robust event study** — Russell 公開 reconstitution 日曆（每年 6 月 announcement + late June effective），對 IWM/IWO/IWN/IWP 與 newly added/deleted constituents 做 announcement-to-effective period RV、相關性、premium-discount proxy；與 line 1221 S&P 500 index inclusion NULL 正交，這裡是 Russell 不同 mechanism（單日 effective vs S&P 連續審查）（來源：經典 Russell reconstitution 文獻 Madhavan + 2026 Russell 重大 mega-cap 重劃實務影響）

### 期刊主題挖掘 batch 2026-06-25（journal_discovery_20260625_0；WebSearch JFEC/RFS/QJE/REStud 2025-26 趨勢層級；selectivity gate 後 6 條入單）
> 來源：journal-topic-discovery Codex fallback（task `journal_discovery_20260625_0`）。先讀 `scripts/agent_prompts/journal_topic_scan.md`，再查 JFEC/RFS/QJE/REStud/Oxford/CFA 2025-26 公開摘要，並用 `rg` 對 research_program.md 去重。剔除已覆蓋軸：能源新聞 topic shock（line 1135）、HFT/Amihud/Corwin liquidity proxy（line 482/1236）、graph/GNN volatility spillover（06-24 header 已剔除）、CIV credit-implied volatility（line 1094）、ETF macro-efficiency/fragility（line 516）、VIX-EPU / generic macro-attention（line 506）、forecast-to-portfolio translation gap（line 1074）。本批只保留方法論 tail/distribution、資料品質、投資人記憶/注意力三類正交軸；所有免費資料版本皆標為 proxy diagnostic，不冒充原文資料 replication。

- [ ] **F-Riesz tail-heterogeneous realized covariance 的低維 proxy：sector ETF covariance tail band 是否改善 min-vol 風險** — 用 SPY + 11 sector ETF / TLT / GLD 日 RV 與 rolling realized covariance，先做可實作近似：DCC/HAR-cov baseline、scalar-t / matrix-F-lite tail shrinkage、以及 asset-specific tail-degree scaling（以各資產 rolling kurtosis / downside semivariance proxy 估），比較 next-week covariance log-score proxy、global-min-vol realized variance、CVaR 與 turnover；只接受 DM/MCS + 成本後改善。與 K1259/MCS、K1532 turnover、06-24 rough/path-dependent HAR 不同，這裡測 covariance-matrix tail heterogeneity 本身（來源：JFEC 2025 F-Riesz realized covariance matrix model；強調 covariance tail fatness 與 cross-coordinate tail heterogeneity）

- [ ] **Admissible constrained multivariate HAR / vMEM guardrail：禁止負 variance 的約束是否比 unconstrained OLS 更穩** — 用 SPY/QQQ/IWM/TLT/GLD/HYG 多資產 RV panel 建 MHAR / vMEM-lite（daily/weekly/monthly lag cross-effects），比較 unconstrained OLS、nonnegative projected coefficients、matrix-inequality/admissible approximation 三版本；評估負 forecast 次數、QLIKE、forecast-error tail、以及 VT/DRP overlay 的 turnover 與 MDD。與現有單資產 HAR/ML ceiling 不同，這是「多變量 volatility system 是否需要參數空間約束」的工程 guardrail（來源：JFEC 2026 vMEM admissible parameter space / constrained MLE）

- [x] **Quadratic-variation estimator disagreement 作 forecast uncertainty：RV estimator risk band 是否預測下期 QLIKE error** — `experiments/k1560/` completed 2026-06-29，verdict **NULL_SHORT_WINDOW**。yfinance daily OHLCV（2018-01-01→2026-06-29 request；effective last date 2026-06-26）+ yfinance 5m rolling window（2026-04-01→2026-06-26），對 SPY/QQQ/IWM/TLT/GLD/HYG 計算 5m RV、10m subsampled RV、realized-kernel-lite、Parkinson/Garman-Klass/Rogers-Satchell/Yang-Zhang 與 close-to-close RV；estimator dispersion 於 origin t 明確 shift 到 target t+1，評估 HAR/GARCH/EWMA/OHLC forecasts 對 total RV（overnight + intraday 5m）的 QLIKE 與 vol-target sizing error。結果方向有弱正號但不穩健：GARCH QLIKE signal coef 0.236、HAC t=1.72、raw p=0.0847、Holm p=0.512；GARCH vol-target absolute log sizing error coef 0.141、t=1.79、raw p=0.0731、Holm p=0.512；未通過任何 signal Holm test。GARCH QLIKE Spearman 正號出現在 5/6 assets，但 HAR QLIKE 僅 TLT 正號、vol-target excess risk 僅 SPY/TLT 正號。結論：短窗 pilot 不支持「RV estimator disagreement 可穩健預測 next-day QLIKE error 或應降槓桿」；可保留為未來有本地 intraday cache 後的長樣本重測方向。

- [x] **Missing / stale market data 作 liquidity-vol prior：缺資料不是清洗問題，而是風險訊號嗎** — `experiments/k1559/` completed 2026-06-29，verdict **CONDITIONAL_PASS**。28 檔 yfinance ETF panel（2014-01-01→2026-06-29 request；86,148 estimation rows）把 missing row / recovery / zero volume / stale price / corporate-action gap 當 t 日 data-quality event，future targets 從 t+1 起算，panel regression 控制 asset FE、lagged 22d RV、dollar volume、price level、SPY 當日 shock，並做 Holm correction。結果：any data-quality event 對 log next-5d RV 係數 0.860（t=5.99，Holm p=5.39e-08），對 log next-22d RV 係數 0.884（t=17.33，Holm p=9.47e-66）；zero-volume / stale-price 對 22d gap risk 也顯著。但證據集中於 QAT/KSA/UAE（只有 3 檔 any-DQ event >=10，且只有 2 檔 within-asset RV22 ratio >1），missing-row 本身只有 1 筆，因此不能宣稱「missing row」普遍預測波動；較誠實結論是：部分薄 country ETF 的 zero-volume / stale-price days 可作 conditional liquidity-risk prior，不是 general alpha signal。

- [ ] **Investor-memory cue 對 RV persistence 的公開代理：當前報酬像過去 crash/rally 時，市場是否過度回想並放大成交與波動** — 用美股大樣本 / sector ETF，為每檔建立歷史 5d/21d return episode library，當今日 return cue 與過去極端 loss/gain episode 相似時，檢定後續 turnover、range-vol、left-tail 與 reversal/continuation 是否異常；可加入新聞 headline count 作 salience control。與 generic sentiment / retail-flow 題不同，這裡是 similarity-based recall cue，而非情緒 level（來源：QJE 2025 Investor Memory and Biased Beliefs；相似回憶會影響預期與交易）

- [ ] **Attention-utility winner focus：贏家被看見、輸家被躲開是否造成 winner-side RV / volume asymmetry** — 用 retail-heavy / meme-risk basket + large-cap control，標記近期新高/高報酬 winner 與近期 loser，測試 market-close 後、週一開盤、財報前後的成交量、range-vol、gap risk 是否對 winner-side attention 更敏感；若可取得 Google Trends/Reddit/Stocktwits public count 則作 attention proxy，否則先用 abnormal volume diagnostic。與 line 506 新聞情緒/EPU 與 line 1102 台股散戶占比不同，這裡是「已知好消息帶來注意力效用」的 winner-vs-loser 不對稱（來源：REStud 2026 Attention Utility；投資人更願意注意已知正面持股資訊，並影響交易）

### 期刊主題挖掘 batch 2026-06-27（journal_discovery_20260626_0；WebSearch JFEC/RoF/RFS/AEA/REStud 2025-26 趨勢層級；selectivity gate 後 8 條入單）
> 來源：Codex hourly journal-discovery。先讀 `scripts/agent_prompts/journal_topic_scan.md`，再查 JFEC/RoF/RFS/AEA/REStud 2025-26 公開摘要與 issue page；用 `rg` 對 research_program.md 去重。剔除已覆蓋軸：CIV、passive mega-cap、ETF AP concentration、stablecoin FX/Treasury、investor-memory、attention-utility、tariff sector RV、generic macro-attention、HFT/Amihud liquidity、VRP term structure。本批只保留 grep=0 或機制明顯正交的新軸；所有 yfinance/RSS/ETF 版本皆標 proxy diagnostic，不冒充原文資料 replication。

- [ ] **Candlestick spot-vol estimator 作日頻 OHLC baseline：K線資訊是否足以打敗 close-to-close / range RV** — 用 yfinance 日 OHLCV 的 SPY/QQQ/IWM/TLT/GLD/HYG 與本機 5-min 片段作 validation，實作 candlestick-based spot volatility estimator、Parkinson/Garman-Klass/Yang-Zhang、close-to-close 三組，先比較 measurement error，再把 lagged estimator 加入 HAR/GARCH 做 OOS QLIKE + DM；與 line 1259 estimator-disagreement 不同，這裡不是把分歧當 uncertainty，而是直接測「OHLC candlestick estimator 可否成為低頻標準 RV proxy」（來源：JFEC 2026 Issue 24(1) Bollerslev et al. candlestick-based spot volatility estimation）。

- [ ] **Coherent VaR/ES estimator guardrail：風險量本身 coherent，不代表估計器 coherent** — 用 SPY/QQQ/IWM/TLT/HYG 日報酬建 rolling VaR/ES，對比 historical simulation、EWMA、Cornish-Fisher、coherent L-estimator weight structure；檢查 ES backtest、capital adequacy ranking flip、以及 VT/DRP 降槓桿觸發次數是否因 estimator 不一致而改變。與 conformal VaR / CVaR-RP 不同，這裡是「risk estimator coherence」工程風控層（來源：JFEC 2026 "Coherent Estimation of Risk Measures"，指出 coherent risk measure 不必然導出 coherent estimator）。

- [ ] **Deep quantile VaR 的公平 baseline 審計：DNN quantile 是否只是吃到高維 covariate advantage** — 用 SPY/QQQ/TLT/HYG 日報酬 + VIX/MOVE/FRED macro lagged features，對 deep quantile estimator、linear quantile regression、CAViaR、HAR-quantile / nonlinear MIDAS 做 VaR/ES forecast，固定 `signal.shift(1)`，用 quantile score、DM/GW、VaR violation independence、ES backtest。與 ML-GARCH ceiling 不同，目標是 tail quantile 而非 conditional mean RV；若 DNN 贏，必須再與同資訊的 gradient boosting / spline quantile baseline 比（來源：JFEC 2024 deep quantile VaR，2025-26 citations 持續成長）。

- [ ] **Social-transmission bias 的公開代理：被貼文成功經驗推上來的高波動股票是否隔日 RV 放大** — 用 Reddit/Stocktwits/Yahoo trending 免費貼文或標題量，標記「近期高報酬且高 RV」個股被討論的 social-transmission shock，檢定 t+1..t+5 abnormal volume、range-vol、gap risk 與 reversal；與 investor-memory/attention-utility 不同，這裡是網路節點傳播 + follow-up purchase channel，不是個人回憶或持股正面注意力（來源：Review of Finance 2025 "Social transmission bias"；高波動、近期贏家貼文更容易引發跟隨買入）。

- [ ] **Tradable factor implementation shortfall：學術因子 paper alpha 轉 ETF 後是否只剩波動與成本** — 用 MTUM/VLUE/QUAL/USMV/RPV/IVE/IWF 等 factor ETF + Fama-French library on-paper factors，估 ETF factor exposure、tracking error、short-leg/cost proxy，量化 2-4% implementation shortfall 是否表現為 lower alpha、higher RV、或 drawdown amplification；與 factor timing / factor crowding 不同，這裡測「paper factor → tradable vehicle」的 implementation gap（來源：Review of Finance 2025 tradable risk factors；ETF shorting costs and transaction costs explain large part of on-paper vs tradable gap）。

- [ ] **Probability-weighted skew-vol pricing：右偏高波動資產是被高估還是承擔 crash co-movement** — 用美股大盤 + current large-cap / sector ETF universe 建 rolling realized skewness、MAX return、downside semivariance 與 idio-vol，測 probability-weighted CAPM 的兩個免費代理預測：(a) right-skew/high-vol 組後續 return 是否低、(b) left-skew/high-vol 組是否有正 alpha 或更高 correlation-risk exposure；與 K1346 lottery basket NULL 不同，這裡是 skew × volatility interaction 與 correlation-premium channel（來源：RFS 2025 Π-CAPM / probability weighting with skewed assets）。

- [ ] **Tariff-uncertainty USD risk-premium wedge：關稅波動是否先進 FX vol 再外溢到 EM/commodity ETF** — 用 2025-26 tariff event calendar / GDELT tariff headline intensity、UUP/DXY proxy、FXY/FXE/CEW/EMLC、EEM/DBC/GLD，檢定 tariff uncertainty shock 對 t+1..t+22 FX realized vol、USD drawdown、EM FX left-tail 的 lead-lag；與 line 1081 tariff sector RV 正交，這裡是 AEA macrotrade 的 exchange-rate risk-premium wedge（來源：AEA P&P 2026 "Global Trade, Tariff Uncertainty, and the US Dollar"）。

- [ ] **US macro news global co-jump beta：各國 ETF 對美國數據驚喜的波動同步化程度** — 用 FRED/BLS/BEA release calendar + surprise proxy（actual-minus-consensus 若免費不可得則用 event-day absolute surprise proxy / Kalshi macro market shock 代理）、EFA/EEM/EWJ/EWG/EWT/EWY/EWZ/INDA，測 event-day cojump、VIX jump、t+5 RV persistence 與 country beta；與 generic EPU/macro-attention 不同，這裡是具體美國 macro release 造成 global financial cycle synchronized jump（來源：REStud 2026 "The U.S., Economic News, and the Global Financial Cycle"；27 國股指、VIX、商品對美國新聞同步跳動）。

### 期刊主題挖掘 batch 2026-06-28（journal_discovery_20260628_0；WebSearch RFS/AER/JEP/Fed/IMF/NATO/WEF/Nature/OECD/IEA 2025-26 趨勢層級；selectivity gate 後 10 條入單）
> 來源：Codex hourly journal-discovery + fresh subagent。先讀 `scripts/agent_prompts/journal_topic_scan.md`，再查 RFS/AER/JEP/Fed/IMF/NATO/WEF/Nature/OECD/IEA 2025-26 公開摘要/報告；主線程以 `rg` 對 research_program.md 去重並抽查來源存在。剔除已覆蓋軸：VT/VIX-MOVE、GARCH/HAR 賽馬、generic risk-model comparison、ETF 分散/60-40/50-50、T+1、ML liquidity proxy、OPEX gamma、foundation TS、Factor MAX、天然氣季節性、sequence risk、tariff event study、investor-memory、attention-utility、AI capex/電力、private-credit/BDC、bond ETF liquidity、commodity inventory、cyber incident。本批只保留可用免費資料或公開事件 calendar 啟動的 cross-domain volatility/risk directions；所有鏈上、新聞、ETF、disclosure proxy 只能標為 diagnostic，不冒充原文資料 replication。

- [ ] **DEX priority-fee pressure 作 ETH/BTC 短期 RV 訊號** — 用 Etherscan/gas fee、DefiLlama DEX volume、Uniswap public data 與 BTC-USD/ETH-USD，建 priority-fee / DEX-volume shock，檢定 t+1/t+5 RV、left-tail、CEX-crypto ETF spillover；免費資料只能做鏈上 proxy，不冒充 address-level mempool replication（來源：RFS 2026 decentralized-exchange price-discovery / on-chain microstructure stream；on-chain market microstructure 進入頂刊）。

- [ ] **BigTech / merchant-platform credit stress 的公開市場 proxy** — 用 SHOP/SQ/PYPL/MELI/AFRM/UPST、IWM/XRT/KRE/HYG，搭 FRED small-business credit 與 delinquency proxy，檢定平台信用收縮是否領先 small-cap / retail / fintech RV；與 BNPL consumer-credit 題不同，聚焦 merchant-platform enforcement 與 bank-credit substitution（來源：RFS 2026 platform lending / platform-bank competition stream；平台金融與銀行競爭成熱門）。

- [ ] **Regulatory-compliance burden beta 作 mid-cap / small-bank RV regime** — 用 Federal Register rule count、RegData / paperwork-burden proxy、行業 compliance-labor exposure，映射 IJR/IWM/KRE/XLF/sector ETF，檢定高 compliance-beta 產業在 regulatory shock 後 RV、downside semivariance、成交量是否放大（來源：RFS 2026 regulatory-compliance stream；法規成本異質性與 business dynamism 成新軸）。

- [ ] **Legacy-asset overhang：技術轉型是否讓舊資產 sector 的信用與 RV 同步惡化** — 用 SEC/XBRL PP&E/intangible proxy、sector ETF、銀行/能源/汽車/工業籃，搭 green-tech / automation / transition-news shock，檢定 legacy-heavy sector 是否有更高 RV persistence 與 credit-spread beta；資料限制標為 public-disclosure proxy（來源：RFS 2026 technological-change / asset-overhang stream；技術轉型金融摩擦成新軸）。

- [x] **Office-CRE refinancing wall 對 regional bank / REIT vol 的 lead-lag** — `experiments/k1570/` completed 2026-06-29，verdict **WEAK_PARTIAL**：FRED CRE delinquency（quarter-end +50d release lag）+ DGS10 + office REIT basket stress 對 KRE/KBE/VNQ/IYR/XLRE/REM/CMBS/office-basket 做 5d/21d forward RV/downside variance HAC 檢定（96 primary tests，Holm/Bonferroni）。只有 `CMBS` 5d forward RV 的兩格倖存：combined CRE pressure t=4.30 Holm p=0.00166、office market stress t=4.04 Holm p=0.00511；KRE/REIT/office basket 無 family-corrected 正向倖存。結論限縮為「公開 CMBS ETF proxy 的短期 RV 訊號」，不支持 broad office-CRE → regional bank / REIT vol pass。

- [ ] **Semiconductor industrial-policy award shock 的 volatility spillover** — 用 CHIPS/NIST award dates、SIA project announcements、SMH/SOXX/NVDA/TSM/ASML + supplier/utility ETF，做 award / delay / cancellation event study 與 t+5/t+22 RV spillover；與 AI capex/電力需求不同，聚焦 public subsidy timing 與供應鏈重估（來源：JEP semiconductor industrial-policy + IMF industrial-policy update 2024-26；subsidy competition 是政策主線）。

- [ ] **Defense-spending boom beta：rearmament 財政 shock 是否改變 defense / industrial / rate vol** — 用 NATO spending path、SIPRI/IMF defense-spending data、ITA/PPA/XAR/European defense proxy、TLT/UUP，檢定 defense-budget surprise 是否領先 defense ETF RV、工業股 beta、長債 vol；與 generic GPR 不同，聚焦可量化財政採購路徑（來源：IMF WEO defense-spending chapter + NATO 5% commitment 2026；國防支出成 macro-investing 熱點）。

- [ ] **Labor-force growth / immigration shock 作 wage-sensitive sector RV prior** — 用 Fed breakeven-employment estimates、FRBSF immigration-flow evidence、BLS/JOLTS/FRED labor series，映射 construction/homebuilder/retail/manufacturing ETF，檢定低 immigration / labor-supply shock 是否領先 XHB/XRT/XLI RV 與 wage-inflation beta（來源：Fed / FRBSF labor-market notes 2026；人口與移民重新定價 macro surprise）。

- [ ] **Maritime chokepoint stress 指數作 commodity / retail / shipping vol 訊號** — 用 GSCPI、Freightos/Harpex/FRED shipping proxies、Panama/Suez/Red Sea/Hormuz event calendar、USO/DBA/XRT/IYT/BDRY，檢定 chokepoint stress 是否領先商品 RV、retail margin-risk RV 與 cross-asset correlation spike；與 tariff event study 正交，聚焦物流節點而非關稅稅率（來源：WEF global supply-chain structural-volatility report + Nature chokepoint study 2025-26；供應鏈進入 structural volatility）。

- [ ] **Critical-minerals export-restriction shock 對 clean-tech / defense / semiconductor RV 的傳導** — 用 OECD export-restriction inventory、IEA critical-minerals outlook、REMX/LIT/COPX/URA/SMH/ITA，建立 rare-earth / graphite / cobalt restriction event set，檢定 t+1/t+22 RV、jump、sector spillover；免費資料只能做 listed ETF / announcement proxy（來源：OECD / IEA critical-minerals reports 2025-26；resource nationalism 與供應集中度快速升溫）。

### 期刊主題挖掘 batch 2026-06-30（journal_discovery_20260629_2；WebSearch JBF/JFEC/RoF/JFutMkt/JFI/JRI/MacroEcon Dynamics/arXiv 2025-26 趨勢層級；硬約束跳出 VT/VIX/GARCH-HAR/ETF分散/50-50/0DTE/private credit/maritime/critical-minerals/labor/defense/CRE 已飽和 cluster，fresh-entity priority CRYPTO/FX/IG_BONDS/MERGER_ARB/VOL_SURFACE/SOVEREIGN_CDS/CAT_BOND；selectivity gate 後 10→7 條入單）
> 來源：journal-topic-discovery (task `journal_discovery_20260629_2`, agent opus/high)。WebSearch 8 期刊軸：JBF FX vol & carry crash (Menkhoff 系列 2025 + Bachmann 2025 Vol 178 ambiguity-carry)、JFEC crypto realized vol & jump、JFI / RFS HYG-LQD ratio regime、RoF merger arbitrage spread vol (AllianceBernstein 2026 outlook + 2025 ScienceDirect deal risk papers)、arXiv 2503.14158 Quintic OU SSR/skew-stickiness、Macroeconomic Dynamics 2025 sovereign CDS volatility as uncertainty + IMF GFSR Apr 2026 EM spread chapter、Journal of Risk and Insurance 2026 Braun extreme-weather risk premium。Selectivity gate 砍 3：convertibles (line 1186 已覆蓋)、prediction market (K1360 NULL 已 closed)、Baltic Dry / freight (line 1307 maritime chokepoint 已覆蓋)。每條 grep ≥3 同義關鍵字確認 backlog 零字面 + 軸級正交；本批刻意挑 fresh-entity (CRYPTO/FX-carry/HY-IG ratio/MNA/SSR/sovereign-CDS/cat-bond)，避開 SPY/VIX/VT 飽和軸。

- [ ] **Global FX volatility-ambiguity 作 carry trade crash-risk 不對稱前哨** — yfinance DBV (Invesco G10 currency carry ETF) + FXE/FXY/UUP/CEW，FRED short-rate proxy 建 carry signal；對 G10 主要貨幣 ETF 計算 cross-currency realized vol dispersion (ambiguity proxy)，檢定高 ambiguity day t+1..t+5 是否預測 carry trade 左尾放大、crash event 加密群聚、unwind 領先 VIX。明確 `signal.shift(1)`；與 K1336 EM FX×vol gate（NULL）正交—— 那是 EM only carry timing，這裡是 G10 ambiguity-level crash prediction，著重 disagreement / volatility-of-volatility 而非 carry-vol gate（來源：JBF 2025 Vol 178 "Global foreign exchange volatility, ambiguity, and currency carry trades"；2025 yen carry crash 與 ambiguity literature 同時躥紅）

- [ ] **Cryptocurrency vol-of-vol pricing 對 BTC/ETH 短期 RV 的增量訊號** — yfinance BTC-USD / ETH-USD 日 OHLCV 與 5-min snapshot（Apr-Jun 2026 cache 範圍）+ IBIT/ETHA spot ETF 成交量 proxy；計算 rolling realized vol-of-vol (21d std of 21d RV)，檢定其作為 covariate 是否改善 HAR-RV / GJR-GARCH 對 BTC/ETH t+1 與 t+5 RV 的 QLIKE 預測；DM-HLN |t|>3 Harvey gate。與 line 543 VVIX→VIX self-prediction（傳統金融 vol-of-vol）正交— 此處是 crypto own-asset vol-of-vol pricing；與 K1291 DEX priority-fee (chain-side proxy) 正交—— 機制是 option-implied vol-of-vol 而非 on-chain microstructure（來源：Journal of Futures Markets 2025 Du et al. "Pricing Cryptocurrency Options With Volatility of Volatility"；JFEC 2024-26 crypto jump literature 持續成長）

- [ ] **HYG/LQD ratio z-score regime 對 SPY/IWM downside RV 的 lead-lag** — yfinance HYG/LQD/SPY/IWM/QQQ + FRED BAMLH0A0HYM2 HY OAS，計算 HYG/LQD 日 ratio 滾動 252d z-score；分高/低 spread regime (z>+1 vs z<-1)，檢定 ratio 5d & 21d 變化是否領先 SPY/IWM downside semivariance 與 left-tail jump count；DM-HLN + Bonferroni-robust。與 line 1094 CIV 三因子（信用 vol 對 SPY RV）正交—— 這是 cross-section spread ratio regime classification 而非 vol decomposition；與 K1098 FOMC credit shock（NULL）正交—— 此處不依賴 FOMC event，是 daily regime conditioning（來源：FRED HY OAS Q2 2026 spreads 接近 long-run mean → 後續尾部風險急速放大歷史先例 2007/2015/2018/2020；SystemTrader / Convex 2026 已用此 ratio 作 risk-off signal）

- [ ] **Merger arbitrage deal spread vol 的 free-data ETF proxy + regulatory-overhang regime** — yfinance MNA (IQ Merger Arbitrage ETF) + SPY/IWM/HYG/^VIX；以 MNA tracking error vs benchmark 作 deal-spread vol proxy，搭配 FTC/DOJ HSR filing public calendar 與 antitrust enforcement event window，檢定 megadeal>$10B announcement 後 t+1..t+22 MNA RV 是否異常放大、與 VIX/HY OAS regime 的交互作用；與既有 backlog 完全空白軸（grep merger arb / deal spread / risk arbitrage = 0 mentions）；無冒充個別 deal-level replication，純 ETF proxy + 公開 calendar（來源：Review of Finance 2025 merger arbitrage spread vol；AllianceBernstein 2026 outlook 指出 2025 Q3 大宗交易+166% YoY；regulatory scrutiny 是 megadeal premium 主驅動）

- [ ] **Skew-Stickiness Ratio (SSR) 動態作 SPX 隔日 RV forecast 增量** — yfinance ^SPX/^VIX/^SKEW + ^VIX9D，建 25-delta call/put skew proxy（SKEW index transform），計算 rolling SSR (skew level / spot RV)；檢定 SSR 變化是否在 HAR-RV/HAR-RV-VIX baseline 上提供 OOS 增量 QLIKE 訊號；DM-HLN Harvey gate。與 K43 VVIX/SKEW overlay (NULL — 該是 strategy timing) 正交—— 這裡用 SSR 本身做 vol forecast covariate 而非 strategy overlay；與 line 543 VVIX → VIX 預測（vol-of-vol level）正交—— SSR 是 skew dynamic 不是 level；與 K530 HAR-VIX (level) 正交—— covariate 是 SSR 而非 VIX itself（來源：arXiv 2503.14158 (March 2025) Quintic OU vol model SPX-VIX joint calibration 重新點燃 SSR 研究；2024 "implied volatility surface (also) is path-dependent" arXiv 2312.15950 提示 skew dynamic 預測力）

- [ ] **Sovereign CDS vol 的 EM USD-bond ETF free-data proxy 作 cross-asset uncertainty 前哨** — yfinance EMB/PCY/ELD/VWOB/CEW + FRED EMBI spread proxy (BAMLEMHBHYCRPIUSOAS)，以 EMB 日 RV 與 spread changes 作 sovereign CDS vol proxy；測 ETF-proxy 拐點是否領先 EEM/EWZ/EWY/EWT/INDA RV 與 USD vol regime；DM-HLN + 三組 maturity ETF cluster-robust。0 mentions in backlog (sovereign CDS / sovereign credit 字面命中 0)；與 line 1284 tariff-USD wedge 正交—— 那是 tariff event-driven FX，這裡是 sovereign credit-stress regime；與 K1336 EM FX carry NULL 正交—— 機制是 EM credit risk premium 而非 carry-vol gate（來源：Macroeconomic Dynamics 2025 "Sovereign CDS volatility as indicator of economic uncertainty"；IMF GFSR April 2026 Ch.1 EM spread vs UST chapter；NBER w13658 sovereign credit 3-PC 共同因子）

- [ ] **Cat bond / 再保險載體個股 vol 對 extreme-weather event 的 dose-response** — yfinance RNR/RE/EG/ARCH/AXS (specialty reinsurance carriers) + KIE (insurance ETF) + ICAT (Cat Bond proxy ETF 若可得，否則用 reinsurer basket equal-weighted)，搭 NOAA Atlantic hurricane database + EM-DAT extreme weather event ledger；檢定 named storm landfall t+1..t+22 reinsurer carriers idio RV 是否較 KIE (broad insurance) 更敏感、左尾 dose-response 對風暴 Saffir-Simpson category 是否單調；與 line 489 NOAA × KIE/XLU/KBWP（broad insurance/utility ETF）正交—— 此處是 specialty reinsurance carrier basket + cat-bond proxy 個體層，非 broad sector；cat bond / reinsurance / RNR 字面 backlog 0 mentions（來源：Journal of Risk and Insurance 2026 Braun "Extreme-weather risk and the cross-section of stock returns"，1995-2019 storm-loss sensitivity 6%/yr return spread；Fed FEDS 2022043 climate-financial-stability 強調 reinsurer balance sheet 為主傳導；2025 PCS Atlantic 損失 record-high 點燃 cat bond demand 與 spread vol）

### 期刊主題挖掘 batch 2026-07-01（journal_discovery_20260701_0；WebSearch RFS/CFA/JPMAM/NBER/BIS 2025-26 趨勢層級；硬約束避開 VT/VIX/HAR-GARCH/ETF fragility/credit-ratio/crypto-vol-of-vol/CRE/defense/critical-minerals 已飽和 cluster；selectivity gate 後 8 條入單）
> 來源：journal-topic-discovery Codex fallback（task `journal_discovery_20260701_0`）。先讀 `scripts/agent_prompts/journal_topic_scan.md` 與 `.claude/skills/research-topic-discovery/SKILL.md`，再查 RFS advance articles、CFA ETF/direct-indexing material、JPMAM ETF conversion notes、NBER household-finance/health papers、BIS tokenisation notes；用 `rg` 對 `direct indexing / tax-loss / N-PX / shareholder democracy / Proud to Not Own Stocks / mortgage lock-in / tokenized MMF / active ETF conversion / sports betting / GLP-1` 去重，字面皆無既有 backlog 命中。本批刻意挑 wealth-tech、household-finance identity、housing lock-in、tokenised collateral、ETF wrapper migration、sports-betting、health-consumption shock 七個 fresh entity；所有版本只標 public-data proxy diagnostic，不冒充原文私有資料 replication。

- [ ] **Direct-indexing / tax-loss-harvesting crowding 是否製造年末 single-stock RV reversal** — 用 S&P 500 / Russell 1000 current constituents（Wikipedia + yfinance/Stooq；承認 survivorship）建 YTD loser / high-unrealized-loss proxy，搭 December tax-loss window、wash-sale 31-day window、ETF replacement basket（sector ETF / same-style ETF）與 abnormal volume，檢定 t+1..t+10 RV、bid-ask range proxy、January reversal 是否在高 loss-stock basket 放大；所有 signal 以 `signal.shift(1)` 進模型，並和 market/sector loser control 做 DID。與 ETF heartbeat / same-index ETF liquidity 題正交—— 那些是 fund-level creation/redemption，這裡是 advisor/direct-indexing tax workflow 造成 single-stock flow crowding（來源：CFA Institute ETF tax-loss harvesting / direct-indexing adoption 討論；direct indexing 進入 wealth-management 主流）。

- [ ] **Shareholder-democracy public-goods vote risk：N-PX / contested ESG proposal 是否預測 firm RV 與 sector spillover** — 用 SEC Form N-PX、company proxy statements、annual-meeting dates、major shareholder proposal outcomes 建公開 vote-event set，標記 environmental/social/public-goods proposal、close vote、large index-manager dissent；對公司股價、同 sector ETF、ESG ETF 做 [-5,+10] abnormal RV、gap risk、downside variance，並用 pre-event realized vol / size / sector fixed effects 控制。與 ESG controversy verified-event 題正交—— 這裡是 governance vote / shareholder-democracy channel，不是事後 controversy shock（來源：RFS 2026 "Voting on Public Goods: Citizens vs Shareholders"；RFS 2024 proxy-advisory / shareholder-engagement stream）。

- [x] **Anti-stockholder identity /「不持股」情緒作 retail-risk-off volatility proxy** → `experiments/research_anti_stockholder_identity_retail_risk_off_volati/` completed 2026-07-02，verdict **WEAK_RAW_ONLY_NO_ROBUST_OOS_PASS**：Google Trends / GDELT phrase probe 在本輪 runtime rate-limited，改採可重跑 public fallback（Wikimedia identity-relevant pageviews normalized by broad finance pageviews + FINRA monthly margin debt lagged 22 trading days + yfinance OHLCV），sample 2020-01-02..2026-07-01（1,633 trading days，24 target cells）。所有 identity / generic-fear attention signal `shift(1)`，FINRA control `shift(22)`，OOS expanding forecast 以 horizon embargo 防 forward-label leakage。最強 raw cell 是 retail/meme 22d RV（coef +0.1604, HAC t=2.85, Holm p=0.106, OOS MSE improvement -4.22%, DM t=-0.72）；retail/meme 22d downside 與 high-idio 22d downside 也只屬 raw weak 且 OOS 變差。結論：這個 Wikimedia public-attention fallback 不是 robust leading retail-risk-off volatility predictor；不可發文宣稱 identity channel 已被否定或驗證，若重開需直接 Google Trends/Gallup/SCF/AAII micro/survey 或 retail-flow data。Canonical `knowledge.json` 寫入依 Codex/Claude 分工延後至 Claude/K1259 gate；本輪只留下 `knowledge_candidate.json`。

- [ ] **Mortgage-rate lock-in / housing turnover freeze 是否領先 homebuilder、brokerage、regional-bank RV** — 用 FRED MORTGAGE30US、MBA/FHFA 或公開 outstanding mortgage rate proxy、existing-home sales / inventory / months-supply release-lagged series，建 lock-in wedge（current mortgage rate minus embedded mortgage-rate proxy）與 housing-turnover surprise；映射 XHB/ITB/HD/LOW/RDFN/Z/KRE/REM，檢定 lock-in wedge 上升後 t+5/t+22 RV、downside variance、cross-correlation 是否放大。與 office-CRE refinancing wall 與 immigration/homebuilder wage 題正交—— 這裡是 household balance-sheet mobility constraint / housing turnover channel（來源：NBER / housing-economics 2025-26 mortgage lock-in 工作論文與 Fed housing lock-in 討論）。

- [x] **Tokenised Treasury / money-market fund AUM shock 是否外溢到 T-bill ETF liquidity-vol** → `experiments/research_tokenised_treasury_money_market_fund_aum_shock_t/` completed 2026-07-02，verdict **WEAK_RAW_ONLY_NO_ROBUST_OOS_PASS**：DefiLlama protocol TVL（BUIDL/USYC/Ondo/USTB/Spiko/Anemoy/OpenEden/MatrixDock/VanEck/ArCoin）+ yfinance BIL/SHV/SGOV/USFR/TFLO daily OHLCV + FRED SOFR/IORB/WRMFNS，sample 2023-04-11..2026-07-01（809 ETF trading days；latest proxy TVL $11.7246bn）。所有 tokenised AUM signal `shift(1)`，retail MMF control `shift(5)`，OOS expanding forecast 以 horizon embargo 防 forward-label leakage。5d range-vol 只有 raw directional（coef +0.0399, HAC t=2.05, Holm p=0.201, OOS DM t=0.56）；22d range-vol 更弱；close-to-close RV 與 Amihud liquidity-vol 不支持。結論：public daily tokenised TVL 不是 robust standalone T-bill ETF liquidity-vol predictor，不可發文宣稱 collateral-flow spillover；若重開需 issuer/transaction-level collateral flow 或 bid-ask/creation-redemption data。

- [ ] **Mutual-fund-to-ETF / active ETF conversion event 是否改變 wrapper liquidity 與 underlying RV** — 用 issuer press releases、SEC 425 / N-14 filings、Morningstar/JPMAM conversion lists 建 conversion event calendar，對 converted ETF、同 category ETF、underlying factor/sector basket 做 announcement-to-conversion 與 listing後 t+1..t+60 volume、premium-discount proxy、tracking-error RV、tax-season flow sensitivity；若無 holdings，只做 category-level proxy diagnostic。與 ETF heartbeat / 351 tax-conversion 題正交—— 這裡是 whole-fund wrapper migration 與 active ETF market microstructure，不是既有 ETF share creation tax arbitrage（來源：JPMAM 2025-26 mutual-fund-to-ETF conversion disclosures；CFA ETF evolution material）。

- [ ] **Sports-betting legalization / betting-handle shock 是否抽走 retail risk budget 並放大 gambling-stock RV** — 用 state legalization / launch dates、public sports-betting handle/revenue、major sports calendar，映射 DKNG/FLUT/PENN/MGM/BETZ、lottery-like retail basket、IWM/ARKK，檢定 launch 或 handle-surprise 後 t+5/t+22 abnormal RV、volume、left-tail 與 retail-risk proxy 是否變化；與 lottery-basket / social-transmission 題正交—— 這裡是 regulated sports-betting access shock 對 household risk budget 的 crowd-out channel（來源：NBER 2024-26 sports-betting household-finance papers，指出 sports betting 對投資、儲蓄與財務壓力有可測影響）。

- [ ] **GLP-1 adoption shock 作跨 sector consumption / healthcare repricing volatility factor** — 用 FDA approval / label-expansion dates、Google Trends（Ozempic/Wegovy/Mounjaro/Zepbound）、public earnings-call mention counts 或 SEC filing text 建 GLP-1 attention/adoption proxy，映射 LLY/NVO、DXCM/TNDM、XLP、restaurant basket、food/snack stocks、healthcare ETF、airlines/apparel baskets，檢定 adoption shock 是否領先 sector RV dispersion、pairwise correlation flip 與 downside semivariance；必須控制 market/healthcare beta 與 earnings-date effects。與 generic healthcare news / consumer discretionary 題正交—— 這裡是 drug-adoption-driven demand reallocation channel（來源：NBER 2026 GLP-1 health-spending / Medicaid coverage papers；GLP-1 已成 household consumption 與 sector allocation 交叉主題）。

### 期刊主題挖掘 batch 2026-07-02（journal_discovery_20260702_0；WebSearch RFS/REStud/RoF/JFEC/CFA/Fed 2025-26 趨勢層級；硬約束避開 off-exchange short-volume、graph/GNN、thematic ETF coherence、CRE/NBFI、stablecoin、AI concentration 等已覆蓋 cluster；selectivity gate 後 6 條入單）
> 來源：journal-topic-discovery Codex fallback + fresh-context worker（task `journal_discovery_20260702_0`）。先讀 `scripts/agent_prompts/journal_topic_scan.md`，再用 `rg` 對 `labor shortage / market-to-book / Tobin / thematic concentration / N-PORT / data-driven investors / blockbuster / deposit flightiness / uninsured deposits / graph / dark / off-exchange` 去重；剔除 dark/off-exchange（K1502 NULL + line 511）、graph/GNN（line 568）、generic thematic ETF coherence（line 1130）、CRE/NBFI bank stress（line 1230/1318）、macro immigration labor-supply（line 1324）等重疊軸。本批只保留可用公開資料啟動、且必須標為 proxy diagnostic 的 narrowed variants；不得冒充原文私有資料 replication。

- [x] **Firm-level labor-shortage exposure 對 wage-sensitive idio RV 的截面預測力** — `experiments/research_firm_level_labor_shortage_exposure_wage_sensitiv/` completed 2026-07-03，verdict **NULL_PUBLIC_PROXY_DIAGNOSTIC**：24 檔 wage-sensitive listed firms、SEC 10-K/10-Q phrase-count proxy `288` filings（`216` 10-Q、`72` 10-K、0 download failures；2023-07-13 至 2026-06-29）、BLS JOLTS `77` monthly observations × `6` industries、regression panel `17420` rows（2023-07-13 至 2026-07-02）。Primary combined signal = lagged SEC labor-shortage text score × lagged BLS openings-minus-quits z-score，0/6 formal pass、2/6 方向為正；最強 combined cell 為 AIQ-relative 5d RV beta `0.01596`, clustered t=`0.7317`, p=`0.4643`，係數遠低於 t>=3 gate。JOLTS-only 5d idio RV 是 follow-up near miss：beta `0.04166`, clustered t=`2.9105`, p=`0.00361`, high-low Welch t=`4.7829`，但仍低於 t>=3 且不是 firm-level SEC text finding。Lookahead：SEC filing_date 後 shift 一個 trading day、JOLTS month-end +35 calendar days 後再 shift 一個 trading day、target 從 t+1 開始，ticker 首份 scored filing 前資料剔除。

- [ ] **Bank market-to-book / Tobin's Q divergence 作 delayed-loss regional-bank RV prior** — 用 KRE/KBE constituent bank market cap、SEC XBRL book equity / HTM-AFS unrealized-loss disclosure、Call Report release-lagged balance sheet，建 market-to-book gap 與 book-vs-market leverage divergence，檢定是否領先銀行個股與 KRE/KBE 的 next-month RV、left-tail、cross-bank correlation spike；若 fundamentals 覆蓋不足，只做 top regional banks diagnostic。與 office-CRE refinancing wall（line 1318）和 NBFI run-pressure（line 1230）正交—— 這裡是 accounting-value lag / delayed-loss repricing，而非單一 CRE exposure 或 fund-flow stress（來源：REStud 2026 "A Q-Theory of Banks"；market/book equity divergence 重回銀行穩定性前沿）。

- [x] **Mutual-fund thematic concentration 是否預測 underlying theme crowding RV** — `experiments/research_mutual_fund_thematic_concentration_underlying_th/` completed 2026-07-03，verdict **PASS_ETF_PROXY**（不是 mutual-fund TCI replication）：16 檔 thematic ETFs 的 yfinance current top holdings、`122` top-holding rows、`76` unique underlying symbols、stock panel `97128` rows / ETF panel `20448` rows（2021-06-01 至 2026-07-02）。Signal = lagged ETF dollar-volume attention × current ETF top-10 HHI × holding weight，按 underlying stock 加總；regressions 含 entity fixed effects + lagged recent 5d RV control + clustered SE。Stock-level gate 4/4 pass：5d stock RV beta `0.03862`, t=`5.2349`；5d downside beta `0.10875`, t=`7.0006`, high-low Welch t=`7.0973`；22d stock RV beta `0.03200`, t=`3.9769`；22d downside beta `0.05025`, t=`5.4840`。結論只限 current ETF top-holdings public proxy：concentration-weighted thematic ETF attention 與 underlying residual RV/downside 上升有關；完整 mutual-fund thematic concentration claim 仍需 historical N-PORT holdings 或作者 TCI data。

- [x] **Data-driven VC screening shock 對 public innovation-stock volatility 的融資閘門效應** — `experiments/research_data_driven_vc_screening_shock_public_innovation/` completed 2026-07-03，verdict **NULL_PUBLIC_PROXY_DIAGNOSTIC**：SEC EDGAR S-1/S-1/A/F-1/F-1/A 日頻 registration count（2018-01-01 至 2026-07-02，total `33576`，innovation-name count `1766`）+ yfinance `ARKK/IPO/IGV/SOXX/AIQ` with `QQQ/SPY` controls；5d/21d forward RV OOS 使用 `attention_z.shift(1)` 與 `target_end_pos < prediction_pos` embargo。10 個 OOS cells 中 6 個方向性改善但 0 個 Harvey pass；最強為 IGV 21d QLIKE improvement `+3.21%`, DM `t=-1.69` 與 IGV 5d `+2.22%`, DM `t=-2.08`，皆未達 `t<=-3`。Aggregate HAC 全部正向 vol-spillover gate FAIL，top-decile SEC attention contrast 反向（5d innovation basket RV shock mean `0.00119` vs non-shock `0.00194`, Welch `t=-6.87`）。結論只限 SEC/yfinance public proxy；不推翻 RFS private-market VC mechanism，若要重開需 Crunchbase/PitchBook/Preqin deal data、VC-backed IPO labels 或 timestamped funding/news feed。

- [ ] **Blockbuster movie / entertainment-attention shock 作 market mood 與 discretionary RV prior** — 用 Wikipedia / Box Office Mojo top movie release dates、opening-weekend gross、Google Trends entertainment attention，映射 SPY/QQQ/IWM、XLY、DIS/NFLX/AMC、gaming / streaming basket，檢定 blockbuster window 後 t+1..t+5 return/RV、downside semivariance、VIX/VRP proxy 是否變化；必須控制 weekday/holiday、market return momentum、macro announcement dates。與 sports-betting legalization（line 1362）和 investor-memory / attention-utility 題正交—— 這裡是 exogenous entertainment mood / attention crowd-out，而非 gambling access 或 investor belief recall（來源：RoF 2025 "Blockbuster or Bust? Silver Screen Effect and Stock Returns"；非金融注意力事件作資產定價 shock 重新升溫）。

- [ ] **Deposit flightiness / uninsured-deposit share 作 regional-bank liability-side RV state variable** — 用 FDIC Call Report uninsured-deposit share、FRED deposits/MMF series、SOFR-IORB / rate-spread、KRE/KBE constituent returns，所有 quarterly bank data 加 release lag 後 `shift(1)`；檢定高 uninsured-deposit share 或 flightiness proxy 是否領先銀行個股 / KRE/KBE next-month RV、left-tail 與 cross-bank correlation spike。與 NBFI run-pressure（line 1230）和 office-CRE（line 1318）正交—— 這裡是 liability-side deposit duration / flightiness，而非 asset-side CRE 或 nonbank fund-flow stress（來源：NY Fed 2025 deposit flightiness、Fed Financial Stability Report 2026 funding-risk chapter、FDIC/OCC 2025-26 deposit-risk updates；銀行負債穩定性仍是金融穩定熱點）。

### 期刊主題挖掘 batch 2026-07-03（journal_discovery_20260702_3；WebSearch JoE/JBES/JBF/JFM/JPM/FAJ/JCommMkt/QJE/AER/JPE/AEJ-Macro 2025-26 趨勢層級；硬約束避開 VT/VIX/GARCH-HAR 賽馬/ETF fragility/CRE/NBFI/stablecoin/AI concentration/labor-shortage/deposit-flightiness/market-to-book/thematic-concentration/data-driven-VC/blockbuster 等 06-30~07-02 已飽和 cluster；selectivity gate 後 10→9 條入單）
> 來源：journal-topic-discovery（task `journal_discovery_20260702_3`，agent general-purpose sonnet/medium，duration ~5 min, 13 tool uses, 114k tokens）。主線程 selectivity gate：agent 產 10 條並附每條 grep 去重確認；主線程 spot-verify 17 個關鍵字全 0 命中（electricity/renewable/day-ahead/farmland/timberland/Giacomini/conditional-predictive/realized-kernel/MedRV/microstructure-noise/wildfire/drought/population-aging/frontier-market/COMEX/lease-rate 皆 0；"aging" 3 命中皆 K1257/mixing/MM-DREX 無關）。gate 掉 #10「人口老化跨國 panel」（年頻 non-tradable diagnostic，monetization 最低）。本批刻意選 fresh-entity/methodology 軸：貴金屬物理稀缺、frontier-EM 相關收斂、方法學×2（Giacomini-White conditional predictive ability + noise-robust realized measures 直接升級既有 vol-forecast 核心）、天然實質資產、電力現貨直接建模、西岸野火乾旱 dose-response、演算法定價反壟斷、FOMC 文本複雜度。趨勢層級來源採公開期刊頁面，不捏造論文標題+作者+期號；用 ETF/proxy 者結論標 proxy diagnostic。

- [ ] **COMEX 黃金/白銀倉儲存量與物理稀缺 proxy 對貴金屬 vol 與實質利率外溢** — CME 每日 COMEX registered/eligible gold/silver warehouse stock 公開報表 + yfinance GLD/SLV/GC=F/SI=F + FRED 10Y TIPS real yield（DFII10），以 registered-inventory depletion rate 與 GLD/SLV AUM 變化作 lease-rate 緊俏度 public proxy（無法取得 LBMA lease rate，明確標 proxy diagnostic），檢定 depletion regime 是否領先貴金屬 RV 與其與實質利率相關性的 flip（來源：JBF/JFM 2025-26 金價創高背景下 physical scarcity / lease-rate 風險溢酬文獻回溫）。

- [ ] **Frontier markets（FM ETF）與 EM 危機相關性收斂是否侵蝕分散效益** — yfinance FM (iShares MSCI Frontier and Select EM) + EEM/VWO + VNM 等成分代理，計算 rolling 63d/252d FM-EM 相關係數與 FM own RV，檢定近期 EM 貨幣/信用壓力事件窗後 FM-EM 相關是否結構性上升、傳統「frontier markets 分散 EM risk」論述是否鬆動（來源：JPM/FAJ 2025-26 討論 frontier markets 逐漸被納入 broader EM beta，分散效益存疑）。

- [ ] **Giacomini-White conditional predictive ability 檢定重驗 HAR vs GARCH 賽馬是否 regime-dependent** — 用既有 SPY/TAIEX HAR-RV vs GJR-GARCH forecast loss differential，不只做 unconditional DM，改跑 Giacomini-White (2006) conditional predictive ability test，以滯後 VIX regime / high-vs-low vol state 當 conditioning instrument，誠實檢定「哪個模型贏」是否只在特定狀態成立而非全樣本單一贏家（來源：Journal of Econometrics/JBES 2025-26 對 unconditional DM 侷限性與 conditional forecast evaluation 方法論重新強調）。

- [x] **天然資源實質資產籃（農地 REIT LAND/FPI、林地 WY/RYN、水資源 ETF PHO/CGW/FIW）的通膨避險與氣候風險 vol 因子** — `experiments/k1614/` extended 2026-07-03，verdict **DIRECTIONAL_ONLY_PUBLIC_PROXY_DIAGNOSTIC**：yfinance adjusted close `LAND/FPI/WY/RYN/PHO/CGW/FIW/SPY/TIP/DBC`（2015-01-01 至 2026-07-03，2,891 trading days each）+ FRED current-vintage CPIAUCSL/PPIACO/T10YIE + USDM California weekly DSCI。Vol-structure replication still rejects homogeneous safe-haven framing: 7-asset RV level avg corr `0.7179` but ΔRV avg corr only `0.3881`; farmland internal corr `0.3545`, timber `0.9112`, water `0.9701`; high-SPY-vol regime raises all 7 RVs (high/low ratio `1.26`–`2.25`). Public-proxy stress diagnostics use conservative release lag + `shift(1)` and next-21d forward RV/downside/correlation targets; 63 HAC regression cells with Holm correction produce **0 safe-haven gate passes** and **0 risk-amplifier gate passes** (`|t|>=3`, Holm p<=0.05). High/low splits are directional only: breakeven/inflation pressure high states often show lower forward RV ratios, while California drought intensification high states raise forward RV ratios (`1.18`–`1.33`), but controlled/Holm-gated evidence does not support a significant inflation hedge, climate-risk vol premium, causal drought exposure, or tradable signal.

- [x] **電力現貨/日前市場價格 RV 直接建模（非電力股 proxy），再生能源滲透率作波動驅動** → `experiments/research_rv_proxy/` completed 2026-07-03，verdict **NULL / CONDITIONAL_PASS_AS_PILOT**：EIA v2 因本機無 `EIA_API_KEY` 不可用，改用 CAISO OASIS public ZIP endpoints（無 key）直接抓 NP15 day-ahead hub LMP、renewable forecast、CA ISO-TAC load forecast，樣本 2024-01-01 至 2025-12-30，OOS 2025 (`n_oos=364`)。正式 forecast gate：HAR + `renewable_share.shift(1)` 未改善 NP15 price-RV QLIKE（0.13078→0.13110，improve -0.25%，DM t=0.76, p=0.447），lagged renewable-share HAC t=0.706。描述性同日 high-renewable days log RV +0.402、negative-price day rate 47.5% vs 2.9%，但不進 PASS gate（未建 publication timestamp/tradability）。結論：直接電力價格 pilot 不支持「lagged renewable penetration improves HAR-RV forecasts」；same-day negative-price/vol pattern 有趣但需 multi-ISO/EIA key 或更穩定 CAISO multi-hub pipeline 才能發佈。

- [x] **加州野火/西部乾旱物理氣候風險對電力公用事業負債股與農業 ETF 的 dose-response** — `experiments/research_etf_dose_response/` completed 2026-07-03，verdict **PARTIAL_PUBLIC_PROXY_DOSE_RESPONSE**：CAL FIRE California Fire Perimeters 5,774 fire rows（acres >= 5,000 event rows `237`）+ USDM California weekly rows `810` + yfinance `PCG/EIX/SRE/XLU/DBA/CORN/WEAT/SPY`（2011-01-03 至 2026-07-02）。野火 final-acre dose 4/4 方向為正但 0/4 formal pass；最強 utility 5d RV log-acre beta `0.1089`, t=`1.47`, p=`0.141`，未達 t>=3 gate。乾旱 1/8 formal pass：4-week California DSCI increase → agriculture ETF 5d RV beta `0.00325`, HAC t=`3.5355`, p=`0.000407`, high-minus-low `0.000286`, Welch t=`6.59`。USDM DSCI 以 `D0+D1+D2+D3+D4` 計算，validStart 加 2 calendar days 後再 shift 一個 trading day；野火 market targets 從 alarm date 後第一個交易日開始，final acres 只作 ex-post physical-dose diagnostic。結論只限 public proxy：快速乾旱惡化與近端 agriculture ETF RV 有關；不支持 broad utility-volatility drought/wildfire claim。

- [x] **演算法定價反壟斷執法（RealPage 式租金演算法案件）事件窗對房東 REIT 與旅遊定價股 vol 的影響** — `experiments/k1615/` completed 2026-07-03，verdict **DIRECTIONAL_ONLY**：DOJ/FTC official event calendar（8 residential RealPage-style milestones + 1 hotel/travel statement）+ yfinance adjusted close `AVB/EQR/UDR/ESS/CPT/MAA` apartment REIT basket、`JETS/MAR/HLT/H/ABNB/BKNG/EXPE` travel-pricing basket、controls `SPY/VNQ/XLY/^VIX`（2022-01-03 至 2026-07-03）。Direct primary tests use log daily RV event-window regressions with Newey-West HAC maxlags=10 and Holm correction. Apartment REIT residential windows are positive but not gateable: coef `0.3655`, HAC t=`2.05`, raw p=`0.0399`, Holm p=`0.0799`, median event/pre RV ratio `1.03x`; travel has only one official event and is non-gateable (t=`0.31`). Conclusion: weak legal-risk proxy lead only; no significant RealPage legal-risk premium, causal exposure, cross-sector spillover, or trading signal.

- [ ] **FOMC 聲明/會議紀要語言複雜度（可讀性/避險語氣）對會後 vol 持續性與 VIX 期限結構的預測力** — Fed 公開 FOMC statement/minutes 全文 + 免費可讀性指標（Flesch-Kincaid/詞彙多樣性）與簡易 hedging-word lexicon（"however"、"uncertain" 等詞頻）建 complexity/tone score，release-lag 後 `shift(1)`，檢定其對會後 t+1..t+5 SPY RV persistence 與 VIX/VIX3M term-structure slope 變化的增量預測力；與既有 generic FOMC event dummy 研究正交——這裡是文本複雜度而非事件本身（來源：JBES/Journal of Econometrics 2025-26 央行溝通文本複雜度與市場反應的 NLP-in-macro 熱點）。

- [ ] **雜訊穩健已實現波動率估計量（Realized Kernel / Two-Scale RV / MedRV）對 HAR 預測力的增量：本機 5-min 資料重估** — 用本機已有 TAIFEX/SPY 5-min tick-derived 資料，實作 jump-robust MedRV 與 noise-robust Realized Kernel / Two-Scale RV，取代標準 5-min RV 作 HAR 輸入，OOS QLIKE/DM/Harvey 對比標準 RV-HAR baseline；與既有 OHLC candlestick estimator 題（line 1291）正交——這裡是 tick/5-min noise-robust kernel 估計量（來源：JBES/Journal of Econometrics 長期熱點 realized-measure microstructure noise correction，2025-26 仍持續有新 kernel 變體研究）。

### 期刊主題挖掘 batch 2026-07-03b（journal_discovery_20260703_2；WebSearch JBF/JFE/RFS/JoE/JFQA/JFM + JPM/FAJ/J.Alt Inv/J.Fixed Income/J.Futures Markets + QJE/AER/JPE/AEJ + JFEC/JBES/JAE 2025-26 趨勢層級；selectivity gate 後 13→10 條入單）
> 來源：journal-topic-discovery（task `journal_discovery_20260703_2`，agent general-purpose sonnet，~6 min, 25 tool uses, 137k tokens；backlog dry / refill no_new_candidates_passing_filter 觸發）。Agent 對既有 30+ batch 逐一 grep 去重並自附**誠實信心分層**（高/中/低）。主線程 selectivity gate 掉 3 條：#7 人口年齡結構 regime（今日稍早 batch 已用同理由 gate 掉「人口老化跨國 panel」— 年頻 non-tradable diagnostic、monetization 最低）、#9 buyback blackout（agent 自標僅財經媒體討論、無學術期刊佐證 → 誠實原則不硬湊）、#13 Basel III Endgame 銀行股（agent 自標與 K1209 同 ticker universe「同殼不同題」風險 + 低信心）。保留 10 條分兩層：**高信心 8 條**（WebSearch 有實據趨勢/已確認發表）+ **中信心 2 條**（領域知識延伸、轉譯屬推測，dispatch 前須再查證，已標 ⚠️）。趨勢層級來源採公開期刊頁面，不捏造論文標題+作者+期號；用 ETF/proxy 者結論須標 proxy diagnostic。

- [ ] **非流動性資產「staleness bias」修正版 RV 估計量對 HAR 預測力的增量** — 用 yfinance 5-min 片段建 zero-return-run-length staleness proxy（低流動 small-cap ETF / ADR），實作 staleness-adjusted RV（staleness-weighted realized kernel）vs 標準 5-min RV，HAR OOS QLIKE + DM 檢定是否改善低流動標的預測；**與 line 1400（noise-robust kernel/MedRV，今日已入單）正交**——那是「微結構量測誤差」，這裡是「非同步/稀薄交易造成的價格 stale」不同 bias 來源（會被誤判為 artificial jump 或 spuriously low vol）（來源：JBES/JFM 2025-26 staleness 是獨立於 missingness/noise 的 bias 來源之持續文獻）。

- [ ] **連續時間 spot-beta（continuous-time Fama-MacBeth）能否改善條件 VaR / vol 預測** — 用本機 5-min TAIFEX/SPY 片段做日內 spot-beta 估計（非傳統 rolling-daily beta），檢定 spot-beta 作 covariate 加入 HAR/GARCH 對次日 VaR breach rate 與 QLIKE 是否優於 60D rolling beta；與既有 idiosyncratic-vol ICAPM 題（K1525，月頻 cross-section）正交（來源：Aït-Sahalia-Jacod-Xiu (2025) continuous-time Fama-MacBeth, RFS；高頻計量重新定義傳統 CAPM 估計是近兩年熱點）。

- [ ] **Cointegration error-correction term 作為 HAR-RV 額外 covariate** — 用高度共整合 ETF-pair（sector ETF vs 成分股籃、或 ADR vs 母股 local-share proxy）建 error-correction 殘差，將 lagged |ECT| 加入 HAR-RV，檢定殘差擴大期是否對兩腳 RV 有增量預測力；與 line 436「Partial Cointegration Hedging」（避險比例應用）正交——這裡把 cointegration 殘差當 vol 預測因子（來源：2025 cointegrated-asset-dynamics vol forecasting 前沿方法趨勢）。

- [ ] **時變因子負荷（time-varying loading）版 Factor-Augmented HAR vs 靜態負荷 HAR** — 用多資產 ETF RV panel 建第一主成分 vol factor，分別用靜態負荷與 Kalman-filter 時變負荷估計，比較 OOS QLIKE/DM；與 line 1276（F-Riesz tail 協方差）、line 1278（constrained multivariate HAR）正交——那兩者處理 tail fatness / 參數約束，這裡處理「因子負荷是否隨時間漂移」（來源：2025-26 time-varying factor-augmented vol forecasting 前沿方法趨勢）。

- [ ] **Realized semicovariance（正/負號分解）作相關性 regime 早期預警** — 用多資產 ETF 高頻/日頻報酬建 signed semicovariance（P+/P-/混合號分量），檢定負號分量（downside co-movement）是否比 total RV/covariance 更早預測相關性 regime break 或跨資產 vol spike；與 line 1276/1278（tail shrinkage / 參數約束）正交（來源：Bollerslev-Patton-Quaedvlieg realized semicovariance JoE 系列持續延伸，2025-26 有新估計量變體）。

- [ ] **Image-rendered candlestick + CNN 視覺分類預測 vol vs 傳統數值估計量** — 用 yfinance OHLCV 生成合成 K 線圖（mplfinance render），訓練輕量 CNN 做視覺 pattern classification 預測次日/次週 RV bucket，對比 HAR/GARCH 數值 baseline 的 QLIKE/rank accuracy；**與 line 1291（JFEC 2026 Bollerslev candlestick 數值公式估計量）正交**——那是解析公式，這裡是 pixel-level 電腦視覺，資料表徵完全不同（來源：Hao (2025) image-based price patterns, Journal of Futures Markets；金融影像式 ML 是 2025 新興子領域）。

- [ ] **貨幣 order-flow proxy 對新興市場股市波動的領先力** — 用 EM 貨幣 ETF（CEW）相對強弱與量價變化構造 order-flow-like pressure proxy（非真實 order flow，須標 proxy diagnostic），檢定其對 EEM/VWO/EWZ/EWY/INDA 等次日/次週 RV 的增量預測力，明確 lag + DM；與 K1336（EM FX carry×vol gate NULL，carry timing）正交——這裡是匯率壓力→股市波動傳導（來源：Macro & Finance in EM Economies (2025) currency order flow → EM stock vol）。

- [ ] **Lottery-demand 對 beta anomaly（BAB）的解釋力分解** — 用 yfinance 美股月頻，按 rolling beta 五分位 + lottery-demand proxy（MAX/idio-vol，複用 K1346/K1503 已建方法）雙重分組，檢定排除高 lottery-demand 名單後 BAB 報酬與 vol-managed 組合效率是否改善；與 K1346（lottery basket vol-of-vol，NULL）、K1503（Factor-MAX 預測 vol 非報酬，MIXED）不同研究問題——這裡測「lottery demand 是否解釋 low-beta anomaly 的報酬來源」（來源：JFQA lottery-demand-based explanation of beta anomaly 研究線）。

- [ ] ⚠️ **中信心（dispatch 前須再查證轉譯）— VC-vintage markdown 週期（IPO ETF + venture-lending BDC proxy）預測小型成長股波動** — 用 FPX/Renaissance IPO ETF 相對大盤報酬與折溢價 proxy + 有 venture-lending 曝險 BDC（HTGC/TRIN/PSEC）建 VC 估值 markdown 週期 proxy，檢定其是否領先 IWO/ARKK 小型成長股籃 RV 與左尾；與 K1188（LP capital-call 壓力，聚焦 alt-manager 母公司股價）正交（來源：JAI 2025-26 permanent-capital/VC 主題確認熱，但「透過上市 proxy 傳導到小型成長股」的具體轉譯為推論，需查證）。

- [ ] ⚠️ **中信心（dispatch 前須再查證轉譯）— 州最低工資調升生效日的邊界不連續設計 → 勞力密集零售/餐飲股波動反應** — 複用 AEJ:Applied 州界不連續識別設計，用公開州最低工資調升生效日曆，對勞力密集零售/餐飲類股（CMG/MCD/SBUX/TGT/M 等曝險不同州比例個股籃）做事件窗 RV/left-tail 比較 + 與非曝險對照股 DID；與既有 labor-shortage 軸（需求面稀缺）不同機制——這裡是明確法規生效日的成本面衝擊 + clean identification（來源：AEJ:Applied 州界最低工資不連續設計是勞動經濟學×asset pricing 交叉持續範式，但 2025-26 特定新文章未證實，屬推測）。

### 期刊主題挖掘 batch 2026-07-04（journal_discovery_20260703_3；WebSearch JPM/FAJ/CFA/JRFM(MDPI)/JFutMkt/FRL/JAI/JoD/AQR 2025-26 趨勢層級；本批**刻意只挖 under-mined 的「交易/投資策略機制」軸**——前 12 批重壓 ML方法/realized-measures/HAR方法學/macro-shock；selectivity gate 後 4→2 條入單）
> 來源：journal-topic-discovery（task `journal_discovery_20260703_3`，agent general-purpose sonnet，~6.6 min, 34 tool uses, 170k tokens；backlog dry / refill no_new_candidates_passing_filter 觸發第 13 批）。**agent 誠實判斷策略軸多已 tapped**：VIX 期限結構交易（line 331/549/587/596/1246/1341）/ drawdown-crash-hedge overlay（line 312/569/660/899 K544/K341）/ risk-parity vol-scaling（K1347/K1532）/ CTA-trend overlay（K1547 NULL）/ FX carry×vol（line 557/1333 K1336）/ bond-equity vol spillover（K1516 NULL）六軸有五軸已紮實覆蓋含多筆完成實驗，WebSearch 反覆搜 2025-26 頂刊新文獻多落空（部落格/業界分析非可驗證期刊）→ agent 依「挖不到就誠實回少數」指令**只回 2 高信心 + 2 中信心，不硬湊 12-15 條**。主線程 selectivity gate 掉 2 中信心：buffer/defined-outcome ETF（agent 自承僅 Morningstar/Cerulli/ETF.com 產業分析、無 2025-26 confirmed 學術期刊 → 依「搜不到期刊趨勢就跳過」硬規則 gate）、VRP 多訊號 circuit-breaker（agent 自承與 line 539/587/K1362 機制相近、「同殼不同題」風險 → gate）。保留 2 條皆主線程 spot-verify dedup（BXM/CNDR/collar/put-write/iron condor/buy-write/altcoin/low-volatility-anomaly 全 0 命中；唯 'covered call' 1 命中經查是 K72 XYLD+VT 單一過往結果、非系統化 overlay 擇時，正交）。趨勢層級來源採公開期刊頁面，不捏造論文標題+作者+期號；用 ETF/benchmark proxy 者結論須標 proxy diagnostic。

- [ ] **選擇權賣方 overlay 策略的 vol-regime 進出場擇時：covered-call / put-write / collar / iron condor 誰在哪個 regime 該關掉** — 用 CBOE 官網公開歷史 benchmark 策略 index level（BXM buy-write、PUT put-write、CLL collar、CNDR iron condor；yfinance ticker 不可得則 CBOE 官網 CSV 免費下載，**非付費 tick/chain**）vs SPX，建 IV percentile / VIX term-structure regime gate 決定進出場，比較 timed vs static overlay 的 **beta-adjusted** Sharpe（須比照 K544 Israelov-style beta-adjust，禁 naive Sharpe）、MDD、危機季報酬；DM/Harvey 檢定 timing 是否真有增量。與 K72（XYLD+VT 不改善 50/50，單一過往結果）正交——這裡是跨 4 個 CBOE benchmark 的系統化 regime 擇時。**直服 Mission #4 策略上架 + 變現**（來源：JRFM(MDPI) 2026-05 "Return Determinants of Option Strategies: Protective Put vs Covered Call"，SET50 指數選擇權 2021-2025，covered call 多數期間贏 protective put、驅動因子含 IV/basis/OI/moneyness；已對 backlog grep 確認 fresh）

- [ ] **加密貨幣 low-volatility anomaly 的 regime 依賴性：低波動幣是否只在特定市場階段有溢酬** — 用 yfinance **top-N 流動性加密貨幣**（BTC/ETH + 主要 altcoin，資料範圍 = yfinance 可抓的 ~top 20-30 幣，**proxy for** FRL 原文 432 幣 Binance universe，須標 proxy diagnostic），仿股票 low-vol anomaly 建月度 realized-vol 排序組合，檢定低波動組合溢酬是否 regime-dependent（牛市早期 vs 修正期/高不確定期），與既有股票 low-vol/min-vol 結果**跨資產類別對照**（非重複）。與既有 crypto 軸（spillover line 462、ETF 時段 line 513、vol-of-vol line 529、stablecoin line 544）全部正交——皆非 cross-section low-vol factor（來源：FRL 2026-03 "Revisiting the low-volatility anomaly in cryptocurrency markets"，432 檔 Binance 幣 2018-01~2025-11，確認 anomaly persistence 與 cyclicality；已對 backlog grep 確認 fresh：altcoin/low-volatility-anomaly/crypto-cross-section 0 命中）

### 期刊主題挖掘 batch 2026-07-04b（interactive 主線程派 journal-topic-scan agent；WebSearch IJF/JFEC/JoE + JPM/FAJ/JAI/Morningstar + Fed/arXiv/CEPR + PBFJ/JFM 2024-26 趨勢層級；硬約束避開兩個 NULL 飽和弧——「獵奇外生 shock → 次日 RV event-window」（K1602/K1604 等 ~15 連 NULL）與「新 covariate → HAR OOS 增量」（K1613/K1616-K1619 連 NULL）；agent 產 12 條、主線程 selectivity gate 後 11 條入單）
> 來源：pool 乾涸（narrative-arc-exhausted）後主線程派 general-purpose agent 跑 scripts/agent_prompts/journal_topic_scan.md（~8.2 min，21 tool uses，123k tokens）。agent 已對 267 條 open backlog + 最近 30 條 knowledge + 8 個飽和 cluster 逐條 dedup，每條標鄰近項差異（#131/#151/#157/#166/#174/#208/#24/#140）。主線程 gate 掉 1 條：buffer/defined-outcome ETF 兌現審計——與 07-04 上一批被 gate 的同主題重複，且同樣只有 Morningstar/practitioner 來源無 confirmed 學術期刊（「搜不到期刊趨勢就跳過」硬規則）。四大弧線群：A 計量方法（reconciliation/pooling/persistence 識別/partial-day nowcast/multiple-testing 總清算）、B 實務產品（volatility laundering/buyback blackout）、C 新資料源（prediction markets/perp funding，2 條標 ⚠️ 資料覆蓋須 dispatch 前驗證）、D 台股主場（價格限制 magnet/TSM ADR 雙上市）。

- [ ] **RV 預測的 forecast reconciliation：跨 horizon（1d/5d/22d）與跨截面（portfolio vs 成分）強制一致性是否免費改善全部預測** — 本機 5-min SPY/TAIFEX RV + yfinance ETF panel；各自跑 HAR@1d/5d/22d 與 portfolio/成分-level variance forecast，MinT/temporal-hierarchy reconciliation 調和後 DM per-horizon 檢定（IJF 2024 reconciliation review + JFEC intraday-decomposition reconciliation + arXiv 2026 MGARCH-portfolio reconciliation）。新弧：coherence 約束，非 covariate 非賽馬；與 #208 期限結構賽馬不同——那是比誰贏，這是強制一致後全體是否變好。
- [ ] **Global vs local：跨資產 pooled HAR（一條服務 ~100 檔）何時打敗 per-asset HAR** — yfinance ~100 檔 ETF/大型股 range-based RV；per-asset OLS-HAR vs pooled vs 分群 pooled，檢定增益是否集中「短樣本資產 + 危機期」；pooled inference 必按日聚合（cluster-robust by date，per K1355 規則）（IJF/M4 cross-learning + arXiv global-NN data-scaling 線）。新弧：估計策略（參數共享結構），非新 predictor。
- [ ] **RV 持續性是真 long memory 還是 level shifts 假象：break-robust 模型 vs ARFIMA/HAR/rough 的識別與預測含意** — SPY 2000-2026 + TAIEX 長樣本 log-RV，估 random-level-shift + spurious-memory 檢定，shift-adjusted 預測 vs HAR/ARFIMA 對比；與 #131（roughness 穩定性）區分：問 persistence 來源識別非粗糙度估計（IJTAF 2025 rough vol survey + JoE spurious-long-memory 經典線 + arXiv 2026；rough-vs-long-memory 辯論重燃，level-shift 第三解是 contrarian 位置）。
- [ ] **部分日資訊的價值曲線：開盤後 N 小時的 partial-day RV 何時就「夠了」預測明天** — 本機 5-min SPY/TAIFEX，partial-day RV(10:30/12:00/14:00/收盤) 逐步 HAR nowcast，畫「資訊到達→OOS QLIKE 改善」曲線找邊際價值歸零點；直接回饋 VT 執行時點（IJF 2025 intraday spot vol + JFEC intraday-decomposition 線）。與 #24（跨資產 feature）/#166（乘法 intraday GARCH spec）不同——這是資訊時點 vs 預測價值的 timing 弧。
- [ ] **波動率 predictor zoo 多重檢定總清算：平台 30+ 已試外生 predictor 的 Romano-Wolf/FDR 統一 OOS 審計** — 重用 1400+ K 中所有「外生變數→RV」實驗（AI 電力/PM2.5/GPR/EPU/credit spread/穩定幣…），統一 pipeline 重跑 + stepdown FWER/FDR，回答「校正後幾個倖存」；天然 methodology 論文素材（Goyal-Welch-Zafirov 2024 equity-premium 總清算 + JF/RFS multiple-testing 主流線；**波動率端沒人做 Welch-Goyal 式總清算**，平台自有 NULL 記錄是獨有樣本）。與「N 套策略 meta」飽和 cluster 區分：這是 predictor 推論層 FWER，非策略 Sharpe 彙整。
- [ ] **Volatility laundering 免費驗證：上市 private-credit 載體的申報波動 vs 去平滑後真波動** — yfinance 上市 BDC（ARCC/BXSL/OBDC）+ BIZD + PRIV（2025 上市 private-credit ETF）+ interval fund NAV（⚠️ 覆蓋度 dispatch 前驗證），Getmansky-Lo-Makarov MA-unsmoothing 估平滑係數，對照 HYG/銀行股 proxy 量化「申報 vol 低估幾倍」（JPM 2024 Anson amortizing volatility + CFA Institute 2025 volatility-laundering 系列；Asness 帶起的最熱實務辯論 × private-credit 入 ETF 時事）。與 #151（BDC 危機前哨=預測弧）不同——這是**測量弧**：申報風險系統性偏誤量化。
- [x] **回購靜默期（buyback blackout）覆蓋率作為市場層級 vol 日曆因子** — `experiments/research_buyback_blackout_vol/` completed 2026-07-04，verdict **NULL_OR_PROXY_LIMITED**：current top-50 S&P large-cap proxy（yfinance market-cap snapshot + `get_earnings_dates(limit=50)`，2020-01-02 至 2026-07-02，1,633 SPY trading days）建 35 日前至 2 日後 blackout coverage；控制 lagged 5d/21d RV、lagged VIX、OPEX、weekday/month FE 後，next-day log-r² coefficient per +10pp = +0.0047、t=0.14，forward 5d coefficient = -0.0387、t=-1.64，high-vs-low coverage annualized abs-return diff -2.18 vol pts（block bootstrap CI [-7.33,+2.40]），OOS QLIKE 改善僅 +0.105%、DM t=-0.17。結論：free-data proxy 不支持「buyback blackout coverage 是 robust SPY vol calendar factor」；限制為 current top-50 survivorship、current weights、無實際 buyback / 10b5-1 / historical constituent weights。
- [ ] **預測市場（Kalshi/Polymarket）合約價格作為免期權 ex-ante 事件不確定性：能否預測 CPI/FOMC 日 realized vol** — Kalshi/Polymarket 免費 public API 抓賽前機率分佈（dispersion/entropy），檢定對事件日 SPY/TLT RV 預測力，與 ^VIX、K1612 FOMC 文字基線對比增量（⚠️ API 歷史資料覆蓋 dispatch 前驗證）（Fed FEDS 2026 Kalshi macro markets + arXiv 2026 Kalshi 預測 crypto vol；prediction markets 2025-26 主流化、學術跟進剛起步）。全新免費資料源弧（risk-neutral 事件機率），與 #140（pre-FOMC IV drift 可交易性）不同問題。
...
### 期刊主題挖掘 batch 2026-07-05（journal_discovery_20260705_0；WebSearch Fed/FEDS/IMF GFSR/FAJ/CFA/SSRN 2025-26 趨勢層級；task pool JSON 修復後派 fresh-context general-purpose agent；selectivity gate 後 8 條入單）
> 來源：journal-topic-discovery（task `journal_discovery_20260705_0`）。先修復 `storage/next_tasks.json` 尾端 `"result":` 截斷造成 claim 機制 JSONDecodeError，再依 `scripts/agent_prompts/journal_topic_scan.md` 派 fresh-context worker 掃近 1-2 年 Fed/FEDS、IMF GFSR、FAJ/CFA 與 SSRN / crypto microstructure 趨勢層級來源。主線程用 `rg` 對 `research_program.md` 去重；剔除泛化 cyber / private-credit / BTC ETF 時段結構 / FOMC generic text 題，保留「同來源但不同機制」的 narrower variants。所有 public API / ETF / disclosure 版本皆為 proxy diagnostic，不冒充原文資料 replication。

- [x] **Hedge-fund Treasury crowding 風險定價（K1683，NULL）** — `experiments/k1683/` 以 CFTC TFF 2Y/5Y/10Y/Bond 四年期的公開 gross-participation proxy 實作（4,192 筆 contract-report rows；名目週五發布後再 lag 一週；899 個 signal origins），不冒充不可公開逐期取得的 Form PF hedge-fund exposure。2013-09 至 2026-07 expanding OOS 的四個主要格子為 0/4 通過：TLT/IEF RV5 QLIKE 雖改善 +1.4026%/+0.4979%，HLN t=-1.2701/-0.5780、BH q 均 0.5635，且 late-half 改善轉為 -0.5407%/-0.2985%；DGS10 五日 yield jump 與 SPY–TLT correlation loss 則惡化 -0.2653%/-1.3396%。公開 proxy 在 2023-01-03→2025-09-30 上升 +25.0946%，與 Fed Form PF 的 gross-exposure 倍增不是同一資料定義，不能互相驗證。結論只是否定「無條件、release-lagged 公開持倉 proxy」的穩健增量預測力，不是否定 forced deleveraging 機制；後續若續做，應改測可得的 funding/margin stress trigger × crowding interaction（來源：K1683；Fed FEDS Notes 2026；CFTC TFF）。
- [ ] **FOMC statement vs press-conference shock 的分布反應拆解** — 用 FOMC statement、press-conference transcript/time stamps + SOFR/Treasury futures event windows，分離 statement shock 與 press-conference shock，檢定二者對 TLT/IEF/ZN=F 次日 RV、rate-vol skew proxy、tail jump 的不同影響；與 line 1425「聲明/紀要語言複雜度」正交——這裡是同日兩階段 market-reaction 分解，不是文本可讀性 level（來源：Fed FEDS 2026-029 “Coarse Statements and Predictive Pressers”）。
- [ ] **Spot Bitcoin ETF flow shock 對 BTC/ETH vol 的影響** — 用 IBIT/FBTC/ARKB/GBTC 等公開 AUM/net-flow/volume（issuer / ETF.com / Farside fallback，dispatch 前驗證覆蓋）+ BTC-USD/ETH-USD，flow shock `shift(1)` 後檢定 t+1/t+5 RV、週末 gap、BTC-ETH beta 與 basis proxy；與既有 BTC ETF 時段結構題正交——那是 ETF-ization 改變交易時鐘，這裡是 ETF 資金流/creation-redemption channel（來源：FAJ 2025 spot Bitcoin ETF discussion）。
- [x] **跨交易所 crypto arbitrage spread 作為短期 RV 訊號（K1682，NULL）** — 受 public REST 可得性限制，實作為 Binance（USDT 同日換成 USD）、Coinbase、Kraken 的 completed UTC 日收盤價離散 proxy，不冒充 synchronized midquote 或可成交 arbitrage gap。BTC/ETH 各 720 個共同日、h=1/5 OOS 407/399 筆；加入昨日離散後，8 個預先指定的 RV QLIKE／5% tail pinball 格子只有 BTC h=1 tail loss 小幅改善 +0.3734%，但 HLN-DM t=-0.3063、BH q=0.8328，0/8 通過，verdict=`NULL_NO_ROBUST_OOS_INCREMENT`。這個 null 只否定該日頻 close proxy 的增量預測力；同步高頻 quote、fee、latency、depth 仍未檢驗（來源：K1682）。
- [ ] **Wrapped-token price discovery / tokenization spillover 對 crypto vol regime 的預警力** — 用 wBTC / cbBTC / stETH 等 wrapped-token 與 native BTC/ETH 的價格、liquidity、bridge-supply 或 DEX pool depth 公開資料，估 information share、basis widening 與 price-discovery latency，檢定 wrapped-token stress 是否領先 BTC/ETH RV regime；與 tokenised Treasury/MMF AUM shock 題正交——那是 T-bill ETF liquidity-vol，這裡是 crypto-native token wrapper basis（來源：SSRN 2025 wrapped-token price-discovery stream + 2026 DeFi/tokenization review）。
- [ ] **短期移動平均距離（SMAD）作為 vol / left-tail predictor** — 用 yfinance 美股/ETF universe 的 close 與 10d moving-average distance 建 salience gap，目標改成 next-day range RV、downside semivariance、left-tail hit rate，而非原始報酬；所有 SMAD signal `shift(1)`，用 FDR/Harvey gate 檢查是否只是技術分析資料探勘。與既有 MA/TF 策略賽馬正交——這裡不交易均線，而是測均線距離是否預示波動/左尾（來源：FAJ 2025 Short-Term Moving Average Distance）。
- [ ] **主權財富基金 private-market 配置上升是否改變公共市場 RV / correlation regime** — 用公開 SWF allocation / annual report / Global SWF summary（dispatch 前驗證歷史覆蓋）建 sovereign-allocator private-market share proxy，對 public equity / credit / REIT / infrastructure ETF basket（SPY/EFA/EEM/HYG/VNQ/PAVE/IGF）做 regime compare，檢定 allocation shift 期間 public-market RV、相關性與 drawdown 是否系統性改變；與 LP liquidity stress / private-credit BDC proxy 正交——這裡是 sovereign allocator rotation（來源：CFA Institute 2026 sovereign wealth funds in private markets + CFA 2026 private markets growth report）。
- [ ] **拉美 private-credit funding-gap 擴張與 EM 波動傳導** — 用 CFA 2026 Latin America private-credit funding-gap 論述作制度背景，建立 EM local-bank / credit / FX ETF proxy（ILF/EWW/ECH/EPU/EWZ/CEW/EMLC/EMB/UUP），檢定 private-credit supply-expansion proxy 或 fundraising/news intensity 是否伴隨 EM RV 壓縮、尾部風險重估或美元敏感度改變；與一般 BDC/private-credit spillover 題正交——這裡是區域 funding-gap / LatAm transmission（來源：CFA Institute 2026 Latin America private credit + CFA 2026 private markets growth report）。

### 期刊主題挖掘 batch 2026-07-08（journal_discovery_20260708_0；WebSearch RFS/JoE/JFEC/IJF + JPM/FAJ/JAI/J.Derivatives/J.Futures Markets + AER/QJE + Journal of Forecasting/JBES 2025-26 趨勢層級；backlog all-covered fallback 觸發；agent 回 7 條 + 主線程 selectivity gate 後 5 條入單）
> 來源：journal-topic-discovery（task `journal_discovery_20260708_0`，agent general-purpose opus/medium，~6.7 min, 31 tool uses, 147k tokens）。agent 誠實判斷 backlog 高度飽和（40+ 前批、260+ open items；vol-forecast 方法賽馬 / tail-VaR-ES / VRP-dispersion / crypto / macro-shock event window / conformal / quantile-VaR / semivariance-HAR / dealer-gamma-0DTE / forecast-reconciliation 皆已覆蓋或 NULL-closed）→ **不硬湊 15 條，只回 7 條真正正交**（4 高信心 + 3 medium ⚠️）。主線程 spot-verify dedup 掉 2 條：「60/20/20 黃金 regime 對沖」已被 line 561（相關 regime 化 60/40）+ line 617（多資產股債金 RS-GARCH 配置）覆蓋；「jump-recapture business-cycle regime HAR」是 line 563（Markov 2-regime HAR + FRED 景氣 gate 檢定 jump 係數轉不顯著）的近乎完全重複。保留 5 條皆經 grep 確認正交。趨勢層級來源採公開期刊頁面，不捏造論文標題+作者+期號；用 range proxy / ETF proxy 者結論須標 proxy diagnostic。
- [ ] **TVP-HAR：時變參數 HAR（狀態空間 / Kalman filter）取代靜態 OLS-HAR** — 對 SPY/0050.TW/TX 日 RV，用 Kalman/random-walk 讓 HAR 的日/週/月係數隨時間演化，DM-HLN + Giacomini-Rossi fluctuation test 檢定時變係數 OOS 是否勝固定係數 HAR，並看係數路徑是否在 leverage/regime 轉換時漂移；yfinance/TAIFEX 免費（來源：Journal of Forecasting 2025 TVP-HAR for VIX 趨勢層級）。與 line 1452 factor-loading TVP（第一主成分 vol factor 的負荷時變）正交——這裡是 **HAR 自身日/週/月係數的連續時變**，非因子負荷；與 line 1478 level-shift（離散斷點）正交（連續演化 ≠ 結構斷點）。
- [ ] **GNAR-HARX：網路增強 HAR（跨資產相關圖的空間自回歸）** — 對 ~30 檔 ETF/大型股 range-based RV panel，先用滾動相關建 network graph，再估 GNAR-HARX（自身 lag + 網路鄰居 RV lag），檢定空間 spillover 項是否在 per-asset HAR 上提供 OOS 增量（DM + MCS，pooled inference 按日聚合 per K1355 cluster 規則）；yfinance 免費（來源：arXiv 2510.24443 GNAR-HARX + IJF network/cross-learning 線趨勢層級）。與 line 526 co-jump/HAR-CJ（jump 計數 channel）正交——這裡是 realized-vol 的**空間自回歸建模**，非 jump channel（GNAR grep=0）。
- [ ] **歐盟碳權（EUA）波動率與 ETS2 2027 上路的結構性 regime 斷點** — 用 KRBN/GRN 等碳權 ETF（yfinance）建 EUA RV proxy，檢定 2025 下半年「碳價與天然氣脫鉤」斷點、HAR forecast，及碳權波動對潔淨能源 ETF（ICLN/TAN/QCLN）的 spillover；ETS2 前置供給機制作 vol-suppression event（來源：Journal of Forecasting / Nature Sci Rep 2025-26 EU ETS 波動 + ETS2 2027 launch 時效題）。與 line 1248 auction-mechanism（拍賣 demand-depth → KRBN RV）正交——這裡是**碳權商品價格本身的波動 + ETS2 監理 regime 斷點 + clean-energy spillover**，時效性強、天生 reader-facing；結論標 ETF proxy diagnostic。
- [ ] ⚠️(medium) **Realized-GARCH-lite：聯合 return + realized-measure 模型（免費日頻 range proxy 解鎖 line 458 blocked 方向）** — 自寫 scipy MLE 估 Realized GARCH（測量方程用 Garman-Klass/Parkinson range 當 realized measure，非 5-min），對 SPY/TLT/0050.TW 比 GARCH(1,1)/GJR/HAR 的 OOS QLIKE + VaR；套件不支援故須 custom MLE（K1213 moat 規則）。line 458 RGARCH-CARR-SK 已列但 blocked on 5-min data（line 30）——**本題用 range proxy 提供免費資料落地路徑**；與 GARCH/HAR 單方程賽馬正交（聯合報酬+RV 測量方程模型類，庫內從未落地）。結論標 range-proxy diagnostic。
- [x] **路徑依賴波動（Guyon-Lekeufack PDV）內生性分解** → **K1666 完成 2026-07-10，CONDITIONAL_PASS_RANGE_PROXY**：yfinance SPY/QQQ daily OHLC（2000-01-03..2026-07-09；usable n=6,415/asset；seed=42）建兩個 PDV features：R1=252d power-law weighted past returns、R2=sqrt weighted past squared returns，全部 `signal.shift(1)`。primary target=Garman-Klass daily range variance proxy（非 5-min RV），C2C 為 robustness。PDV-only GK R²：SPY full/holdout 0.501/0.420，QQQ 0.551/0.345；C2C 較弱（SPY 0.244/0.206，QQQ 0.232/0.135）。Expanding HAR(d,w,m) OOS 2004-12-29..2026-07-09：date-clustered GK QLIKE vs HAR，HAR+R1 +7.49% (DM t=-3.90)，HAR+R1+R2 +8.79% (DM t=-4.53)；subperiod 2005-2009 與 2020-2026 pass，2010-2019 方向正但未過 Harvey（t=-2.61）。Caveat：C2C QLIKE 亦改善但 MSE 可嚴重惡化，SPY GK MSE 也惡化；因此結論是 **daily range-proxy / QLIKE-specific conditional evidence**，不可包裝成 Guyon-Lekeufack 5-min RV replication。reviewer=Codex primary path CONDITIONAL_PASS；knowledge=e4973432。

### 期刊主題挖掘 batch 2026-07-09（journal_discovery_20260709_0；WebSearch JFutMkt/JFEC/JoF/JBES/RoF/JPM/FAJ/QJE-AER-JPE-Econometrica/NBER 共 14 次 2025-26 趨勢層級；backlog all-covered fallback 觸發；agent general-purpose opus/low 回 6 條 + 主線程 spot-verify dedup 後全 6 條入單）
> 來源：journal-topic-discovery（task `journal_discovery_20260709_0`，agent general-purpose opus，~7.4 min, 29 tool uses, 122k tokens）。agent 誠實判斷 backlog 高度飽和（30+ 前批、480+ open items；conformal-VaR / connectedness-QVAR / path-dependent-rough-HAR / F-Riesz-RCov / SSR-skew / VT-no-trade-band / prediction-market / MOVE-VIX-VRP / LLM-text-vol / diurnal / backwardation-regime 皆已飽和或 NULL-closed）→ **不硬湊 15 條，只回 6 條真正正交**（2 高信心 + 1 中高 + 2 中 + 1 中低 ⚠️）。主線程 spot-verify：realized-beta / climate-CPU / bottom-up-VaR / interval-fuzzy 四軸 grep 全 0 命中確認正交；bear-beta 題與 line 1461（lottery-demand 解釋 BAB anomaly）鄰接，但「bear beta = 崩盤保險通道」的下檔溢酬**來源歸因**是 1461 未覆蓋角度 → 保留並註記 dispatch 時聚焦 crash-insurance channel（勿與 1461 的 BAB 報酬分解重複）。趨勢層級來源採公開期刊頁面，不捏造論文標題+作者+期號；用 range/ETF proxy 者結論須標 proxy diagnostic。
- [x] **氣候政策不確定性（CPU）指數 → 商品期貨 RV 增量** → **K1668 completed 2026-07-10，NULL_NO_OOS_CPU_INCREMENT**：用 Gavriilidis/Känzig/Raghavan/Stock 官方月頻 U.S. CPU（1985-01..2026-04；primary=`cpu_index_narrow`）+ yfinance commodity ETF adjusted close（USO/UNG/DBA/CORN/WEAT/GLD）建月頻 realized variance proxy；所有 HAR/CPU features 先按月建 raw signal 再 `signal.shift(1)`，target month t 只用 t-1 資訊。Expanding OOS log-HAR vs HAR+lagged CPU：overall date-clustered QLIKE improvement **-2.489%**（HAR 0.2361 vs HAR+CPU 0.2420，DM t=+1.564，p=0.120），sector 皆負（energy -1.515%、agriculture -2.866%、metal -2.876%），資產只有 WEAT 點估 +1.891% 但 DM t=-0.398；無 Harvey pass。多國 equal-weight GCPU proxy（policyuncertainty.com，pre-2020 robustness）overall 亦負 -4.766%（DM t=+1.597）。結論：免費 ETF proxy 不支持 lagged CPU 是 next-month commodity RV 增量 predictor；不可外推為反駁 JFutMkt futures/event-window connectedness 證據。reviewer=Codex primary path PASS_NULL_RESULT；knowledge=f0be4112。
- [ ] **已實現 beta 預測 vs 常數 beta 的最小變異避險比率賽馬**（高信心，contrarian）— yfinance 日頻（或本機 5-min SPY/sector）建 rolling realized beta，誠實檢定「時變 realized beta 是否真的打敗 prior-year constant beta」於次期 minimum-variance 避險比率（對 hedged-portfolio realized variance 做 DM/HLN）；Reeves-Wu 的反直覺結論（TVR beta 不勝 constant）是否在 free-data 也成立。realized-beta grep=0，與 line 339（MV hedge 引用）、1299（TWD 幣別避險）皆正交（來源：JFEC realized-integrated-beta stream 2025-26 + Reeves-Wu 2013 contrarian）。
- [ ] **VaR/ES 預測的 bottom-up（成分加總）vs top-down（組合層直估）誰贏**（中高信心）— yfinance 多資產 basket（SPY/TLT/GLD/HYG/QQQ），比較「先預測各成分 VaR/ES 再加總」與「直接對組合報酬估 VaR/ES」的 Kupiec/Christoffersen/DQ/ES backtest 與 MCS；測 cross-sectional aggregation 是否改善尾部覆蓋。aggregation grep=0，與 line 1335（coherent estimator guardrail）、conformal 弧正交——這裡是 aggregation direction 本身（來源：Journal of Forecasting 2025 cross-sectional aggregation ES/VaR）。
- [ ] ⚠️(medium) **Bear-beta vs speculative-beta：下檔風險溢酬到底來自崩盤保險還是樂透偏好**（中信心）— yfinance 美股月度截面，建 downside/bear beta（對深度下跌 regime 的條件 beta）與 speculative beta（高 idio-vol/lottery proxy）雙排序組合，Fama-MacBeth + vol-conditioning 檢定溢酬歸因；明確 lag。**dispatch 聚焦 bear-beta = crash-insurance channel**（下檔崩盤條件 beta 的溢酬），勿與 line 1461（lottery-demand 解釋 BAB **報酬**分解）、K1346（lottery basket VoV，NULL）重複——這裡問「下檔溢酬來源歸因：崩盤保險 vs 樂透」（來源：RoF 2023-25「Bear Beta or Speculative Beta」reconciliation stream）。
- [ ] ⚠️(medium) **動能因子崩盤：動態（Daniel-Moskowitz）vs 常數（Barroso）波動率縮放的 MDD/尾部改善** — yfinance MTUM 或自建截面動能組合，跨 2020/2022/2025 三類回撤，比較常數 12% vol-target 與 forecast-based 動態縮放對 momentum crash 的 MDD、左偏、淨成本後 Sharpe 改善；與 line 516（Moreira-Muir 因子動物園 vol-managing premium 存活性）正交——這裡聚焦 crash-tail/MDD 縮減與 dynamic-vs-constant scaling 機制（來源：JPM 2026 Baltussen-Van Vliet 150yr/40國 + Barroso-Santa Clara 2015 + Daniel-Moskowitz JFE 2016）。
- [x] ⚠️(medium-low) **區間值（interval-valued / range-based）RV 預測 vs 點預測 HAR** → **K1670 completed 2026-07-10，NULL_NO_INTERVAL_EDGE**：yfinance adjusted OHLC（SPY/QQQ/IWM/TLT/GLD/HYG；request start 2005-01-01）把 next-day interval 定義為 `[log(Low_t/Close_{t-1}), log(High_t/Close_{t-1})]`，用 expanding OOS（initial 1000、21d refit）比較 calibrated scalar point-HAR interval baseline、center-radius interval HAR、direct lower/upper bounds HAR；所有 interval features 先建 raw signal 再 `signal.shift(1)`，並以 80% train containment 校準 interval width。Aggregate date-clustered 結果：center-radius interval 的 interval MSE 只改善 **+0.034%**（DM t=-0.318，p=0.751），coverage 78.34% vs point-HAR 78.41%，interval score 反而 -0.096%；direct bounds interval MSE **-1.347%**（DM t=+0.902），coverage 77.80%。QQQ 單資產 center-radius DM t=-4.08 但被 IWM/GLD/HYG 抵消，未過 pooled/date-clustered inference。結論：透明 daily OHLC interval-valued proxy 不支持比 calibrated point-HAR interval 更好的 next-day interval forecast；不可改寫成 scalar RV QLIKE 證據。reviewer=Codex primary path PASS_NULL_RESULT；knowledge=b52e118a。

### 期刊主題挖掘 batch 2026-07-10（journal_discovery_20260710_0；WebSearch FRL/JFE/RFS/JBF/QJE/JFEC/RCFS 2024-26 趨勢層級；backlog all-covered fallback 觸發；fresh-context worker 回 10 條 + 主線程 selectivity gate 後 7 條入單）
> 來源：Codex hourly journal-discovery。先讀 `scripts/agent_prompts/journal_topic_scan.md`，再查近 1-2 年財金/實務/計量/經濟期刊公開頁面；主線程用 `rg` 對 `research_program.md` / `knowledge.json` 去重。剔除 3 條：美元主導性（已與 DXY/UUP regime / global FX ambiguity 多軸鄰近）、clean-energy climate uncertainty（撞 Green-minus-brown / climate-energy vol spread）、EM ETF vol-regime generic（過寬且與 EM FX/vol regime 題鄰近）。保留題皆要求免費資料代理並標 proxy diagnostic；趨勢層級來源採公開期刊頁面，不捏造論文題名以外細節。
- [ ] **原創新聞率 vs 轉載新聞率的 vol premium** — 用 GDELT / RSS / 公開新聞簇把同一事件拆成 `original-source ratio` 與 `reprint ratio`，再配 SPY 與 sector ETF 做 next-week RV、downside semivariance、jump-risk 預測；明確 `signal.shift(1)`，並控制總新聞量與 VIX。與 generic sentiment / news count 題正交——這裡測「資訊流品質」而非新聞強度（來源：Finance Research Letters 2024 original-vs-reprinted news price-volume relation；資訊傳播鏈品質成為新聞流研究新切口）。
- [ ] **政策新聞類型 mix 對 sector vol dispersion 的影響** — 用 Baker-Bloom-Davis EMV / GDELT / policyuncertainty 分類，把 tax / monetary / regulation / trade / inflation 等政策新聞 share 建成日/月頻 state，檢定 sector RV dispersion、downside risk 與 sector rotation 的 lead-lag；yfinance sector ETF 免費資料，必做 VIX 與市場 RV 控制。與單一 EPU/CPU level 題正交——這裡問政策新聞「組成」是否解釋橫斷面波動分化（來源：JFE 2026 policy news and stock market volatility；40 類新聞 tracker 與 firm-level volatility exposure 是 2025-26 明確趨勢）。
- [ ] **Fraud / enforcement contagion 的 peer-vol spillover** — 用 SEC Accounting and Auditing Enforcement Releases、DOJ press release、PPP fraud / corporate scandal verified event set，檢定事件公司同業與 sector ETF 在 t+1..t+10 的 RV、left-tail、bid-ask proxy 是否擴散；事件必手工/程式雙重驗證，避免新聞 rumor。與 broad ESG controversy / cyber incident 題正交——這裡是法律/詐欺 enforcement shock 的 peer-vol contagion（來源：RFS 2025 fraud contagion stream；social connection + fraud diffusion 使 enforcement shock 成為可驗證事件源）。
- [ ] **Saliency-driven stock price manipulation 的 crash-risk proxy** — 用 abnormal search / Reddit / Stocktwits / news burst 當 saliency shock，再用 SEC pump-and-dump / market-manipulation enforcement case 作弱標籤，檢定小盤股與 meme basket 的 t+1/t+5 left-tail、RV、gap risk 是否被放大；需 FDR/Harvey gate 防資料探勘。與一般 attention 或 social-transmission 題正交——這裡聚焦 manipulation/crash-risk channel（來源：Journal of Banking & Finance 2025 investor attention and stock price manipulation；saliency × manipulation 是新興行為風險題）。
- [ ] **Bank deposits and stock-market cycle：存款流失是否放大 regional-bank vol** — 用 FDIC Call Reports / H.8 / FRED deposit proxies、KRE/XLF 與 regional bank basket，檢定 stock-market boom/crash 與存款增減 state 是否領先銀行 RV、downside semivariance、信用相關 drawdown；所有報表資料按發布 lag 處理。與 bond-fund run / bank M/B 題正交——這裡是 household deposit cycle 造成銀行 funding fragility（來源：RFS bank deposits and stock market channel；家庭資產配置與銀行資金來源連動可轉成 VolPred funding-risk proxy）。
- [ ] **Information-theoretic latent risk-state 作 vol regime classifier** — 用純價格 cross-section（SPY/sector ETF/factor portfolios）與少量 FRED macro factors 估 information-theoretic / entropy-based latent pricing-kernel state，再用 state 預測 next-week RV、drawdown、tail loss，對照 HAR/EWMA/Markov-switching baseline；若 implementation 過重，先用 entropy/SDF proxy diagnostic。與單一 predictor 題正交——先抽 model-agnostic risk state 再做 VolPred regime forecast（來源：JFEC 2025 information-theoretic asset pricing model；非參數 SDF / entropy 方法進入資產定價主流）。
- [x] **地理型 investor attention / local information advantage 的波動效應** → **K1680 completed 2026-07-11，RETROSPECTIVE_NULL**：官方 RCFS demo（4 匿名 firms × 50 states × 52 weeks；10,400 rows）只作同週 spatial/news sanity；主 pilot 用 K1588 pinned yfinance OHLCV 與 6 檔 current-HQ 州／全美 Google Trends（2021-07..2026-07），ticker/SPY 同 payload ratio 後做 52w rolling z，明確 `signal.shift(1)`。六檔共同 retrospective evaluation：風險 target 80 週、national attention 68 週；**0/4** 通過 CW one-sided + Holm + Harvey t≥3。RV model-scale MSE +0.798%，但 QLIKE **-0.395%**，CW t=1.88、Holm p=0.120；gap MSE -0.922%、CS spread -0.138%；local signal 對 national attention MSE +0.533%，CW t=1.67、Holm p=0.143。Google 歷史抽樣／修訂 vintage 不可重建，故只能稱 current-vintage pseudo-OOS diagnostic，不能宣稱 real-time OOS、因果 local-information advantage，亦不能概括「所有 geographic attention 都無效」。knowledge entry 依 task gate（至少 CONDITIONAL_PASS）不寫；完整證據 `experiments/k1680/`。

### 期刊主題挖掘 batch 2026-07-11（journal_discovery_20260711_0；JoE/JBES/RFS/JFM + ReStud/QJE/AEJ:Macro + Fed/NY Fed/USPTO 2025-26；backlog all-covered fallback 觸發；三線 21 候選 + 主線 selectivity gate 後 10 條入單）
> 來源：`research-topic-discovery` 輕量流程；財金計量、實務、經濟跨領域三線並行掃描，主線逐條檢查官方期刊／政府來源、免費資料、發布時點與 `research_program.md` 近義重複。剔除已覆蓋的 factor-vintage、concentration、generic attention、term-premium、bond-fund run、coherent-risk、generic inventory predictor 等弧；保留題皆要求 point-in-time/release lag、forward-label embargo 與多重檢定，proxy 版不得冒充原文資料 replication。
- [ ] **Intrinsic-time RV：成交／價格活動時間是否優於固定 5 分鐘取樣** — 用本機 TAIFEX tick 同時計算 clock-time、trade-time、hitting-time、realized-business-time RV；先以 noise-robust RV 比 measurement error，再把各 measure 放進同資訊 HAR，做 OOS QLIKE、horizon-specific DM-HLN、MCS 與 `block=max(10,H)` bootstrap。與 Realized Kernel/MedRV 題正交：這裡改「取樣時鐘」，不是再換 estimator（來源：[Journal of Econometrics 2025 intrinsic-time volatility](https://www.sciencedirect.com/science/article/pii/S0304407625002039)；同 issue [endogenous-sampling jump test](https://www.sciencedirect.com/science/article/pii/S030440762500185X)）。
- [ ] **ML forecast comparison 的 estimation-risk inference audit** — 在相同 SPY/QQQ RV 任務重跑 Lasso/樹模型 vs HAR，比較傳統 DM 與納入 learner 收斂速度、loss-score 條件、sample split 的 predictive-ability inference；用 Monte Carlo 檢查 size，外層 holdout 防 selector/learner 自身 overfit。與 C+ ML-ceiling 賽馬正交：本題問「既有顯著性檢定是否有效」，不再疊新架構（來源：[JBES 2026, Extending Predictive-Ability Inference to Machine Learning](https://www.tandfonline.com/doi/abs/10.1080/07350015.2025.2562964)）。
- [ ] **基金風險揭露「說到做到」程度是否預警 risk surprise** — 用 SEC N-1A 風險文字建立 disclosed-risk factor set，以 rolling incremental R² 衡量文字對基金報酬的解釋力，再測低 informativeness 是否領先 tracking-error RV、drawdown 與贖回 shock；N-1A amendment 依實際 filing timestamp 對齊，fund/month FE、雙向 clustered SE、BH。與 thematic concentration 正交：這裡是「揭露風險 × 實際風險吻合度」（來源：[RFS 2026](https://academic.oup.com/rfs/advance-article-abstract/doi/10.1093/rfs/hhag020/8688482)；免費 [EDGAR API](https://www.sec.gov/search-filings/edgar-application-programming-interfaces) + [N-PORT bulk data](https://www.sec.gov/data-research/sec-markets-data/form-n-port-data-sets)）。
- [ ] **FCM 清算集中度是否在高波動期排擠小型交易者並放大商品流動性風險** — 用 CFTC 月度 FCM customer-segregated assets 算 HHI，按實際發布日 lag，接 DCOT non-reportable share 與 CL/GC/ZC/ZS volume、range RV；估 commodity×month panel、`FCM-HHI × high-vol` interaction 與雙向 clustered SE，預註冊「異質、非 universal」假說。與期貨持倉/庫存題正交：訊號是 clearing-intermediary concentration（來源：[Journal of Futures Markets 2026](https://onlinelibrary.wiley.com/doi/10.1002/fut.70116)；[CFTC FCM data](https://www.cftc.gov/MarketReports/financialfcmdata/index.htm)）。
- [ ] **EIA 天然氣庫存公告遇假日移到週五：低注意力是否造成延遲波動吸收** — 以 EIA 公告日曆及庫存變化建 surprise（無免費 consensus 時用只含當時資料的 AR forecast），比較正常週四與假日週五之當日反應、次交易日 RV/continuation/reversal；估 `abs(surprise) × Friday`，用 HAC + release-week block bootstrap。與 generic inventory predictor 正交：這是日曆誘發的 attention quasi-experiment（來源：[Journal of Futures Markets 2026](https://onlinelibrary.wiley.com/doi/10.1002/fut.70104)；[作者研究中心公開稿](https://business.ucdenver.edu/jpmorgancenter/content/trader-attention-and-market-reaction-fundamental-news-evidence-natural-gas-futures)）。
- [ ] **Treasury 稅期／拍賣交割的 TGA reserve-drain shock 是否放大 repo 與 duration RV** — 用預先可知的 tax/auction-settlement calendar × lagged reserve-abundance state，配 Treasury DTS、H.4.1、NY Fed TGCR/BGCR/SOFR 與 TLT/IEF/ZN=F；event local projection 與 expanding OOS 都禁止使用當日 DTS（T+1 才可得）。與 repo-basis/CFTC crowding 正交：這裡是固定日曆流量 shock × reserve scarcity（來源：[Federal Reserve FEDS Notes 2025](https://www.federalreserve.gov/econres/notes/feds-notes/fluctuations-in-the-treasury-general-account-and-their-effect-on-the-feds-balance-sheet-20250806.html)）。
- [ ] **Primary-dealer settlement fails 作 Treasury market-plumbing 預警** — 用 FR2004 Treasury/MBS/agency fails-to-deliver/receive 週資料，嚴格從週四發布後才讓 fails z-score 進模型，預測下一週 TLT/IEF/MBB RV、range 與股債相關；threshold local projection + HAR/HAR+fails expanding OOS，依 target H 設 HAC/embargo。與 equity T+1 settlement 制度斷點正交：這是 collateral daisy-chain 的實際壓力量（來源：[NY Fed fails primer](https://www.newyorkfed.org/markets/pridealers_failsprimer.html)；[NY Fed data hub](https://www.newyorkfed.org/markets/data-hub)）。
- [ ] **Patent-publication innovation news → sector vol/dispersion regime** — 用 USPTO PatentsView 的實際 publication/grant date 與 CPC→sector mapping，建只含當時已公開資訊的 novelty/count surprise，測 XLK/SMH/XBI 等 t0–t5 RV、dispersion、co-jump；申請日或 full-sample technology shock 只能做非交易性解釋，不得冒充 PIT signal。與 AI capex/generic news sentiment 正交：這是可驗證的 innovation-news 公開事件（來源：[Review of Economic Studies 2025](https://doi.org/10.1093/restud/rdaf086)；[USPTO PatentsView](https://www.uspto.gov/subscription-center/2026/patentsview-releases-q4-2025-data-update)）。
- [ ] **Funding distress 下 stock-price informativeness collapse 是否領先 crash breadth / sector correlation** — 優先重用論文 replication measure；否則以 yfinance 個股 panel + FRED funding stress 建保守 proxy，估 monthly informativeness state × funding-stress interaction 對下一月 crash breadth、sector correlation、RV，並對照 stress-only 與 Amihud-only。與一般 liquidity predictor 正交：機制是「股票被拿來取現，流動性需求侵蝕價格資訊」（來源：[QJE 2026](https://academic.oup.com/qje/advance-article/doi/10.1093/qje/qjag029/8698258)；免費 [replication data/code](https://doi.org/10.7910/DVN/KRMMS7)）。
- [ ] **AS/AD「bad uncertainty」分解 → cross-asset tail regime** — 用 AEA replication package + ALFRED/FRED，把 aggregate-supply / aggregate-demand shock 的 Gaussian uncertainty 與負偏 bad uncertainty 分開；只能用單邊 expanding/filtering state，禁止 full-sample smoothed state，檢定 next-month SPY/TLT/GLD/UUP/HYG downside-RV、stock-bond correlation 與 VaR。與 uncertainty-level × disagreement 正交：這裡有明確 shock channel 分解（來源：[AEJ: Macroeconomics 2026](https://www.aeaweb.org/articles?id=10.1257%2Fmac.20220076)）。
...
- [x] **「量先價行 / 爆量長黑是出貨」** → **K1671 completed 2026-07-10，PARTIAL_FOR_VOLUME_LEADS_PRICE__FALSE_FOR_BLACK_CANDLE_DISTRIBUTION**：獨立複核 K1659/K1667，9 資產（SPY/QQQ/IWM/AAPL/MSFT/NVDA/0050.TW/2330.TW/2317.TW），成交量門檻改為更嚴格 `volume_t > 2 × previous_20d_avg_volume`，事件訊號 `signal.shift(1)` 預測隔日報酬，seed=42。18 個 primary cells 中僅 2317.TW 爆量後隔日上漲通過 BH 5%（n=293，hit-rate +9.53pp，均報酬差 +0.522%，CI [+0.229%, +0.845%]，q=0.0265）；2330.TW raw positive 但 BH 後不存活。Date-level pooled diagnostic 為正（+0.293%，HAC t=4.10），顯示 attention drift 可能存在於特定個股/事件日，但不是普世規則。爆量長黑隔日續跌再次不成立：0/9 資產支持，pooled signed return 對「續跌」方向為 -0.344%（t=-2.17），放空策略 Sharpe=-0.55、MDD=-89%。結論：量先價行「部分、資產依賴」；爆量長黑出貨「不支持」。
...
### 老闆 Telegram msg877 文章拆解（2026-07-16；財經捕手「SpaceX 跌破發行價」總經文）

> **拆解方法**：先查知識庫（LanceDB 語意搜尋）確認覆蓋，再驗資料可得性，才判斷可做性。四個論點只有一個進 backlog。

1. ❌ **「債務壓力 → 股價/vol 溢出」（文章主論點）— 已覆蓋且全 NULL，aggregate 版不再做**。K872：FRED HY OAS（BAMLH0A0HYM2）對 fwd vol raw r=0.55 但 VIX r=0.71，HY-VIX corr=0.77（資訊冗餘），**控制 VIX 後 delta-R² = 0**；T14：credit+yield 僅 +1.6% incremental R²（統計顯著但經濟 trivial，credit overlay 反而降低表現）；K1621：EM sovereign credit vol proxies 不 lead EM equity vol（NULL）；另 K1529 / K1515 / K1538 同軸。結論：**aggregate 信用壓力的資訊已被 VIX 吃掉**，文章的洞察在總體層面沒有增量。
2. ⚠️ **issuer-specific credit → issuer equity vol（文章真正在講的層面）— 未覆蓋，但資料 blocked**。SPCX 自己的公司債殖利率 7.5% / CDS 上升 → 自己的股價疲軟，是 firm-level 傳導，與上面測過的 aggregate 完全不同層次；等價於 Merton structural model 的 firm-level 直接檢定。Blocker：個股 CDS 需 Markit（付費）、公司債逐筆需 FINRA TRACE（免費但需 scrape/申請）。**列 P4 待資料決策**，不在資料解決前開題。
3. ✅ **IPO 市場熱度作為 risk-appetite → vol 領先指標** — 未覆蓋 + 資料已驗證可得，**排入 backlog**。文章第二論點「重磅 IPO 利多不漲 / 破發 = 市場動能休息訊號」的可檢定版本：用 Renaissance IPO ETF（yfinance `IPO`；**已驗證 n=3,204，2013-10-16→2026-07-15**，涵蓋 2018Q4 / 2020 COVID / 2022 三次空頭）相對 SPY 的動能與 drawdown 當 IPO 市場熱度 proxy，檢定其對 SPY 已實現波動的 OOS 預測增量。設計：HAR-RV baseline + `shift(1)` 訊號、expanding OOS、QLIKE、DM-HLN + Harvey |t|>3、canonical HAC bandwidth。差異化：K531 測 FRED sentiment（EPU / Michigan / SKEW）、N182 測 VIX/GARCH z-score，**都沒測 IPO 市場熱度這條 risk-appetite 通道**；預期它與 VIX 高度相關（同 K872 教訓），故主檢定必須是**控制 VIX 後的增量**，不是 raw 相關。
4. ❌ **ARM / SPCX / SKHY 個案事件研究 — 樣本不足，不做**。已驗證：SPCX n=22 天（2026-06-12 上市）、ARM n=710 天。重磅 IPO 事件母體 <30，檢定力不足；只能當文章敘事，不能當研究證據。

- [ ] **IPO 市場熱度（Renaissance IPO ETF 相對 SPY）對 SPY 波動率的 OOS 預測增量** — 見上 §3；主檢定 = 控制 VIX 後的 delta-QLIKE 與 DM-HLN，raw 相關不算數。

### 期刊主題挖掘 batch 2026-07-14（journal_discovery_20260713_0；JoE/JBES/JFE/RFS/JFM + ReStud/QJE 2024-26；backlog all-covered fallback 後補 10 條）
> 來源：`research-topic-discovery` 輕量流程；計量、財金實務、經濟跨領域三線共掃描 2024-26 正式期刊，再由主線核對 publisher／政府 primary source、免費資料、發布時點及 `research_program.md`／knowledge 近義題。剔除已做 NULL 的 capital-gains-overhang、generic attention、bond-fund run、一般 higher-moment HAR 與既有 macro co-jump 弧。下列題皆須先凍結資料可得時點，rolling/expanding 估計只用當時資料；replication/proxy 設計不得冒充原文完整資料重現。
- [x] **偏誤波動代理的可靠度加權評估目標** → **K1704 完成 PROXY_ROBUST_STATISTICAL_RANKING（2026-07-16，Codex post-run review PASS + certified）**：本機 TX 日盤 3,548 天（2012-01-02..2026-07-14，raw bytes 逐檔 SHA-256 復驗）、共同 OOS ledger 2,016 天（2018-02-01..2026-07-14）；六個觀察代理 target（1/5/10-min RV、range、day r²、可靠度加權 consensus）QLIKE 排名全部 `HAR_RV5 < EWMA_R2 < GJR_GARCH`，每個 target 的 10% stationary-bootstrap MCS 皆 singleton {HAR_RV5}（1,000 reps, seed 42），early/late split 亦然。Consensus QLIKE：HAR 0.1839 vs EWMA 0.2405 (-23.5%) vs GJR 0.2593 (-29.1%)；full-OOS HAC DM t=-4.63/-4.24（過 Harvey）。可靠度權重與偏誤中心全部只用過去樣本 + leave-one-proxy-out；eligibility 先於 calibration 凍結、fail closed。限制：僅證明排名對「觀察代理選擇」穩健，不推 latent-IV 最優；early-OOS MCS elimination p=0.093（5% 門檻下不淘汰）；1/5/10-min RV 共用 ticks 的相關測量誤差未識別。knowledge=k1704_certified_20260716。
  - 差異／來源：不同於 K1660 單一縮放錯誤與 K1560 estimator-dispersion predictor，這裡正式修復 evaluation target（[Journal of Econometrics 2025](https://www.sciencedirect.com/science/article/pii/S0304407625001228)，DOI 10.1016/j.jeconom.2025.106068）。
- [ ] **多期距 × 多分位數的聯合預測最適性 gate** — 重用既有 1/5/10 日、1%/5% VaR ledger，對照逐格 Kupiec/Christoffersen/DQ 與多條分位數回歸的聯合 autocalibration 檢定；重疊期距採 embargo、HAC／區塊抽樣，預測必須在 `t-h` 已生成。
  - 差異／來源：不同於逐格 backtest，本題問整張風險曲面是否聯合最適，避免挑格顯著（[JBES 2024](https://www.tandfonline.com/doi/abs/10.1080/07350015.2024.2316091)；作者提供 `quantoptimR`）。
- [ ] **相依結構預測評分的誠實度回溯稽核** — 對既有 Joe、DCC 結果重建單邊滾動 marginal PIT，先驗證邊際，再用 lexicographic 邊際＋相依分數與 two-step predictive-ability 檢定，並做非同步收盤敏感度。
  - 差異／來源：不是新增 copula；本題檢查 margins 不同時直接比 loss 是否把 marginal edge 誤歸因為 dependence edge（[JBES 2026](https://www.tandfonline.com/doi/full/10.1080/07350015.2025.2602855)；[公開稿](https://arxiv.org/abs/2410.04165)）。
- [ ] **零股／報送規則的幽靈成交量污染稽核** — 用 BRK.A/BRK.B 與配對高價股公開 OHLCV，圍繞零股報送起點及 2024 修正做中斷時間序列、合成控制與 placebo-date 檢定，確認成交量衝擊、Amihud、量價波動預測 edge 是否為 one-share 機制造成；所有 feature lag 1。
  - 差異／來源：不是 FINRA short-volume predictor，而是 data-lineage measurement audit（[RFS 2025](https://academic.oup.com/rfs/article-abstract/38/3/623/7632093)；[FAJ 2025](https://rpc.cfainstitute.org/research/financial-analysts-journal/2025/fractional-solution-stock-market-mystery)）。
- [x] **美國最小跳動單位試驗的 pre-spread 波動異質性** → **K1706 completed 2026-07-16，PASS（range-only confirmatory）**：用官方 Tick Size Pilot 分組與 2016-09 FINRA/CHX B.I 事前 spread，固定 `<10¢`／`>15¢` 分層；公開日 OHLCV 覆蓋 2,064 檔。精確 stock/date FE 下，窄 spread 層 post range `+19.74 bps`（RI p=.002，Holm=.016），寬層 `-9.35 bps`；stock-level 窄減寬異質性 `+29.75 bps`（RI p=.001，Holm=.004），兩個 range placebo 的 Holm p 均為 1.0。RV5、log dollar volume、Amihud 在 Holm 後皆未拒絕，只能稱 non-rejection diagnostic，不能稱等價或零效果。日 OHLCV proxy 不是 TAQ/MIDAS intraday measure，RI 只保留固定 spread 層內 treated counts，另有公開行情覆蓋／survivorship 限制；Codex post-boundary review PASS，knowledge=`k1706_reviewed_20260716`，完整證據見 `experiments/k1706/`。
  - 差異／來源：制度自然實驗，不是一般 microstructure predictor（[Journal of Financial Markets 2026](https://www.sciencedirect.com/science/article/abs/pii/S1386418125000643)；[SEC 資料](https://www.sec.gov/data-research/tick-size-pilot-program/tick-size-pilot-data-resources)；[FINRA 分組](https://www.finra.org/rules-guidance/key-topics/tick-size-pilot-program/test-group-assignments)）。
- [ ] **全球波動下機構去風險對小型股特質波動／流動性的傳導** — 優先用論文公開 replication；延伸才以 SEC 13F 並按 filing date（最長 +45 日落差）建 small→large rotation，預測下一季特質 RV、downside RV 與 Amihud；用 firm/date 雙向 clustered panel、固定窗 OOS、CW/DM-HLN 檢定。
  - 差異／來源：不是 generic VIX→market RV，而是機構資金跨 size 傳導（[Journal of Financial Economics 2025](https://www.sciencedirect.com/science/article/pii/S0304405X25000868)；[公開 replication](https://data.mendeley.com/datasets/x6p3r3jg6h/2)）。
- [ ] **企業新聞類別的事前尾厚度 → 公告後 RV／反轉** — 以 SEC 8-K item code 分類，每次 filing 前只用該類別歷史估 tail ratio/Hill index，再預測 t+1..t+5 異常區間 RV、volume 與 reversal，控制當次 shock、market RV/VIX 與 firm FE；16:00 ET 後 acceptance 移至下一交易日。
  - 差異／來源：不是 sentiment/news count；禁止 full-sample category ranking，訊號是類別歷史 tail geometry（[Review of Economic Studies 2026](https://doi.org/10.1093/restud/rdaf037)；[SEC EDGAR API](https://www.sec.gov/search-filings/edgar-application-programming-interfaces)）。
- [ ] **弱總經訊號過度反應、強訊號不足反應的非線性波動吸收** — 以 ALFRED first-release vintage 與當時可得估計建 CPI/NFP/GDP surprise，預註冊弱／強分箱並以 spline 檢定 t+1..t+5 反轉、延續與 RV decay；08:30 反應可看當日 open→close，但 forecast label 從次日開始，修訂值禁入。
  - 差異／來源：不同於 macro co-jump，本題識別 surprise 強度 × absorption 曲線（[QJE 2025](https://academic.oup.com/qje/article/140/1/335/7821262)，DOI 10.1093/qje/qjae032）。
- [x] **選擇權競價的 price-improvement／liquidity benefit 是否在壓力期崩解** — K1707 完成公開 pseudo-data adequacy audit：1,161,488 rows 只有 16 個偽日期、3 個匿名標的、0 個事前 VIX≥30 日，且含 6 個週末日期，故四項預註冊 support gate 全數失敗，verdict=`INSUFFICIENT_STRESS_SUPPORT`；未估 interaction、不作 null 宣稱、不寫 PASS-family knowledge。未來真實 OPRA 樣本至少需 80 日、30 個高壓日與 10 個標的才重啟 confirmatory inference。
