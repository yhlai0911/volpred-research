...
- [ ] **台灣投資人多幣別 hedge ratio 的 ex-post realized vol-minimization：靜態 vs DCC vs regime-switching** — yfinance USD/TWD（^TWD=X 或 FXY/FXB/FXE cross-rate proxy）+ 0050.TW/^TWII vs SPY/^GSPC/EFA/EEM，建 (a) static MV hedge ratio、(b) DCC-GARCH 動態、(c) HMM regime-switching 三規格，比較 hedged portfolio realized vol、downside semivariance、tail HE 與 Sharpe；與既有面向 I 期貨避險不同，這裡是 currency overlay on equity（來源：CFA 2026 Currency Management Refresher Reading + Wellington 2026 multi-asset currency exposure；台灣投資人多幣別 hedge 為 retail 客群高需求題）
