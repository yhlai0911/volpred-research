...
- [ ] ⚠️(medium) **路徑依賴波動（Guyon-Lekeufack PDV）內生性分解：日 RV 有多少是「報酬路徑內生」而非外生新聞** — 建 PDV 的兩個特徵（過去報酬 power-law 加權趨勢項 R1 + 過去平方報酬加權 √項），量測其對日 RV 的 R²（文獻稱 ~65% 內生），並把 R1（leverage-trend）當獨立 covariate 測 HAR 上的增量；yfinance SPY/QQQ（來源：Guyon-Lekeufack 2023 PDV + arXiv 2503.00851 HAR-PD 2025）。與 line 1294（rough/path/Markovian **賽馬**）正交——這裡問**內生 vs 外生變異分解 + leverage 通道 attribution**，非誰預測贏；PDV/Guyon grep=0，dispatch 前再 spot-verify 非殼變。
