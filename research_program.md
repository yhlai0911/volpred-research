...
- [ ] **Tokenised Treasury / money-market fund AUM shock 是否外溢到 T-bill ETF liquidity-vol** — 用 RWA.xyz / issuer public pages（BUIDL、OUSG 等）或 DefiLlama RWA dashboard 建 tokenised Treasury/MMF AUM growth，搭 SHV/BIL/SGOV/USFR/TFLO、SOFR/IORB、BTC/ETH ETF volume，檢定 AUM shock 是否領先 short-end Treasury ETF high-low spread proxy、RV、flow-vol 與 crypto collateral stress；所有鏈上與 issuer AUM 只能作 diagnostic，不宣稱完整 collateral ledger。與 stablecoin reserve/T-bill RV 題正交—— 那是 stablecoin supply/reserve flow，這裡是 tokenised fund / on-chain collateral wrapper flow（來源：BIS 2025-26 tokenisation / tokenised money-market-fund notes）。
