...
|---|---|---|---|
| **P5** | **vt-crowding-abm** | 🟢 **K1261+K1262+K1262b evidence COMPREHENSIVE (2026-04-27)** — v1 reproduce GREEN 33/33 + v2 撤回 → **K1261 Phase 1** (10,500 sims H1+) + **K1262 Phase 2** (16,800 sims scaling × window robustness, **H1+ STRONGLY SUPPORTED** 12/12 cells) + **K1262b OAT** (16,000 sims λ/γ ±50% sensitivity, **H1+ confirmed robust to λ/γ** 5/5 cells, calibration EXACT VT=70% match P5 paper Table 2). **NotebookLM「70% threshold = λ/γ knife-edge mathematical artifact」critique 被 K1262b 直接 rebut — VT range {70%,70%,70%,70%,100%}, TF=30% in 5/5, MR≤VT in 5/5; both qualitative ordering AND magnitude robust to λ/γ ±50% perturbations**. K1261+K1262 兩輪 code review CONDITIONAL PASS 0 CRITICAL；K1262b code review pending (in-flight). 強 evidence「positive-feedback crowding 是 generic family, VT 是 representative case」, 完整 robustness 三 phase (strategy-spec × market-microstructure × NoiseControl null). **P5 paper rewrite reviewer-resistant** — 等用戶 confirm narrative shift → v3 framing change | 不投稿；等用戶 confirm reframe → v3 paper rewrite (1-2 週 主線程 effort)。所有 robustness gates 已過：strategy-spec (K1262)、market-microstructure (K1262b)、NoiseControl null control (K1261)、calibration (P5-style detector EXACT) |
...
| **P10** | **crypto-fear-channel** | ✅ **reproduce gate 100% (7/7) + 6/6 Codex gaps closed (2026-04-27)** — pre-body review 歸檔 + body_v0_intro.tex H1+H3 fixes + 全 Codex Gap A-F closed: ✓ claim-to-JSON / ✓ k1025.py read / ✓ **lit review 18 DOIs verified** (`research_notes/lit_review_dois_2026_04_27.md`, 6 topics × 2-4 papers, 13 NEW + 5 already-cited corrected; bonus: catch Bouri2020 outline+body_v0 typo FRL 37:101764→QREF 77:156-164) / ✓ outline reconcile (K639 lag 2-10 + K746b 反向 attribution) / ✓ reproduce scaffold / ✓ data snapshot (4116 rows auto_adjust=False) | 主線程接手 §2-§6 body drafting（per p10_status_2026_04_27.md，2-3 週 effort）；prerequisites 全 met |
