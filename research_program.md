...
- [ ] **Mutual-fund thematic concentration 是否預測 underlying theme crowding RV** — 用 RFS replication / author TCI data（若可得）、SEC N-PORT holdings、13F/ETF holdings fallback 建 fund-level thematic concentration / holdings-overlap proxy，檢定 TCI 上升後 underlying stocks 的 residual-correlation、theme ETF RV、theme drawdown 與 reversal 是否放大；若只用 ETF holdings，必須標「ETF proxy，不是 mutual-fund TCI replication」。與 line 1130 thematic ETF coherence decay 題正交—— 那是 theme 本身共同風險是否崩解，這裡是 fund/holding concentration 的 positioning-crowding channel（來源：RFS 2026 "Thematic Concentration and Mutual Fund Performance"；theme concentration 由報酬研究延伸成 crowding-risk prior）。
