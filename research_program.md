...
- [ ] 動能因子時序動量的「隔夜 vs 日內」拆解 — yfinance SPY/QQQ 及個股 1h bar，把 12-1 月動能信號分別回測「只持隔夜（close→open）」vs「只持日內（open→close）」兩子策略，計算兩段的 Sharpe、偏態、最大 DD；retail order-flow heuristic 解釋隔夜 vs 日內 return gap；與既有 VRP 隔夜/日內拆解（line 470, 608, 596+）正交：這裡是動能 cross-sectional × overnight/intraday × 子策略 backtest，不是 VRP variance decomposition（來源：SSRN 2024-25 動能 × 隔夜/日內拆解；retail ebb and flow）
- [ ] 實現高階矩（realized kurtosis）作 vol 爆發預測的增量因子 — yfinance SPY + TAIEX 5-min bar 計算 daily realized kurtosis，在 HAR-RV 框架外加入 RK 作 predictor，檢定對未來 5d realized vol 的 OOS QLIKE 改善；分市況（低 vs 高 vol regime）比較增量力；與既有 realized skewness（06-14b）正交：聚焦四階矩 fat-tail 訊號，非偏態（來源：Review of Accounting and Finance / JFQA 高階矩文獻；RK → vol 爆發整合 HAR 框架尚少）
- [ ] EPU 作為跨資產 vol regime **switching trigger**（非 incremental predictor）— Baker-Bloom-Davis EPU 月頻 ×（TAIEX/SPY）日 realized vol，**Markov-switching GARCH** 估計 EPU 衝擊上升期的 vol 狀態轉移概率與條件分佈；與既有 line 505「EPU/Google Trends 增益 HAR-RV」正交：那是 incremental linear predictor，這裡是 regime-shift mechanism 識別（來源：FRB 2025 Costs of Rising Uncertainty + ScienceDirect EPU-vol regime；2025 EPU 達歷史峰值）
