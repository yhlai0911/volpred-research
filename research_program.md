...
| **P6** | **prg-periodic-garch** | 🟢 **v4 review done 4.2★, HOLD for v4.1 (2026-04-27)** — v4 batch 5 actions apply 完 → v4 paper-review-cycle 並行 latex+citation+cross-paper meta 啟動：**latex 4.2★** (v3 4.1 → +0.1, 1 NEW MAJOR §4-§4.5 SPY DM 6.00 vs 5.24 cross-section 數值微差 + 1 NEW MED Bollerslev1996 vs BollerslevWooldridge1992 QML cite + 3 cosmetic MIN), **citation GREEN PASS** (0/1/2), **cross-paper meta**: NotebookLM 2 critical issues 維持 CLOSED+STRENGTHENED; K1260 contribution intact; M-NEW-1-v4 是內部 disambiguation 不是 portfolio-level issue. 6-criteria gate: **5/6 PASS + 1 marginal** (criterion 4 真實接受率 desk-accept 30%). **HOLD for v4.1 (~20 min batch fix)**: §4.5 Table 5 移除 PRG-vs-GJR row + Bollerslev1996→BollerslevWooldridge1992 + 3 cosmetic MIN. v4.1 後預期 4.4-4.6★ + FRL desk-accept 35-45% + 6/6 PASS → 升 ready_for_submission | 不直投稿；v4.1 fix 後 6/6 gate PASS 升 ready_for_submission |
