...
- [ ] **氣候政策不確定性（CPU）指數 → 商品期貨 RV 增量**（高信心）— 用 Gavriilidis 免費 news-based CPU/GCPU 月頻指數（policyuncertainty.com）作 HAR 外生 covariate，檢定對 USO/UNG/DBA/CORN/WEAT/GLD 次期 realized vol 的 OOS QLIKE+DM 增量，並測 sector heterogeneity（農產/金屬曝險最高）；signal.shift(1)。與 line 479（純 UNG/DBA 季節性、無 CPU 訊號）正交（來源：JFutMkt 2026 + NBER w34762 CPU-as-supply-shock；CPU→commodity vol 是 2025-26 明確上升趨勢）。
