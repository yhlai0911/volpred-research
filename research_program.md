...
- [ ] **Short interest 急升 + borrow rate proxy：squeeze-risk 是否預測 small-cap 個股下 5d 高 RV** — FINRA 公開 short interest semi-monthly + yfinance Russell 2000 constituents（IWM holdings），建 short interest ratio 21d change z-score + days-to-cover proxy（shares short ÷ avg volume），shift(1) 後檢定 top-decile shock 是否預測 5d RV、jump intensity、left-tail loss；與既有 retail attention 軸正交，這裡是 short-side limits-to-arbitrage channel（來源：JFQA 2025 short-sale constraint arbitrage limit 持續主題；2026 GME-style squeeze 餘波 + 監管討論）
