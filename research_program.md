...
- [ ] Giacomini-Rossi fluctuation test 回溯 VolPred 既有 DM ledger — 對 A4f/CF-Rolling vs HAR 的既有 2718-row loss differential 序列跑時變相對績效（fluctuation test），找出優勢是否集中特定 regime；純內部資料零額外成本（來源：Giacomini-Rossi 波動檢定成為 2025-26 forecast-eval 標配）。差異化：把庫內靜態 DM 升級為時變 inference，強化方法學護城河。
