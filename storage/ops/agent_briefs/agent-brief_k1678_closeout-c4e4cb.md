# Task: 收尾孤兒實驗 K1678（寫入 knowledge.json）

**Model**: opus / xhigh (per model_router, task_type=experiment)
**Task ID**: dreaming_orphaned_experiment_k1678
**Owner token**: hourly-slot-1-215e3e27ac5b46b78a013cb996257633

## 背景

`experiments/K1678/` 已有完整結果但**沒有任何 consumer**（knowledge.json / feed.json / paper / open task 都找不到 'k1678'）。
dreaming 巡檢在 2026-07-12 首次偵測，已餓死 97.9h（P3 門檻 72h）。本任務目的：把結果正式收進機構記憶，讓它不再是孤兒。

## 你要做的事（依序）

1. **讀結果**：`experiments/K1678/K1678_results.json` + `experiments/K1678/README.md` + `experiments/K1678/K1678.py`（確認方法、樣本、lag 處理）。
   - 特別檢查：signal 有沒有 `.shift(1)`？baseline 用同樣 lag 嗎？結果好得不像真的 = 90% 有 bug。
2. **讀既有 Codex review**：`experiments/K1678/codex_review.md` 已存在。
...
3. **寫 knowledge.json**（canonical: `/Users/yhlai0911/volpred-research/storage/knowledge.json`，用既有 helper script 或 append 相同 schema，**不要手改成不合法 JSON**）：
   - 必含 `experiment_id`（K1678）、reviewer provenance（哪個 model / 哪份 review / verdict / 日期）、方法摘要、樣本期間、主要數字（照抄 results.json，**禁止四捨五入成漂亮數字、禁止編造**）。
   - **Null result 照實寫 — null 也是結果。** 不要把不顯著寫成「有趨勢」。
4. **判斷可發佈性**：若結論夠強且對讀者有價值 → 在 `storage/next_tasks.json` 新增一筆 `daily_article` task（description 註明 source K1678），**不要自己寫文章**。若是 null/太技術 → 不排文章，直接說明理由。
...
你在 registered linked worktree `.claude/worktrees/dispatch-slot-1-215e3e27-k1678`（branch 同名）工作。
