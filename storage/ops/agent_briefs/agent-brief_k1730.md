# K1730 — 隔夜/日內波動率溢酬（VRP）clustering 與星期效應擇時的可交易性

**Model**: opus / xhigh (per model_router)
**Task**: K1730 (experiment lane, worktree topology)
...
## 交付物（三件套，寫入 worktree）
- `experiments/k1730/README.md`：motivation + 資料契約 + method + **lookahead policy 明述** + success criteria + 結果摘要 + 局限。
- `experiments/k1730/k1730.py`：可重跑，`seed=42`，明確 `.shift(1)` lag，segment 計算與成本模型清楚。
- `experiments/k1730/K1730_results.json`：byte-traceable 輸出（每個 README 數字都能對應到 json key）。
- 圖表（ACF、by-weekday bar、equity curve with/without cost）放 `experiments/k1730/`。
...
完成後產出 `experiments/k1730/review_verdict.json`（Codex review；quota 擋則 fallback subagent / audit）。未達 **CONDITIONAL_PASS** 不得宣稱結論、不得寫 knowledge（K1259：agent 禁寫 knowledge.json，由主線程收件時寫）。
