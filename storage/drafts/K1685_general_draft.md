---
title: "把恐慌指數餵給波動率模型，七年半後的成績單"
audience: general
k_id: K1685
experiment_refs: ["k1685_garchx_oos_extension"]
tags: [VIX, 波動率預測, 模型比較, SPY]
description: "課本裡的波動率模型只吃股價自己的歷史。如果把市場的恐慌情緒也餵進去，預測會不會更準？我們用 1,890 個交易日、七年半的樣本外資料把這件事跑到底，也把它站不住腳的地方一併攤開。"
---
...
| 樣本 | 交易日 | 不看 VIX 的模型 | 把 VIX 餵進去的模型 | 誤差降低 |
|---|---:|---:|---:|---:|
...
這個差距不是運氣。用標準的預測比較檢定去算，檢定值 t = 3.966，純屬巧合的機率是 0.000076，低於萬分之一。

門檻也不是隨便挑的：金融學界試過的模型太多，光要求 t 大於 2 會撈出一堆假發現，所以近年的方法論文獻建議把門檻拉到 3。餵了 VIX 的版本過得去。

![四段期間的預測誤差比較，餵了 VIX 的模型誤差都比較低](storage/drafts/assets/k1685_qlike_by_regime.png)
...
![把測試期終點逐月往後拉，檢定值一路為正、沒有一次翻盤](storage/drafts/assets/k1685_dm_t_path.png)
...
![累積誤差差距逐月往上，優勢是一天一天累積出來的](storage/drafts/assets/k1685_cumulative_gap.png)
...
兩個模型的完整學名、參數上下界、誤差指標的數學定義、檢定的落後期設定與全部敏感度表格，都在對應的實驗頁與結果檔裡（實驗代號見本文標籤）。方法論依據為 Patton (2011) 對雜訊代理穩健的損失函數、Diebold 與 Mariano (1995) 的預測準確度比較，以及 Harvey、Liu 與 Zhu (2016) 提出的多重檢定門檻。
