---
title: "小波分析能不能改善波動率預測？我們直接拿它對上 HAR，結論是：看你怎麼比"
audience: research
status: draft
tags:
  - wavelet
...
但這次實驗我們直接拿小波對上「波動率預測界的工作馬」HAR 模型，得到一個讓人冷靜的答案：**有時贏一點，有時輸一點，統計上幾乎沒差。**
...
![小波模型對 HAR 的比較檢定 p-value（static 與 rolling 估計法並列）](experiments/k479/k479_dm_pvalues.png)
...
![樣本內 R² 比較：六個模型解釋力幾乎完全由 HAR 決定](experiments/k479/k479_r2_compare.png)
...
第一，**過去文獻常宣稱「小波改善波動率預測」**，但很多論文只報一種估計法、一個資料集、一段期間的 R² 提升。我們這次同時跑 static + rolling，差距瞬間從「贏一點」變「輸一點」。**估計方式敏感度沒被控制 = 結論不穩**。
...
第三，**對波動率預測實務的啟示**：HAR 的多尺度（1 日、5 日、22 日）已經是一種「人為但有效」的頻率分解；小波雖然數學上更優雅，但**在這個任務上沒帶來額外資訊**。Occam's razor 提醒我們：簡單方法經得起考驗。HAR 的三層滯後已經把波動率裡的多尺度結構吸進去了，小波再切一刀，挖不到剩下的訊號。

## 結論與下一步

這次實驗的標題答案是：**小波在波動率預測上不是「萬能改良藥」**。它在某些 spec、某些估計方式、某些期間下會看起來贏，但這些勝利在統計檢定下都不顯著，而且翻轉非常容易。
...
但在我們承擔得起這些更複雜實驗之前，這次的紀律是：**任何「新方法贏舊方法」的宣稱，至少要跑兩種估計法、兩個比較檢定、一個 cross-OOS。否則你看到的可能只是估計法敏感度的鏡像。**
