...
K475 測的就是這個問題，用的方法傻到有點好笑：把 GJR 和 HAR 的波動率預測值，直接 50/50 加在一起。
...
這不是一期的結果，是五期加起來算平均排名，Ens_GJR_HAR 都是第一。5 期裡有 3 期它排 r² 第一名，另外 2 期也排在前三。
...
K475 的核心發現用一句話說：對 SPY 做跨 2015-2025 五段 OOS 測試，GJR 和 HAR 的等權平均（Ens_GJR_HAR）在波動率預測和 VaR 兩個面向同時表現最好，平均 QLIKE 比最好的單模型低 6.4%。
