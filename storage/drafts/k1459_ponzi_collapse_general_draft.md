---
title: "為什麼龐氏一定會崩？問題不在道德，先在數學"
audience: general
tags: [龐氏騙局, 現金流模型, 臨界條件, 投資人保護, Madoff]
experiment_refs: [k1459_ponzi_collapse_math]
description: "把『龐氏一定崩』這句話寫成可以驗算的數學：臨界條件 g* = r 與每月需要補進的新錢規模。"
---
...
![龐氏崩潰熱圖：承諾報酬 × 新人增長率 → 撐多久才崩](experiments/k1459_ponzi_collapse_math/k1459_collapse_heatmaps.png)
...
但如果你的 outstanding capital 已經到 175 億美元，那每個月光是支付 1% 報酬，就要 1.75 億美元的新錢。
...
![三情境每輪新進人數路徑：穩態 1% / 臨界 1% / 激進 10%](experiments/k1459_ponzi_collapse_math/k1459_scenario_entrants.png)
...
![三情境 reserve 路徑：何時撐住、何時開始下行](experiments/k1459_ponzi_collapse_math/k1459_scenario_reserves.png)
...
**數據來源**：K1459 龐氏崩潰數學實驗（experiments/k1459_ponzi_collapse_math/）。Madoff 數字以 Reuters/Time（2015）報導的 trustee 估計約 17.5B 損失規模為背景；本文用此規模搭配模型 `本輪新錢 = (r+w) × A_{t-1}` 推導每月所需新增資金。Charles Ponzi 50% 情境下的 1,922 與 5,766 由 `A_t = N0 × (1+r)^t` 與 `E_t = r × A_{t-1}` 解析推出。
