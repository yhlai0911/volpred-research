...
![θ₂ 27 次重估的時間序列：圍繞 0 上下震盪，bootstrap CI 涵蓋 0](https://qxhfgdfzazwpkdgesavm.supabase.co/storage/v1/object/public/article-images/k1067_theta2_evolution.png)
...
![事件日 vs 非事件日的逐日損失差異：兩邊都圍繞 0，事件日沒有顯著偏離](https://qxhfgdfzazwpkdgesavm.supabase.co/storage/v1/object/public/article-images/k1067_event_window_analysis.png)
...
![台積電對 K1064 三個 ETF 變體的兩模型比較統計強度比較：四個都在 0 附近，沒有方向性訊號](https://qxhfgdfzazwpkdgesavm.supabase.co/storage/v1/object/public/article-images/k1067_dm_comparison.png)
...
- **預測時序**：所有訊號都是用 t-1 的資訊預測 t 的波動率，code 採用 lag-1 手動對齊（訓練端 `eav_lag[1:] = eav_vals[:-1]`、OOS forecast 端 `eav_lag_val = eav_arr[abs_idx - 1]`），結構上排除同日洩漏；不是 pandas 的 `signal.shift(1)`，但等效。Garman-Klass 波動率代理也使用同一交易日的 OHLC 計算，與訊號日對齊。
...
### 延伸閱讀
- [更準的波動率模型，真的能賺更多嗎？](/reports/mile_f2340786)
