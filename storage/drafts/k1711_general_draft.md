...
K1711 換了一個更嚴的問法。我們把候選模型全部丟進一個池子：老公式這邊有 HAR、HAR-A（多了一個非對稱項，讓大跌後的波動反應更快），AI 這邊有裸用的 TimesFM、TTM，還有它們校準後的版本、以及跟 HAR 做組合的版本。接著跑一個叫「模型信心集合」（Model Confidence Set，MCS）的程序，問的是：**哪些模型可以被統計證據排除掉？** 留在集合裡的模型，意思是「沒有足夠證據說它比最佳的差」，落選的則是「有證據判定它確實比較差」。
...
先看最直接的數字。評分用 QLIKE，這是波動率預測的標準損失函數，數字越低代表預測越準。三個市場的一步預測結果如下（QLIKE，越低越好）：
...
![各模型 QLIKE 比最佳模型 HAR-A 高多少](storage/drafts/k1711_general_assets/fig1_qlike_excess.png)
...
有意思的地方在後半段。我們對 AI 的原始預測做了一道很輕的手術：Mincer–Zarnowitz 校準，白話說就是用一條迴歸，把它系統性高估或低估的偏誤修掉。以 SPY 為例，TimesFM 的 QLIKE 從裸用的 0.4798 降到校準後的 0.3837，幾乎貼平 HAR 的 0.3840；台股和台指期也是同一個走勢。跟 HAR 直接做加權組合，效果類似。
...
問題是，進了集合不等於贏。把三個市場按日期先平均、再跑一次跨市場的信心集合（2,344 個共同交易日），存活下來的只剩兩個模型：HAR-A（QLIKE 0.2955）和 AI＋HAR 的校準組合 COMB-MZ（0.3003）。兩者並存在集合裡，意思是分不出高下；而數字最小的那個，還是純血的 HAR-A。整份實驗跑下來，沒有任何一個含 AI 的模型，在任何一個市場，拿到「顯著比 HAR 更準」的成績。
...
第一，這是回溯式的 pseudo-OOS，不是真正的即時預測。TimesFM 和 TTM 都是後來才發布的模型，它們預訓練吃過的資料，是否包含了評估期裡等價的歷史模式，我們沒辦法完全稽核。輸入端沒有偷看未來，但權重裡記了什麼，是個黑盒。所以這個窗口只能叫「retrospective pseudo-OOS」，不能宣稱是即時實戰。

第二，信心集合是一個「未被拒絕」的集合，不是「證明相等」的集合。集合大小會受損失函數、波動率代理、bootstrap 設定影響。留在裡面，永遠只代表現有樣本量還排除不掉它，不代表它跟最佳一樣好。
...
**數據與方法**：實驗編號 K1711。市場 SPY、0050.TW、台指期 TX，評分期間 2016-07 至 2026-07，逐資產樣本 2,426 至 2,519 筆，跨市場共同交易日 2,344 筆。波動代理方面，SPY 與 0050 用 yfinance 日 OHLC 的 Garman–Klass variance，TX 用 TAIFEX 5 分鐘 RV。TSFM checkpoint 為 `google/timesfm-2.5-200m-pytorch`（TimesFM 2.5, 200M）與 `ibm-granite/granite-timeseries-ttm-r2`（TTM），context=512。損失函數 Patton QLIKE，一步預測（h=1）。模型信心集合用 Hansen–Lunde–Nason（2011），5,000 次 bootstrap，α=0.10，seed 固定 20260714。

**延伸閱讀（同一研究線的相關實驗）**：
- K1259 建立本平台的模型信心集合基礎設施，是本實驗評分程序的地基。
- K1535 等實驗早已顯示，在公平資訊集下，複雜 ML 模型通常難以顯著勝過 HAR 家族；K1711 把這條結論延伸到最新的基礎模型。
- K1355 提供跨資產 pooled 推論的正確做法（先按日期聚合再檢定，不把同日多市場當成獨立樣本），本實驗的跨市場信心集合遵循此規範。

**可復現**：完整程式與結果在 `experiments/k1711/`，主腳本 `k1711.py`，結果檔 `k1711_results.json`，本文圖表由 `k1711_results.json` 直接讀值繪製。
