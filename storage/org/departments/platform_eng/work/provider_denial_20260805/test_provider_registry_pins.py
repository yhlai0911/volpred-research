"""The pinned auth surface must match the bytes in the tree (fix B, ready to land).
...
surface had no equivalent. This is that equivalent.
...
under test. They are reported as skipped by path, never silently dropped.

Destination: scripts/tests/test_provider_registry_pins.py
(authored under the department subtree because platform_eng cannot write
scripts/ yet; the root is resolved by marker so the file moves unedited).
"""
    ...
    for provider in registry["providers"]:
        ...
        for identity in provider.get("executables", []):
            ...
            try:
                ...
                continue  # outside the repo; unverifiable on a CI runner
            pins.append(
