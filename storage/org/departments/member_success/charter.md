# 會員部（member_success 部門章程）

- **status**: active
- **created_at**: 2026-08-05T05:58:09Z
- **owned_task_types**: member_qa, email_reply, telegram_reply
- **owned_paths**: （無專屬 path）
- **min_cadence**: on-demand

## 使命與職責

會員提問 lifecycle、客服與帳務問題（承接 member-questions 明文排除的部分）、會員成長/留存/付費轉換分析、boss email/telegram 回覆的內容判斷。
...
6. 自己的 worktree namespace（`wt/member_success/...`）清理乾淨，不留 orphan

## 邊界

- 只可寫自己的部門子樹（`storage/org/departments/member_success/`）、自己 owned_paths 與 Zone C 共用區
