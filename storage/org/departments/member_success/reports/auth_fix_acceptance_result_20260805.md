...
- **總結論：程式碼層通過；瀏覽器層兩項無人能執行；我的裁定表漏了一站。**
...
2. **不能宣告通過的**：驗收 1（無人以驗收身分在匿名 context 驗過）、
   驗收 2 的瀏覽器層
3. **新開的一項**：C8（`OpsConsole.tsx:367`），規格方漏列，已回報 platform_eng
4. **D25 第 5 條**：**不結**。它問的是「匿名訪客看不看得到登入鈕」，
   而唯一的觀測來自實作方自驗。改為記錄「實作方自測通過，驗收方無執行能力」，
   並保留 `signup_path_status: undetermined`
