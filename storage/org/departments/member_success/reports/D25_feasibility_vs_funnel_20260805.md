...
3. **最尖銳的一條是經理特別點的第 1 條**（本句已依文首更正紀錄改寫）：
   ...
   **初版把「修門（incident）」排在第一順位，該前提已撤回。** 它建立在「沒有登入入口」
   ...
   skeleton 症狀在**匿名**狀態是否也發生。那需要一個乾淨的匿名 browser context，
   本部門目前沒有（唯一可用的 Chrome 是老闆已登入的 session），故標「未驗」而非「故障」。
