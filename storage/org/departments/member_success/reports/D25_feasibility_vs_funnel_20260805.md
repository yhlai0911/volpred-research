...
> **更正紀錄（2026-08-05 19:0x 台灣時間，由 member_success 下一班補）**
> 本報告初版第 5 列宣稱「站上沒有登入入口」，**該宣稱是錯的，已撤回**。
> 錯因：據以判斷的 grep 只掃 `Nav*.tsx` / `Header*.tsx` / `layout.tsx` 三個檔的中文字串與
> `signInWithOAuth`，而登入元件是獨立的 `src/components/AuthButton.tsx`，`layout.tsx` 只
> **import** 它、本身不含那些字串——**那個 grep 從一開始就不可能命中它**。
> 實際上 `AuthButton` 掛在 `app/layout.tsx:174` 與 `:180`，全站每頁 nav 都有登入按鈕；
> 匿名 client bundle 也含全部登入字串與 inline 的 `NEXT_PUBLIC_SUPABASE_URL`。
> 第 5 列判定由「❌推翻」改為「⚠️未驗」，第 3 句「牆前面那道門也打不開」一併撤回。
> **排序結論因此改變：「修門」不是已證實的前置阻塞，量測（G3/G1）才是確定的第一順位。**
> 其餘各列與第二、三部分規格不受影響——它們的數據皆為 Supabase 實查與線上字串實測，
> 不依賴這條錯誤推論。
...
| 5 | YP 的登入牆是設計過的轉換裝置（半透明遮罩、「免費使用 30 種工具」、6 個好處、「先逛逛」逃生口），我們該學 | §4.5 登入牆 | **（本列初版結論已撤回，見文首更正紀錄。）** 我們**有**登入入口：`AuthButton` 全域掛在 `app/layout.tsx:174,180`，每頁 nav 皆有，匿名 bundle 亦含全部登入字串。但我們**沒有 YP 那種登入牆**——現況是一顆裸按鈕，沒有遮罩、沒有價值主張文案、沒有「先逛逛」逃生口，也沒有任何 `signup_prompt_shown` 級的埋點可證明它被看見。另有一項獨立且未解的觀察：已登入 owner 的真實 Chrome 上，/questions 的「提出你的問題」卡片永久停在 skeleton（該卡片內另有一個 `signInWithOAuth`，questions/page.tsx:277）；**（2026-08-06 更新）該 skeleton 症狀已修復並由本部門在乾淨匿名 context 實測結案**：commit 785ca70 部署後，`/questions` 與 `/v3/questions` 兩版的登入鈕與提問卡皆正常 render，未卡骨架。本列的結構性結論不變——我們有入口、但沒有 YP 那種被設計過的登入牆 | ⚠️修正（宣稱方向成立：牆要設計；但前提不是「沒有入口」而是「入口沒被設計成牆」） |
...
| 10 | 「文章 → 個人化工具 → 訂閱」這條線 B0–B5 約 5–7 週會第一次真的存在 | §6 | 這條線的**出口**（結帳）沒有落地層——無 orders／subscriptions／entitlement 推導（見第 3 條）。**入口**（註冊）技術上是通的，但停流 111 天、且完全沒有儀器可判斷有多少人走到那裡（見第 5、6 條）。B0–B5 做完會得到一條出口不通、入口盲測的中段 | 🔺前置 |
...
3. **最尖銳的一條是經理特別點的第 1 條**（本句已依文首更正紀錄改寫）：
   YP 的洞見是「牆該切在市場涵蓋而非功能存在」。我們的問題不在牆切在哪裡——
   **我們根本還沒有牆：門是開著的（全域 AuthButton），但門口沒有任何轉換設計，
   也沒有任何儀器能告訴我們有多少人走到門口。** 排序因此是：
   `量測（G3/G1） → 訂閱骨架（G2） → 立牆（門口的轉換設計＋CTA，需老闆核准）`。
   A0 可與量測並行，因為它不動引擎、也不依賴任何權限。

   **初版把「修門（incident）」排在第一順位，該前提已撤回。** 它建立在「沒有登入入口」
   這個錯誤結論上。剩下的真實待驗項只有一個，且遠比 incident 小：/questions 卡片的
   skeleton 症狀在**匿名**狀態是否也發生。

   **（2026-08-06 00:0x 結案）已驗，答案是：修復後不再發生。** 治理部裁定唯讀查看自家站
   不受 `computer_use` 管轄後，本部門在乾淨匿名 context（全新分頁、無登入 session）實測
   `/questions` 與 `/v3/questions`：nav 登入鈕正常 render，提問卡完整 render
   （輸入框 +「Google 登入」），未卡骨架。受驗版本為 frontend-v2-fix commit 785ca70。
   **此為修復後的觀測事實，不回推歷史**——修復前的匿名行為在部署發生後已永久無法量測。
   唯一仍未驗的是「帶著不符 schema 的 localStorage」那條路徑（注入需要 JS 執行，
   被權限層 deny），但該失效鏈已被 S4 結構性消滅（`authLoading` 不再由該 effect 決定），
   詳見 `auth_fix_acceptance_result_20260805.md`。
...
1. `signup_prompt_shown` **不是多餘的，而且它的必要性今天剛被自己證明了一次。**
   本部門初版曾誤判「入口從未 render」並發成 incident，錯因是靠 grep 與人工開瀏覽器去
   推斷一件本該由數據直接回答的事（見文首更正紀錄）。有了這個事件，「入口有沒有被看見」
   不再需要任何人去猜：`signup_prompt_shown` 連續數日為 0 才是入口壞了，非 0 就是沒壞。
   **這一條的價值高於其他四條**——它同時是漏斗指標與故障偵測器。
...
**依賴**：無。（初版寫「三個事件在註冊入口修好之前埋了也不會有資料，故 G3 與 incident
修復需同批驗收」——該依賴建立在已撤回的錯誤前提上，一併取消。入口本來就在，
G3 可以立刻獨立實作與驗收。）
