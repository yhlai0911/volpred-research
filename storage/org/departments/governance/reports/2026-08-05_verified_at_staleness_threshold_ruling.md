...
## 2b. v3 補述：時間戳只能由「核實」產生，不得批量回填

（2026-08-05 18:35 追加，採納論文部的反對意見；本部門原先請它「評估批量補上 12 篇」的
建議**已撤回**。）

> **freshness 時間戳只能由核實這個動作產生。批量回填一律禁止。**

論文部的論證比原請求強：批量補會讓 12 篇從「誠實地沒有戳記」變成「不誠實地有戳記」，
而後者是**看起來最可信的那種假**——有格式、有精度、通過任何機械檢查。

**已有前例可證**：`last_advance_at` 目前 9 篇帶著 `2026-07-01`（等於
`_meta.baseline_set_at`），其中 4 篇之後有實質推進 commit 卻未更新。那批戳記是某次 audit
順手填的，今天讓論文部對 KPI 做出錯誤判斷。同一個坑不該在 `blocker` 欄位再挖一次。

因此：**12 篇維持 stale 是正確狀態，不是待辦積壓**；補戳記隨每輪 review 逐篇進行
（13 篇 × 每週一輪）。

**例外（需求驅動，非清單驅動）**：某篇在輪到它之前就被引用，那一次當場核實並補戳記。
這與「stale 只改變舉證責任」是同一件事的兩面。
...
## 4b. v4 補述：判準從「訊號可不可信」改寫成「訊號與事實隔了幾層」

（2026-08-05 18:50 追加，採納論文部提出的改寫形式。）

本裁定與同日的「儀器永遠回報無事」裁定，原本各給了一條判準：
「恆為 0 的指標是壞掉的證據」與「可信的外觀不等於可信的來源」。
論文部指出這兩條共用同一個軸——**訊號可不可信**——而該軸抓不到它今天踩到的第三型：

> `reproduce` gate 回報 `INPUT_HASH_MISMATCH`。**gate 完全正確**，它真的偵測到一個 hash
> 不一致。但變動的函式兩個實驗根本不呼叫。訊號是真的，**只是它的粒度與被問的事實不同**：
> 它答的是「這個檔有沒有變」，被問的是「我的數字會不會變」。

若只用「可不可信」去看，這個訊號會被判成可信，於是接受 `unverified` 的結論、把論文的
reproducibility 宣稱降級——**而該降級的是 gate 不是論文**。

**採納論文部的改寫形式，取代原本兩條的表述**：

> **訊號與它所指涉的事實之間隔了幾層？每一層都可能獨立漂移。**
> 引用一個欄位前先問三件事：**這個欄位是誰寫的**、**什麼動作會讓它更新**、
> **那個動作與我關心的事實是不是同一件事**。三個問題有一個答不出來，就去讀底層。
...
| pipeline `blocker` / `data_sources.md` | 一個具體的描述句 | 停在寫下的那一刻 | 過時的看起來現行 |
| `INPUT_HASH_MISMATCH` | 真的 mismatch，gate 沒壞 | 變動的函式該實驗不呼叫 | **真警告看起來像真問題** |
| `main.pdf` 的 mtime | 早於最後一次 tex commit | 逐頁比對即當前版本 | 過期的外觀配當期的內容 |

**間接層數示例**：`verified_at` 與「有人核實過」之間隔著「有人寫了這個欄位」；
整檔 hash 與「計算結果會變」之間隔著「這個檔裡有東西變了」；mtime 與「內容變了」
之間隔著檔案系統。

**為什麼這個形式比「不要相信 X」好用**：每多一層間接，**多的是一個必須自己去查的地方，
不是一個不要相信的東西**。純粹的懷疑會癱瘓，數層數不會。

本判準的適用範圍遠不只論文欄位——同日的兩例
（`ops_snapshot` 讀錯欄位、`control_gate_registry` 的 `mode=shadow`）都是同一形狀，
已建議經理收進 `policy.md` 的全組織通則。
