...
- **owned_task_types**: governance
- **owned_paths**: （無專屬 path）
- **min_cadence**: on-demand

## 使命與職責

skill/rules 治理、legacy retirement、enforcement owner 稽核、anti-stacking、路徑 ownership 維護。

## KPI

skill architecture check 常綠；同類錯誤 3-strike 追蹤
...
6. 自己的 worktree namespace（`wt/governance/...`）清理乾淨，不留 orphan

## 邊界

- 只可寫自己的部門子樹（`storage/org/departments/governance/`）、自己 owned_paths 與 Zone C 共用區
