...
- **owned_task_types**: daily_article, daily_digest, event_article, trending_repost
- **owned_paths**: storage/drafts/
- **min_cadence**: on-demand

## 使命與職責

讀者導向文章：日常文章、每日導讀、事件文章、trending 改寫；選題查重、撰寫、發佈（走 feed-publisher）、懶人包圖組。

## KPI

draft 池不空；release 節奏不斷；讀者回訪與分享指標
...
6. 自己的 worktree namespace（`wt/content/...`）清理乾淨，不留 orphan

## 邊界

- 只可寫自己的部門子樹（`storage/org/departments/content/`）、自己 owned_paths 與 Zone C 共用區
