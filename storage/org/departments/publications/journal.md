...
## 2026-08-05 21:1x — PRG v8 gate 複驗＋Edit 1 處置確認（老闆點名最急）— `outcome=done`（驗證）＋ `blocked`（套用）

工作項 `item_20260805T134851468670Z_prg-v8-staleness-gate-round-sta`（經理 P1，老闆 telegram
msg 1615-1619 點名）。經理本輪盤點發現這件事在 canonical 池裡沒有任何一張任務、論文部待辦為 0
——老闆點名最急的事實際上沒有人在做。派工層的漏，不是本部門的。

### 做完的三件

1. **Gate 獨立複驗**（沒有引用經理的代驗結果）：sha256 `8852326a…c86ed` / 30,408 bytes，
   兩項皆相符。
2. **補驗經理沒驗的那一格：六個 FIND 的全檔唯一性。** gate 只證明整檔未變，不證明每個 FIND
   唯一命中——一個字串若出現兩次，套用者會改到錯的那一處，而 hash 完全通過、gate 全綠。
   結果六筆全部 `count=1` 且命中原行號（207/198/39/118/111/195）。**verdict: ROUND VALID。**
   腳本 `scratchpad/verify_prg_v8_anchors.py` 唯讀可重跑。
3. **Edit 1 處置確認不變**：移除兩個 citep 的最小修法。三條理由，都與投稿策略無關——
   (a) 舉證責任在稿件，撤回指控不需外部確認、換成讚許則需要；(b) **就算不投 FRL 也必須撤**，
   無證據的具名方法論指控本身就不能留，FRL 只讓後果更難看而非撤除的理由（我明確反對
   「換個期刊就能留著」這個推論）；(c) 稿件 `:103`/`:106` 與結論段自我矛盾。

### 仍然擋住：「誰套用」是空的

重測非引用舊結論——settings 檔未變更（deny 仍在、外於 allow），且實際跑了一次 Edit 1 的逐字
替換仍是 `File is in a directory that is denied`。**未以任何方式繞過**（無 `sed -i`、
無 shell 重導向、無 python 寫檔）。

### 這輪挖出的一個操作陷阱（已送經理併入 platform_eng 那張單）

**解鎖是兩步不是一步。** 改生成端 settings 之後，**執行中的 pane 不會自動生效**——權限是
attach 當下載入的快照。今早 registry 已 grant `paper/`，我這班仍要 re-attach 才看得到，
同一個機制。若只做第一步，平台工程部會回報「已修」，而這裡照樣擋，然後雙方再花一輪對帳。

### 給經理的取捨（我沒有偏好，但有時間差）

A（D40 原路線，主線程套用）時間成本為零，交接件已是可貼上的形態；B（解鎖後由本部門套用）
需要改 settings ＋ re-attach 兩個非同步步驟。而 round 一旦有人先動 `main.tex` 就整輪作廢，
所以「立刻」有實質意義。選誰仍由經理定。

記錄：`review_rounds/prg-periodic-garch/v8_review_20260805/GATE_RECHECK.md`。
