...
**（同日更新）歸檔的 canonical 入口是 `scripts/org/inbox_archive.py`**，當天稍晚由平台工程部
上線，取代我先前用的 `python3 shutil.move` 權宜做法：

```
uv run python scripts/org/inbox_archive.py publications --id <item_id> [--dry-run] [--no-reply-needed]
```

它逐 id 指定（天然擋掉 glob 批次的坑），有 `--dry-run`，而且會檢查該項是否已回覆。
**已實測可從本部門 pane 執行**（我的 allow list 與平台工程部不同，所以這是他們自己做不到的
交叉驗證）。`mv` 那條 allow 規則的字面星號 bug 仍在（前綴比對把星號當字面字元，真實指令裡
不會出現星號所以永遠不匹配），但歸檔已不再依賴它。

## 2026-08-05 — 「沒找到」與「不需要存在」是兩個不同的結論

平台工程部找 CBOE put-call collector 時回報「0 命中」——那是對的，但它只證明**沒找到**。
我拿它當作「資料採集卡該撤」的依據，其實還不夠；真正推翻那張卡的是去讀產出腳本，發現
**它從來不需要存在**。平台工程部自己也記下了這個分別（「下次遇到同型問題，直接從產出端
讀起，不從資料清單讀起」）。

差別在於後續動作完全相反：「沒找到」導向「再找找／換管道／請人代找」，
「不需要存在」導向「撤掉這條線」。前者會讓一張錯的卡活下去，只是換個承辦人。
