...
## 報表可信度的唯一判準（2026-08-05 D12 定案，取代所有「內容是不是 0」的直覺）

**判準是「期間結束了沒」＋「機制認不認為它 due」，不是 `totals.billable_total == 0`。**
三個分支，讀到任何一份 token 報表都先走這棵樹：

1. 期間**未結束** → 0 或中間值都正常，**不是缺陷**，停。
   （`week_range` / `date` 一定要讀，我前三輪就是跳過這步才連續答錯同一題）
2. 期間已結束、`build_token_usage_maintenance()` 標它 `due` → **會自癒**，等它跑，不人工回填。
3. 期間已結束、機制**不會**重產 → **真缺陷，該回填**。← 我前三輪從沒問到的那一題
...
## 部門常規：連續改口是訊號，順著訊號找資料格式缺陷（經理 D18 指定寫成常規）

**觸發條件**：發現自己（或前一輪的自己）對**同一個欄位／同一個判斷**改口 ≥2 次。

**錯誤反應**：再改一次結論，把措辭寫得更小心。
**正確反應**：停下來問「**為什麼我會反覆答錯同一題**」，答案通常不在我身上，
而在那份資料——它缺了能回答這題的欄位，所以每次都得靠外部線索去推斷，推斷就會飄。

實例（2026-08-05，`weekly_2026-07-31.json`）：
我三輪的說法是「需回填」→「不需回填」→ 清單裡又寫成「已知壞、還沒人動」。
往定義挖之後發現：那份 payload 沒有 `generated_at`、沒有 `period_end`、沒有
`period_complete`，所以「這個 0 是還沒發生還是漏記了」**光讀檔案答不出來**——
完整性存在檔案 mtime 裡，不在報表內容裡。修的是資料格式，不是我的措辭。

順著這條線挖出的東西比原問題大得多：47/98 份報表寫在期間結束前、五份 weekly 永久
凍結在部分期間值、F5 這個全新缺陷、以及 mtime 不隨 git 走的移機風險。
**如果只改結論，這四件一件都不會被發現。**

判準一句話：**改口 ≥2 次 → 去看那份資料能不能自我描述；不能，那才是要修的東西。**

## 對外風險的排序原則（2026-08-05 經理 D18 認可）

**「0 很顯眼會被抓到；部分期間的非 0 值不會有任何人懷疑。」**
一個明顯錯的數字會被修，一個看起來合理的錯數字會被引用。所以排優先序時：

- 「通過了檢查的錯誤」> 「沒通過檢查的錯誤」（前者會被當基準用 —— F5 就是這型）
