...
- **⚠️ 禁止在 hourly fire 內編輯正在執行自己的 wrapper（`cron_hourly_dispatch.sh` / `~/.volpred/bin/cron_hourly_dispatch.sh`）**（2026-07-02 21:07 incident，見 error_log）：bash 對長腳本邊執行邊 re-read 磁碟源檔，fire 若 mid-run 改該檔，wrapper teardown 讀到行號位移的內容 → syntax error → 崩在 `=== [hourly_dispatch] exit N ===` banner 之前 → 無 exit marker → `host_cron_fail` 卡假 CRITICAL。此 wrapper 的邏輯改動一律在**互動 session**（執行體非該 wrapper）做，或接受「改完下一班才生效」；改完必 `bash -n` 驗證兩份且**不在同一 fire 觸發它自己**。（其他非 hourly-dispatch 的 `cron_*.sh` wrapper 不受此限，因為不是當前 fire 的執行體。）
