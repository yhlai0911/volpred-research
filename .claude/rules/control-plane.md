...
- **⚠️ 派工執行體的程式改動規則（2026-07-05 更新 — 執行體已從 shell wrapper 換成 supervisor daemon）**：hourly dispatch 自 2026-07-04 cutover 起由 `com.volpred.dispatch-supervisor`（`scripts/dispatch_supervisor/*.py` 常駐 daemon）執行；legacy `cron_hourly_dispatch.sh` 已 launchctl-disabled（留作一鍵回滾 artifact，Deliverable 8 歸檔前勿刪）。改 supervisor code 的規矩：(a) daemon 常駐，**改完必 `launchctl kickstart -k gui/<UID>/com.volpred.dispatch-supervisor` 重載才生效**；(b) 重載前必查 `jq '.current_job' storage/ops/dispatch_state.json` 為 null（restart 的 orphan-cleanup 會 kill in-flight worker）；(c) `scripts/cron_hourly_dispatch_prompt.md` 改動不需重載（每次 fire 重讀）。歷史脈絡（bash wrapper 邊執行邊 re-read、2026-07-02 21:07 incident）見 error_log；其他仍在用的 `cron_*.sh` wrapper 若要「fire 內改自己」仍適用舊禁令原理。
