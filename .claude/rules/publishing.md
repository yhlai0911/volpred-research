...
- **已註冊系列 canonical list**（新增系列在此登記：系列名 + 一句定義 + 集數/範例）：
  - **迷思實驗室 EP.X｜** 🧪（boss 2026-07 於 Telegram 定名，2026-07-06 補記）— 拿一個投資人常聽的
    說法／網路熱門傳言，用**真數據 + 統計檢定**驗一次、可複現（呼應本平台 AI 自主研究招牌）。
    **每篇掛集數**，格式 `迷思實驗室 EP.X｜<標題>`（X 依發佈順序遞增，全形｜）。首發 3 集：
    **EP.1** 開盤第一小時真的最危險嗎？（`mile_467226e7`）、**EP.2** 融資餘額創高＝要見頂？
    （`mile_0f7d1501`）、**EP.3** 黃金真的是股災避風港嗎？（`mile_08820c3d`）。
    ⚠️ feed 中其他 mid-title 含「迷思」的**舊文**（Sell in May／財報季選股／美股跟單／策略越簡單）
    **不屬**本系列，**不要亂掛 EP**。
  - **會員提問｜** — member_qa 既有慣例（沿用；member-questions skill 已在用）。
- 系列名 / 集數可由 boss 隨時 rename / 增列（口頭一句即改此清單）。下一集 EP 編號 = 續 EP.3 之後。
