...
- **已註冊系列 canonical list**（新增系列在此登記）。系列有**兩種品牌化機制**，都算正式系列：
  ...
  **機制 A — 標題前綴 `系列名｜標題`**（一般 reader-facing 文章用；分隔符全形 `｜`）：
  - **迷思實驗室 EP.X｜** 🧪（boss 2026-07 於 Telegram 定名，2026-07-06 補記）— 拿一個投資人常聽的
    說法／網路熱門傳言，用**真數據 + 統計檢定**驗一次、可複現（呼應本平台 AI 自主研究招牌）。
    **每篇掛集數**，格式 `迷思實驗室 EP.X｜<標題>`（X 依發佈順序遞增）。首發 3 集：**EP.1** 開盤第一
    小時（`mile_467226e7`）、**EP.2** 融資餘額創高（`mile_0f7d1501`）、**EP.3** 黃金避風港
    （`mile_08820c3d`）。下一集續編 EP.4。⚠️ feed 中其他 mid-title 含「迷思」的**舊文**（Sell in
    May／財報季選股／美股跟單／策略越簡單）**不屬**本系列，**不要亂掛 EP**。
