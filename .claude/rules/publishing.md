...
>
> **鐵律**：(1) 分隔符恆全形 `｜`；(2) 同一主題多篇留 1 篇 `published`、其餘 `unpublished`（**不可 `draft`** — release-pool 會重發）；(3) **動任何文章（retitle / unpublish / dedup）前先讀該文 `status` 驗 ground truth，不可憑 `published_at` 假設已發佈**（draft 也可能帶 published_at）；(4) **registry 只登記「成員關係」（`members`），不存 status** — status 的唯一真相是 `feed.json`。2026-07-14 前的 schema 用 `members_published` / `members_draft` 存了第二份 status，而 publish 流程從不回寫它，於是每次 draft 被 release 就漂移一次（該日 4 篇 taiwan_uav）。**禁止在 registry 重新引入任何 status 欄位**；`tests/test_alerts.py::test_series_registry_stores_no_status_copy` 機械擋。
...
- **已註冊系列 canonical list**（新增系列在此登記）。系列有**兩種品牌化機制**，都算正式系列：

  **機制 A — 標題前綴 `系列名｜標題`**（一般 reader-facing 文章用；分隔符全形 `｜`）：
  - **迷思實驗室｜** 🧪（boss 2026-07 於 Telegram 定名；2026-07-06 boss 決定**不掛集數、純前綴**）—
    拿一個投資人常聽的說法／網路熱門傳言，用**真數據 + 統計檢定**驗一次、可複現（呼應 AI 自主研究招牌）。
    格式 `迷思實驗室｜<標題>`（**不用 EP.X 集數**）。判定屬本系列 = 「投資人常聽的說法／網路傳言 +
    真數據驗證」，內部研究線代號多為 `research_myth_*` / `newroute_*_myth`。已上線 8 篇（開盤第一小時
    `mile_467226e7`／融資餘額 `mile_0f7d1501`／黃金 `mile_08820c3d`／盤整越久 `mile_e082beb8`／
    VIX 破30抄底 `mile_4d67163f`／美股大跌台股補跌 `mile_d1a83b4f`／漲跌停磁吸 `mile_4fe0a026`／
    創新高急殺 `mile_bd564eb7`）；台積電打噴嚏 `mile_e7b6e075` 尚為 draft，釋出時已預掛前綴。
  ...
  **機制 B — 前端專屬區塊品牌化（標題本身不加前綴）**：
  - **每日精選導讀**（daily_digest）— **是正式系列**，但品牌化在前端元件而非標題：
    `frontend-v2-fix/src/components/DailyDigestSection.tsx` 已渲染完整報頭「每日精選導讀」+ **第 X 期**
    （issueNumber）+ 日期 + 編按導言 + 主圖 + 本期精選各篇。因為報頭已含系列名與期數，**標題本身
    不加前綴**（否則與報頭雙重「每日精選導讀」贅字）；title 用專題式標題、**不可**以 `每日精選導讀｜`
    起頭。它是系列 —— 只是靠前端 masthead + 期數 呈現系列身分，不靠標題前綴。

- 系列名 / 集數可由 boss 隨時 rename / 增列（口頭一句即改此清單）。

**真正不加前綴的 non-series 情況**：一次性文章（單一 K 研究、單次事件）—— 不屬任何系列。

**寫作 agent 適用**：派 daily_article / trending_repost / event_article agent 時，若該篇屬**機制 A**
系列，brief 必明寫「標題格式 `系列名｜<你的標題>`，分隔符用全形 `｜` 不是半形 `|`」；若屬 **daily_digest**
（機制 B），維持專題式標題不加前綴，系列身分由前端報頭 + 期數呈現。
...
傳給 gate。精確同階段 coverage 才停止，跨階段 K／arc／標題相似只提示 writer
依該 slot 的 information-state contract 做差異化。
