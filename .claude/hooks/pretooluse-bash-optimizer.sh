...
if _inside_dispatch_fire \
  ...
  DENY_REASON="🚫 禁止在 dispatch fire 內 spawn headless agent（claude -p / agy -p）。這是 2026-07-12 三次 hang_killed 的 root cause：fire 有 3000s hard cap，研究 agent 要跑 20-60min —— 父等子就燒完 cap 被 SIGKILL（三次 duration 全是 3001.3s），父先走子就變 ppid=1 孤兒、worktree 成果沒人收（K1681 白做）。這不是 timeout 調太小，是把 60 分鐘的工作塞進 50 分鐘的容器。改法：把 brief 用 Write 寫成檔案，再 uv run python scripts/compute_queue.py enqueue-agent --brief-file /tmp/brief.md --model claude-opus-4-8 --effort xhigh --source-task-id <task_id> --cwd <registered-worktree> --followup-brief '<收件時要做什麼>' --followup-task-type experiment。agent 由 */15 的 detached compute worker 執行（不受 fire cap 限制），成果由後續某一班 fire 在 PHASE A 收 —— 這正是 heavy compute 一直在走的路。本班 fire 的職責是 enqueue 後立刻結束，不是坐在那裡等。"
