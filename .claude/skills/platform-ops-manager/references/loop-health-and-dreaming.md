...
| **L2b 派工失敗** | `dispatch_supervisor/alerts.py`（daemon 內建，唯一 owner — 2026-07-05 明確化） | hourly dispatch 的失敗/掛/額度/認證 alert | completion_failure / hang / **work_timeout**（own deadline，不當作 hang）/ quota（outage-scoped，一次事故一信）/ auth / orphan / loop_crash。`host_cron_fail` **刻意不覆蓋** supervisor（legacy log 已凍結）；dashboard health_cron 只量 daemon 存活（dispatch_state.json mtime），不量成敗——成敗歸這層 |
| **L2c alert dedup 分工** | daemon `dispatch_supervisor/alerts.py`（per-class 秒級窗，存 dispatch_state）＝**防洪**；ops `src/volpred/ops/alerts.py`（`alert_dedup.json` 24h/(level,title) + 30d retention prune）＝**防轟炸** | 同一 alert 流的兩層節流 — 切面不同刻意不合併：daemon 擋 crash-loop 秒級洪水（送達前），ops 擋跨小時重複轟炸（送達端） | 兩檔 docstring 互指（2026-07-20 ops-master D4）；daemon 過洪水閘後走 `send-alert --force` 是設計非繞過 |
| **L2d 任務池水位** | `src/volpred/ops/pool_pressure.py`（`pool_admits_new_work` — 唯一 admission owner） | 「自動生成器現在可不可以再產新任務」 | drain-first latch（pending > `config/supervisor_rules.json::pending_caps.pending_cap`＝80 時擋；退出需 pending < cap **且**連續 3 個完整日 succeeded ≥ created）。閘在 4 個 generator entry point（`refill_task_pool.refill` / `generate_research_backlog.generate` / `generate_diverse_tasks.generate` / `dreaming_review.apply_auto_dispatch`），**不在 writer 層**——`append_task_record` 同時承載 telegram/gmail/老闆指派，閘那裡＝預設擋掉急件，且大宗生成器走 `write_tasks_locked` 根本繞得過。豁免：ingress 不呼叫本閘；`reader_facing`（time_critical，時效過了價值歸零）；dreaming 的 `critical`（那是壞了不是 backlog）。`continue_task_dispatch` 的 `REFILL_FLOOR=4` 降為冗餘外層。觀測者非第二 owner：`daily_checkup::check_task_pool_pressure`（淨增連續 3 日為正→critical）＋ `append_task_record` 的 `[pool_pressure] WARN` route-around 偵測 |
...
| `PostToolUse` | `Edit&#124;Write&#124;MultiEdit&#124;NotebookEdit` | `scripts/hooks/record_fire_manifest.py` | fire 內每次檔案編輯寫入 producer manifest；shadow attribution fail-open | 每次檔案編輯成功後 |
