...
| **Gemini** | 方法論建議、文獻連結、robustness 建議 | **首選** `agy -p "<prompt>"`（OAuth 訂閱免費）；**fallback** `scripts/gemini_ask.py`（直打 PAID API，會觸發費用 email alert，僅 agy 不可用時才用） | 學術框架、cross-reference、新測試建議 |
