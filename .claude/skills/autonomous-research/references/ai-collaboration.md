...
| **Gemini** | 方法論建議、文獻連結、robustness 建議 | **一律** `agy -p "<prompt>"`（OAuth 訂閱免費）；`scripts/gemini_ask.py` 付費 API **已停用**（exit 2 per boss msg936；僅 break-glass `VOLPRED_ALLOW_PAID_GEMINI=1`） | 學術框架、cross-reference、新測試建議 |
