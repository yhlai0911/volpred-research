-- Production ownership transaction for a safe publisher reconcile batch.
--
-- The durable owner/request/attempt tables already carry an effect_family
-- discriminator. This migration adds the safe reconcile family without exposing
-- those private tables.  Five narrow public wrappers exist only because the
...
VALUES (
  ...
  1,
  clock_timestamp(),
  'migration:operations_core_publisher_reconcile_ownership',
...
BEGIN
  IF p_expected_owner NOT IN ('legacy', 'operations_core')
    ...
    RAISE EXCEPTION
      'publisher reconcile batch ownership transfer fields are invalid';
  ELSIF p_target_owner = 'legacy'
      AND p_rollback_of_generation
        IS DISTINCT FROM p_expected_generation THEN
    RAISE EXCEPTION
      'publisher reconcile batch rollback must identify current generation';
  ELSIF p_target_owner = 'operations_core'
      AND p_rollback_of_generation IS NOT NULL THEN
    RAISE EXCEPTION
      'publisher reconcile batch cutover cannot carry rollback generation';
  END IF;
  ...
  IF ownership.owner <> p_expected_owner
    ...
    IF replay.effect_family IS NULL
      ...
      RAISE EXCEPTION
        'publisher reconcile batch ownership compare-and-set failed: '
        'expected %/% found %/%',
        p_expected_owner, p_expected_generation,
        ownership.owner, ownership.generation;
    END IF;
    RETURN public.volpred_read_publisher_article_reconcile_owner();
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'publisher reconcile batch ownership transfer requires zero active attempts';
  END IF;
...
DECLARE
  ...
  event_at timestamptz;
BEGIN
  articles := p_payload -> 'articles';
  canonical_feed_sha256 := p_payload ->> 'canonical_feed_sha256';
  IF p_owner_generation IS NULL
      ...
      OR NOT p_payload ?& ARRAY[
        'schema_version', 'canonical_feed_sha256', 'articles'
      ]
      OR p_payload ->> 'schema_version'
        <> 'publisher-article-reconcile.v1'
      OR canonical_feed_sha256 !~ '^[0-9a-f]{64}$'
      OR jsonb_typeof(articles) <> 'array'
      OR pg_catalog.jsonb_array_length(articles) = 0
      OR EXISTS (
        SELECT 1
        FROM pg_catalog.jsonb_array_elements(articles) AS item(article)
        WHERE jsonb_typeof(item.article) <> 'object'
          OR item.article ->> 'id' IS NULL
          OR item.article ->> 'id'
            !~ '^[A-Za-z0-9][A-Za-z0-9_-]*$'
      )
      OR (
        SELECT count(*) <> count(DISTINCT item.article ->> 'id')
        FROM pg_catalog.jsonb_array_elements(articles) AS item(article)
      )
      OR articles <> (
        SELECT pg_catalog.jsonb_agg(item.article ORDER BY item.article ->> 'id')
        FROM pg_catalog.jsonb_array_elements(articles) AS item(article)
      ) THEN
    RAISE EXCEPTION
      'owned publisher reconcile batch request fields are invalid';
  END IF;
  ...
  IF ownership.owner <> 'operations_core'
    ...
    RAISE EXCEPTION
      'operations core does not own publisher reconcile batch generation %',
      p_owner_generation;
  END IF;

  request_identity := jsonb_build_object(
    'schema_version', 'owned-publisher-reconcile-request.v1',
    'effect_family', ownership.effect_family,
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'owned-publisher-reconcile:' || btrim(p_idempotency_key),
      0
    )
  );
  ...
  IF existing.idempotency_key IS NOT NULL THEN
    IF existing.request_sha256 <> request_sha256
      ...
      RAISE EXCEPTION
        'owned publisher reconcile batch idempotency key conflicts '
  ...
  article_count := pg_catalog.jsonb_array_length(articles);
  work_id :=
    'work_owned_reconcile_' || substr(request_sha256, 1, 32);
  effect_id :=
    'effect_owned_reconcile_' || substr(request_sha256, 1, 32);
  ...
  FROM volpred_ops.submit_work(
    work_id,
    'owned-publisher-reconcile-work:' || btrim(p_idempotency_key),
    'publisher.reconcile_batch',
    'publisher.article.reconcile',
    'Reconcile ' || article_count::text || ' publisher articles',
    2,
    ARRAY['supabase-article-reconcile-effect'],
    ARRAY['supabase-article-reconcile-readback'],
    'safe',
    'auto',
    payload_ref,
    NULL,
  ...
  FROM volpred_ops.request_effect(
    effect_id,
    'owned-publisher-reconcile-effect:' || btrim(p_idempotency_key),
    work.id,
    work.version,
    ownership.effect_family,
    target_ref,
    ...
    payload_sha256,
    'safe',
    'publisher.article.supabase.reconcile.readback',
    target_ref,
    btrim(p_actor_ref),
    request_sha256
  );
...
BEGIN
  IF p_owner_generation IS NULL
    ...
    RAISE EXCEPTION 'owned publisher reconcile batch begin fields are invalid';
  END IF;
  ...
  IF ownership.owner <> 'operations_core'
      OR ownership.generation <> p_owner_generation THEN
    RAISE EXCEPTION
      'publisher reconcile batch ownership lost: '
  ...
  IF NOT (
    ...
    RAISE EXCEPTION
      'owned publisher reconcile batch work is not available: %',
      work.status;
  END IF;
  ...
  IF message.available_at > event_at
    ...
    RAISE EXCEPTION
      'owned publisher reconcile batch effect is not available: %',
      message.status;
  END IF;
  ...
  IF effect.effect_kind <> ownership.effect_family
    ...
    RAISE EXCEPTION
      'owned publisher reconcile batch effect contract drifted';
  END IF;
  ...
  authority_request_sha256 := encode(
    sha256(
      convert_to(
        jsonb_build_object(
          'schema_version', 'owned-publisher-reconcile-authority.v1',
          'owner_generation', ownership.generation,
          'work_id', work.id,
  ...
  IF encode(sha256(payload_bytes), 'hex')
    ...
    RAISE EXCEPTION
      'owned publisher reconcile batch durable payload hash mismatch';
  END IF;
  ...
  RETURN jsonb_build_object(
    'schema_version', 'owned-publisher-reconcile-attempt.v1',
    'owner_generation', ownership.generation,
    'work_id', work.id,
    'work_version', work.version,
...
BEGIN
  IF p_owner_generation IS NULL
    ...
    RAISE EXCEPTION
      'owned publisher reconcile batch settlement fields are invalid';
  END IF;
  ...
  IF owned_attempt.owner_generation <> ownership.generation
    ...
    RAISE EXCEPTION
      'owned publisher reconcile batch attempt identity mismatch';
  END IF;
  IF owned_attempt.status <> 'started' THEN
    IF owned_attempt.reported_outcome <> p_outcome
      ...
      RAISE EXCEPTION
        'owned publisher reconcile batch settlement conflicts '
  ...
  ELSIF owned_attempt.lease_expires_at <= clock_timestamp() THEN
    RAISE EXCEPTION 'owned publisher reconcile batch attempt lease expired';
  END IF;
  ...
  IF work.version <> p_work_version
    ...
    RAISE EXCEPTION 'publisher reconcile batch work lease lost';
  END IF;
  ...
  IF attempt_receipt.disposition = 'delivered' THEN
    ...
    FROM volpred_ops.complete_work(
      ...
      'owned publisher reconcile batch delivered with exact Supabase read-back'
    );
  ...
  ELSE
    ...
    SET status = 'failed',
        ...
        result_summary = 'owned publisher reconcile batch dead-lettered',
    ...
    VALUES (
      ...
      'failed',
      attempt_receipt.evidence_ref,
      'owned publisher reconcile batch dead-lettered',
      event_at
    );
    INSERT INTO volpred_ops.work_events (
...
COMMENT ON FUNCTION
  public.volpred_read_publisher_article_reconcile_owner()
IS
  'Read the unique owner generation for publisher safe reconcile.';
COMMENT ON FUNCTION
  public.volpred_transfer_publisher_article_reconcile_owner(
    text, bigint, text, text, text, bigint
  )
IS
  'CAS transfer publisher reconcile ownership with rollback identity.';
COMMENT ON FUNCTION
  public.volpred_request_owned_publisher_article_reconcile(
    bigint, text, jsonb, text
  )
IS
  'Create immutable reconcile WorkItem, payload, EffectRequest and outbox.';
COMMENT ON FUNCTION
  public.volpred_begin_owned_publisher_article_reconcile(
    bigint, text, text, integer, text, text, text
  )
