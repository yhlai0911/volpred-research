-- Production ownership transaction for one publisher article projection.
--
-- The durable owner/request/attempt tables already carry an effect_family
-- discriminator.  This migration adds the publisher family without exposing
...
VALUES (
  ...
  1,
  clock_timestamp(),
  'migration:operations_core_publisher_article_ownership',
...
BEGIN
  ...
  IF ownership.owner <> p_expected_owner
    ...
    IF replay.effect_family IS NULL
      ...
      RAISE EXCEPTION
        'publisher article ownership compare-and-set failed: '
        'expected %/% found %/%',
        p_expected_owner, p_expected_generation,
        ownership.owner, ownership.generation;
    END IF;
    RETURN public.volpred_read_publisher_article_sync_owner();
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'publisher article ownership transfer requires zero active attempts';
  END IF;
...
BEGIN
  ...
  IF p_owner_generation IS NULL
      ...
      OR NOT p_payload ?& ARRAY['schema_version', 'article']
      OR p_payload ->> 'schema_version'
        <> 'publisher-article-sync.v1'
      OR jsonb_typeof(article) <> 'object'
      OR article_slug IS NULL
      OR article_slug !~ '^[A-Za-z0-9][A-Za-z0-9_-]*$'
      OR (
        article ? 'tags'
        AND (
          jsonb_typeof(article -> 'tags') <> 'array'
          OR EXISTS (
            SELECT 1
    ...
    RAISE EXCEPTION
      'owned publisher article request fields are invalid';
  END IF;
  ...
  IF ownership.owner <> 'operations_core'
    ...
    RAISE EXCEPTION
      'operations core does not own publisher article generation %',
      p_owner_generation;
  END IF;
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'owned-publisher-article:' || btrim(p_idempotency_key),
      0
    )
  );
  ...
  IF existing.idempotency_key IS NOT NULL THEN
    IF existing.request_sha256 <> request_sha256
      ...
      RAISE EXCEPTION
        'owned publisher article idempotency key conflicts '
  ...
  work_id :=
    'work_owned_publisher_' || substr(request_sha256, 1, 32);
  effect_id :=
    'effect_owned_publisher_' || substr(request_sha256, 1, 32);
  payload_ref := 'effect-payload:' || effect_id || ':article';
  target_ref := 'supabase:articles/' || article_slug;
  ...
  FROM volpred_ops.submit_work(
    work_id,
    'owned-publisher-work:' || btrim(p_idempotency_key),
    'publisher.single_article',
    'publisher.article.sync',
    coalesce(nullif(btrim(article_title), ''), article_slug),
    2,
    ARRAY['supabase-article-effect'],
    ARRAY['supabase-article-readback'],
    'safe',
    'auto',
    payload_ref,
    NULL,
  ...
  FROM volpred_ops.request_effect(
    effect_id,
    'owned-publisher-effect:' || btrim(p_idempotency_key),
    work.id,
    work.version,
    ownership.effect_family,
    target_ref,
    ...
    payload_sha256,
    'safe',
    'publisher.article.supabase.readback',
    target_ref,
    btrim(p_actor_ref),
    request_sha256
  );
...
BEGIN
  IF p_owner_generation IS NULL
    ...
    RAISE EXCEPTION 'owned publisher article begin fields are invalid';
  END IF;
  ...
  IF ownership.owner <> 'operations_core'
      OR ownership.generation <> p_owner_generation THEN
    RAISE EXCEPTION
      'publisher article ownership lost: '
  ...
  IF NOT (
    ...
    RAISE EXCEPTION
      'owned publisher article work is not available: %',
      work.status;
  END IF;
  ...
  IF message.available_at > event_at
    ...
    RAISE EXCEPTION
      'owned publisher article effect is not available: %',
      message.status;
  END IF;
  ...
  IF effect.effect_kind <> ownership.effect_family
      OR effect.target_ref
        <> 'supabase:articles/'
          || split_part(effect.target_ref, '/', 2)
      OR effect.acknowledgement_kind
        <> 'publisher.article.supabase.readback'
      OR effect.acknowledgement_target_ref <> effect.target_ref THEN
    RAISE EXCEPTION
      'owned publisher article effect contract drifted';
  END IF;
  ...
  authority_request_sha256 := encode(
    sha256(
      convert_to(
        jsonb_build_object(
          'schema_version', 'owned-publisher-article-authority.v1',
          'owner_generation', ownership.generation,
          'work_id', work.id,
  ...
  IF encode(sha256(payload_bytes), 'hex')
    ...
    RAISE EXCEPTION
      'owned publisher article durable payload hash mismatch';
  END IF;
  ...
  RETURN jsonb_build_object(
    'schema_version', 'owned-publisher-article-attempt.v1',
    'owner_generation', ownership.generation,
    'work_id', work.id,
    'work_version', work.version,
...
BEGIN
  IF p_owner_generation IS NULL
    ...
    RAISE EXCEPTION
      'owned publisher article settlement fields are invalid';
  END IF;
  ...
  IF owned_attempt.owner_generation <> ownership.generation
    ...
    RAISE EXCEPTION
      'owned publisher article attempt identity mismatch';
  END IF;
  IF owned_attempt.status <> 'started' THEN
    IF owned_attempt.reported_outcome <> p_outcome
      ...
      RAISE EXCEPTION
        'owned publisher article settlement conflicts '
  ...
  ELSIF owned_attempt.lease_expires_at <= clock_timestamp() THEN
    RAISE EXCEPTION 'owned publisher article attempt lease expired';
  END IF;
  ...
  IF work.version <> p_work_version
    ...
    RAISE EXCEPTION 'publisher article work lease lost';
  END IF;
  ...
  IF attempt_receipt.disposition = 'delivered' THEN
    ...
    FROM volpred_ops.complete_work(
      ...
      'owned publisher article delivered with exact Supabase read-back'
    );
  ...
  ELSE
    ...
    SET status = 'failed',
        ...
        result_summary = 'owned publisher article dead-lettered',
    ...
    VALUES (
      ...
      'failed',
      attempt_receipt.evidence_ref,
      'owned publisher article dead-lettered',
      event_at
    );
    INSERT INTO volpred_ops.work_events (
...
COMMENT ON FUNCTION
  public.volpred_read_publisher_article_sync_owner()
IS
  'Read the unique owner generation for publisher article Supabase sync.';
COMMENT ON FUNCTION
  public.volpred_transfer_publisher_article_sync_owner(
    text, bigint, text, text, text, bigint
  )
IS
  'CAS transfer publisher sync ownership with explicit rollback identity.';
COMMENT ON FUNCTION
  public.volpred_request_owned_publisher_article_sync(
    bigint, text, jsonb, text
  )
IS
  'Atomically create publisher WorkItem, payload, EffectRequest and outbox.';
COMMENT ON FUNCTION
  public.volpred_begin_owned_publisher_article_sync(
    bigint, text, text, integer, text, text, text
  )
