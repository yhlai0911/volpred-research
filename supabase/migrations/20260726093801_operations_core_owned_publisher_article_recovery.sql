-- Recover publisher article sync attempts after a worker crash or a due
-- provider retry.  The public service-role RPC is the only mutation seam:
-- it locks one exact-family candidate, records immutable recovery evidence,
-- and starts the next fenced attempt in the same transaction.
...
CREATE TABLE volpred_ops.owned_publisher_article_recovery_receipts (
...
CREATE INDEX owned_publisher_article_recovery_receipts_effect_idx
  ON volpred_ops.owned_publisher_article_recovery_receipts (
    effect_id, recovery_attempt_count, recovered_at
  );

CREATE INDEX owned_notification_attempts_recovery_candidate_idx
  ON volpred_ops.owned_notification_attempts (
    lease_expires_at, effect_id, attempt_count
  )
  WHERE status IN ('started', 'retry_scheduled');

ALTER TABLE volpred_ops.owned_publisher_article_recovery_receipts
...
GRANT USAGE, SELECT ON SEQUENCE
  volpred_ops.owned_publisher_article_recovery_receipts_sequence_seq
TO volpred_ops_definer;

CREATE POLICY owned_publisher_article_recovery_receipts_definer_select
  ON volpred_ops.owned_publisher_article_recovery_receipts
  FOR SELECT TO volpred_ops_definer USING (true);
CREATE POLICY owned_publisher_article_recovery_receipts_definer_insert
  ON volpred_ops.owned_publisher_article_recovery_receipts
...
BEGIN
  SELECT pg_catalog.pg_get_functiondef(
    pg_catalog.to_regprocedure(
      'public.volpred_begin_owned_publisher_article_sync(bigint,text,text,integer,text,text,text)'
    )
  )
...
$old_begin$,
  ...
  IF EXISTS (
    ...
    WHERE predecessor.effect_id = owned_request.effect_id
      AND predecessor.status IN ('started', 'retry_scheduled')
      AND NOT EXISTS (
        SELECT 1
        FROM volpred_ops.owned_publisher_article_recovery_receipts
          AS recovery_receipt
        WHERE recovery_receipt.effect_id = predecessor.effect_id
          AND recovery_receipt.candidate_attempt_count =
            predecessor.attempt_count
      )
  ) THEN
    RAISE EXCEPTION
      'owned publisher article predecessor requires recovery';
  END IF;
...
$new_begin$
  ...
  IF rewritten_definition = function_definition THEN
    IF pg_catalog.strpos(
      ...
      RAISE EXCEPTION
        'owned publisher article begin recovery fence pre-image did not match';
    END IF;
  ELSE
...
BEGIN
  IF p_owner_generation IS NULL OR p_owner_generation <= 0
    ...
    RAISE EXCEPTION 'owned publisher article recovery fields are invalid';
  END IF;
  ...
  IF candidate_attempt.effect_id IS NULL THEN
    RETURN jsonb_build_object(
      'schema_version', 'owned-publisher-article-recovery.v1',
      'recovered', false
    );
  END IF;
  ...
  recovery_evidence_ref :=
    'owned-publisher-article-recovery:' || candidate_attempt.effect_id
  ...
  recovery_evidence_sha256 := encode(
    sha256(
      convert_to(
        jsonb_build_object(
          'schema_version',
            'owned-publisher-article-recovery-receipt.v1',
  ...
  INSERT INTO volpred_ops.owned_publisher_article_recovery_receipts (
  ...
  IF (recovered_attempt ->> 'attempt_count')::integer
    ...
    RAISE EXCEPTION
      'owned publisher article recovery did not advance attempt count';
  END IF;
...
IS
  'Recover one exact-family publisher sync crash or due retry through a fenced transaction.';
